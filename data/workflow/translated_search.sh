...
if [ -n "$QUERY_NUCL" ]; then
    if notExists "${TMP_PATH}/q_orfs_aa.dbtype"; then
        ...
        "$BIOSNAKE" extractorfs "$1" "${TMP_PATH}/q_orfs_aa" ${ORF_PAR} \
...
if [ -n "$NO_TARGET_INDEX" ]; then
    if notExists "${TMP_PATH}/t_orfs_aa.dbtype"; then
        # shellcheck disable=SC2086
        "$BIOSNAKE" extractorfs "$2" "${TMP_PATH}/t_orfs_aa" ${ORF_PAR} \
...
#        # shellcheck disable=SC2086
#        "$BIOSNAKE" prefilter "${QUERY}" "${TARGET}" "${TMP_PATH}/q_orfs_aa_pref" --min-ungapped-score 3 -s 3 -k 6 --diag-score 0 --spaced-kmer-mode 0 --max-seqs 1 ${THREAD_COMP_PAR} \
...
#        # shellcheck disable=SC2086
#        "$BIOSNAKE" createsubdb "${TMP_PATH}/q_orfs_aa_filter.list" "${QUERY}" "${TMP_PATH}/q_orfs_aa_filter" ${CREATESUBDB_PAR} \
...
if notExists "$3.dbtype"; then
    # shellcheck disable=SC2086
    "$BIOSNAKE" offsetalignment "$1" "$QUERY_ORF" "$2" "$TARGET_ORF" "${TMP_PATH}/aln"  "$3" ${OFFSETALIGNMENT_PAR} \
        || fail "Offset step died"
fi

if [ -n "$REMOVE_TMP" ]; then
    # shellcheck disable=SC2086
    "$BIOSNAKE" rmdb "${TMP_PATH}/q_orfs_aa" ${VERBOSITY}
    # shellcheck disable=SC2086
    "$BIOSNAKE" rmdb "${TMP_PATH}/t_orfs_aa" ${VERBOSITY}
    # shellcheck disable=SC2086
    "$BIOSNAKE" rmdb "${TMP_PATH}/aln" ${VERBOSITY}
    if [ -n "${REFERENCE_FILTER}" ]; then
        # shellcheck disable=SC2086
        "$BIOSNAKE" rmdb "${TMP_PATH}/q_orfs_aa_pref" ${VERBOSITY}
        # shellcheck disable=SC2086
        "$BIOSNAKE" rmdb "${TMP_PATH}/q_orfs_aa_filter" ${VERBOSITY}
