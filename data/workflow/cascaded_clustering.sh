...
while [ "$STEP" -lt "$STEPS" ]; do
    ...
    if notExists "${TMP_PATH}/pref_step$STEP.dbtype"; then
         # shellcheck disable=SC2086
        $RUNNER "$BIOSNAKE" prefilter "$INPUT" "$INPUT" "${TMP_PATH}/pref_step$STEP" ${TMP} \
    ...
    if notExists "${TMP_PATH}/aln_step$STEP.dbtype"; then
         # shellcheck disable=SC2086
        $RUNNER "$BIOSNAKE" "${ALIGN_MODULE}" "$INPUT" "$INPUT" "${TMP_PATH}/pref_step$STEP" "${TMP_PATH}/aln_step$STEP" ${TMP} \
    ...
    if notExists "${TMP_PATH}/clu_step$STEP.dbtype"; then
         # shellcheck disable=SC2086
        "$BIOSNAKE" clust "$INPUT" "${TMP_PATH}/aln_step$STEP" "${TMP_PATH}/clu_step$STEP" ${TMP} \
    ...
    if [ "$STEP" -eq "$((STEPS-1))" ]; then
       if [ -n "$REASSIGN" ]; then
          if notExists "${TMP_PATH}/clu.dbtype"; then
            # shellcheck disable=SC2086
            "$BIOSNAKE" mergeclusters "$SOURCE" "${TMP_PATH}/clu" "${TMP_PATH}/clu_redundancy" ${CLUSTER_STR} \
            || fail "Merging of clusters has died"
          fi
       else
            # shellcheck disable=SC2086
            "$BIOSNAKE" mergeclusters "$SOURCE" "$2" "${TMP_PATH}/clu_redundancy" ${CLUSTER_STR} $MERGECLU_PAR \
            || fail "Merging of clusters has died"
       fi
    else
        if notExists "$NEXTINPUT.dbtype"; then
            # shellcheck disable=SC2086
            "$BIOSNAKE" createsubdb "${TMP_PATH}/clu_step$STEP" "$INPUT" "$NEXTINPUT" ${VERBOSITY} --subdb-mode 1 \
...
if [ -n "$REASSIGN" ]; then
    ...
    if notExists "${TMP_PATH}/aln.dbtype"; then
        # shellcheck disable=SC2086
        $RUNNER "$BIOSNAKE" "${ALIGN_MODULE}" "$SOURCE" "$SOURCE" "${TMP_PATH}/clu" "${TMP_PATH}/aln" ${ALIGNMENT_REASSIGN_PAR} \
    ...
    if notExists "${TMP_PATH}/clu_not_accepted.dbtype"; then
        # shellcheck disable=SC2086
        "$BIOSNAKE" subtractdbs "${TMP_PATH}/clu" "${TMP_PATH}/aln" "${TMP_PATH}/clu_not_accepted" --e-profile 100000000 -e 100000000 ${THREADSANDCOMPRESS} \
    ...
    if notExists "${TMP_PATH}/clu_accepted.dbtype"; then
        # shellcheck disable=SC2086
        "$BIOSNAKE" subtractdbs "${TMP_PATH}/clu" "${TMP_PATH}/clu_not_accepted" "${TMP_PATH}/clu_accepted" --e-profile 100000000 -e 100000000 ${THREADSANDCOMPRESS} \
                 || fail "subtractdbs2 reassign died"
    fi
    if notExists "${TMP_PATH}/clu_not_accepted_swap.dbtype"; then
        # shellcheck disable=SC2086
        "$BIOSNAKE" swapdb "${TMP_PATH}/clu_not_accepted" "${TMP_PATH}/clu_not_accepted_swap" ${THREADSANDCOMPRESS} \
    ...
    if notExists "${TMP_PATH}/seq_wrong_assigned.dbtype"; then
        # shellcheck disable=SC2086
        "$BIOSNAKE" createsubdb "${TMP_PATH}/clu_not_accepted_swap" "$SOURCE" "${TMP_PATH}/seq_wrong_assigned" ${VERBOSITY} \
                 || fail "createsubdb1 reassign died"
    fi
    # build seed sequences
    if notExists "${TMP_PATH}/seq_seeds.dbtype"; then
        # shellcheck disable=SC2086
        "$BIOSNAKE" createsubdb "${TMP_PATH}/clu" "$SOURCE" "${TMP_PATH}/seq_seeds" ${VERBOSITY} \
    ...
    if notExists "${TMP_PATH}/seq_wrong_assigned_pref.dbtype"; then
        ...
        # shellcheck disable=SC2086
        $RUNNER "$BIOSNAKE" prefilter "${TMP_PATH}/seq_wrong_assigned" "${TMP_PATH}/seq_seeds.merged" "${TMP_PATH}/seq_wrong_assigned_pref" ${PREFILTER_REASSIGN_PAR} \
                 || fail "Prefilter reassign died"
    fi
    if notExists "${TMP_PATH}/seq_wrong_assigned_pref_swaped.dbtype"; then
        # shellcheck disable=SC2086
        "$BIOSNAKE" swapdb "${TMP_PATH}/seq_wrong_assigned_pref" "${TMP_PATH}/seq_wrong_assigned_pref_swaped" ${THREADSANDCOMPRESS} \
                 || fail "swapdb2 reassign died"
    fi
    if notExists "${TMP_PATH}/seq_wrong_assigned_pref_swaped_aln.dbtype"; then
        # shellcheck disable=SC2086
        $RUNNER "$BIOSNAKE" "${ALIGN_MODULE}" "${TMP_PATH}/seq_seeds.merged" "${TMP_PATH}/seq_wrong_assigned" \
    ...
    if notExists "${TMP_PATH}/seq_wrong_assigned_pref_swaped_aln_ocol.dbtype"; then
        # shellcheck disable=SC2086
        "$BIOSNAKE" filterdb "${TMP_PATH}/seq_wrong_assigned_pref_swaped_aln" "${TMP_PATH}/seq_wrong_assigned_pref_swaped_aln_ocol" --trim-to-one-column ${THREADSANDCOMPRESS} \
    ...
    if notExists "${TMP_PATH}/clu_accepted_plus_wrong.dbtype"; then
        # combine clusters
        # shellcheck disable=SC2086
        "$BIOSNAKE" mergedbs "${TMP_PATH}/seq_seeds.merged" "${TMP_PATH}/clu_accepted_plus_wrong" "${TMP_PATH}/clu_accepted" \
    ...
    if notExists "${TMP_PATH}/missing.single.seqs.db.dbtype"; then
        ...
        # shellcheck disable=SC2086
        "$BIOSNAKE" tsv2db "${TMP_PATH}/missing.single.seqs" "${TMP_PATH}/missing.single.seqs.db" --output-dbtype 6 ${VERBCOMPRESS} \
    ...
    if notExists "${TMP_PATH}/clu_accepted_plus_wrong_plus_single.dbtype"; then
        # combine clusters
        # shellcheck disable=SC2086
        "$BIOSNAKE" mergedbs "${SOURCE}" "${TMP_PATH}/clu_accepted_plus_wrong_plus_single" "${TMP_PATH}/clu_accepted_plus_wrong" \
    ...
    eval TMP="\$$PARAM"
    # shellcheck disable=SC2086
    "$BIOSNAKE" clust "${SOURCE}" "${TMP_PATH}/clu_accepted_plus_wrong_plus_single" "${2}" ${TMP} \
            || fail "Clustering step $STEP died"
    ...
    if [ -n "$REMOVE_TMP" ]; then
        ...
        # shellcheck disable=SC2086
        "$BIOSNAKE" rmdb "${TMP_PATH}/clu_not_accepted" ${VERBOSITY}
        # shellcheck disable=SC2086
        "$BIOSNAKE" rmdb "${TMP_PATH}/clu_accepted" ${VERBOSITY}
        # shellcheck disable=SC2086
        "$BIOSNAKE" rmdb "${TMP_PATH}/clu_not_accepted_swap" ${VERBOSITY}
        # shellcheck disable=SC2086
        "$BIOSNAKE" rmdb "${TMP_PATH}/seq_wrong_assigned" ${VERBOSITY}
        # shellcheck disable=SC2086
        "$BIOSNAKE" rmdb "${TMP_PATH}/seq_seeds" ${VERBOSITY}
        # shellcheck disable=SC2086
        "$BIOSNAKE" rmdb "${TMP_PATH}/seq_seeds.merged" ${VERBOSITY}
        # shellcheck disable=SC2086
        "$BIOSNAKE" rmdb "${TMP_PATH}/seq_wrong_assigned_pref" ${VERBOSITY}
        # shellcheck disable=SC2086
        "$BIOSNAKE" rmdb "${TMP_PATH}/seq_wrong_assigned_pref_swaped" ${VERBOSITY}
        # shellcheck disable=SC2086
        "$BIOSNAKE" rmdb "${TMP_PATH}/seq_wrong_assigned_pref_swaped_aln" ${VERBOSITY}
        # shellcheck disable=SC2086
        "$BIOSNAKE" rmdb "${TMP_PATH}/seq_wrong_assigned_pref_swaped_aln_ocol" ${VERBOSITY}
        rm -f "${TMP_PATH}/missing.single.seqs"
        rm -f "${TMP_PATH}/clu_accepted_plus_wrong.tsv"
        # shellcheck disable=SC2086
        "$BIOSNAKE" rmdb "${TMP_PATH}/missing.single.seqs.db" ${VERBOSITY}
        # shellcheck disable=SC2086
        "$BIOSNAKE" rmdb "${TMP_PATH}/clu_accepted_plus_wrong" ${VERBOSITY}
        # shellcheck disable=SC2086
        "$BIOSNAKE" rmdb "${TMP_PATH}/clu_accepted_plus_wrong_plus_single" ${VERBOSITY}
...
if [ -n "$REMOVE_TMP" ]; then
    ...
    while [ "$STEP" -lt "$STEPS" ]; do
        # shellcheck disable=SC2086
        "$BIOSNAKE" rmdb "${TMP_PATH}/pref_step$STEP" ${VERBOSITY}
        # shellcheck disable=SC2086
        "$BIOSNAKE" rmdb "${TMP_PATH}/aln_step$STEP" ${VERBOSITY}
        # shellcheck disable=SC2086
        "$BIOSNAKE" rmdb "${TMP_PATH}/clu_step$STEP" ${VERBOSITY}
        STEP=$((STEP+1))
    done
    ...
    while [ "$STEP" -lt "$STEPS" ]; do
        # shellcheck disable=SC2086
        "$BIOSNAKE" rmdb "${TMP_PATH}/input_step$STEP" ${VERBOSITY}
        STEP=$((STEP+1))
    done
