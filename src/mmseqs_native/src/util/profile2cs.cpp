...
#pragma omp for schedule(dynamic, 1000)
      for (size_t i = 0; i < entries; ++i) {
        ...
        {
            if
        (subMat.subMatrix[ProfileStates::hh2mmseqsAAorder((int)result[k])][seq.consensus_sequence[k]]<0)
            {
                std::cout<<"Pos: "<<k<<",
        "<<subMat.num2aa[ProfileStates::hh2mmseqsAAorder((int)result[k])]<<"-"<<subMat.num2aa[seq.consensus_sequence[k]]<<"("<<subMat.subMatrix[ProfileStates::hh2mmseqsAAorder((int)result[k])][seq.consensus_sequence[k]]<<")
