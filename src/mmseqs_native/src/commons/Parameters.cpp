...
Parameters::Parameters()
    : scoringMatrixFile("INVALID", "INVALID"),
      ...
      PARAM_S(PARAM_S_ID, "-s", "Sensitivity",
              "Sensitivity: 1.0 faster; 4.0 fast; 7.5 sensitive", typeid(float),
              (void *)&sensitivity, "^[0-9]*(\\.[0-9]+)?$",
              MMseqsParameter::COMMAND_PREFILTER),
      PARAM_K(PARAM_K_ID, "-k", "k-mer length",
              "k-mer length (0: automatically set to optimum)", typeid(int),
              (void *)&kmerSize, "^[0-9]{1}[0-9]*$",
              MMseqsParameter::COMMAND_PREFILTER |
                  MMseqsParameter::COMMAND_CLUSTLINEAR |
                  MMseqsParameter::COMMAND_EXPERT),
      PARAM_THREADS(PARAM_THREADS_ID, "--threads", "Threads",
                    "Number of CPU-cores used (all by default)", typeid(int),
                    (void *)&threads, "^[1-9]{1}[0-9]*$",
                    MMseqsParameter::COMMAND_COMMON),
      PARAM_COMPRESSED(PARAM_COMPRESSED_ID, "--compressed", "Compressed",
                       "Write compressed output", typeid(int),
                       (void *)&compressed, "^[0-1]{1}$",
                       MMseqsParameter::COMMAND_COMMON),
      PARAM_ALPH_SIZE(PARAM_ALPH_SIZE_ID, "--alph-size", "Alphabet size",
                      "Alphabet size (range 2-21)", typeid(MultiParam<int>),
                      (void *)&alphabetSize, "",
                      MMseqsParameter::COMMAND_PREFILTER |
                          MMseqsParameter::COMMAND_CLUSTLINEAR |
                          MMseqsParameter::COMMAND_EXPERT),
      PARAM_MAX_SEQ_LEN(
          PARAM_MAX_SEQ_LEN_ID, "--max-seq-len", "Max sequence length",
          "Maximum sequence length", typeid(int), (void *)&maxSeqLen,
          "^[0-9]{1}[0-9]*",
          MMseqsParameter::COMMAND_COMMON | MMseqsParameter::COMMAND_EXPERT),
      PARAM_DIAGONAL_SCORING(
          PARAM_DIAGONAL_SCORING_ID, "--diag-score", "Diagonal scoring",
          "Use ungapped diagonal scoring during prefilter", typeid(bool),
          (void *)&diagonalScoring, "",
          MMseqsParameter::COMMAND_PREFILTER | MMseqsParameter::COMMAND_EXPERT),
      PARAM_EXACT_KMER_MATCHING(
          PARAM_EXACT_KMER_MATCHING_ID, "--exact-kmer-matching",
          "Exact k-mer matching",
          "Extract only exact k-mers for matching (range 0-1)", typeid(int),
          (void *)&exactKmerMatching, "^[0-1]{1}$",
          MMseqsParameter::COMMAND_PREFILTER | MMseqsParameter::COMMAND_EXPERT),
      PARAM_MASK_RESIDUES(
          PARAM_MASK_RESIDUES_ID, "--mask", "Mask residues",
          "Mask sequences in k-mer stage: 0: w/o low complexity masking, 1: "
          "with low complexity masking",
          typeid(int), (void *)&maskMode, "^[0-1]{1}",
          MMseqsParameter::COMMAND_PREFILTER | MMseqsParameter::COMMAND_EXPERT),
      PARAM_MASK_LOWER_CASE(
          PARAM_MASK_LOWER_CASE_ID, "--mask-lower-case",
          "Mask lower case residues",
          "Lowercase letters will be excluded from k-mer search 0: include "
          "region, 1: exclude region",
          typeid(int), (void *)&maskLowerCaseMode, "^[0-1]{1}",
          MMseqsParameter::COMMAND_PREFILTER | MMseqsParameter::COMMAND_EXPERT),
      PARAM_MIN_DIAG_SCORE(
          PARAM_MIN_DIAG_SCORE_ID, "--min-ungapped-score",
          "Minimum diagonal score",
          "Accept only matches with ungapped alignment score above threshold",
          typeid(int), (void *)&minDiagScoreThr, "^[0-9]{1}[0-9]*$",
          MMseqsParameter::COMMAND_PREFILTER | MMseqsParameter::COMMAND_EXPERT),
      PARAM_K_SCORE(
          PARAM_K_SCORE_ID, "--k-score", "k-score",
          "k-mer threshold for generating similar k-mer lists", typeid(int),
          (void *)&kmerScore, "^[0-9]{1}[0-9]*$",
          MMseqsParameter::COMMAND_PREFILTER | MMseqsParameter::COMMAND_EXPERT),
      PARAM_MAX_SEQS(PARAM_MAX_SEQS_ID, "--max-seqs", "Max results per query",
                     "Maximum results per query sequence allowed to pass the "
                     "prefilter (affects sensitivity)",
                     typeid(int), (void *)&maxResListLen, "^[1-9]{1}[0-9]*$",
                     MMseqsParameter::COMMAND_PREFILTER),
      PARAM_SPLIT(
          PARAM_SPLIT_ID, "--split", "Split database",
          "Split input into N equally distributed chunks. 0: set the best "
          "split automatically",
          typeid(int), (void *)&split, "^[0-9]{1}[0-9]*$",
          MMseqsParameter::COMMAND_PREFILTER | MMseqsParameter::COMMAND_EXPERT),
      PARAM_SPLIT_MODE(
          PARAM_SPLIT_MODE_ID, "--split-mode", "Split mode",
          "0: split target db; 1: split query db; 2: auto, depending on main "
          "memory",
          typeid(int), (void *)&splitMode, "^[0-2]{1}$",
          MMseqsParameter::COMMAND_PREFILTER | MMseqsParameter::COMMAND_EXPERT),
      PARAM_SPLIT_MEMORY_LIMIT(PARAM_SPLIT_MEMORY_LIMIT_ID,
                               "--split-memory-limit", "Split memory limit",
                               "Set max memory per split. E.g. 800B, 5K, 10M, "
                               "1G. Default (0) to all available system memory",
                               typeid(ByteParser), (void *)&splitMemoryLimit,
                               "^(0|[1-9]{1}[0-9]*(B|K|M|G|T)?)$",
                               MMseqsParameter::COMMAND_COMMON |
                                   MMseqsParameter::COMMAND_PREFILTER |
                                   MMseqsParameter::COMMAND_EXPERT),
      PARAM_DISK_SPACE_LIMIT(
          PARAM_DISK_SPACE_LIMIT_ID, "--disk-space-limit", "Disk space limit",
          "Set max disk space to use for reverse profile searches. E.g. 800B, "
          "5K, 10M, 1G. Default (0) to all available disk space in the temp "
          "folder",
          typeid(ByteParser), (void *)&diskSpaceLimit,
          "^(0|[1-9]{1}[0-9]*(B|K|M|G|T)?)$",
          MMseqsParameter::COMMAND_COMMON | MMseqsParameter::COMMAND_PREFILTER |
              MMseqsParameter::COMMAND_EXPERT),
      PARAM_SPLIT_AMINOACID(
          PARAM_SPLIT_AMINOACID_ID, "--split-aa", "Split by amino acid",
          "Try to find the best split boundaries by entry lengths",
          typeid(bool), (void *)&splitAA, "$", MMseqsParameter::COMMAND_EXPERT),
      PARAM_SUB_MAT(
          PARAM_SUB_MAT_ID, "--sub-mat", "Substitution matrix",
          "Substitution matrix file", typeid(MultiParam<char *>),
          (void *)&scoringMatrixFile, "",
          MMseqsParameter::COMMAND_COMMON | MMseqsParameter::COMMAND_EXPERT),
      PARAM_SEED_SUB_MAT(
          PARAM_SEED_SUB_MAT_ID, "--seed-sub-mat", "Seed substitution matrix",
          "Substitution matrix file for k-mer generation",
          typeid(MultiParam<char *>), (void *)&seedScoringMatrixFile, "",
          MMseqsParameter::COMMAND_PREFILTER | MMseqsParameter::COMMAND_EXPERT),
      PARAM_NO_COMP_BIAS_CORR(
          PARAM_NO_COMP_BIAS_CORR_ID, "--comp-bias-corr", "Compositional bias",
          "Correct for locally biased amino acid composition (range 0-1)",
          typeid(int), (void *)&compBiasCorrection, "^[0-1]{1}$",
          MMseqsParameter::COMMAND_PREFILTER | MMseqsParameter::COMMAND_ALIGN |
              MMseqsParameter::COMMAND_PROFILE |
              MMseqsParameter::COMMAND_EXPERT),
      PARAM_SPACED_KMER_MODE(
          PARAM_SPACED_KMER_MODE_ID, "--spaced-kmer-mode", "Spaced k-mers",
          "0: use consecutive positions in k-mers; 1: use spaced k-mers",
          typeid(int), (void *)&spacedKmer, "^[0-1]{1}",
          MMseqsParameter::COMMAND_PREFILTER | MMseqsParameter::COMMAND_EXPERT),
      PARAM_REMOVE_TMP_FILES(
          PARAM_REMOVE_TMP_FILES_ID, "--remove-tmp-files",
          "Remove temporary files", "Delete temporary files", typeid(bool),
          (void *)&removeTmpFiles, "",
          MMseqsParameter::COMMAND_COMMON | MMseqsParameter::COMMAND_EXPERT),
      PARAM_INCLUDE_IDENTITY(PARAM_INCLUDE_IDENTITY_ID, "--add-self-matches",
                             "Include identical seq. id.",
                             "Artificially add entries of queries with "
                             "themselves (for clustering)",
                             typeid(bool), (void *)&includeIdentity, "",
                             MMseqsParameter::COMMAND_PREFILTER |
                                 MMseqsParameter::COMMAND_ALIGN |
                                 MMseqsParameter::COMMAND_EXPERT),
      PARAM_PRELOAD_MODE(
          PARAM_PRELOAD_MODE_ID, "--db-load-mode", "Preload mode",
          "Database preload mode 0: auto, 1: fread, 2: mmap, 3: mmap+touch",
          typeid(int), (void *)&preloadMode, "[0-3]{1}",
          MMseqsParameter::COMMAND_COMMON | MMseqsParameter::COMMAND_EXPERT),
      PARAM_SPACED_KMER_PATTERN(
          PARAM_SPACED_KMER_PATTERN_ID, "--spaced-kmer-pattern",
          "Spaced k-mer pattern", "User-specified spaced k-mer pattern",
          typeid(std::string), (void *)&spacedKmerPattern, "^1[01]*1$",
          MMseqsParameter::COMMAND_PREFILTER | MMseqsParameter::COMMAND_EXPERT),
      PARAM_LOCAL_TMP(
          PARAM_LOCAL_TMP_ID, "--local-tmp", "Local temporary path",
          "Path where some of the temporary files will be created",
          typeid(std::string), (void *)&localTmp, "",
          MMseqsParameter::COMMAND_PREFILTER | MMseqsParameter::COMMAND_EXPERT),
      // alignment
      PARAM_ALIGNMENT_MODE(PARAM_ALIGNMENT_MODE_ID, "--alignment-mode",
                           "Alignment mode",
                           "How to compute the alignment:\n0: automatic\n1: "
                           "only score and end_pos\n2: also start_pos and "
                           "cov\n3: also seq.id\n4: only ungapped alignment",
                           typeid(int), (void *)&alignmentMode, "^[0-4]{1}$",
                           MMseqsParameter::COMMAND_ALIGN),
      PARAM_E(PARAM_E_ID, "-e", "E-value threshold",
              "List matches below this E-value (range 0.0-inf)", typeid(double),
              (void *)&evalThr,
              "^([-+]?[0-9]*\\.?[0-9]+([eE][-+]?[0-9]+)?)|[0-9]*(\\.[0-9]+)?$",
              MMseqsParameter::COMMAND_ALIGN),
      PARAM_C(PARAM_C_ID, "-c", "Coverage threshold",
              "List matches above this fraction of aligned (covered) residues "
              "(see --cov-mode)",
              typeid(float), (void *)&covThr, "^0(\\.[0-9]+)?|^1(\\.0+)?$",
              MMseqsParameter::COMMAND_ALIGN |
                  MMseqsParameter::COMMAND_CLUSTLINEAR),
      PARAM_COV_MODE(PARAM_COV_MODE_ID, "--cov-mode", "Coverage mode",
                     ...
                     "needs to be at least x% of the other seq. length",
                     typeid(int), (void *)&covMode, "^[0-5]{1}$",
                     MMseqsParameter::COMMAND_ALIGN),
      PARAM_SEQ_ID_MODE(PARAM_SEQ_ID_MODE_ID, "--seq-id-mode", "Seq. id. mode",
                        "0: alignment length 1: shorter, 2: longer sequence",
                        typeid(int), (void *)&seqIdMode, "^[0-2]{1}$",
                        MMseqsParameter::COMMAND_ALIGN),
      PARAM_MAX_REJECTED(PARAM_MAX_REJECTED_ID, "--max-rejected", "Max reject",
                         "Maximum rejected alignments before alignment "
                         "calculation for a query is stopped",
                         typeid(int), (void *)&maxRejected, "^[1-9]{1}[0-9]*$",
                         MMseqsParameter::COMMAND_ALIGN),
      PARAM_MAX_ACCEPT(PARAM_MAX_ACCEPT_ID, "--max-accept", "Max accept",
                       "Maximum accepted alignments before alignment "
                       "calculation for a query is stopped",
                       typeid(int), (void *)&maxAccept, "^[1-9]{1}[0-9]*$",
                       MMseqsParameter::COMMAND_ALIGN),
      PARAM_ADD_BACKTRACE(PARAM_ADD_BACKTRACE_ID, "-a", "Add backtrace",
                          "Add backtrace string (convert to alignments with "
                          "mmseqs convertalis module)",
                          typeid(bool), (void *)&addBacktrace, "",
                          MMseqsParameter::COMMAND_ALIGN),
      PARAM_REALIGN(
          PARAM_REALIGN_ID, "--realign", "Realign hits",
          "Compute more conservative, shorter alignments (scores and E-values "
          "not changed)",
          typeid(bool), (void *)&realign, "",
          MMseqsParameter::COMMAND_ALIGN | MMseqsParameter::COMMAND_EXPERT),
      PARAM_MIN_SEQ_ID(
          PARAM_MIN_SEQ_ID_ID, "--min-seq-id", "Seq. id. threshold",
          "List matches above this sequence identity (for clustering) (range "
          "0.0-1.0)",
          typeid(float), (void *)&seqIdThr, "^0(\\.[0-9]+)?|1(\\.0+)?$",
          MMseqsParameter::COMMAND_ALIGN),
      PARAM_MIN_ALN_LEN(PARAM_MIN_ALN_LEN_ID, "--min-aln-len",
                        "Min alignment length",
                        "Minimum alignment length (range 0-INT_MAX)",
                        typeid(int), (void *)&alnLenThr, "^[0-9]{1}[0-9]*$",
                        MMseqsParameter::COMMAND_ALIGN),
      PARAM_SCORE_BIAS(
          PARAM_SCORE_BIAS_ID, "--score-bias", "Score bias",
          "Score bias when computing SW alignment (in bits)", typeid(float),
          (void *)&scoreBias, "^-?[0-9]*(\\.[0-9]+)?$",
          MMseqsParameter::COMMAND_ALIGN | MMseqsParameter::COMMAND_EXPERT),
      PARAM_REALIGN_SCORE_BIAS(
          PARAM_REALIGN_SCORE_BIAS_ID, "--realign-score-bias",
          "Realign score bias", "Additional bias when computing realignment",
          typeid(float), (void *)&realignScoreBias, "^-?[0-9]*(\\.[0-9]+)?$",
          MMseqsParameter::COMMAND_ALIGN | MMseqsParameter::COMMAND_EXPERT),
      PARAM_REALIGN_MAX_SEQS(
          PARAM_REALIGN_MAX_SEQS_ID, "--realign-max-seqs", "Realign max seqs",
          "Maximum number of results to return in realignment", typeid(int),
          (void *)&realignMaxSeqs, "^[0-9]{1}[0-9]*$",
          MMseqsParameter::COMMAND_ALIGN | MMseqsParameter::COMMAND_EXPERT),
      PARAM_ALT_ALIGNMENT(PARAM_ALT_ALIGNMENT_ID, "--alt-ali",
                          "Alternative alignments",
                          "Show up to this many alternative alignments",
                          typeid(int), (void *)&altAlignment,
                          "^[0-9]{1}[0-9]*$", MMseqsParameter::COMMAND_ALIGN),
      PARAM_GAP_OPEN(
          PARAM_GAP_OPEN_ID, "--gap-open", "Gap open cost", "Gap open cost",
          typeid(MultiParam<int>), (void *)&gapOpen, "^[0-9]{1}[0-9]*$",
          MMseqsParameter::COMMAND_ALIGN | MMseqsParameter::COMMAND_EXPERT),
      PARAM_GAP_EXTEND(
          PARAM_GAP_EXTEND_ID, "--gap-extend", "Gap extension cost",
          "Gap extension cost", typeid(MultiParam<int>), (void *)&gapExtend,
          "^[0-9]{1}[0-9]*$",
          MMseqsParameter::COMMAND_ALIGN | MMseqsParameter::COMMAND_EXPERT),
      PARAM_ZDROP(
          PARAM_ZDROP_ID, "--zdrop", "Zdrop",
          "Maximal allowed difference between score values before alignment is "
          "truncated  (nucleotide alignment only)",
          typeid(int), (void *)&zdrop, "^[0-9]{1}[0-9]*$",
          MMseqsParameter::COMMAND_ALIGN | MMseqsParameter::COMMAND_EXPERT),
      // clustering
      PARAM_CLUSTER_MODE(
          PARAM_CLUSTER_MODE_ID, "--cluster-mode", "Cluster mode",
          "0: Set-Cover (greedy)\n1: Connected component (BLASTclust)\n2,3: "
          "Greedy clustering by sequence length (CDHIT)",
          typeid(int), (void *)&clusteringMode, "[0-3]{1}$",
          MMseqsParameter::COMMAND_CLUST),
      PARAM_CLUSTER_STEPS(
          PARAM_CLUSTER_STEPS_ID, "--cluster-steps",
          "Cascaded clustering steps", "Cascaded clustering steps from 1 to -s",
          typeid(int), (void *)&clusterSteps, "^[1-9]{1}$",
          MMseqsParameter::COMMAND_CLUST | MMseqsParameter::COMMAND_EXPERT),
      PARAM_CASCADED(PARAM_CASCADED_ID, "--single-step-clustering",
                     "Single step clustering",
                     "Switch from cascaded to simple clustering workflow",
                     typeid(bool), (void *)&singleStepClustering, "",
                     MMseqsParameter::COMMAND_CLUST),
      PARAM_CLUSTER_REASSIGN(
          PARAM_CLUSTER_REASSIGN_ID, "--cluster-reassign", "Cluster reassign",
          "Cascaded clustering can cluster sequence that do not fulfill the "
          "clustering criteria.\nCluster reassignment corrects these errors",
          typeid(bool), (void *)&clusterReassignment, "",
          MMseqsParameter::COMMAND_CLUST),
      // affinity clustering
      PARAM_MAXITERATIONS(
          PARAM_MAXITERATIONS_ID, "--max-iterations",
          "Max connected component depth",
          "Maximum depth of breadth first search in connected component "
          "clustering",
          typeid(int), (void *)&maxIteration, "^[1-9]{1}[0-9]*$",
          MMseqsParameter::COMMAND_CLUST | MMseqsParameter::COMMAND_EXPERT),
      PARAM_SIMILARITYSCORE(
          PARAM_SIMILARITYSCORE_ID, "--similarity-type", "Similarity type",
          "Type of score used for clustering. 1: alignment score 2: sequence "
          "identity",
          typeid(int), (void *)&similarityScoreType, "^[1-2]{1}$",
          MMseqsParameter::COMMAND_CLUST | MMseqsParameter::COMMAND_EXPERT),
      // logging
      PARAM_V(PARAM_V_ID, "-v", "Verbosity",
              "Verbosity level: 0: quiet, 1: +errors, 2: +warnings, 3: +info",
              typeid(int), (void *)&verbosity, "^[0-3]{1}$",
              MMseqsParameter::COMMAND_COMMON),
      // convertalignments
      PARAM_FORMAT_MODE(
          PARAM_FORMAT_MODE_ID, "--format-mode", "Alignment format",
          "Output format: 0: BLAST-TAB, 1: SAM, 2: BLAST-TAB + query/db "
          "length, 3: Pretty HTML",
          typeid(int), (void *)&formatAlignmentMode, "^[0-3]{1}$"),
      PARAM_FORMAT_OUTPUT(
          PARAM_FORMAT_OUTPUT_ID, "--format-output", "Format alignment output",
          "Choose comma separated list of output columns from: "
          "query,target,evalue,gapopen,pident,fident,nident,qstart,qend,"
          "qlen\ntstart,tend,tlen,alnlen,raw,bits,cigar,qseq,tseq,qheader,"
          "theader,qaln,taln,qframe,tframe,mismatch,qcov,tcov\nqset,qsetid,"
          "tset,tsetid,taxid,taxname,taxlineage,qorfstart,qorfend,torfstart,"
          "torfend",
          typeid(std::string), (void *)&outfmt, ""),
      PARAM_DB_OUTPUT(PARAM_DB_OUTPUT_ID, "--db-output", "Database output",
                      "Return a result DB instead of a text file", typeid(bool),
                      (void *)&dbOut, "", MMseqsParameter::COMMAND_EXPERT),
      // --include-only-extendablediagonal
      PARAM_RESCORE_MODE(
          PARAM_RESCORE_MODE_ID, "--rescore-mode", "Rescore mode",
          "Rescore diagonals with:\n0: Hamming distance\n1: local alignment "
          "(score only)\n2: local alignment\n3: global alignment\n4: longest "
          "alignment fulfilling window quality criterion",
          typeid(int), (void *)&rescoreMode, "^[0-4]{1}$"),
      PARAM_WRAPPED_SCORING(
          PARAM_WRAPPED_SCORING_ID, "--wrapped-scoring",
          "Allow wrapped scoring",
          "Double the (nucleotide) query sequence during the scoring process "
          "to allow wrapped diagonal scoring around end and start",
          typeid(bool), (void *)&wrappedScoring, "",
          MMseqsParameter::COMMAND_ALIGN | MMseqsParameter::COMMAND_EXPERT),
      PARAM_FILTER_HITS(PARAM_FILTER_HITS_ID, "--filter-hits",
                        "Remove hits by seq. id. and coverage",
                        "Filter hits by seq.id. and coverage", typeid(bool),
                        (void *)&filterHits, "",
                        MMseqsParameter::COMMAND_EXPERT),
      PARAM_SORT_RESULTS(PARAM_SORT_RESULTS_ID, "--sort-results",
                         "Sort results",
                         "Sort results: 0: no sorting, 1: sort by E-value "
                         "(Alignment) or seq.id. (Hamming)",
                         typeid(int), (void *)&sortResults, "^[0-1]{1}$",
                         MMseqsParameter::COMMAND_EXPERT),
      // result2msa
      PARAM_MSA_FORMAT_MODE(PARAM_MSA_FORMAT_MODE_ID, "--msa-format-mode",
                            "MSA format mode",
                            "Format MSA as: 0: binary cA3M DB\n1: binary ca3m "
                            "w. consensus DB\n2: aligned FASTA DB\n3: aligned "
                            "FASTA w. header summary\n4: STOCKHOLM flat file",
                            typeid(int), (void *)&msaFormatMode, "^[0-4]{1}$"),
      PARAM_ALLOW_DELETION(PARAM_ALLOW_DELETION_ID, "--allow-deletion",
                           "Allow deletions", "Allow deletions in a MSA",
                           typeid(bool), (void *)&allowDeletion, ""),
      PARAM_SUMMARY_PREFIX(PARAM_SUMMARY_PREFIX_ID, "--summary-prefix",
                           "Summary prefix", "Set the cluster summary prefix",
                           typeid(std::string), (void *)&summaryPrefix, "",
                           MMseqsParameter::COMMAND_EXPERT),
      PARAM_SKIP_QUERY(PARAM_SKIP_QUERY_ID, "--skip-query", "Skip query",
                       "Skip the query sequence", typeid(bool),
                       (void *)&skipQuery, "", MMseqsParameter::COMMAND_EXPERT),
      // convertmsa
      PARAM_IDENTIFIER_FIELD(
          PARAM_IDENTIFIER_FIELD_ID, "--identifier-field", "Identifier field",
          "Field from STOCKHOLM comments for choosing the MSA identifier: 0: "
          "ID, 1: AC. If the respective comment does not exist, the name of "
          "the first sequence will become the identifier",
          typeid(int), (void *)&identifierField, "^[0-1]{1}$",
          MMseqsParameter::COMMAND_COMMON),
      // msa2profile
      PARAM_MATCH_MODE(PARAM_MATCH_MODE_ID, "--match-mode", "Match mode",
                       "0: Columns that have a residue in the first sequence "
                       "are kept, 1: columns that have a residue in "
                       "--match-ratio of all sequences are kept",
                       typeid(int), (void *)&matchMode, "^(0|1)$",
                       MMseqsParameter::COMMAND_PROFILE),
      PARAM_MATCH_RATIO(
          PARAM_MATCH_RATIO_ID, "--match-ratio", "Match ratio",
          "Columns that have a residue in this ratio of all sequences are kept",
          typeid(float), (void *)&matchRatio, "^0(\\.[0-9]+)?|1(\\.0+)?$",
          MMseqsParameter::COMMAND_PROFILE),
      // result2profile
      PARAM_MASK_PROFILE(
          PARAM_MASK_PROFILE_ID, "--mask-profile", "Mask profile",
          "Mask query sequence of profile using tantan [0,1]", typeid(int),
          (void *)&maskProfile, "^[0-1]{1}$",
          MMseqsParameter::COMMAND_PROFILE | MMseqsParameter::COMMAND_EXPERT),
      PARAM_E_PROFILE(
          PARAM_E_PROFILE_ID, "--e-profile", "Profile E-value threshold",
          "Include sequences matches with < E-value thr. into the profile "
          "(>=0.0)",
          typeid(double), (void *)&evalProfile,
          "^([-+]?[0-9]*\\.?[0-9]+([eE][-+]?[0-9]+)?)|([0-9]*(\\.[0-9]+)?)$",
          MMseqsParameter::COMMAND_PROFILE),
      PARAM_FILTER_MSA(
          PARAM_FILTER_MSA_ID, "--filter-msa", "Filter MSA",
          "Filter msa: 0: do not filter, 1: filter", typeid(int),
          (void *)&filterMsa, "^[0-1]{1}$",
          MMseqsParameter::COMMAND_PROFILE | MMseqsParameter::COMMAND_EXPERT),
      PARAM_FILTER_MAX_SEQ_ID(
          PARAM_FILTER_MAX_SEQ_ID_ID, "--max-seq-id",
          "Maximum seq. id. threshold",
          "Reduce redundancy of output MSA using max. pairwise sequence "
          "identity [0.0,1.0]",
          typeid(float), (void *)&filterMaxSeqId, "^0(\\.[0-9]+)?|1(\\.0+)?$",
          MMseqsParameter::COMMAND_PROFILE | MMseqsParameter::COMMAND_EXPERT),
      PARAM_FILTER_QSC(
          PARAM_FILTER_QSC_ID, "--qsc", "Minimum score per column",
          "Reduce diversity of output MSAs using min. score per aligned "
          "residue with query sequences [-50.0,100.0]",
          typeid(float), (void *)&qsc, "^\\-*[0-9]*(\\.[0-9]+)?$",
          MMseqsParameter::COMMAND_PROFILE | MMseqsParameter::COMMAND_EXPERT),
      PARAM_FILTER_QID(
          PARAM_FILTER_QID_ID, "--qid", "Minimum seq. id.",
          "Reduce diversity of output MSAs using min.seq. identity with query "
          "sequences [0.0,1.0]",
          typeid(float), (void *)&qid, "^0(\\.[0-9]+)?|1(\\.0+)?$",
          MMseqsParameter::COMMAND_PROFILE | MMseqsParameter::COMMAND_EXPERT),
      PARAM_FILTER_COV(
          PARAM_FILTER_COV_ID, "--cov", "Minimum coverage",
          "Filter output MSAs using min. fraction of query residues covered by "
          "matched sequences [0.0,1.0]",
          typeid(float), (void *)&covMSAThr, "^0(\\.[0-9]+)?|1(\\.0+)?$",
          MMseqsParameter::COMMAND_PROFILE | MMseqsParameter::COMMAND_EXPERT),
      PARAM_FILTER_NDIFF(
          PARAM_FILTER_NDIFF_ID, "--diff", "Select N most diverse seqs",
          "Filter MSAs by selecting most diverse set of sequences, keeping at "
          "least this many seqs in each MSA block of length 50",
          typeid(int), (void *)&Ndiff, "^[1-9]{1}[0-9]*$",
          MMseqsParameter::COMMAND_PROFILE | MMseqsParameter::COMMAND_EXPERT),
      PARAM_WG(
          PARAM_WG_ID, "--wg", "Global sequence weighting",
          "Use global sequence weighting for profile calculation", typeid(bool),
          (void *)&wg, "",
          MMseqsParameter::COMMAND_PROFILE | MMseqsParameter::COMMAND_EXPERT),
      PARAM_PCA(
          PARAM_PCA_ID, "--pca", "Pseudo count a",
          "Pseudo count admixture strength", typeid(float), (void *)&pca,
          "^[0-9]*(\\.[0-9]+)?$",
          MMseqsParameter::COMMAND_PROFILE | MMseqsParameter::COMMAND_EXPERT),
      PARAM_PCB(
          PARAM_PCB_ID, "--pcb", "Pseudo count b",
          "Pseudo counts: Neff at half of maximum admixture (range 0.0-inf)",
          typeid(float), (void *)&pcb, "^[0-9]*(\\.[0-9]+)?$",
          MMseqsParameter::COMMAND_PROFILE | MMseqsParameter::COMMAND_EXPERT),
      // sequence2profile
      PARAM_NEFF(PARAM_NEFF_ID, "--neff", "Neff",
                 "Neff included into context state profile (1.0,20.0)",
                 typeid(float), (void *)&neff, "^[0-9]*(\\.[0-9]+)?$",
                 MMseqsParameter::COMMAND_PROFILE),
      PARAM_TAU(PARAM_TAU_ID, "--tau", "Tau",
                "Tau: context state pseudo count mixture (0.0,1.0)",
                typeid(float), (void *)&tau, "[0-9]*(\\.[0-9]+)?$",
                MMseqsParameter::COMMAND_PROFILE),
      // createtsv
      PARAM_TARGET_COLUMN(
          PARAM_TARGET_COLUMN_ID, "--target-column", "Target column",
          "Select a target column (default 1), 0 if no target id exists",
          typeid(int), (void *)&targetTsvColumn, "^[0-9]*$"),
      PARAM_FIRST_SEQ_REP_SEQ(PARAM_FIRST_SEQ_REP_SEQ_ID, "--first-seq-as-repr",
                              "First sequence as representative",
                              "Use the first sequence of the clustering result "
                              "as representative sequence",
                              typeid(bool), (void *)&firstSeqRepr, "",
                              MMseqsParameter::COMMAND_MISC),
      PARAM_FULL_HEADER(PARAM_FULL_HEADER_ID, "--full-header",
                        "Add full header",
                        "Replace DB ID by its corresponding Full Header",
                        typeid(bool), (void *)&fullHeader, ""),
      PARAM_IDX_SEQ_SRC(
          PARAM_IDX_SEQ_SRC_ID, "--idx-seq-src", "Sequence source",
          "0: auto, 1: split/translated sequences, 2: input sequences",
          typeid(int), (void *)&idxSeqSrc, "^[0-2]{1}$",
          MMseqsParameter::COMMAND_MISC),

      // result2stats
      PARAM_STAT(
          PARAM_STAT_ID, "--stat", "Statistics to be computed",
          "One of: linecount, mean, doolittle, charges, seqlen, firstline",
          typeid(std::string), (void *)&stat, ""),
      // linearcluster
      PARAM_KMER_PER_SEQ(
          PARAM_KMER_PER_SEQ_ID, "--kmer-per-seq", "k-mers per sequence",
          "k-mers per sequence", typeid(int), (void *)&kmersPerSequence,
          "^[1-9]{1}[0-9]*$", MMseqsParameter::COMMAND_CLUSTLINEAR),
      PARAM_KMER_PER_SEQ_SCALE(
          PARAM_KMER_PER_SEQ_SCALE_ID, "--kmer-per-seq-scale",
          "Scale k-mers per sequence",
          "Scale k-mer per sequence based on sequence length as kmer-per-seq "
          "val + scale x seqlen",
          typeid(MultiParam<float>), (void *)&kmersPerSequenceScale,
          "^0(\\.[0-9]+)?|1(\\.0+)?$", MMseqsParameter::COMMAND_CLUSTLINEAR),
      PARAM_INCLUDE_ONLY_EXTENDABLE(
          PARAM_INCLUDE_ONLY_EXTENDABLE_ID, "--include-only-extendable",
          "Include only extendable", "Include only extendable", typeid(bool),
          (void *)&includeOnlyExtendable, "",
          MMseqsParameter::COMMAND_CLUSTLINEAR),
      PARAM_IGNORE_MULTI_KMER(PARAM_IGNORE_MULTI_KMER_ID, "--ignore-multi-kmer",
                              "Skip repeating k-mers",
                              "Skip k-mers occurring multiple times (>=2)",
                              typeid(bool), (void *)&ignoreMultiKmer, "",
                              MMseqsParameter::COMMAND_CLUSTLINEAR |
                                  MMseqsParameter::COMMAND_EXPERT),
      PARAM_HASH_SHIFT(PARAM_HASH_SHIFT_ID, "--hash-shift", "Shift hash",
                       "Shift k-mer hash initialization", typeid(int),
                       (void *)&hashShift, "^[1-9]{1}[0-9]*$",
                       MMseqsParameter::COMMAND_CLUSTLINEAR |
                           MMseqsParameter::COMMAND_EXPERT),
      PARAM_PICK_N_SIMILAR(PARAM_PICK_N_SIMILAR_ID, "--pick-n-sim-kmer",
                           "Add N similar to search",
                           "Add N similar k-mers to search", typeid(int),
                           (void *)&pickNbest, "^[1-9]{1}[0-9]*$",
                           MMseqsParameter::COMMAND_CLUSTLINEAR |
                               MMseqsParameter::COMMAND_EXPERT),
      PARAM_ADJUST_KMER_LEN(
          PARAM_ADJUST_KMER_LEN_ID, "--adjust-kmer-len", "Adjust k-mer length",
          "Adjust k-mer length based on specificity (only for nucleotides)",
          typeid(bool), (void *)&adjustKmerLength, "",
          MMseqsParameter::COMMAND_CLUSTLINEAR |
              MMseqsParameter::COMMAND_EXPERT),
      PARAM_RESULT_DIRECTION(
          PARAM_RESULT_DIRECTION_ID, "--result-direction", "Result direction",
          "result is 0: query, 1: target centric", typeid(int),
          (void *)&resultDirection, "^[0-1]{1}$",
          MMseqsParameter::COMMAND_CLUSTLINEAR |
              MMseqsParameter::COMMAND_EXPERT),

      // workflow
      PARAM_RUNNER(
          PARAM_RUNNER_ID, "--mpi-runner", "MPI runner",
          "Use MPI on compute cluster with this MPI command (e.g. \"mpirun -np "
          "42\")",
          typeid(std::string), (void *)&runner, "",
          MMseqsParameter::COMMAND_COMMON | MMseqsParameter::COMMAND_EXPERT),
      PARAM_REUSELATEST(
          PARAM_REUSELATEST_ID, "--force-reuse",
          "Force restart with latest tmp",
          "Reuse tmp filse in tmp/latest folder ignoring parameters and "
          "version changes",
          typeid(bool), (void *)&reuseLatest, "",
          MMseqsParameter::COMMAND_COMMON | MMseqsParameter::COMMAND_EXPERT),
      // search workflow
      PARAM_NUM_ITERATIONS(
          PARAM_NUM_ITERATIONS_ID, "--num-iterations", "Search iterations",
          "Number of iterative profile search iterations", typeid(int),
          (void *)&numIterations, "^[1-9]{1}[0-9]*$",
          MMseqsParameter::COMMAND_PROFILE),
      PARAM_START_SENS(PARAM_START_SENS_ID, "--start-sens", "Start sensitivity",
                       "Start sensitivity", typeid(float), (void *)&startSens,
                       "^[0-9]*(\\.[0-9]+)?$"),
      PARAM_SENS_STEPS(
          PARAM_SENS_STEPS_ID, "--sens-steps", "Search steps",
          "Number of search steps performed from --start-sens to -s",
          typeid(int), (void *)&sensSteps, "^[1-9]{1}$"),
      PARAM_SLICE_SEARCH(
          PARAM_SLICE_SEARCH_ID, "--slice-search", "Slice search mode",
          "For bigger profile DB, run iteratively the search by greedily "
          "swapping the search results",
          typeid(bool), (void *)&sliceSearch, "",
          MMseqsParameter::COMMAND_PROFILE | MMseqsParameter::COMMAND_EXPERT),
      PARAM_STRAND(PARAM_STRAND_ID, "--strand", "Strand selection",
                   "Strand selection only works for DNA/DNA search 0: reverse, "
                   "1: forward, 2: both",
                   typeid(int), (void *)&strand, "^[0-2]{1}$",
                   MMseqsParameter::COMMAND_EXPERT),
      PARAM_ORF_FILTER(PARAM_ORF_FILTER_ID, "--orf-filter", "ORF filter",
                       "Prefilter query ORFs with non-selective before search",
                       typeid(int), (void *)&orfFilter, "^[0-1]{1}$"),
      PARAM_ORF_FILTER_S(
          PARAM_ORF_FILTER_S_ID, "--orf-filter-s", "ORF filter sensitivity",
          "Sensitivity used for query ORF prefiltering", typeid(float),
          (void *)&orfFilterSens, "^[0-9]*(\\.[0-9]+)?$"),
      PARAM_ORF_FILTER_E(
          PARAM_ORF_FILTER_E_ID, "--orf-filter-e", "ORF filter e-value",
          "E-value threshold used for query ORF prefiltering", typeid(double),
          (void *)&orfFilterEval,
          "^([-+]?[0-9]*\\.?[0-9]+([eE][-+]?[0-9]+)?)|[0-9]*(\\.[0-9]+)?$"),
      PARAM_LCA_SEARCH(
          PARAM_LCA_SEARCH_ID, "--lca-search", "LCA search mode",
          "Efficient search for LCA candidates", typeid(bool),
          (void *)&lcaSearch, "",
          MMseqsParameter::COMMAND_PROFILE | MMseqsParameter::COMMAND_EXPERT),
      // easysearch
      PARAM_GREEDY_BEST_HITS(PARAM_GREEDY_BEST_HITS_ID, "--greedy-best-hits",
                             "Greedy best hits",
                             "Choose the best hits greedily to cover the query",
                             typeid(bool), (void *)&greedyBestHits, ""),
      // extractorfs
      PARAM_ORF_MIN_LENGTH(
          PARAM_ORF_MIN_LENGTH_ID, "--min-length", "Min codons in orf",
          "Minimum codon number in open reading frames", typeid(int),
          (void *)&orfMinLength, "^[1-9]{1}[0-9]*$"),
      PARAM_ORF_MAX_LENGTH(
          PARAM_ORF_MAX_LENGTH_ID, "--max-length", "Max codons in length",
          "Maximum codon number in open reading frames", typeid(int),
          (void *)&orfMaxLength, "^[1-9]{1}[0-9]*$"),
      PARAM_ORF_MAX_GAP(PARAM_ORF_MAX_GAP_ID, "--max-gaps", "Max orf gaps",
                        "Maximum number of codons with gaps or unknown "
                        "residues before an open reading frame is rejected",
                        typeid(int), (void *)&orfMaxGaps,
                        "^(0|[1-9]{1}[0-9]*)$"),
      PARAM_CONTIG_START_MODE(
          PARAM_CONTIG_START_MODE_ID, "--contig-start-mode",
          "Contig start mode",
          "Contig start can be 0: incomplete, 1: complete, 2: both",
          typeid(int), (void *)&contigStartMode, "^[0-2]{1}"),
      PARAM_CONTIG_END_MODE(
          PARAM_CONTIG_END_MODE_ID, "--contig-end-mode", "Contig end mode",
          "Contig end can be 0: incomplete, 1: complete, 2: both", typeid(int),
          (void *)&contigEndMode, "^[0-2]{1}"),
      PARAM_ORF_START_MODE(
          PARAM_ORF_START_MODE_ID, "--orf-start-mode", "Orf start mode",
          "Orf fragment can be 0: from start to stop, 1: from any to stop, 2: "
          "from last encountered start to stop (no start in the middle)",
          typeid(int), (void *)&orfStartMode, "^[0-2]{1}"),
      PARAM_ORF_FORWARD_FRAMES(PARAM_ORF_FORWARD_FRAMES_ID, "--forward-frames",
                               "Forward frames",
                               "Comma-separated list of frames on the forward "
                               "strand to be extracted",
                               typeid(std::string), (void *)&forwardFrames, ""),
      PARAM_ORF_REVERSE_FRAMES(PARAM_ORF_REVERSE_FRAMES_ID, "--reverse-frames",
                               "Reverse frames",
                               "Comma-separated list of frames on the reverse "
                               "strand to be extracted",
                               typeid(std::string), (void *)&reverseFrames, ""),
      PARAM_USE_ALL_TABLE_STARTS(PARAM_USE_ALL_TABLE_STARTS_ID,
                                 "--use-all-table-starts",
                                 "Use all table starts",
                                 "Use all alternatives for a start codon in "
                                 "the genetic table, if false - only ATG (AUG)",
                                 typeid(bool), (void *)&useAllTableStarts, ""),
      PARAM_TRANSLATE(PARAM_TRANSLATE_ID, "--translate", "Translate orf",
                      "Translate ORF to amino acid", typeid(int),
                      (void *)&translate, "^[0-1]{1}"),
      PARAM_CREATE_LOOKUP(
          PARAM_CREATE_LOOKUP_ID, "--create-lookup", "Create lookup",
          "Create database lookup file (can be very large)", typeid(int),
          (void *)&createLookup, "^[0-1]{1}", MMseqsParameter::COMMAND_EXPERT),
      // indexdb
      PARAM_CHECK_COMPATIBLE(
          PARAM_CHECK_COMPATIBLE_ID, "--check-compatible", "Check compatible",
          "0: Always recreate index, 1: Check if recreating index is needed, "
          "2: Fail if index is incompatible",
          typeid(int), (void *)&checkCompatible, "^[0-2]{1}$",
          MMseqsParameter::COMMAND_MISC),
      PARAM_SEARCH_TYPE(PARAM_SEARCH_TYPE_ID, "--search-type", "Search type",
                        "Search type 0: auto 1: amino acid, 2: translated, 3: "
                        "nucleotide, 4: translated nucleotide alignment",
                        typeid(int), (void *)&searchType, "^[0-4]{1}"),
      // createdb
      PARAM_USE_HEADER(PARAM_USE_HEADER_ID, "--use-fasta-header",
                       "Use fasta header",
                       "Use the id parsed from the fasta header as the index "
                       "key instead of using incrementing numeric identifiers",
                       typeid(bool), (void *)&useHeader, ""),
      PARAM_ID_OFFSET(
          PARAM_ID_OFFSET_ID, "--id-offset", "Offset of numeric ids",
          "Numeric ids in index file are offset by this value", typeid(int),
          (void *)&identifierOffset, "^(0|[1-9]{1}[0-9]*)$"),
      PARAM_DB_TYPE(PARAM_DB_TYPE_ID, "--dbtype", "Database type",
                    "Database type 0: auto, 1: amino acid 2: nucleotides",
                    typeid(int), (void *)&dbType, "[0-2]{1}"),
      PARAM_CREATEDB_MODE(
          PARAM_CREATEDB_MODE_ID, "--createdb-mode", "Createdb mode",
          "Createdb mode 0: copy data, 1: soft link data and write new index "
          "(works only with single line fasta/q)",
          typeid(int), (void *)&createdbMode, "^[0-1]{1}$"),
      PARAM_SHUFFLE(PARAM_SHUFFLE_ID, "--shuffle", "Shuffle input database",
                    "Shuffle input database", typeid(bool),
                    (void *)&shuffleDatabase, ""),
      PARAM_WRITE_LOOKUP(PARAM_WRITE_LOOKUP_ID, "--write-lookup",
                         "Write lookup file",
                         "write .lookup file containing mapping from internal "
                         "id, fasta id and file number",
                         typeid(int), (void *)&writeLookup, "^[0-1]{1}",
                         MMseqsParameter::COMMAND_EXPERT),
      PARAM_USE_HEADER_FILE(PARAM_USE_HEADER_FILE_ID, "--use-header-file",
                            "Use header DB",
                            "use the sequence header DB instead of the body to "
                            "map the entry keys",
                            typeid(bool), (void *)&useHeaderFile, ""),
      // splitsequence
      PARAM_SEQUENCE_OVERLAP(PARAM_SEQUENCE_OVERLAP_ID, "--sequence-overlap",
                             "Overlap between sequences",
                             "Overlap between sequences", typeid(int),
                             (void *)&sequenceOverlap, "^(0|[1-9]{1}[0-9]*)$"),
      PARAM_SEQUENCE_SPLIT_MODE(PARAM_SEQUENCE_SPLIT_MODE_ID,
                                "--sequence-split-mode", "Sequence split mode",
                                "Sequence split mode 0: copy data, 1: soft "
                                "link data and write new index,",
                                typeid(int), (void *)&sequenceSplitMode,
                                "^[0-1]{1}$"),
      PARAM_HEADER_SPLIT_MODE(
          PARAM_HEADER_SPLIT_MODE_ID, "--headers-split-mode",
          "Header split mode",
          "Header split mode: 0: split position, 1: original header",
          typeid(int), (void *)&headerSplitMode, "^[0-1]{1}$"),
      // gff2db
      PARAM_GFF_TYPE(PARAM_GFF_TYPE_ID, "--gff-type", "GFF type",
                     "Type in the GFF file to filter by", typeid(std::string),
                     (void *)&gffType, ""),
      // translatenucs
      PARAM_TRANSLATION_TABLE(
          PARAM_TRANSLATION_TABLE_ID, "--translation-table",
          "Translation table",
          "1) CANONICAL, 2) VERT_MITOCHONDRIAL, 3) YEAST_MITOCHONDRIAL, 4) "
          "MOLD_MITOCHONDRIAL, 5) INVERT_MITOCHONDRIAL, 6) CILIATE\n9) "
          "FLATWORM_MITOCHONDRIAL, 10) EUPLOTID, 11) PROKARYOTE, 12) "
          "ALT_YEAST, 13) ASCIDIAN_MITOCHONDRIAL, 14) "
          "ALT_FLATWORM_MITOCHONDRIAL\n15) BLEPHARISMA, 16) "
          "CHLOROPHYCEAN_MITOCHONDRIAL, 21) TREMATODE_MITOCHONDRIAL, 22) "
          "SCENEDESMUS_MITOCHONDRIAL\n23) THRAUSTOCHYTRIUM_MITOCHONDRIAL, 24) "
          "PTEROBRANCHIA_MITOCHONDRIAL, 25) GRACILIBACTERIA, 26) PACHYSOLEN, "
          "27) KARYORELICT, 28) CONDYLOSTOMA\n 29) MESODINIUM, 30) PERTRICH, "
          "31) BLASTOCRITHIDIA",
          typeid(int), (void *)&translationTable, "^[1-9]{1}[0-9]*$",
          MMseqsParameter::COMMAND_MISC | MMseqsParameter::COMMAND_EXPERT),
      // createseqfiledb
      PARAM_ADD_ORF_STOP(PARAM_ADD_ORF_STOP_ID, "--add-orf-stop",
                         "Add orf stop",
                         "Add stop codon '*' at complete start and end",
                         typeid(bool), (void *)&addOrfStop, ""),
      // createseqfiledb
      PARAM_MIN_SEQUENCES(
          PARAM_MIN_SEQUENCES_ID, "--min-sequences", "Min sequences",
          "Minimum number of sequences a cluster may contain", typeid(int),
          (void *)&minSequences, "^[1-9]{1}[0-9]*$"),
      PARAM_MAX_SEQUENCES(
          PARAM_MAX_SEQUENCES_ID, "--max-sequences", "Max sequences",
          "Maximum number of sequences a cluster may contain", typeid(int),
          (void *)&maxSequences, "^[1-9]{1}[0-9]*$"),
      PARAM_HH_FORMAT(
          PARAM_HH_FORMAT_ID, "--hh-format", "HH format",
          "Format entries to use with hhsuite (for singleton clusters)",
          typeid(bool), (void *)&hhFormat, ""),
      // filterdb
      PARAM_FILTER_COL(PARAM_FILTER_COL_ID, "--filter-column", "Filter column",
                       "column", typeid(int), (void *)&filterColumn,
                       "^[1-9]{1}[0-9]*$"),
      PARAM_COLUMN_TO_TAKE(
          PARAM_COLUMN_TO_TAKE_ID, "--column-to-take", "Column to take",
          "column to take in join mode. If -1, the whole line is taken",
          typeid(int), (void *)&columnToTake, "^(-1|0|[1-9]{1}[0-9]*)$"),
      PARAM_FILTER_REGEX(
          PARAM_FILTER_REGEX_ID, "--filter-regex", "Filter regex",
          "Regex to select column (example float: [0-9]*(.[0-9]+)? "
          "int:[1-9]{1}[0-9])",
          typeid(std::string), (void *)&filterColumnRegex, "^.*$"),
      PARAM_FILTER_POS(PARAM_FILTER_POS_ID, "--positive-filter",
                       "Positive filter",
                       "Used in conjunction with --filter-file. If true, out  "
                       "= in \\intersect filter ; if false, out = in - filter",
                       typeid(bool), (void *)&positiveFilter, ""),
      PARAM_FILTER_FILE(PARAM_FILTER_FILE_ID, "--filter-file", "Filter file",
                        "Specify a file that contains the filtering elements",
                        typeid(std::string), (void *)&filteringFile, ""),
      PARAM_FILTER_EXPRESSION(
          PARAM_FILTER_EXPRESSION_ID, "--filter-expression",
          "Filter expression",
          "Specify a mathematical expression to filter lines",
          typeid(std::string), (void *)&filterExpression, ""),
      PARAM_MAPPING_FILE(PARAM_MAPPING_FILE_ID, "--mapping-file",
                         "Mapping file",
                         "Specify a file that translates the keys of a DB to "
                         "new keys, TSV format",
                         typeid(std::string), (void *)&mappingFile, ""),
      PARAM_TRIM_TO_ONE_COL(
          PARAM_TRIM_TO_ONE_COL_ID, "--trim-to-one-column",
          "Trim to one column",
          "Output only the column specified by --filter-column", typeid(bool),
          (void *)&trimToOneColumn, ""),
      PARAM_EXTRACT_LINES(PARAM_EXTRACT_LINES_ID, "--extract-lines",
                          "Extract N lines", "Extract n lines of each entry",
                          typeid(int), (void *)&extractLines,
                          "^[1-9]{1}[0-9]*$"),
      PARAM_COMP_OPERATOR(
          PARAM_COMP_OPERATOR_ID, "--comparison-operator",
          "Numerical comparison operator",
          "Filter by comparing each entry row numerically by using the le) "
          "less-than-equal, ge) greater-than-equal or e) equal operator",
          typeid(std::string), (void *)&compOperator, ""),
      PARAM_COMP_VALUE(PARAM_COMP_VALUE_ID, "--comparison-value",
                       "Numerical comparison value",
                       "Filter by comparing each entry to this value",
                       typeid(double), (void *)&compValue, "^.*$"),
      PARAM_SORT_ENTRIES(
          PARAM_SORT_ENTRIES_ID, "--sort-entries", "Sort entries",
          "Sort column set by --filter-column, by 0: no sorting, 1: "
          "increasing, 2: decreasing, 3: random shuffle",
          typeid(int), (void *)&sortEntries, "^[1-9]{1}[0-9]*$"),
      PARAM_BEATS_FIRST(PARAM_BEATS_FIRST_ID, "--beats-first", "Beats first",
                        "Filter by comparing each entry to the first entry",
                        typeid(bool), (void *)&beatsFirst, ""),
      PARAM_JOIN_DB(PARAM_JOIN_DB_ID, "--join-db", "join to DB",
                    "Join another database entry with respect to the database "
                    "identifier in the chosen column",
                    typeid(std::string), (void *)&joinDB, ""),
      // besthitperset
      PARAM_SIMPLE_BEST_HIT(PARAM_SIMPLE_BEST_HIT_ID, "--simple-best-hit",
                            "Use simple best hit",
                            "Update the p-value by a single best hit, or by "
                            "best and second best hits",
                            typeid(bool), (void *)&simpleBestHit, ""),
      PARAM_ALPHA(PARAM_ALPHA_ID, "--alpha", "Alpha",
                  "Set alpha for combining p-values during aggregation",
                  typeid(float), (void *)&alpha, "^0(\\.[0-9]+)?|^1(\\.0+)?$"),
      PARAM_SHORT_OUTPUT(
          PARAM_SHORT_OUTPUT_ID, "--short-output", "Short output",
          "The output database will contain only the spread p-value",
          typeid(bool), (void *)&shortOutput, ""),
      PARAM_AGGREGATION_MODE(
          PARAM_AGGREGATION_MODE_ID, "--aggregation-mode", "Aggregation mode",
          "Combined P-values computed from 0: multi-hit, 1: minimum of all "
          "P-values, 2: product-of-P-values, 3: truncated product",
          typeid(int), (void *)&aggregationMode, "^[0-4]{1}$"),
      // concatdb
      PARAM_PRESERVEKEYS(PARAM_PRESERVEKEYS_ID, "--preserve-keys",
                         "Preserve the keys",
                         "The keys of the two DB should be distinct, and they "
                         "will be preserved in the concatenation",
                         typeid(bool), (void *)&preserveKeysB, ""),
      PARAM_TAKE_LARGER_ENTRY(
          PARAM_TAKE_LARGER_ENTRY_ID, "--take-larger-entry",
          "Take the larger entry",
          "Only keep the larger entry (dataSize >) in the concatenation, both "
          "databases need the same keys in the index",
          typeid(bool), (void *)&takeLargerEntry, ""),
      // offsetalignment
      PARAM_CHAIN_ALIGNMENT(PARAM_CHAIN_ALIGNMENT_ID, "--chain-alignments",
                            "Chain overlapping alignments",
                            "Chain overlapping alignments", typeid(int),
                            (void *)&chainAlignment, "^[0-1]{1}",
                            MMseqsParameter::COMMAND_EXPERT),
      PARAM_MERGE_QUERY(PARAM_MERGE_QUERY_ID, "--merge-query", "Merge query",
                        "Combine ORFs/split sequences to a single entry",
                        typeid(int), (void *)&mergeQuery, "^[0-1]{1}",
                        MMseqsParameter::COMMAND_EXPERT),
      // tsv2db
      PARAM_OUTPUT_DBTYPE(
          PARAM_OUTPUT_DBTYPE_ID, "--output-dbtype", "Output database type",
          "Set database type for resulting database: Amino acid sequences 0, "
          "Nucl. seq. 1, Profiles 2, Alignment result 5, Clustering result 6, "
          "Prefiltering result 7, Taxonomy result 8, Indexed database 9, cA3M "
          "MSAs 10, FASTA or A3M MSAs 11, Generic database 12, Omit dbtype "
          "file 13, Bi-directional prefiltering result 14, Offsetted headers "
          "15",
          typeid(int), (void *)&outputDbType, "^(0|[1-9]{1}[0-9]*)$"),
      // diff
      PARAM_USESEQID(PARAM_USESEQID_ID, "--use-seq-id",
                     "Match sequences by their ID",
                     "Sequence ID (Uniprot, GenBank, ...) is used for "
                     "identifying matches between the old and the new DB",
                     typeid(bool), (void *)&useSequenceId, ""),
      // prefixid
      PARAM_PREFIX(PARAM_PREFIX_ID, "--prefix", "Prefix",
                   "Use this prefix for all entries", typeid(std::string),
                   (void *)&prefix, ""),
      PARAM_TSV(PARAM_TSV_ID, "--tsv", "Tsv", "Return output in TSV format",
                typeid(bool), (void *)&tsvOut, ""),
      // summarize headers
      PARAM_HEADER_TYPE(PARAM_HEADER_TYPE_ID, "--header-type", "Header type",
                        "Header Type: 1: Uniclust, 2: Metaclust", typeid(int),
                        (void *)&headerType, "[1-2]{1}"),
      // mergedbs
      PARAM_MERGE_PREFIXES(PARAM_MERGE_PREFIXES_ID, "--prefixes",
                           "Merge prefixes",
                           "Comma separated list of prefixes for each entry",
                           typeid(std::string), (void *)&mergePrefixes, "",
                           MMseqsParameter::COMMAND_EXPERT),
      PARAM_MERGE_STOP_EMPTY(
          PARAM_MERGE_STOP_EMPTY_ID, "--merge-stop-empty",
          "Stop merge after empty",
          "Don't continue merging entries after an empty entry", typeid(bool),
          (void *)&mergeStopEmpty, "", MMseqsParameter::COMMAND_EXPERT),
      // summarizeresult
      PARAM_OVERLAP(PARAM_OVERLAP_ID, "--overlap", "Overlap threshold",
                    "Maximum overlap of covered regions", typeid(float),
                    (void *)&overlap, "^[0-9]*(\\.[0-9]+)?$"),
      // msa2profile
      PARAM_MSA_TYPE(PARAM_MSA_TYPE_ID, "--msa-type", "MSA type",
                     "MSA Type: 0: cA3M, 1: A3M, 2: FASTA", typeid(int),
                     (void *)&msaType, "^[0-2]{1}$"),
      // extractalignedregion
      PARAM_EXTRACT_MODE(PARAM_EXTRACT_MODE_ID, "--extract-mode",
                         "Extract mode", "Extract from 1: Query, 2: Target",
                         typeid(int), (void *)&extractMode, "^[1-2]{1}$"),
      // convertkb
      PARAM_KB_COLUMNS(PARAM_KB_COLUMNS_ID, "--kb-columns", "UniprotKB columns",
                       "list of indices of UniprotKB columns to be extracted",
                       typeid(std::string), (void *)&kbColumns, ""),
      PARAM_RECOVER_DELETED(
          PARAM_RECOVER_DELETED_ID, "--recover-deleted", "Recover deleted",
          "Find and recover deleted sequences during updating of clustering",
          typeid(bool), (void *)&recoverDeleted, ""),
      // filtertaxdb
      PARAM_TAXON_LIST(PARAM_TAXON_LIST_ID, "--taxon-list", "Selected taxa",
                       "Taxonomy ID, possibly multiple values separated by ','",
                       typeid(std::string), (void *)&taxonList, ""),
      // view
      PARAM_ID_LIST(PARAM_ID_LIST_ID, "--id-list", "Selected entries with key",
                    "Entries to be printed separated by ','",
                    typeid(std::string), (void *)&idList, ""),
      PARAM_IDX_ENTRY_TYPE(
          PARAM_IDX_ENTRY_TYPE_ID, "--idx-entry-type", "Index entry type",
          "0: sequence, 1: src sequence, 2: header, 3: src header", typeid(int),
          (void *)&idxEntryType, "^[0-3]{1}$"),
      // lca and addtaxonomy
      PARAM_PICK_ID_FROM(PARAM_PICK_ID_FROM_ID, "--pick-id-from",
                         "Extract mode", "Query 1, Target 2", typeid(int),
                         (void *)&pickIdFrom, "^[1-2]{1}$"),
      PARAM_LCA_RANKS(PARAM_LCA_RANKS_ID, "--lca-ranks", "LCA ranks",
                      "Add column with specified ranks (',' separated)",
                      typeid(std::string), (void *)&lcaRanks, ""),
      PARAM_BLACKLIST(PARAM_BLACKLIST_ID, "--blacklist", "Taxon blacklist",
                      "Comma separated list of ignored taxa in LCA computation",
                      typeid(std::string), (void *)&blacklist,
                      "([0-9]+,)?[0-9]+"),
      PARAM_TAXON_ADD_LINEAGE(
          PARAM_TAXON_ADD_LINEAGE_ID, "--tax-lineage",
          "Column with taxonomic lineage",
          "0: don't show, 1: add all lineage names, 2: add all lineage taxids",
          typeid(int), (void *)&showTaxLineage, "^[0-2]{1}$"),
      // aggregatetax
      PARAM_MAJORITY(PARAM_MAJORITY_ID, "--majority", "Majority threshold",
                     "minimal fraction of agreement among taxonomically "
                     "assigned sequences of a set",
                     typeid(float), (void *)&majorityThr,
                     "^0(\\.[0-9]+)?|^1(\\.0+)?$"),
      PARAM_VOTE_MODE(PARAM_VOTE_MODE_ID, "--vote-mode", "Vote mode",
                      "Mode of assigning weights to compute majority. 0: "
                      "uniform, 1: minus log E-value, 2: score",
                      typeid(int), (void *)&voteMode, "^[0-2]{1}$"),
      // taxonomyreport
      PARAM_REPORT_MODE(PARAM_REPORT_MODE_ID, "--report-mode", "Report mode",
                        "Taxonomy report mode 0: Kraken 1: Krona", typeid(int),
                        (void *)&reportMode, "^[0-1]{1}$"),
      // createtaxdb
      PARAM_NCBI_TAX_DUMP(
          PARAM_NCBI_TAX_DUMP_ID, "--ncbi-tax-dump", "NCBI tax dump directory",
          "NCBI tax dump directory. The tax dump can be downloaded here "
          "\"ftp://ftp.ncbi.nih.gov/pub/taxonomy/taxdump.tar.gz\"",
          typeid(std::string), (void *)&ncbiTaxDump, ""),
      PARAM_TAX_MAPPING_FILE(
          PARAM_TAX_MAPPING_FILE_ID, "--tax-mapping-file",
          "Taxonomy mapping file",
          "File to map sequence identifier to taxonomical identifier",
          typeid(std::string), (void *)&taxMappingFile, ""),
      PARAM_TAX_MAPPING_MODE(PARAM_TAX_MAPPING_MODE_ID, "--tax-mapping-mode",
                             "Taxonomy mapping mode",
                             "Map taxonomy based on sequence database 0: "
                             ".lookup file 1: .source file",
                             typeid(int), (void *)&taxMappingMode,
                             "^[0-1]{1}$"),
      PARAM_TAX_DB_MODE(PARAM_TAX_DB_MODE_ID, "--tax-db-mode",
                        "Taxonomy db mode",
                        "Create taxonomy database as: 0: .dmp flat files "
                        "(human readable) 1: binary dump (faster readin)",
                        typeid(int), (void *)&taxDbMode, "^[0-1]{1}$"),
      // expandaln
      PARAM_EXPANSION_MODE(
          PARAM_EXPANSION_MODE_ID, "--expansion-mode", "Expansion mode",
          "Update score, E-value, and sequence identity by 0: input alignment "
          "1: rescoring the inferred backtrace",
          typeid(int), (void *)&expansionMode, "^[0-2]{1}$"),
      // taxonomy
      PARAM_LCA_MODE(
          PARAM_LCA_MODE_ID, "--lca-mode", "LCA mode",
          "LCA Mode 1: single search LCA , 2/3: accelerated 2bLCA, 4: top hit",
          typeid(int), (void *)&taxonomySearchMode, "^[1-4]{1}$"),
      PARAM_TAX_OUTPUT_MODE(
          PARAM_TAX_OUTPUT_MODE_ID, "--tax-output-mode", "Taxonomy output mode",
          "0: output LCA, 1: output alignment 2: output both", typeid(int),
          (void *)&taxonomyOutputMode, "^[0-2]{1}$"),
      // createsubdb, filtertaxseqdb
      PARAM_SUBDB_MODE(
          PARAM_SUBDB_MODE_ID, "--subdb-mode", "Subdb mode",
          "Subdb mode 0: copy data 1: soft link data and write index",
          typeid(int), (void *)&subDbMode, "^[0-1]{1}$"),
      PARAM_TAR_INCLUDE(PARAM_TAR_INCLUDE_ID, "--tar-include",
                        "Tar Inclusion Regex",
                        "Include file names based on this regex",
                        typeid(std::string), (void *)&tarInclude, "^.*$"),
      PARAM_TAR_EXCLUDE(PARAM_TAR_EXCLUDE_ID, "--tar-exclude",
                        "Tar Exclusion Regex",
                        "Exclude file names based on this regex",
                        typeid(std::string), (void *)&tarExclude, "^.*$"),
      // for modules that should handle -h themselves
      PARAM_HELP(PARAM_HELP_ID, "-h", "Help", "Help", typeid(bool),
                 (void *)&help, "", MMseqsParameter::COMMAND_HIDDEN),
      PARAM_HELP_LONG(PARAM_HELP_LONG_ID, "--help", "Help", "Help",
  ...
  // needed for slice search, however all its parameters are already present in
  // searchworkflow searchworkflow = combineList(searchworkflow, sortresult);
...
// void Parameters::printUsageMessage(const Command& command, const unsigned int
// outputFlag, const char* extraText) {
//    const std::vector<MMseqsParameter*>& parameters = *command.params;
//    std::ostringstream ss;
//    ss << "usage: " << binary_name << " " << command.cmd << " " <<
//    command.usage << (parameters.size() > 0 ? " [options]" : "") << "\n"; if
//    (extraText != NULL) {
//        ss << extraText;
//    }
...
//            if (par->category & categories[i].category && (printExpert ||
//            isExpert == false)) {
//                maxParamWidth = std::max(strlen(parameters[j]->name),
//                maxParamWidth);
//            }
//        }
//    }
...
//            if (par->category & categories[i].category && (printExpert ||
//            isExpert == false ) && alreadyPrint == false ) {
...
//            }
//            ss << paramString << std::string(maxParamWidth <
//            paramString.size()? 1 : maxParamWidth - paramString.size(), ' ');
//
//            if(printHeader==true){
//                //ss << " ";
...
//                paramString.clear();
//                const MMseqsParameter * par = parameters[j];
//                bool isExpert = (par->category &
//                MMseqsParameter::COMMAND_EXPERT); hasExpert |= isExpert; bool
//                alreadyPrint = alreadyPrintMap[par->uniqid]; if (par->category
//                & categories[i].category && (printExpert || isExpert == false)
//                && alreadyPrint == false ) {
...
//                        valueString = ByteParser::format(*((size_t *)
//                        par->value), 'a', 'h');
...
//                        valueString =
//                        MultiParam<char*>::format(*((MultiParam<char*> *)
//                        par->value));
//                    } else if (par->type == typeid(MultiParam<int>)) {
//                        paramString.append(" TWIN"); //nucl:VAL,aa:VAL"
//                        valueString =
//                        MultiParam<int>::format(*((MultiParam<int> *)
//                        par->value));
//                    } else if (par->type == typeid(MultiParam<float>)) {
//                        paramString.append(" TWIN"); //nucl:VAL,aa:VAL"
//                        valueString =
//                        MultiParam<float>::format(*((MultiParam<float> *)
//                        par->value));
//                    }
//
//                    ss << " " << paramString << std::string(maxParamWidth <
//                    paramString.size()? 1 : maxParamWidth -
//                    paramString.size(), ' ');
//
//                    ss << " ";
...
//        ss << "\nreferences:\n";
//        for (unsigned int pos = 0; pos != sizeof(command.citations) *
//        CHAR_BIT; ++pos) {
//            unsigned int citation = 1 << pos;
//            if (command.citations & citation && citations.find(citation) !=
//            citations.end()) {
...
//        ss << "\n" << "Show an extended list of options by calling '" <<
//        binary_name << " " << command.cmd << " -h'.\n";
//    }
...
//
// void Parameters::parseParameters(int argc, const char *pargv[], const Command
...
//        if (par[parIdx]->uniqid == PARAM_HELP_ID || par[parIdx]->uniqid ==
//        PARAM_HELP_LONG_ID) {
//            canHandleHelp = true;
//        }
//    }
...
//        const bool longParameter = (pargv[argIdx][0] == '-' &&
//        pargv[argIdx][1] == '-'); if (longParameter || (pargv[argIdx][0] ==
//        '-')) {
//            if ((parseFlags & PARSE_REST) && longParameter && pargv[argIdx][2]
//            == '\0') {
...
//            if (canHandleHelp == false && (parameter.compare("-h") == 0 ||
//            parameter.compare("--help") == 0)) {
...
//                    if (typeid(bool) != par[parIdx]->type && argIdx + 1 ==
//                    argc) {
//                        printUsageMessage(command, outputFlags);
//                        Debug(Debug::ERROR) << "Missing argument " <<
//                        par[parIdx]->name << "\n"; EXIT(EXIT_FAILURE);
//                    }
//
//                    if (par[parIdx]->wasSet) {
//                        printUsageMessage(command, outputFlags);
//                        Debug(Debug::ERROR) << "Duplicate parameter " <<
//                        par[parIdx]->name << "\n"; EXIT(EXIT_FAILURE);
//                    }
//
...
//                        int nomatch = regexec(&regex, pargv[argIdx+1], 0,
//                        NULL, 0); regfree(&regex);
//                        // if no match found or two matches found (we want
//                        exactly one match) if (nomatch){
//                            printUsageMessage(command, 0xFFFFFFFF);
//                            Debug(Debug::ERROR) << "Error in argument " <<
//                            par[parIdx]->name << "\n"; EXIT(EXIT_FAILURE);
//                        }else{
//                            *((int *) par[parIdx]->value) =
//                            atoi(pargv[argIdx+1]); par[parIdx]->wasSet = true;
//                        }
//                        argIdx++;
...
//                        exactly one match) if (nomatch){
//                            printUsageMessage(command, 0xFFFFFFFF);
//                            Debug(Debug::ERROR) << "Error in argument regex "
//                            << par[parIdx]->name << "\n"; EXIT(EXIT_FAILURE);
//                        } else {
//                            size_t value = ByteParser::parse(pargv[argIdx+1]);
//                            if (value == ByteParser::INVALID_SIZE) {
//                                printUsageMessage(command, 0xFFFFFFFF);
//                                Debug(Debug::ERROR) << "Error in value parsing
//                                " << par[parIdx]->name << "\n";
//                                EXIT(EXIT_FAILURE);
//                            } else {
...
//                            val = base64_decode(val.c_str() + 4, val.size() -
//                            4);
//                        }
...
//                            << par[parIdx]->name << "\n"; EXIT(EXIT_FAILURE);
//                        } else {
//                            *((MultiParam<char*> *) par[parIdx]->value) =
//                            value; par[parIdx]->wasSet = true;
//                        }
//                        argIdx++;
//                    }else if (typeid(MultiParam<int>) == par[parIdx]->type) {
//                        MultiParam<int> value =
//                        MultiParam<int>(pargv[argIdx+1]); if (value.aminoacids
//                        == INT_MAX || value.nucleotides == INT_MAX) {
//                            printUsageMessage(command, 0xFFFFFFFF);
//                            Debug(Debug::ERROR) << "Error in value parsing "
//                            << par[parIdx]->name << "\n"; EXIT(EXIT_FAILURE);
//                        } else {
...
//                    {
//                        MultiParam<float> value =
//                        MultiParam<float>(pargv[argIdx + 1]); if
//                        (value.aminoacids == FLT_MAX || value.nucleotides ==
//                        FLT_MAX) {
//                            printUsageMessage(command, 0xFFFFFFFF);
//                            Debug(Debug::ERROR) << "Error in value parsing "
//                            << par[parIdx]->name << "\n"; EXIT(EXIT_FAILURE);
//                        } else {
//                            *((MultiParam<float> *) par[parIdx]->value) =
//                            value; par[parIdx]->wasSet = true;
//                        }
//                        argIdx++;
...
//                        compileRegex(&regex, par[parIdx]->regex);
//                        int nomatch = regexec(&regex, pargv[argIdx+1], 0,
//                        NULL, 0); regfree(&regex); if (nomatch){
//                            printUsageMessage(command, 0xFFFFFFFF);
//                            Debug(Debug::ERROR) << "Error in argument " <<
//                            par[parIdx]->name << "\n"; EXIT(EXIT_FAILURE);
//                        }else{
//                            double input = strtod(pargv[argIdx+1], NULL);
//                            *((float *) par[parIdx]->value) =
//                            static_cast<float>(input); par[parIdx]->wasSet =
//                            true;
//                        }
//                        argIdx++;
...
//                            par[parIdx]->name << "\n"; EXIT(EXIT_FAILURE);
//                        }else{
//                            *((double *) par[parIdx]->value) =
//                            strtod(pargv[argIdx+1], NULL); par[parIdx]->wasSet
//                            = true;
//                        }
//                        argIdx++;
...
//                        par[parIdx]->wasSet = true;
//                    } else {
//                        Debug(Debug::ERROR) << "Wrong parameter type in
//                        parseParameters. Please inform the developers\n";
//                        EXIT(EXIT_FAILURE);
//                    }
//
...
//                std::vector<MMseqsParameter *>::const_iterator index =
//                par.end(); int maxDistance = 0; for
//                (std::vector<MMseqsParameter *>::const_iterator it =
//                par.begin(); it != par.end(); ++it) {
//                    int distance =
//                    DistanceCalculator::localLevenshteinDistance(parameter,
//                    (*it)->name); if (distance > maxDistance) {
//                        maxDistance = distance;
//                        index = it;
//                    }
//                }
//
//                Debug(Debug::ERROR) << "Unrecognized parameter \"" <<
//                parameter << "\""; if (index != par.end()) {
//                    Debug(Debug::ERROR) << ". Did you mean \"" <<
//                    (*index)->name << "\" (" << (*index)->display << ")?\n";
//                } else {
//                    Debug(Debug::ERROR) << "\n";
//                }
//
...
//            ssize_t size = cygwin_conv_path(CCP_WIN_A_TO_POSIX | CCP_RELATIVE,
//            path, NULL, 0); if (size < 0) {
//                Debug(Debug::ERROR) << "Could not convert cygwin path!\n";
//                EXIT(EXIT_FAILURE);
//            } else {
//                char *posix = new char[size];
//                if (cygwin_conv_path(CCP_WIN_A_TO_POSIX | CCP_RELATIVE, path,
//                posix, size)) {
//                    Debug(Debug::ERROR) << "Could not convert cygwin path!\n";
//                    EXIT(EXIT_FAILURE);
//                }
...
//
//
//    bool ignorePathCountChecks = command.databases.empty() == false &&
//    command.databases[0].specialType & DbType::ZERO_OR_ALL && filenames.size()
//    == 0; const size_t MAX_DB_PARAMETER = 6; if (ignorePathCountChecks ==
//    false && command.databases.size() > MAX_DB_PARAMETER) {
//        Debug(Debug::ERROR) << "Use argv if you need more than " <<
//        MAX_DB_PARAMETER << " db parameters" << "\n"; EXIT(EXIT_FAILURE);
//    }
//
//    if (ignorePathCountChecks == false && filenames.size() <
//    command.databases.size()){
//        printUsageMessage(command, outputFlags);
//        Debug(Debug::ERROR) << "Not enough input paths provided. ";
//        if (command.databases.size() == 1) {
//            Debug(Debug::ERROR) << "1 path is required.\n";
//        } else {
//            Debug(Debug::ERROR) << command.databases.size() << " paths are
//            required.\n";
//        }
//        EXIT(EXIT_FAILURE);
//    }
...
//    bool isEndVar = false;
//    if(command.databases.empty() == false && command.databases[0].validator !=
//    NULL) {
//        if (command.databases.size() >= 2) {
//            for(size_t i = 0; i < command.databases.size();i++){
//                if(i == 0){
//                    isStartVar |= (command.databases[i].specialType &
//                    DbType::VARIADIC);
//                } else if(i == command.databases.size() - 1){
//                    isEndVar |= (command.databases[i].specialType &
//                    DbType::VARIADIC);
//                } else {
//                    isMiddleVar |= (command.databases[i].specialType &
//                    DbType::VARIADIC);
//                }
//
//            }
...
//        }
//        if (ignorePathCountChecks == false && isVar == false &&
//        filenames.size() > command.databases.size()) {
//            printUsageMessage(command, outputFlags);
//            Debug(Debug::ERROR) << "Too many input paths provided. Only " <<
//            SSTR(command.databases.size()) << " are allowed\n";
//            EXIT(EXIT_FAILURE);
//        }
//    }
...
//            break;
//        default:
//            // Do not abort execution if we expect a variable amount of
//            parameters if (parseFlags & PARSE_VARIADIC)
//                break;
//            // FALLTHROUGH
//        case 0:
...
//        bool isAminoAcid   = (strcmp(scoringMatrixFile.aminoacids,
//        substitutionMatrices[i].name.c_str()) == 0); bool isNucleotide  =
//        (strcmp(scoringMatrixFile.nucleotides,
//        substitutionMatrices[i].name.c_str()) == 0); bool isSeedAminoAcid   =
//        (strcmp(seedScoringMatrixFile.aminoacids,
//        substitutionMatrices[i].name.c_str()) == 0); bool isSeedNucleotide  =
//        (strcmp(seedScoringMatrixFile.nucleotides,
//        substitutionMatrices[i].name.c_str()) == 0); if (isAminoAcid ||
//        isNucleotide|| isSeedAminoAcid|| isSeedNucleotide) {
//            std::string matrixData((const char
//            *)substitutionMatrices[i].subMatData,
//            substitutionMatrices[i].subMatDataLen); std::string matrixName =
//            substitutionMatrices[i].name; if(isAminoAcid) {
//                free(scoringMatrixFile.aminoacids);
//                scoringMatrixFile.aminoacids =
//                BaseMatrix::serialize(matrixName, matrixData);
//            }
//            if(isNucleotide) {
//                free(scoringMatrixFile.nucleotides);
//                scoringMatrixFile.nucleotides =
//                BaseMatrix::serialize(matrixName, matrixData);
//            }
//            if(isSeedAminoAcid) {
//                free(seedScoringMatrixFile.aminoacids);
//                seedScoringMatrixFile.aminoacids =
//                BaseMatrix::serialize(matrixName, matrixData);
//            }
//            if(isSeedNucleotide) {
//                free(seedScoringMatrixFile.nucleotides);
//                seedScoringMatrixFile.nucleotides =
//                BaseMatrix::serialize(matrixName, matrixData);
//            }
//        }
//    }
...
//        checkIfDatabaseIsValid(command, argc, pargv, isStartVar, isMiddleVar,
//        isEndVar);
//    }
//
//    if (printPar == true) {
...
//
// void Parameters::checkIfDatabaseIsValid(const Command& command, int argc,
// const char** argv, bool isStartVar, bool isMiddleVar, bool isEndVar) {
//    size_t fileIdx = 0;
...
//                argumentDist = (filenames.size() - command.databases.size());
//            } else if ((command.databases[dbIdx].specialType &
//            DbType::VARIADIC) && isMiddleVar) {
//                argumentDist = (filenames.size() - command.databases.size());
//            }
//
...
//                }
//
//                if (filenames[fileIdx] != "stdin" &&
//                FileUtil::fileExists((filenames[fileIdx]).c_str()) == false &&
//                FileUtil::fileExists((filenames[fileIdx] + ".dbtype").c_str())
//                == false) {
//                    printParameters(command.cmd, argc, argv, *command.params);
//                    Debug(Debug::ERROR) << "Input " << filenames[fileIdx] << "
//                    does not exist\n"; EXIT(EXIT_FAILURE);
//                }
//                int dbtype =
//                FileUtil::parseDbType(filenames[fileIdx].c_str()); if
//                (db.specialType & DbType::NEED_HEADER &&
//                FileUtil::fileExists((filenames[fileIdx] +
//                "_h.dbtype").c_str()) == false &&
//                Parameters::isEqualDbtype(dbtype, Parameters::DBTYPE_INDEX_DB)
//                == false) {
//                    printParameters(command.cmd, argc, argv, *command.params);
//                    Debug(Debug::ERROR) << "Database " << filenames[fileIdx]
//                    << " needs header information\n"; EXIT(EXIT_FAILURE);
//                }
//                if (db.specialType & DbType::NEED_TAXONOMY) {
//                    std::vector<std::string> missingFiles =
//                    findMissingTaxDbFiles(filenames[fileIdx]); if
//                    (missingFiles.empty() == false) {
//                        printParameters(command.cmd, argc, argv,
//                        *command.params); printTaxDbError(filenames[fileIdx],
//                        missingFiles); EXIT(EXIT_FAILURE);
//                    }
//                }
//                if (db.specialType & DbType::NEED_LOOKUP &&
//                FileUtil::fileExists((filenames[fileIdx] + ".lookup").c_str())
//                == false) {
//                    printParameters(command.cmd, argc, argv, *command.params);
//                    Debug(Debug::ERROR) << "Database " << filenames[fileIdx]
//                    << " needs a lookup file\n"; EXIT(EXIT_FAILURE);
...
//                for (size_t i = 0; i < db.validator->size() && dbtypeFound ==
//                false; i++) {
...
//                    {
//                        dbtypeFound =
//                        (FileUtil::fileExists(filenames[fileIdx].c_str()) ==
//                        true &&
//                                       FileUtil::directoryExists(filenames[fileIdx].c_str())
//                                       == false);
//                    } else if (validatorDbtype ==
//                    Parameters::DBTYPE_DIRECTORY) {
//                        dbtypeFound =
//                        FileUtil::directoryExists(filenames[fileIdx].c_str());
//                    } else if (Parameters::isEqualDbtype(db.validator->at(i),
//                    dbtype)) {
//                        dbtypeFound = true;
//                    }
//                }
...
//                    printParameters(command.cmd, argc, argv, *command.params);
//                    Debug(Debug::ERROR) << "Input database \"" <<
//                    filenames[fileIdx] << "\" has the wrong type ("
//                                        << Parameters::getDbTypeName(dbtype)
//                                        << ")\nAllowed input:\n";
//                    for (size_t i = 0; i < db.validator->size(); ++i) {
//                        Debug(Debug::ERROR) << "- " <<
//                        Parameters::getDbTypeName(db.validator->at(i)) <<
//                        "\n";
//                    }
//                    EXIT(EXIT_FAILURE);
//                }
...
//                if (FileUtil::directoryExists(filenames[fileIdx].c_str()) ==
//                false) {
//                    if (FileUtil::makeDir(filenames[fileIdx].c_str()) ==
//                    false) {
//                        printParameters(command.cmd, argc, argv,
//                        *command.params); Debug(Debug::ERROR) << "Cannot
//                        create temporary directory " << filenames[dbIdx] <<
//                        "\n"; EXIT(EXIT_FAILURE);
//                    } else {
//                        Debug(Debug::INFO) << "Create directory " <<
//                        filenames[dbIdx] << "\n";
//                    }
//                }
//                fileIdx++;
...
//                {
//                    Debug(Debug::WARNING) << filenames[fileIdx] << " exists
...
//
// void Parameters::printParameters(const std::string &module, int argc, const
// char* pargv[],
...
//    for (int i = 0; i < argc; i++) {
//        // don't expose users to the interal b64 masking of whitespace
//        characters if (strncmp("b64:", pargv[i], 4) == 0) {
//            Debug(Debug::INFO) << "'" << base64_decode(pargv[i] + 4,
//            strlen(pargv[i]) - 4) << "' ";
//        } else {
...
//    ss << std::boolalpha;
//
//    ss << std::setw(maxWidth) << std::left  << "MMseqs Version:" << "\t" <<
//    version << "\n";
//
//
//    for (size_t i = 0; i < par.size(); i++) {
...
//            MultiParam<char*>
//            tmpPar(BaseMatrix::unserializeName(param->aminoacids).c_str(),
...
//            ss << MultiParam<char*>::format(*((MultiParam<char*>
//            *)par[i]->value));
//        } else if(typeid(MultiParam<int>) == par[i]->type) {
//            ss << MultiParam<int>::format(*((MultiParam<int>
//            *)par[i]->value));
//        } else if(typeid(MultiParam<float>) == par[i]->type) {
//            ss << MultiParam<float>::format(*((MultiParam<float>
//            *)par[i]->value));
...
std::vector<MMseqsParameter *> Parameters::combineList(
  ...
  for (size_t i = 0; i < par.size(); ++i) {
    ...
    if (typeid(int) == par[i]->type) {
      ...
      ss << *((double *)par[i]->value) << " ";
               ...
               PARAM_SEED_SUB_MAT.uniqid == par[i]->uniqid) {
      MultiParam<char *> *param = ((MultiParam<char *> *)par[i]->value);
      MultiParam<char *> tmpPar(
          BaseMatrix::unserializeName(param->aminoacids).c_str(),
          BaseMatrix::unserializeName(param->nucleotides).c_str());
      ...
      if (value.find_first_of(" \n\t") != std::string::npos) {
        ss << par[i]->name
           << " b64:" << base64_encode(value.c_str(), value.size()) << " ";
      } else {
        ss << par[i]->name << " " << value << " ";
      }
    } else if (typeid(std::string) == par[i]->type) {
      ...
      ss << par[i]->name << " ";
      std::string value =
          MultiParam<char *>::format(*((MultiParam<char *> *)par[i]->value));
      // see above
      if (value.find_first_of(" \n\t") != std::string::npos) {
        ss << par[i]->name
           << " b64:" << base64_encode(value.c_str(), value.size()) << " ";
...
std::vector<MMseqsParameter *> Parameters::removeParameter(
    const std::vector<MMseqsParameter *> &par, const MMseqsParameter &x) {
  std::vector<MMseqsParameter *> newParamList;
  for (size_t i = 0; i < par.size(); i++) {
    MMseqsParameter *p = par[i];
    if (p->name != x.name) newParamList.push_back(p);
  }
  return newParamList;
}

void Parameters::overrideParameterDescription(MMseqsParameter &par,
                                              const char *description,
                                              const char *regex, int category) {
  if (description != NULL) {
...
std::vector<int> Parameters::getOutputFormat(
  ...
  if (formatMode == Parameters::FORMAT_ALIGNMENT_SAM ||
      formatMode == Parameters::FORMAT_ALIGNMENT_HTML) {
    needSequences = true;
    needBacktrace = true;
    return formatCodes;
  }
  ...
  for (size_t i = 0; i < outformatSplit.size(); ++i) {
    if (outformatSplit[i].compare("query") == 0) {
      code = Parameters::OUTFMT_QUERY;
    } else if (outformatSplit[i].compare("target") == 0) {
      code = Parameters::OUTFMT_TARGET;
    } else if (outformatSplit[i].compare("evalue") == 0) {
      code = Parameters::OUTFMT_EVALUE;
    } else if (outformatSplit[i].compare("gapopen") == 0) {
      code = Parameters::OUTFMT_GAPOPEN;
    } else if (outformatSplit[i].compare("pident") == 0) {
      code = Parameters::OUTFMT_PIDENT;
    } else if (outformatSplit[i].compare("fident") == 0) {
      code = Parameters::OUTFMT_FIDENT;
    } else if (outformatSplit[i].compare("nident") == 0) {
      code = Parameters::OUTFMT_NIDENT;
    } else if (outformatSplit[i].compare("qstart") == 0) {
      code = Parameters::OUTFMT_QSTART;
    } else if (outformatSplit[i].compare("qend") == 0) {
      code = Parameters::OUTFMT_QEND;
    } else if (outformatSplit[i].compare("qlen") == 0) {
      code = Parameters::OUTFMT_QLEN;
    } else if (outformatSplit[i].compare("tstart") == 0) {
      code = Parameters::OUTFMT_TSTART;
    } else if (outformatSplit[i].compare("tend") == 0) {
      code = Parameters::OUTFMT_TEND;
    } else if (outformatSplit[i].compare("tlen") == 0) {
      code = Parameters::OUTFMT_TLEN;
    } else if (outformatSplit[i].compare("alnlen") == 0) {
      code = Parameters::OUTFMT_ALNLEN;
    } else if (outformatSplit[i].compare("raw") == 0) {
      needSequences = true;
      code = Parameters::OUTFMT_RAW;
    } else if (outformatSplit[i].compare("bits") == 0) {
      code = Parameters::OUTFMT_BITS;
    } else if (outformatSplit[i].compare("cigar") == 0) {
      needBacktrace = true;
      code = Parameters::OUTFMT_CIGAR;
    } else if (outformatSplit[i].compare("qseq") == 0) {
      needSequences = true;
      code = Parameters::OUTFMT_QSEQ;
    } else if (outformatSplit[i].compare("tseq") == 0) {
      needSequences = true;
      code = Parameters::OUTFMT_TSEQ;
    } else if (outformatSplit[i].compare("qheader") == 0) {
      needFullHeaders = true;
      code = Parameters::OUTFMT_QHEADER;
    } else if (outformatSplit[i].compare("theader") == 0) {
      needFullHeaders = true;
      code = Parameters::OUTFMT_THEADER;
    } else if (outformatSplit[i].compare("qaln") == 0) {
      needBacktrace = true;
      needSequences = true;
      code = Parameters::OUTFMT_QALN;
    } else if (outformatSplit[i].compare("taln") == 0) {
      needBacktrace = true;
      needSequences = true;
      code = Parameters::OUTFMT_TALN;
    } else if (outformatSplit[i].compare("qframe") == 0) {
      code = Parameters::OUTFMT_QFRAME;
    } else if (outformatSplit[i].compare("tframe") == 0) {
      code = Parameters::OUTFMT_TFRAME;
    } else if (outformatSplit[i].compare("mismatch") == 0) {
      code = Parameters::OUTFMT_MISMATCH;
    } else if (outformatSplit[i].compare("qcov") == 0) {
      code = Parameters::OUTFMT_QCOV;
    } else if (outformatSplit[i].compare("tcov") == 0) {
      code = Parameters::OUTFMT_TCOV;
    } else if (outformatSplit[i].compare("qset") == 0) {
      needLookup = true;
      needSource = true;
      code = Parameters::OUTFMT_QSET;
    } else if (outformatSplit[i].compare("qsetid") == 0) {
      needLookup = true;
      needSource = true;
      code = Parameters::OUTFMT_QSETID;
    } else if (outformatSplit[i].compare("tset") == 0) {
      needLookup = true;
      code = Parameters::OUTFMT_TSET;
    } else if (outformatSplit[i].compare("tsetid") == 0) {
      needLookup = true;
      needSource = true;
      code = Parameters::OUTFMT_TSETID;
    } else if (outformatSplit[i].compare("taxid") == 0) {
      needTaxonomyMapping = true;
      code = Parameters::OUTFMT_TAXID;
    } else if (outformatSplit[i].compare("taxname") == 0) {
      needTaxonomyMapping = true;
      needTaxonomy = true;
      code = Parameters::OUTFMT_TAXNAME;
    } else if (outformatSplit[i].compare("taxlineage") == 0) {
      needTaxonomyMapping = true;
      needTaxonomy = true;
      code = Parameters::OUTFMT_TAXLIN;
    } else if (outformatSplit[i].compare("qorfstart") == 0) {
      code = Parameters::OUTFMT_QORFSTART;
    } else if (outformatSplit[i].compare("qorfend") == 0) {
      code = Parameters::OUTFMT_QORFEND;
    } else if (outformatSplit[i].compare("torfstart") == 0) {
      code = Parameters::OUTFMT_TORFSTART;
    } else if (outformatSplit[i].compare("torfend") == 0) {
      code = Parameters::OUTFMT_TORFEND;
...
void Parameters::setSubstitutionMatrices(std::string aminoacids,
  ...
  for (size_t i = 0; i < substitutionMatrices.size(); i++) {
    ...
    if (isAminoAcid || isNucleotide) {
      std::string matrixData((const char *)substitutionMatrices[i].subMatData,
                             substitutionMatrices[i].subMatDataLen);
...
void Parameters::setSeedSubstitutionMatrices(std::string aminoacids,
                                             std::string nucleotides) {
  for (size_t i = 0; i < substitutionMatrices.size(); i++) {
