...
typedef struct kseq_buffer {
  ...
  kseq_buffer() : buffer(nullptr), position(0), length(0){};
  kseq_buffer(char* buffer, size_t length)
      : buffer(buffer), position(0), length(length){};
} kseq_buffer_t;

inline ssize_t kseq_buffer_reader(kseq_buffer_t* inBuffer, char* outBuffer,
                                  size_t nbyte) {
