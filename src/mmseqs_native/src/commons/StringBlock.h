 ...
 public:
  ...
  void compact() {
    ...
    for (T i = 0; i < entryCount; ++i) {
      if (i != 0 &&
          strcmp(getString(indices[i]), getString(indices[i - 1])) == 0) {
  ...
  static size_t memorySize(const StringBlock& block) {
    return sizeof(size_t) + 2 * sizeof(T) + block.byteCapacity * sizeof(char) +
           block.entryCapacity * sizeof(T);
  }
  ...
  static StringBlock* unserialize(const char* mem) {
    ...
    return new StringBlock<T>(byteCapacity, entryCapacity, entryCount, data,
                              offsets);
  }

 private:
  StringBlock(size_t byteCapacity, T entryCapacity, T entryCount, char* data,
              T* offsets)
