...
std::string MultiParam<T>::format(const MultiParam<T>& multiparam) {
  if (multiparam.nucleotides == multiparam.aminoacids) {
    ...
    return std::string("nucl:") + SSTR(multiparam.nucleotides) +
           ",aa:" + SSTR(multiparam.aminoacids);
  }
}
...
MultiParam<int>::MultiParam(const char* parametercstring) {
  if (strchr(parametercstring, ',') != NULL) {
    if (sscanf(parametercstring, "aa:%d,nucl:%d", &aminoacids, &nucleotides) !=
            2 &&
        sscanf(parametercstring, "nucl:%d,aa:%d", &nucleotides, &aminoacids) !=
            2) {
      nucleotides = INT_MAX;
      aminoacids = INT_MAX;
    }
...
MultiParam<float>::MultiParam(const char* parametercstring) {
  if (strchr(parametercstring, ',') != NULL) {
    if (sscanf(parametercstring, "aa:%f,nucl:%f", &aminoacids, &nucleotides) !=
            2 &&
        sscanf(parametercstring, "nucl:%f,aa:%f", &nucleotides, &aminoacids) !=
            2) {
      nucleotides = FLT_MAX;
      aminoacids = FLT_MAX;
    }
...
MultiParam<char*>::MultiParam(const char* filename) {
  ...
  if ((split = strchr(filename, ',')) != NULL) {
    ...
    if (strncmp("aa:", first, strlen("aa:")) == 0 &&
        strncmp("nucl:", second, strlen("nucl:")) == 0) {
      aminoacids = strndup(first + 3, split - first - 5);
      nucleotides = strdup(second + 5);
    } else if (strncmp("nucl:", first, strlen("nucl:")) == 0 &&
               strncmp("aa:", second, strlen("aa:")) == 0) {
...
std::string MultiParam<char*>::format(const MultiParam<char*>& file) {
  /*if (strncmp(file.nucleotides, file.aminoacids, strlen(file.aminoacids)) ==
  0) { return file.nucleotides; } else {*/
  return std::string("nucl:") + file.nucleotides + ",aa:" + file.aminoacids;
  //}
}

bool MultiParam<char*>::operator==(const char* other) const {
  return strncmp(other, nucleotides, strlen(nucleotides)) == 0 ||
         strncmp(other, aminoacids, strlen(aminoacids)) == 0;
}

bool MultiParam<char*>::operator==(const std::string& other) const {
  return strncmp(other.c_str(), nucleotides, strlen(nucleotides)) == 0 ||
         strncmp(other.c_str(), aminoacids, strlen(aminoacids)) == 0;
}

bool MultiParam<char*>::operator==(const MultiParam<char*>& other) const {
  return strncmp(other.nucleotides, nucleotides, strlen(nucleotides)) == 0 &&
         strncmp(other.aminoacids, aminoacids, strlen(aminoacids)) == 0;
}
