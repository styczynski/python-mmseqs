...
class MathUtil {
 public:
  ...
  static bool isWithinModRang(unsigned int start, unsigned int end,
                              unsigned int n, unsigned int mod) {
  ...
  static inline T ipow(int base, int exponent) {
    T res = 1;
    for (int i = 0; i < exponent; i++) res = res * base;
    return res;
  }
  ...
  static inline float flog2(float x) {
    if (x <= 0) return -128;
    MAY_ALIAS(int) *px =
        (int *)(&x);  // store address of float as pointer to long int
    float e = (float)(((*px & 0x7F800000) >> 23) -
                      0x7f);  // shift right by 23 bits and subtract 127 = 0x7f
                              // => exponent
    *px = ((*px & 0x007FFFFF) | 0x3f800000);  // set exponent to 127 (i.e., 0)
    ...
    x *=
        (1.441740 +
         x * (-0.7077702 +
              x * (0.4123442 + x * (-0.1903190 +
                                    x * 0.0440047))));  // 5'th order polynomial
                                                        // approx. of log(1+x)
    return x + e;
  }
  ...
  static inline double fpow2(float x) {
    ...
    MAY_ALIAS(int) *px =
        (int *)(&x);  // store address of float as pointer to long int
    float tx = (x - 0.5f) + (3 << 22);  // temporary value for truncation: x-0.5
                                        // is added to a large integer (3<<22),
    ...
    //   x = 1.0f + dx*(0.69606564f           // cubic apporoximation of 2^x for
    //   x in the range [0, 1]
    //            + dx*(0.22449433f           // Gives relative deviation
    //            < 1.5E-4
    //            + dx*(0.07944023f)));       // Speed: 1.9E-8s
    x = 1.0f + dx * (0.693019f  // polynomial approximation of 2^x for x in the
                                // range [0, 1]
                     + dx * (0.241404f  // Gives relative deviation < 4.6E-6
                             + dx * (0.0520749f  // Speed: 2.1E-8s
                                     + dx * 0.0134929f)));
    //   x = 1.0f + dx*(0.693153f             // polynomial apporoximation of
    //   2^x for x in the range [0, 1]
    //            + dx*(0.240153f             // Gives relative deviation
    //            < 2.3E-7
  ...
  static inline unsigned short sadd16(const unsigned short a,
...
#error EXPONENT_BITS and MANTISSA_BITS must sum to 16
  ...
  static float convertCharToFloat(char a) {
    ...
    return ldexpf(
        (exponent > 0 ? HIDDEN_BIT | mantissa : mantissa << 1) / ONE_FLOAT,
        exponent - EXCESS);
  }
#pragma GCC diagnostic pop
  ...
  static char convertNeffToChar(const float neff) {
    ...
    return std::max(static_cast<unsigned char>(1),
                    static_cast<unsigned char>(retVal + 0.5));
  }
  ...
  static void computeFactorial(double *output, const size_t range) {
    ...
    for (size_t score = 1; score < range; score++) {
      ...
      const double S_fact = std::min(std::numeric_limits<double>::max(),
                                     sqrt(2 * M_PI * scoreDbl) *
                                         pow(scoreDbl / exp(1), scoreDbl) *
                                         exp(1 / (12 * scoreDbl)));
  ...
  static inline float NormalizeTo1(float *array, int length,
                                   const double *def_array = NULL) {
    float sum = 0.0f;
