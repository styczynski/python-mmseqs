 ...
 public:
  ...
  ~Debug() {
    if (level <= ERROR && level <= debugLevel) {
      std::cout << std::flush;
      if (interactive) {
        std::cerr << "\033[" << Color::FG_RED << "m" << buffer << "\033["
                  << Color::FG_DEFAULT << "m";
        ;
      } else {
        std::cerr << buffer;
      ...
      if (interactive) {
        std::cout << "\033[" << Color::FG_YELLOW << "m" << buffer << "\033["
                  << Color::FG_DEFAULT << "m";
      } else {
        std::cout << buffer;
      }
   ...
   private:
    ...
    std::string buildItemString(size_t id) {
      ...
      if (exp < 3) {
        ...
        int fmtElm =
            sprintf(fmtbuffer, "%.2f",
                    static_cast<float>(id + 1) / static_cast<float>(base));
        line.append(fmtbuffer, fmtElm);
    ...
    void updateProgress() {
      ...
      if (interactive == false) {
        if (totalEntries == SIZE_MAX) {
          ...
          float progress = (totalEntries == 1)
                               ? 1.0
                               : (static_cast<float>(id) /
                                  static_cast<float>(totalEntries - 1));
        ...
        if (totalEntries == SIZE_MAX) {
          ...
          float prevPrintedProgress =
              (totalEntries == 1) ? 0.0
                                  : (static_cast<float>(prevPrintedId) /
                                     static_cast<float>(totalEntries - 1));
          if (progress - prevPrintedProgress > 0.01 ||
              id == (totalEntries - 1) || id == 0) {
            std::string line;
            line.push_back('[');
            int pos = BARWIDTH * progress;
            ...
            if (id == 0) {
              ...
              //                            long long msec = (time_t)((eta -
              //                            sec) * 1e3); std::cout << timeDiff
              //                            << "\t" << progress << std::endl;
              line.append("eta ");
              if (sec >= 3600) {
