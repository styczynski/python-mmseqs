...
class HeaderSummarizer {
 public:
  virtual std::string summarize(mmseqs_output* out, const std::vector<std::string>& headers) = 0;
  virtual ~HeaderSummarizer(){};
};

class UniprotHeaderSummarizer : public HeaderSummarizer {
 public:
  std::string summarize(mmseqs_output* out, const std::vector<std::string>& headers);
