...
std::vector<std::string> readData(std::string fasta_filename) {
  ...
  if (fasta_file == NULL) {
    std::cout << "Could not open " << fasta_filename << std::endl;
    EXIT(1);
  }
...
int main(int, const char**) {
  ...
  std::vector<std::string> sequences =
      readData("/Users/mad/Documents/databases/rfam/Rfam.fasta");
  for (size_t seq_i = 0; seq_i < sequences.size(); seq_i++) {
    ...
    for (size_t seq_j = 0; seq_j < sequences.size(); seq_j++) {
      dbSeq->mapSequence(2, 2, sequences[seq_j].c_str(),
                         sequences[seq_j].size());
      int32_t maskLen = query->L / 2;
      EvalueComputation evalueComputation(100000, &subMat, gap_open,
                                          gap_extend);
      s_align alignment =
          aligner.ssw_align(dbSeq->numSequence, dbSeq->L, gap_open, gap_extend,
                            0, 10000, &evalueComputation, 0, 0.0, maskLen);
      if (mode == 0) {
