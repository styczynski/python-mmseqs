...
int main(int, const char**) {
  ...
  std::cout << "Check getDataByDBKey: " << reader2.getDataByDBKey(2, 0)
            << std::endl;
  std::cout << "Check getDataByDBKey: " << reader2.getDataByDBKey(6, 0)
            << std::endl;
  std::cout << "Check getDataByDBKey: " << reader2.getDataByDBKey(1, 0)
            << std::endl;
  std::cout << "Check getDataByDBKey: " << reader2.getDataByDBKey(111, 0)
            << std::endl;
  std::cout << "Check getDataByDBKey: " << reader2.getDataByDBKey(12, 0)
            << std::endl;
  ...
  std::cout << "Check length: " << (reader2.getSeqLen(reader2.getId(111)) == 13)
            << std::endl;
  std::cout << "Check length: " << (reader2.getSeqLen(reader2.getId(12)) == 10)
            << std::endl;
  reader2.close();
  // test sort mode
  ...
  for (size_t i = 0; i < reader3.getSize(); i++) {
    ...
    std::cout << id << "\t" << reader3.getSeqLen(i) << "\t"
              << reader3.getData(i, 0);
  }
