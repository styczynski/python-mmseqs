...
int main(int argc, const char* argv[]) {
  ...
  ExtendedSubstitutionMatrix extMattwo(subMat.subMatrix, 2,
                                       subMat.alphabetSize);
  ExtendedSubstitutionMatrix extMatthree(subMat.subMatrix, 3,
                                         subMat.alphabetSize);
  ...
  printf("\nNormal int code: ");
  for (int i = 'A'; i < 'Z'; i++) printf("%d\t", subMat.aa2num[i]);
  ...
  for (int x = 0; x < 10; x++) {
    ...
    while (s->hasNextKmer(kmer_size)) {
      ...
      //        std::cout << "MaxScore: " <<
      //        extMattwo.scoreMatrix[idx_val]->back().first<< "\n";
