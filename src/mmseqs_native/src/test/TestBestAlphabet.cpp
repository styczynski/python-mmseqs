...
void printReduced(size_t alphabetSize, size_t reducedSize, size_t* map2reduced,
                  float* probsAA, float** probsPair) {
  ...
  if (reducedSize < 30) {
    ...
    printf("\n");
    for (size_t a = 0; a < reducedSize; a++) {
      for (size_t b = 0; b < reducedSize;
           b++) {  // test each pair only once, a < b
...
int main(int, const char**) {
  const unsigned int kmerSize =
      1;  ////////// <= change k-mer size here! (1,..,3) /////////////

  /////////                                 |   ////////////////
  ///////// Choose substitution matrix here V ! ////////////////
  // SubstitutionMatrix
  // subMat("/Users/soeding/Projects/MMseqs2/MMseqs2/data/blosum100.out", 2.0,
  // -0.0f);
  SubstitutionMatrix subMat(
      "/Users/soeding/Projects/MMseqs2/MMseqs2/data/PAM70.out", 2.0, -0.0f);

  const size_t numAAs = subMat.alphabetSize - 1;  // size of original alphabet
  const size_t numKmers = pow(numAAs, kmerSize);
  float seqid = 0.0;
  for (size_t a = 0; a < numAAs; a++) seqid += subMat.probMatrix[a][a];
  ...
  DBReader<unsigned int> seqDb(
      dbPath.c_str(), dbIndexPath.c_str(), 1,
      DBReader<unsigned int>::USE_DATA | DBReader<unsigned int>::USE_INDEX);
  seqDb.open(DBReader<unsigned int>::NOSORT);

  Sequence rseqKmer(65536, Parameters::DBTYPE_AMINO_ACIDS, &subMat, kmerSize,
                    false, false);
  ...
  for (size_t id = 0; id < seqDb.getSize(); id++) {
    ...
    while (rseqKmer.hasNextKmer() && sumKmerCnts < 20000 * numKmers) {
      ...
      unsigned pos = 0;
      while (size_t(kmer[pos]) != numAAs && pos < kmerSize) {
        pos++;
      }
      unsigned increment =
          (pos == kmerSize ? 1 : 0);  // add 1.0 only if no X found in k-mer

      kmerCnts[indexer.int2index(kmer)] += increment;
      sumKmerCnts += increment;

      if ((sumKmerCnts % (2000 * numKmers)) == 0)
        printf("%i k-mers counted\n", sumKmerCnts);
      //      printf("kmer=%2i  incr=%1u count[%zu]=%u
      //      sumKmerCnts=%u\n",kmer[0], increment,  indexer.int2index(kmer),
      //      kmerCnts[indexer.int2index(kmer)], sumKmerCnts );
    }
  }
      ...
      switch (kmerSize) {
        case 1:
          printf("index=%4zu, counts[%2zu] = %8u, prob=%.3e\n", index, index,
                 kmerCnts[index], float(kmerCnts[index]) / float(sumKmerCnts));
          break;
        case 2:
          printf("index=%4zu, counts[%2zu-%2zu] = %8u, prob=%.3e\n", index,
                 index / 20, index % 20, kmerCnts[index],
                 float(kmerCnts[index]) / float(sumKmerCnts));
          break;
        case 3:
          printf("index=%4zu, counts[%2zu-%2zu-%2zu] = %8u, prob=%.3e\n", index,
                 index / 400, (index / 20) % 20, index % 20, kmerCnts[index],
                 float(kmerCnts[index]) / float(sumKmerCnts));
          break;
      }
    }
  ...
  size_t* map2reduced = new size_t[numKmers];  // maps letter of full alphabet
                                               // to letter in reduced alphabet
  float* probsKmer =
      new float[numKmers];  // probs p(a|A) for letter a of reduced alphabet A
  float** probsPair =
      new float*[numKmers];  // probs p(a,b|A) for letters a,b of reduced
                             // alphabet A to be aligned

  for (size_t a = 0; a < numKmers; a++) {
    probsPair[a] = new float[numKmers];
  }

  float probHom = 0.0;  // prob. of two homologous residues to have a match;
                        // measures sensitivity
  float probRan = 0.0;  // prob. of two random residues to have a match;
  ...
  for (size_t a = 0; a < numKmers; a++) {
    for (size_t b = 0; b < numKmers; b++) {
      ...
      for (size_t j = 0; j < kmerSize; j++) {
        ...
        bRest = bRest / numAAs;
        probsPair[a][b] *= subMat.probMatrix[aa_a][aa_b] / subMat.pBack[aa_a] /
                           subMat.pBack[aa_b];
      }
  ...
  // Shrink the size of the reduced alphabet by one letter at a time by greedily
  // merging two letters per iteration
  size_t reducedSize;
  for (reducedSize = numKmers - 1; reducedSize > 2; reducedSize--) {
    float objFunCurr =
        log(probHom) /
        log(probRan);  // objective function of current reduced alphabet
    float objFunMin = FLT_MAX;  // best objective function so far
    size_t aMin = 0, bMin = 0;  // best pair of letters found so far

    // For each possible pair of letters a,b to merge, compute objective
    // function and pick best pair
    for (size_t a = 0; a < reducedSize + 1; a++) {
      for (size_t b = a + 1; b < reducedSize + 1;
           b++) {  // test each pair only once, a < b
        float objFun = log(probHom + 2.0 * probsPair[a][b]) /
                       log(probRan + 2.0 * probsKmer[a] * probsKmer[b]);
        if (objFun < objFunMin) {
          objFunMin = objFun;
          aMin = a;
          bMin = b;
    ...
    printf(
        "Alphabet size = %2zu. Merge letters %4zu and %4zu: objective function "
        " %.5f => %0.5f\n",
    ...
    for (size_t a = 0; a < reducedSize + 1; a++) {
      probsPair[a][aMin] +=
          probsPair[a][bMin];  // add probs of columns aMin and bMin
    ...
    for (size_t b = 0; b < reducedSize + 1; b++) {
      probsPair[aMin][b] +=
          probsPair[bMin][b];  // add probs of rows aMin and bMin
