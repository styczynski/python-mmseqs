...
int main(int, const char**) {
  ...
  ScoreMatrix extMatthree =
      ExtendedSubstitutionMatrix::calcScoreMatrix(subMat, 3);

  Indexer idx(subMat.alphabetSize, kmer_size);
  std::cout << "Sequence (id 0):\n";
  const char* sequence = "PATWPCLVALG";
  std::cout << sequence << "\n\n";
  Sequence* s = new Sequence(10000, Parameters::DBTYPE_AMINO_ACIDS, &subMat,
                             kmer_size, false, false);
  ...
  while (s->hasNextKmer()) {
    ...
    for (size_t pos = 0; pos < kmer_list.second; pos++) {
      ...
      std::cout << "\t";
      for (size_t i = 0; i < kmer_size; i++) std::cout << testKmer[i] << " ";
      std::cout << "\t";
