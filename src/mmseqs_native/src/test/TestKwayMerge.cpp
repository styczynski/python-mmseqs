...
struct KmerEntry {
  ...
  short diagonal;
  KmerEntry(){};
  KmerEntry(unsigned int seqId, short diagonal)
      : seqId(seqId), diagonal(diagonal) {}
};

void mergeKmerEntryLists(KmerEntry **entries, size_t *entrySizes,
                         const int fileCnt);
...
typedef std::priority_queue<KmerPosition, std::vector<KmerPosition>,
...
size_t queueNextEntry(KmerPositionQueue &queue, int file, size_t offsetPos,
  ...
  while (entries[offsetPos + pos].seqId != UINT_MAX) {
    queue.push(KmerPosition(repSeqId, entries[offsetPos + pos].seqId,
                            entries[offsetPos + pos].diagonal, file));
    pos++;
  }
...
void mergeKmerEntryLists(KmerEntry **entries, size_t *entrySizes,
  ...
  for (int file = 0; file < fileCnt; file++) {
    offsetPos[file] =
        queueNextEntry(queue, file, 0, entries[file], entrySizes[file]);
  }
