...
int main(int argc, const char* argv[]) {
  ...
  SubstitutionMatrix subMat(
      "/Users/aluucard/Documents/workspace/kClust2/data/blosum30.out", 8.0);
  ...
  ExtendedSubstitutionMatrix extMat(subMat.subMatrix, kmer_size,
                                    subMat.alphabetSize);
  ...
  Sequence* s =
      new Sequence(10000, subMat.aa2num, subMat.num2aa, 0, kmer_size, false);
  ...
  for (int i = 0; i < subMat.alphabetSize; i++)
    printf("%c\t", subMat.num2aa[i]);
  printf("\nNormal : ");
  for (int i = 65; i < 'Z'; i++) printf("%d\t", subMat.aa2num[i]);
  printf("\nReduced: ");
  for (int i = 65; i < 'Z'; i++) printf("%d\t", subMat.aa2num[i]);

  std::cout << "\nInt reduced sequence:\n";
  for (int i = 0; i < s->L; i++) std::cout << s->int_sequence[i] << " ";
  std::cout << "\n";
  ...
  while (s->hasNextKmer()) {
    ...
    std::cout << "MaxScore: "
              << extMat.scoreMatrix->score[idx_val * extMat.size] << "\n";
  }
