...
class Util {
 public:
  static void decomposeDomain(biosnake_output* out, size_t domain_size, size_t world_rank,
  ...
  static char touchMemory(biosnake_output* out, const char *memory, size_t size);
  ...
  static std::string &implode(const std::vector<std::string> &splits, char sep,
                              std::string &str) {
    for (std::vector<std::string>::const_iterator it = splits.begin();
         it != splits.end(); ++it) {
      str += (*it);
      if (it + 1 != splits.end()) {
        str += sep;
  ...
  static bool getLine(const char *data, size_t dataLength, char *buffer,
                      size_t bufferLength);
  ...
  static inline size_t getLastNonWhitespace(char *data, size_t len) {
    ...
    if (counter && data[counter] == '\0') counter--;
  ...
  static inline size_t skipNoneWhitespace(const char *data) {
    // A value different from zero (i.e., true) if indeed c is a white-space
  ...
  static std::pair<std::string, std::string> createTmpFileNames(
      const std::string &db, const std::string &dbindex, int count) {
  ...
  static std::pair<std::string, std::string> databaseNames(
  ...
  static inline size_t getWordsOfLine(const char *data, const char **words,
                                      size_t maxElement) {
    ...
    if (elementCounter < maxElement) words[elementCounter] = data;

    return elementCounter;
  }

  static std::pair<ssize_t, ssize_t> getFastaHeaderPosition(
  ...
  static void checkAllocation(biosnake_output* out, void *pointer, std::string message);
  ...
  static std::map<unsigned int, std::string> readLookup(
      biosnake_output* out, const std::string &lookupFile, const bool removeSplit = false);
  ...
  static int swapCoverageMode(biosnake_output* out, int covMode);
  ...
  static size_t computeMemory(biosnake_output* out, size_t limit);
};
#endif
