...
struct Tantan {
  ...
  Tantan(const char *seqBeg, const char *seqEnd, int maxRepeatOffset,
         const const_double_ptr *likelihoodRatioMatrix, double repeatProb,
         double repeatEndProb, double repeatOffsetProbDecay,
         double firstGapProb, double otherGapProb) {
  ...
  double forwardTotal() {
    double fromForeground =
        std::accumulate(foregroundProbs.begin(), foregroundProbs.end(), 0.0);
  ...
  void addEndCounts(double forwardProb, double totalProb,
                    double *transitionCounts) {
    double toEnd = forwardProb * b2b / totalProb;
    transitionCounts[0] += toEnd;
  }

  void addTransitionCounts(double forwardProb, double totalProb,
                           double *transitionCounts) {
  ...
  bool isNearSeqBeg() { return seqPtr - seqBeg < maxRepeatOffset; }
  ...
  void calcForwardTransitionAndEmissionProbs() {
    ...
    for (int i = 0; i < maxOffset; ++i) {
      ...
      fromForeground += f;
      foregroundBeg[i] =
          (b * b2fProbs[i] + f * f2f0) * lrRow[(int)seqPtr[-i - 1]];
    }
...
int maskSequences(char *seqBeg, char *seqEnd, int maxRepeatOffset,
  ...
  getProbabilities(seqBeg, seqEnd, maxRepeatOffset, likelihoodRatioMatrix,
                   repeatProb, repeatEndProb, repeatOffsetProbDecay,
                   firstGapProb, otherGapProb, probabilities);

  return maskProbableLetters(seqBeg, seqEnd, probabilities, minMaskProb,
                             maskTable);
}
...
void getProbabilities(const char *seqBeg, const char *seqEnd,
  ...
  Tantan tantan(seqBeg, seqEnd, maxRepeatOffset, likelihoodRatioMatrix,
                repeatProb, repeatEndProb, repeatOffsetProbDecay, firstGapProb,
                otherGapProb);
...
void countTransitions(const char *seqBeg, const char *seqEnd,
                      int maxRepeatOffset,
                      const const_double_ptr *likelihoodRatioMatrix,
                      double repeatProb, double repeatEndProb,
                      double repeatOffsetProbDecay, double firstGapProb,
                      double otherGapProb, double *transitionCounts) {
  Tantan tantan(seqBeg, seqEnd, maxRepeatOffset, likelihoodRatioMatrix,
                repeatProb, repeatEndProb, repeatOffsetProbDecay, firstGapProb,
                otherGapProb);
