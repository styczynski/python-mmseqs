...
DBConcat::DBConcat(biosnake_output* output, const std::string &dataFileNameA,
    ...
    for (size_t id = 0; id < indexSizeA; id++) {
      ...
      if (write) {
        ...
        if (takeLargerEntry == true) {
          ...
          size_t dataSizeB =
              std::max(dbB.getEntryLen(idB), trimRight) - trimRight;
...
#pragma omp for schedule(dynamic, 10)
    for (size_t id = 0; id < indexSizeB; id++) {
      ...
      if (write) {
        ...
        if (takeLargerEntry) {
          ...
          size_t dataSizeA =
              std::max(dbA.getEntryLen(idB), trimRight) - trimRight;
  ...
  if (shouldConcatSource) {
    ...
    for (itB = sourceMapB.begin(); itB != sourceMapB.end(); itB++) {
      ...
      char *tmpBuff =
          Itoa::u32toa_sse2(static_cast<uint64_t>(newSetIdB), buffer);
      line.append(buffer, tmpBuff - buffer - 1);
      line.append(1, '\t');
...
unsigned int DBConcat::dbAKeyMap(unsigned int key) {
  if (sameDatabase) return key;

  std::pair<unsigned int, unsigned int> *originalMap = std::upper_bound(
      keysA, keysA + indexSizeA, key, compareKeyToFirstEntry());
...
unsigned int DBConcat::dbBKeyMap(unsigned int key) {
  ...
  std::pair<unsigned int, unsigned int> *originalMap = std::upper_bound(
      keysB, keysB + indexSizeB, key, compareKeyToFirstEntry());
  return originalMap->second;
}
...
void setDbConcatDefault(Parameters *par) { par->threads = 1; }

int concatdbs(biosnake_output *out, Parameters &par) {
