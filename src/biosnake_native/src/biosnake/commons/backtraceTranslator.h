...
class BacktraceTranslator {
 public:
  BacktraceTranslator(biosnake_output* output): out(output) {
  ...
  /* translate results takes an pairwise alignment between sequence AB
   (resultAB), and BC (resultBC) to infer an alignment between AC (resultAC)
   ...
   C: --TG--   /
  */
  void translateResult(const Matcher::result_t& resultAB,
                       const Matcher::result_t& resultBC,
    ...
    if (startBab < startBbc) {
      ...
      int bOffset = 0;
      int btOffset = 0;
      while (bOffset < distanceInB &&
             btOffset < static_cast<int>(resultAB.backtrace.size())) {
        bOffset += (resultAB.backtrace[btOffset] == 'M' ||
                    resultAB.backtrace[btOffset] == 'D');
        aOffset += (resultAB.backtrace[btOffset] == 'M' ||
                    resultAB.backtrace[btOffset] == 'I');
        btOffset++;
      }
      offsetBbc = 0;
      offsetBab = btOffset;
      ...
      int btOffset = 0;
      while (bOffset < distanceInB &&
             btOffset < static_cast<int>(resultBC.backtrace.size())) {
        bOffset += (resultBC.backtrace[btOffset] == 'M' ||
                    resultBC.backtrace[btOffset] == 'I');
        cOffset += (resultBC.backtrace[btOffset] == 'M' ||
                    resultBC.backtrace[btOffset] == 'D');
        btOffset++;
      }
      offsetBab = 0;
      offsetBbc = btOffset;
