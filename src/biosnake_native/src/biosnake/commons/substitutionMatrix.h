...
// Can read a straight-forward substitution matrix file (.mat ending of the
// file) and probabilities file (.out ending of the file). If a probabilities
// file is given, it calculates a biased matrix (produces shorter = more precise
// alignments).
//
...
class SubstitutionMatrix : public BaseMatrix {
 public:
  SubstitutionMatrix(biosnake_output* output, const char *filename, float bitFactor, float scoreBias);
  ...
  static void calcLocalAaBiasCorrection(const BaseMatrix *m,
                                        const unsigned char *int_sequence,
                                        const int N, float *compositionBias);
  static void calcProfileProfileLocalAaBiasCorrection(
      short *profileScores, const size_t profileAASize, const int N,
      size_t alphabetSize);
  static void calcProfileProfileLocalAaBiasCorrectionAln(biosnake_output* out, int8_t *profileScores,
                                                         int N,
                                                         size_t alphabetSize,
                                                         BaseMatrix *subMat);
  static void calcGlobalAaBiasCorrection(const BaseMatrix *m,
                                         ...
                                         const int N);
  bool estimateLambdaAndBackground(const double **mat, int alphabetSize,
                                   double *pBack, double &lambda);
  ...
  struct FastMatrix {
    ...
    FastMatrix(const char **matrix, const char *matrixData,
               const size_t asciiStart)
  ...
  static FastMatrix createAsciiSubMat(BaseMatrix &submat) {
    ...
    return FastMatrix((const char **)matrix, (const char *)matrixData,
                      asciiStart);
  }
