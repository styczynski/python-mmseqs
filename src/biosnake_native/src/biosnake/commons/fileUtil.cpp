...
bool FileUtil::fileExists(biosnake_output* out, const char *fileName) {
  struct stat st;
  return stat(fileName, &st) == 0;
}

bool FileUtil::fileExistsAndIsNotEmpty(biosnake_output* out, const char *fileName) {
...
bool FileUtil::directoryExists(biosnake_output* out, const char *directoryName) {
  struct stat st;
  return stat(directoryName, &st) == 0 && S_ISDIR(st.st_mode);
}

bool FileUtil::makeDir(biosnake_output* out, const char *directoryName, const int mode) {
  return mkdir(directoryName, mode) == 0;
}

void *FileUtil::mmapFile(biosnake_output* out, FILE *file, size_t *dataSize) {
  struct stat sb;
...
void FileUtil::munmapData(biosnake_output* out, void *ptr, size_t dataSize) {
  if (munmap(ptr, dataSize) < 0) {
    out->failure("Failed to munmap memory");
  }
}

FILE *FileUtil::openFileOrDie(biosnake_output* out, const char *fileName, const char *mode,
                              bool shouldExist) {
...
size_t FileUtil::countLines(biosnake_output* out, const char *name) {
...
void FileUtil::deleteTempFiles(biosnake_output* out, const std::list<std::string> &tmpFiles) {
...
void FileUtil::writeFile(biosnake_output* out, const std::string &pathToFile,
...
std::string FileUtil::dirName(biosnake_output* out, const std::string &file) {
  size_t pos = file.find_last_of("\\/");
  return (std::string::npos == pos) ? "." : file.substr(0, pos);
}

std::string FileUtil::baseName(biosnake_output* out, const std::string &file) {
  size_t pos = file.find_last_of("\\/");
  return (std::string::npos == pos) ? file
                                    : file.substr(pos + 1, file.length());
}

size_t FileUtil::getFreeSpace(biosnake_output* out, const char *path) {
  struct statvfs stat;
...
std::string FileUtil::getRealPathFromSymLink(biosnake_output* out, const std::string path) {
...
std::string FileUtil::getHashFromSymLink(biosnake_output* out, const std::string path) {
  char *p = realpath(path.c_str(), NULL);
  if (p == NULL) {
    out->failure("Could not get path of {}", path);
  }
...
void FileUtil::symlinkAlias(biosnake_output* out, const std::string &file, const std::string &alias) {
...
std::string FileUtil::getCurrentWorkingDirectory(biosnake_output* out) {
...
void FileUtil::symlinkAbs(biosnake_output* out, const std::string &target, const std::string &link) {
...
size_t FileUtil::getFileSize(biosnake_output* out, const std::string &fileName) {
  struct stat stat_buf;
  int rc = stat(fileName.c_str(), &stat_buf);
  return rc == 0 ? stat_buf.st_size : -1;
}

bool FileUtil::symlinkExists(biosnake_output* out, const std::string &path) {
  struct stat buf;
  int result = lstat(path.c_str(), &buf);
  return (result == 0);
}

void FileUtil::copyFile(biosnake_output* out, const char *src, const char *dst) {
...
FILE *FileUtil::openAndDelete(biosnake_output* out, const char *fileName, const char *mode) {
...
std::vector<std::string> FileUtil::findDatafiles(biosnake_output* out, const char *datafiles) {
...
void FileUtil::remove(biosnake_output* out, const char *file) {
  if (std::remove(file) != 0) {
    out->failure("Could not delete {}", file);
  }
}

void FileUtil::move(biosnake_output* out, const char *src, const char *dst) {
...
int FileUtil::parseDbType(biosnake_output* out, const char *name) {
...
std::string FileUtil::createTemporaryDirectory(
    biosnake_output* out, std::string baseTmpPath, const std::string &tmpPath,
...
void FileUtil::fixRlimitNoFile(biosnake_output* out) {
