...
Orf::Orf(biosnake_output* output, const unsigned int requestedGenCode, bool useAllTableStarts): out(output) {
  ...
  if (useAllTableStarts) {
    // if useAllTableStarts we take all alternatives for start codons from the
    // table
...
Matcher::result_t Orf::getFromDatabase(biosnake_output* out, const size_t id,
  ...
  // compute orf length
  size_t orfLen = std::max(orfLocOnContigParsed.from, orfLocOnContigParsed.to) -
                  std::min(orfLocOnContigParsed.from, orfLocOnContigParsed.to) +
                  1;
  Matcher::result_t orfToContigResult(
      contigKey, 1, 1, 0, 1, 0, orfLen, 0, (orfLen - 1), orfLen,
      orfLocOnContigParsed.from, orfLocOnContigParsed.to, contigLen, "");
...
bool Orf::setSequence(const char *seq, size_t length) {
  ...
  if ((length + VECSIZE_INT) > bufferSize) {
    ...
    free(reverseComplement);
    sequence =
        (char *)mem_align(ALIGN_INT, (length + VECSIZE_INT) * sizeof(char));
    reverseComplement =
        (char *)mem_align(ALIGN_INT, (length + VECSIZE_INT) * sizeof(char));
...
std::pair<const char *, size_t> Orf::getSequence(
    const SequenceLocation &location) {
  assert(location.to > location.from);
  size_t length = (location.to - location.from) + 1;
  if (location.strand == Orf::STRAND_PLUS) {
    return sequence ? std::make_pair(sequence + location.from, length)
                    : std::make_pair("", 0);
  } else {
    return reverseComplement
               ? std::make_pair(reverseComplement + location.from, length)
               : std::make_pair("", 0);
  }
}

void Orf::findAll(std::vector<Orf::SequenceLocation> &result,
                  const size_t minLength, const size_t maxLength,
                  const size_t maxGaps, const unsigned int forwardFrames,
                  const unsigned int reverseFrames,
                  const unsigned int startMode) {
  if (forwardFrames != 0) {
    // find ORFs on the forward sequence
    findForward(sequence, sequenceLength, result, minLength, maxLength, maxGaps,
                forwardFrames, startMode, STRAND_PLUS);
  }

  if (reverseFrames != 0) {
    // find ORFs on the reverse complement
    findForward(reverseComplement, sequenceLength, result, minLength, maxLength,
                maxGaps, reverseFrames, startMode, STRAND_MINUS);
  }
}
...
template <int N>
#ifndef AVX2
inline bool isInCodons(const char *sequence, simd_int codons,
                       simd_int codons2) {
#else
...
void Orf::findForward(const char *sequence, const size_t sequenceLength,
                      std::vector<SequenceLocation> &result,
                      const size_t minLength, const size_t maxLength,
                      const size_t maxGaps, const unsigned int frames,
                      const unsigned int startMode, const Strand strand) {
  // An open reading frame can beginning in any of the three codon start
  // position Frame 0:  AGA ATT GCC TGA ATA AAA GGA TTA CCT TGA TAG GGT AAA
  ...
  // We want to walk over the memory only once so we calculate which codon we
  // are in and save the values of our state machine in these arrays
  ...
  for (size_t i = 0; i < sequenceLength - (FRAMES - 1); i += FRAMES) {
    for (size_t position = i; position < i + FRAMES; position++) {
      ...
      codon[0] =
          sequence[position + 0] == CHAR_MAX
              ? CHAR_MAX
              : sequence[position + 0] & static_cast<unsigned char>(~0x20);
      codon[1] =
          sequence[position + 1] == CHAR_MAX
              ? CHAR_MAX
              : sequence[position + 1] & static_cast<unsigned char>(~0x20);
      codon[2] =
          sequence[position + 2] == CHAR_MAX
              ? CHAR_MAX
              : sequence[position + 2] & static_cast<unsigned char>(~0x20);
      ...
      bool isLast =
          !thisIncomplete && isIncomplete(sequence + position + FRAMES);

      // START_TO_STOP returns the longest fragment such that the first codon is
      // a start ANY_TO_STOP returns the longest fragment LAST_START_TO_STOP
      // retruns last encountered start to stop, no start codons in the middle
      ...
      if ((startMode == START_TO_STOP)) {
        shouldStart =
            ...
            ((startCodonCount > 4)
                 ? isInCodons<8>(codon, startCodonsHi, startCodonsLo)
      ...
      if (isInsideOrf[frame] && (stop || isLast)) {
        ...
        // we include the stop codon here --> inconsistent because stops are not
        // included in other orfs
        ...
        if (countLength[frame] == 0 && stop) continue;
        // if final codon the last in the frame but not a stop - include it
        // (stops are never included):
        ...
        if ((countGaps[frame] > maxGaps) || (countLength[frame] > maxLength) ||
            (countLength[frame] < minLength)) {
          continue;
        }

        result.emplace_back(from[frame], to, !hasStartCodon[frame], !stop,
                            strand);
      }
    }
  }
...
Orf::SequenceLocation Orf::parseOrfHeader(const char *data) {
  ...
  if (columns >= 2) {
    ...
    if (pos > 0) {
      ...
      if ((entry[1][pos] == '-' || entry[1][pos] == '+') &&
          hasNumericStart == true && hasPlusMinus == false) {
        hasPlusMinus = true;
        isPlus = (entry[1][pos] == '+');
    ...
    //            } else  else if ((entry[1][i] >= '0' && entry[1][i] <= '9') &&
    //            hasNumericStart == true && hasPlusMinus == true) {
    //                hasNumericEnd = true;
    //            } else {
    //                break;
    //            }
    found =
        hasNumericStart && hasPlusMinus && hasNumericEnd && lenStart != NULL;
    if (found) {
...
size_t Orf::writeOrfHeader(char *buffer, unsigned int key, size_t fromPos,
                           size_t toPos, bool hasIncompleteStart,
