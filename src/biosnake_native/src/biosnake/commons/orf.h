...
class Orf {
 public:
  ...
  enum Strand { STRAND_PLUS = 1, STRAND_MINUS = -1 };

  enum Frame { FRAME_1 = (1u << 0), FRAME_2 = (1u << 1), FRAME_3 = (1u << 2) };

  enum StartMode { START_TO_STOP = 0, ANY_TO_STOP = 1, LAST_START_TO_STOP = 2 };
  ...
  struct SequenceLocation {
    ...
    Strand strand;
    SequenceLocation(size_t from, size_t to, bool hasIncompleteStart,
                     bool hasIncompleteEnd, Strand strand)
        : id(0),
          from(from),
          to(to),
  ...
  Orf(biosnake_output* output, const unsigned int requestedGenCode, bool useAllTableStarts);
  ~Orf();

  bool setSequence(const char* sequence, size_t sequenceLength);

  /// Find all ORFs in both orientations that are at least orfMinLength and at
  /// most orfMaxLength long. Report results as SequenceLocations. seq must be
  /// in iupac. Do not allow more than max_seq_gap consecutive N-or-gap bases in
  /// an ORF
  void findAll(std::vector<SequenceLocation>& result,
               const size_t minLength = 1, const size_t maxLength = SIZE_MAX,
  ...
  static size_t writeOrfHeader(char* buffer, unsigned int key, size_t fromPos,
