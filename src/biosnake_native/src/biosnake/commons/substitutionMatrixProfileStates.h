...
class SubstitutionMatrixProfileStates : public BaseMatrix {
 public:
  SubstitutionMatrixProfileStates(biosnake_output* output, std::string matrixName, double **probMatrix,
    ...
    generateSubMatrix(probMatrix, rMatrix, this->subMatrix, alphabetSize,
                      bitFactor, scoreBias);
    // remove X
  ...
  ~SubstitutionMatrixProfileStates() {
    delete ps;
    // delete too much memory in the BaseMatrix destructor if I do not set this
    // back to org. alph. size
    alphabetSize = origAlphabetSize;
  };

  float *getProfileVectorForState(size_t k) { return ps->getProfile(k); }
