...
#ifndef BIOSNAKE_INTERVALARRAY_H
#define BIOSNAKE_INTERVALARRAY_H
...
class IntervalArray {
 public:
  ...
  unsigned char getLowMask(unsigned int rest) {
    const unsigned char mask[8] = {0xFF, 0xFE, 0xFC, 0xF8,
                                   0xF0, 0xE0, 0xC0, 0x80};
    return mask[rest];
  }

  unsigned char getHighRest(unsigned int rest) {
    const unsigned char mask[8] = {0xFF, 0x7F, 0x3F, 0x1F,
                                   0x0F, 0x07, 0x03, 0x01};
    return mask[rest];
  }
  ...
  size_t getRangesSize() { return ranges.size(); }
  ...
  int findIndex(int low, int high) {
    ...
    it = std::lower_bound(ranges.begin(), ranges.end(), val,
                          Range::compareStart);
    if (it == ranges.end()) {
      it--;
    }
  ...
  Range getRange(int index) { return ranges[index]; }
...
#endif  // BIOSNAKE_INTERVALARRAY_H
