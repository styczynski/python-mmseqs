...
class BaseMatrix {
 public:
  BaseMatrix(biosnake_output* output);
  ...
  // generate the substitution matrix given the probability matrix, background
  // probabilities and the alphabet size
  static void generateSubMatrix(double** probMatrix, double** subMatrix,
                                float** subMatrixPseudoCounts, int size,
                                bool containsX);

  // generate a short data type substitution matrix
  static void generateSubMatrix(double** probMatrix,
                                float** subMatrixPseudoCounts,
                                short** subMatrix, int size, bool containsX,
                                double bitFactor = 1.0,
                                double scoringBias = 0.0);
  ...
  static void computeBackground(double** probMat, double* pBack,
  ...
  ProbabilityMatrix(BaseMatrix& matrix) : alphabetSize(matrix.alphabetSize) {
    ...
    for (int i = 0; i < matrix.alphabetSize; ++i) {
      ...
      for (int j = 0; j < matrix.alphabetSize; ++j) {
        probMatrix[i][j] =
            matrix.probMatrix[i][j] / (matrix.pBack[i] * matrix.pBack[j]);
      }
    }
  }
  ~ProbabilityMatrix() {
