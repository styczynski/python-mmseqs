...
#include <biosnake/commons/memoryTracker.h>
 ...
 public:
  struct Index {
    ...
    static bool compareById(const Index& x, const Index& y) {
      ...
      if (y.offset < x.offset) return false;
  ...
  struct LookupEntry {
    ...
    static bool compareById(const LookupEntry& x, const LookupEntry& y) {
      if (x.id < y.id) return true;
      if (y.id < x.id) return false;
      if (x.entryName < y.entryName) return true;
      if (y.entryName < x.entryName) return false;
      if (x.fileNumber < y.fileNumber) return true;
      if (y.fileNumber < x.fileNumber) return false;
      return false;
    }
  ...
  // = USE_DATA|USE_INDEX
  DBReader(biosnake_output* output, const char* dataFileName, const char* indexFileName, int threads,
           int mode);

  DBReader(biosnake_output* output, Index* index, size_t size, size_t aaDbSize, T lastKey, int dbType,
  ...
  static const int SORT_BY_LINE = 4;  // the local IDs correspond to the line
                                      // number in the original index file
  static const int SHUFFLE = 5;
  static const int HARDNOSORT = 6;  // do not even sort by ids.
  static const int SORT_BY_ID_OFFSET = 7;
  static const int SORT_BY_OFFSET =
      8;  // only offset sorting saves memory and does not support random access
  ...
  char* getDataForFile(size_t fileIdx) { return dataFiles[fileIdx]; }

  size_t getDataFileCnt() { return dataFileCnt; }
  ...
  static void moveDatafiles(biosnake_output* out, const std::vector<std::string>& files,
                            const std::string& destination);

  static void moveDb(biosnake_output* out, const std::string& srcDbName,
                     const std::string& dstDbName);

  static void removeDb(biosnake_output* out, const std::string& databaseName);

  static void softlinkDb(biosnake_output* out, const std::string& databaseName,
                         const std::string& outDb,
                         DBFiles::Files dbFilesFlags = DBFiles::ALL);
  static void copyDb(biosnake_output* out, const std::string& databaseName, const std::string& outDb,
                     DBFiles::Files dbFilesFlags = DBFiles::ALL);

  char* mmapData(FILE* file, size_t* dataSize);

  bool readIndex(char* data, size_t indexDataSize, Index* index,
                 size_t& dataSize);
  ...
  static DBReader<unsigned int>* unserialize(biosnake_output* out, const char* data, int threads);
  ...
  struct compareIndexLengthPairByIdKeepTrack {
    bool operator()(
        const std::pair<Index, std::pair<size_t, unsigned int> >& lhs,
        const std::pair<Index, std::pair<size_t, unsigned int> >& rhs) const {
  ...
  struct comparePairByIdAndOffset {
    bool operator()(const std::pair<unsigned int, Index>& lhs,
                    const std::pair<unsigned int, Index>& rhs) const {
      if (lhs.second.id < rhs.second.id) return true;
      if (rhs.second.id < lhs.second.id) return false;
      if (lhs.second.offset < rhs.second.offset) return true;
      if (rhs.second.offset < lhs.second.offset) return false;
      return false;
    }
  };

  struct comparePairByOffset {
    bool operator()(const std::pair<unsigned int, size_t>& lhs,
                    const std::pair<unsigned int, size_t>& rhs) const {
  ...
  int isCompressed() { return isCompressed(dbtype); }

  static int isCompressed(int dbtype);

  void setSequentialAdvice();

  void decomposeDomainByAminoAcid(size_t worldRank, size_t worldSize,
