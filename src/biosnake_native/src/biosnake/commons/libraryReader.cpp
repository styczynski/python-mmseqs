...
int LibraryReader::ReadInt(biosnake_output* out, const char* line, const char* label,
...
// line read from file pointer 'fp'.
double LibraryReader::ReadDouble(biosnake_output* out, const char* line, const char* label,
                                 const char* errmsg = NULL) {
  double rv = 0;
  if (strstr(line, label)) {
...
// line read from file pointer 'fp'.
std::string LibraryReader::ReadString(biosnake_output* out, const char* line, const char* label,
                                      const char* errmsg = NULL) {
  std::string rv;
  if (strstr(line, label)) {
...
// line read from file pointer 'fp'.
bool LibraryReader::ReadBool(biosnake_output* out, const char* line, const char* label,
                             const char* errmsg = NULL) {
  bool rv = false;
  if (strstr(line, label)) {
    const char* ptr = line + strlen(label);
...
std::vector<std::string> LibraryReader::tokenize(const char* str,
                                                 char sep = ' ') {
  ...
  while (str[pos] != '\0') {
    ...
    while (str[pos] != '\0' && str[pos] == sep) pos++;
