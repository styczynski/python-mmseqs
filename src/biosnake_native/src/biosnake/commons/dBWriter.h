...
class DBWriter : public MemoryTracker {
 public:
  DBWriter(biosnake_output* output, const char* dataFileName, const char* indexFileName,
  ...
  void writeEnd(unsigned int key, unsigned int thrIdx = 0,
                bool addNullByte = true, bool addIndexEntry = true);

  void writeData(const char* data, size_t dataSize, unsigned int key,
                 unsigned int threadIdx = 0, bool addNullByte = true,
                 bool addIndexEntry = true);

  static size_t indexToBuffer(char* buff1, unsigned int key, size_t offsetStart,
                              size_t len);
  ...
  static void writeDbtypeFile(biosnake_output* out, const char* path, int dbtype, bool isCompressed);

  size_t getStart(unsigned int threadIdx) { return starts[threadIdx]; }

  size_t getOffset(unsigned int threadIdx) { return offsets[threadIdx]; }
 ...
 private:
  ...
  static void mergeResults(biosnake_output* out,
  ...
  static void mergeIndex(biosnake_output* out, const char** indexFilenames, unsigned int fileCount,
                         const std::vector<size_t>& dataSizes);

  static void sortIndex(biosnake_output* out, const char* inFileNameIndex,
                        const char* outFileNameIndex,
