...
BaseMatrix::BaseMatrix(biosnake_output* output): out(output) {
...
void BaseMatrix::initMatrixMemory(int alphabetSize) {
  ...
  // init the background probabilities, joint probability and scoring matrices
  // with zeros
  ...
  for (int i = 0; i < alphabetSize; i++) {
    ...
    subMatrixPseudoCounts[i] =
        (float*)malloc_simd_float(alphabetSize * sizeof(float));
...
void BaseMatrix::print(short** matrix, char* num2aa, int size) {
  ...
  short avg = 0;
  printf("     ");
  for (int i = 0; i < size; i++) printf("%4c ", num2aa[i]);
  std::cout << "\n";
...
void BaseMatrix::print(double** matrix, char* num2aa, int size) {
  ...
  double avg = 0.0;
  printf("%7c ", ' ');
  for (int i = 0; i < size; i++) printf("%7c ", num2aa[i]);
  std::cout << "\n";
  for (int i = 0; i < size; i++) {
    printf("%7c ", num2aa[i]);
    for (int j = 0; j < size; j++) {
...
void BaseMatrix::computeBackground(double** probMat, double* pBack,
                                   int alphabetSize, bool containsX) {
  for (int i = 0; i < alphabetSize; i++) {
    pBack[i] = 0;
    for (int j = 0; j < alphabetSize; j++) {
      pBack[i] += probMat[i][j];
...
void BaseMatrix::generateSubMatrix(double** probMatrix, double** subMatrix,
                                   float** subMatrixPseudoCounts, int size,
                                   bool containsX) {
  ...
  for (int i = 0; i < size; i++) {
    for (int j = 0; j < size; j++) {
      subMatrixPseudoCounts[i][j] =
          probMatrix[i][j] / (pBack[j]);  // subMatrixPseudoCounts[a][b]=P(a|b)
    }
  }
...
void BaseMatrix::generateSubMatrix(double** probMatrix,
                                   float** subMatrixPseudoCounts,
                                   short** subMatrix, int size, bool containsX,
                                   double bitFactor, double scoringBias) {
  double** sm = new double*[size];
  for (int i = 0; i < size; i++) sm[i] = new double[size];
  ...
  for (int i = 0; i < size; i++) {
    for (int j = 0; j < size; j++) {
      ...
      subMatrix[i][j] =
          (pValNBitScale < 0.0) ? pValNBitScale - 0.5 : pValNBitScale + 0.5;
    }
  }

  for (int i = 0; i < size; i++) delete[] sm[i];
  delete[] sm;
}

std::string BaseMatrix::getMatrixName() { return matrixName; }

double BaseMatrix::getBackgroundProb(size_t) {
  out->failure("getBackground is not Impl. for this type of Matrix");
}

size_t BaseMatrix::memorySize(std::string& matrixName,
