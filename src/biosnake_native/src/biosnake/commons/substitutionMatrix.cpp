...
SubstitutionMatrix::SubstitutionMatrix(biosnake_output* output, const char *filename, float bitFactor,
                                       float scoreBias)
  ...
  generateSubMatrix(probMatrix, subMatrixPseudoCounts, subMatrix, alphabetSize,
                    true, bitFactor, scoreBias);
}
...
bool SubstitutionMatrix::estimateLambdaAndBackground(const double **scoreMatrix,
  ...
  lambda = calculate_lambda(&pointers[0], alphabetSize, &letterProbs1[0] - 1,
                            &letterProbs2[0] - 1);
...
void SubstitutionMatrix::calcLocalAaBiasCorrection(
    const BaseMatrix *m, const unsigned char *int_sequence, const int N,
...
void SubstitutionMatrix::calcProfileProfileLocalAaBiasCorrection(
  ...
  for (size_t aa = 0; aa < alphabetSize; aa++) pnul[aa] /= N;
  for (int i = 0; i < N; i++) {
    const int minPos = std::max(0, (i - windowSize / 2));
    const int maxPos = std::min(N, (i + windowSize / 2));
    const int windowLength = maxPos - minPos;
    ...
    for (size_t aa = 0; aa < alphabetSize; aa++) {
      profileScores[i * profileAASize + aa] = static_cast<int>(
          (profileScores + (i * profileAASize))[aa] - aaSum[aa] / windowLength);
    }
  }
  delete[] aaSum;
  delete[] pnul;
...
void SubstitutionMatrix::calcProfileProfileLocalAaBiasCorrectionAln(
  ...
  float *pnul = new float[N];  // expected score of the prof ag. a random
                               // (blosum bg dist) seq
  ...
  for (int i = 0; i < N; i++) {
    ...
    for (size_t aa = 0; aa < alphabetSize; aa++) {
      profileScores[i + aa * N] = static_cast<int8_t>(
          *(profileScores + i + N * aa) - aaSum[aa] / windowLength);
    }
  }
  delete[] aaSum;
  delete[] pnul;
...
void SubstitutionMatrix::calcGlobalAaBiasCorrection(const BaseMatrix *m,
  ...
  for (int i = 0; i < N; i++) {
    ...
    for (size_t aa = 0; aa < 20; aa++) {
      ...
      //                   static_cast<int>((profileScores + (i *
      //                   profileAASize))[aa] -  aaSum[aa]/windowLength) );
      // std::cout << i << "\t" << (profileScores + (i * profileAASize))[aa] <<
      // "\t" <<  aaSum[aa]/windowLength << "\t" <<  (profileScores + (i *
      // profileAASize))[aa] -  aaSum[aa]/windowLength << std::endl;
      profileScores[i * profileAASize + aa] = static_cast<int>(
          (profileScores + (i * profileAASize))[aa] - aaSum[aa] / windowLength);
      //            avg += static_cast<int>((profileScores + (i *
      //            profileAASize))[aa] -  aaSum[aa]/windowLength);
    }
  }
...
void SubstitutionMatrix::readProbMatrix(const std::string &matrixData,
                                        const bool containsX) {
  ...
  if (xIsPositive == false) {
    for (int i = 0; i < alphabetSize - 1; i++) {
      pBack[i] = pBack[i] *
                 (1.0 - pBack[static_cast<int>(aa2num[static_cast<int>('X')])]);
    }
  }
  ...
  for (int i = 0; i < alphabetSize; i++) {
    ...
    for (int j = 0; j < alphabetSize; j++) {
      probMatrix[i][j] =
          std::exp(lambda * probMatrix[i][j]) * pBack[i] * pBack[j];
    }
  }
}

std::pair<int, bool> SubstitutionMatrix::setAaMappingDetectAlphSize(
