...
//
// Calls SSE2 parallelized calculation of Smith-Waterman alignment and
// non-parallelized traceback afterwards.
//
...
class Matcher {
 public:
  ...
  struct result_t {
    ...
    result_t(unsigned int dbkey, int score, float qcov, float dbcov,
        ...
        : dbKey(dbkey),
          ...
          qEndPos(qEndPos),
          qLen(qLen),
          dbStartPos(dbStartPos),
          dbEndPos(dbEndPos),
          dbLen(dbLen),
          queryOrfStartPos(queryOrfStartPos),
          queryOrfEndPos(queryOrfEndPos),
          dbOrfStartPos(dbOrfStartPos),
          dbOrfEndPos(dbOrfEndPos),
          backtrace(backtrace){};
  ...
  Matcher(biosnake_output* output, int querySeqType, int maxSeqLen, BaseMatrix *m,
  ...
  result_t getSWResult(Sequence *dbSeq, const int diagonal, bool isReverse,
                       const int covMode, const float covThr,
                       const double evalThr, unsigned int alignmentMode,
                       unsigned int seqIdMode, bool isIdentical,
  ...
  // need for sorting the results
  static bool compareHitsByPosAndStrand(const result_t &first,
                                        const result_t &second) {
    if (first.dbKey != second.dbKey) {
      return first.dbKey < second.dbKey;
    }
    bool qFirstRev = (first.qStartPos > first.qEndPos);
    bool qSecondRev = (second.qStartPos > second.qEndPos);
    if (qSecondRev < qFirstRev) return false;
    if (qFirstRev < qSecondRev) return true;
    bool dbFirstRev = (first.dbStartPos > first.dbEndPos);
    bool dbSecondRev = (second.dbStartPos > second.dbEndPos);
    if (dbSecondRev < dbFirstRev) return false;
    if (dbFirstRev < dbSecondRev) return true;
    int firstQStartPos = std::min(first.qStartPos, first.qEndPos);
    int secondQStartPos = std::min(second.qStartPos, second.qEndPos);
  ...
  static result_t parseAlignmentRecord(biosnake_output* out, const char *data,
                                       bool readCompressed = false);

  static void readAlignmentResults(biosnake_output* out, std::vector<result_t> &result, char *data,
                                   bool readCompressed = false);
  ...
  static size_t resultToBuffer(char *buffer, const result_t &result,
  ...
  static void updateResultByRescoringBacktrace(
