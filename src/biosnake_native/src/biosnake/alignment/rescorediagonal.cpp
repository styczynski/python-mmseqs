...
float parsePrecisionLib(biosnake_output* out, const std::string &scoreFile, double targetSeqid,
  ...
  targetCov =
      static_cast<float>(static_cast<int>((targetCov + 0.0001) * 10)) / 10;
  while (std::getline(in, line)) {
    ...
    if (MathUtil::AreSame(cov, targetCov) &&
        MathUtil::AreSame(seqid, targetSeqid) && precision >= targetPrecision) {
      return scorePerCol;
    }
  }
...
int doRescorediagonal(biosnake_output* out, Parameters &par, DBWriter &resultWriter,
  ...
  if (par.filterHits) {
    ...
    std::string libraryString =
        (par.covMode == Parameters::COV_MODE_BIDIRECTIONAL)
            ? std::string((const char *)CovSeqidQscPercMinDiag_lib,
      ...
      for (size_t id = start; id < (start + bucketSize); id++) {
        ...
        if (*data != '\0') {
          ...
          if (reversePrefilterResult == true && queryLen > queryRevSeqLen) {
            queryRevSeq =
                static_cast<char *>(realloc(queryRevSeq, queryLen + 1));
          ...
          if (reversePrefilterResult == true) {
            ...
            for (int pos = queryLen - 1; pos > -1; pos--) {
              unsigned char res =
                  subMat->aa2num[static_cast<int>(querySeq[pos])];
              queryRevSeq[(queryLen - 1) - pos] =
                  subMat->num2aa[nuclMatrix->reverseResidue(res)];
            }
          }
        ...
        for (size_t entryIdx = 0; entryIdx < results.size(); entryIdx++) {
          ...
          const bool isIdentity =
              (queryId == targetId && (par.includeIdentity || sameQTDB))
                  ? true
                  : false;
          ...
          int alnLen = 0;
          float targetCov =
              static_cast<float>(diagonalLen) / static_cast<float>(dbLen);
          float queryCov = static_cast<float>(diagonalLen) /
                           static_cast<float>(origQueryLen);

          Matcher::result_t result;
          if (par.rescoreMode == Parameters::RESCORE_MODE_HAMMING) {
            int idCnt = (static_cast<float>(distance));
            seqId = Util::computeSeqId(par.seqIdMode, idCnt, origQueryLen,
                                       dbLen, diagonalLen);
            alnLen = diagonalLen;
          } else if (par.rescoreMode == Parameters::RESCORE_MODE_SUBSTITUTION ||
                     par.rescoreMode == Parameters::RESCORE_MODE_ALIGNMENT ||
                     par.rescoreMode ==
                         Parameters::RESCORE_MODE_GLOBAL_ALIGNMENT ||
                     par.rescoreMode ==
                         Parameters::RESCORE_MODE_WINDOW_QUALITY_ALIGNMENT) {
            evalue = evaluer.computeEvalue(distance, origQueryLen);
            bitScore =
                static_cast<int>(evaluer.computeBitScore(distance) + 0.5);
            ...
            if (par.rescoreMode == Parameters::RESCORE_MODE_ALIGNMENT ||
              ...
              // -1 since diagonal is computed from sequence Len which starts by
              // 1
              if (diagonal >= 0) {
              ...
              //                                int qAlnLen = std::max(qEndPos -
              //                                qStartPos, static_cast<int>(1));
              //                                int dbAlnLen = std::max(dbEndPos
              //                                - dbStartPos,
              //                                static_cast<int>(1)); seqId =
              //                                (alignment.score1 /
              //                                static_cast<float>(std::max(qAlnLength,
              //                                dbAlnLength)))  * 0.1656 +
              //                                0.1141;

              // compute seq.id if hit fulfills e-value but not by seqId
              // criteria
              if (evalue <= par.evalThr || isIdentity) {
                int idCnt = 0;
                for (int i = qStartPos; i <= qEndPos; i++) {
                  char qLetter =
                      querySeqToAlign[i] & static_cast<unsigned char>(~0x20);
                  char tLetter = targetSeq[dbStartPos + (i - qStartPos)] &
              ...
              queryCov =
                  SmithWaterman::computeCov(qStartPos, qEndPos, origQueryLen);
              targetCov =
                  SmithWaterman::computeCov(dbStartPos, dbEndPos, dbLen);
              if (isReverse) {
                qStartPos = queryLen - qStartPos - 1;
                qEndPos = queryLen - qEndPos - 1;
              }
              result = Matcher::result_t(
                  results[entryIdx].seqId, bitScore, queryCov, targetCov, seqId,
                  evalue, alnLen, qStartPos, qEndPos, origQueryLen, dbStartPos,
                  dbEndPos, dbLen, backtrace);
            }
          }

          // float maxSeqLen = std::max(static_cast<float>(targetLen),
          // static_cast<float>(queryLen));
          float currScorePerCol =
              static_cast<float>(distance) / static_cast<float>(diagonalLen);
          ...
          // --filter-hits
          bool hasToFilter =
              (par.filterHits == true && currScorePerCol >= scorePerColThr);
          if (isIdentity || hasToFilter ||
              (hasAlnLen && hasCov && hasSeqId && hasEvalue)) {
            if (par.rescoreMode == Parameters::RESCORE_MODE_ALIGNMENT ||
                par.rescoreMode == Parameters::RESCORE_MODE_GLOBAL_ALIGNMENT ||
                par.rescoreMode ==
                    Parameters::RESCORE_MODE_WINDOW_QUALITY_ALIGNMENT) {
              alnResults.emplace_back(result);
            } else if (par.rescoreMode ==
                       Parameters::RESCORE_MODE_SUBSTITUTION) {
              hit_t hit;
        ...
        if (par.sortResults > 0 && alnResults.size() > 1) {
          SORT_SERIAL(alnResults.begin(), alnResults.end(),
                      Matcher::compareHits);
        }
        for (size_t i = 0; i < alnResults.size(); ++i) {
          size_t len = Matcher::resultToBuffer(buffer, alnResults[i],
...
int rescorediagonal(biosnake_output *out, Parameters &par) {
  ...
  if (BiosnakeMPI::rank == 0) {
    ...
    for (int proc = 0; proc < BiosnakeMPI::numProc; ++proc) {
      std::pair<std::string, std::string> tmpFile =
          Util::createTmpFileNames(outfile, outfileIndex, proc);
