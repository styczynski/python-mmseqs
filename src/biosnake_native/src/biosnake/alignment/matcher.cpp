...
Matcher::Matcher(biosnake_output* output, int querySeqType, int maxSeqLen, BaseMatrix *m,
  ...
  if (Parameters::isEqualDbtype(
          querySeqType, Parameters::DBTYPE_PROFILE_STATE_PROFILE) == false) {
...
void Matcher::initQuery(Sequence *query) {
  currentQuery = query;
  if (Parameters::isEqualDbtype(query->getSequenceType(),
                                Parameters::DBTYPE_NUCLEOTIDES)) {
    nuclaligner->initQuery(query);
  } else if (Parameters::isEqualDbtype(query->getSeqType(),
...
Matcher::result_t Matcher::getSWResult(Sequence *dbSeq, const int diagonal,
                                       bool isReverse, const int covMode,
                                       const float covThr, const double evalThr,
                                       unsigned int alignmentMode,
                                       unsigned int seqIdMode, bool isIdentity,
                                       bool wrappedScoring) {
  ...
  if (Parameters::isEqualDbtype(dbSeq->getSequenceType(),
    ...
    if (isIdentity == false) {
      alignment = aligner->ssw_align(dbSeq->numSequence, dbSeq->L, gapOpen,
                                     gapExtend, alignmentMode, evalThr, evaluer,
                                     covMode, covThr, maskLen);
    } else {
      alignment = aligner->scoreIdentical(dbSeq->numSequence, dbSeq->L, evaluer,
                                          alignmentMode);
    }
    if (alignmentMode == Matcher::SCORE_COV_SEQID) {
      if (isIdentity == false) {
        if (alignment.cigar) {
          int32_t targetPos = alignment.dbStartPos1,
                  queryPos = alignment.qStartPos1;
          for (int32_t c = 0; c < alignment.cigarLen; ++c) {
            char letter = SmithWaterman::cigar_int_to_op(alignment.cigar[c]);
            uint32_t length =
                SmithWaterman::cigar_int_to_len(alignment.cigar[c]);
            backtrace.reserve(length);

            for (uint32_t i = 0; i < length; ++i) {
              if (letter == 'M') {
                if (dbSeq->numSequence[targetPos] ==
                    currentQuery->numSequence[queryPos]) {
                  aaIds++;
                }
                ++queryPos;
                ++targetPos;
  ...
  if (alignmentMode == Matcher::SCORE_COV ||
      alignmentMode == Matcher::SCORE_COV_SEQID) {
    qcov = alignment.qCov;
  ...
  unsigned int alnLength =
      Matcher::computeAlnLength(qStartPos, qEndPos, dbStartPos, dbEndPos);
  ...
  int bitScore =
      static_cast<int>(evaluer->computeBitScore(alignment.score1) + 0.5);

  result_t result;
  if (isReverse) {
    result = result_t(dbSeq->getDbKey(), bitScore, qcov, dbcov, seqId, evalue,
                      alnLength, qStartPos, qEndPos, origQueryLen, dbEndPos,
                      dbStartPos, dbSeq->L, backtrace);
  } else {
    result = result_t(dbSeq->getDbKey(), bitScore, qcov, dbcov, seqId, evalue,
                      alnLength, qStartPos, qEndPos, origQueryLen, dbStartPos,
                      dbEndPos, dbSeq->L, backtrace);
  }
...
void Matcher::readAlignmentResults(biosnake_output* out, std::vector<result_t> &result, char *data, bool readCompressed) {
  if (data == NULL) {
    return;
  }
...
float Matcher::estimateSeqIdByScorePerCol(uint16_t score, unsigned int qLen,
                                          unsigned int tLen) {
  float estimatedSeqId =
      (score / static_cast<float>(std::max(qLen, tLen))) * 0.1656 + 0.1141;
...
Matcher::result_t Matcher::parseAlignmentRecord(biosnake_output* out, const char *data,
                                                bool readCompressed) {
  ...
  size_t alnLength =
      Matcher::computeAlnLength(adjustQstart, qEnd, adjustDBstart, dbEnd);
  ...
  switch (columns) {
    ...
    case ALN_RES_WITH_BT_COL_CNT:
      if (readCompressed) {
        return Matcher::result_t(targetId, score, qCov, dbCov, seqId, eval,
                                 alnLength, qStart, qEnd, qLen, dbStart, dbEnd,
                                 dbLen, -1, -1, -1, -1,
                                 std::string(entry[10], entry[11] - entry[10]));
      } else {
        return Matcher::result_t(
            targetId, score, qCov, dbCov, seqId, eval, alnLength, qStart, qEnd,
            qLen, dbStart, dbEnd, dbLen, -1, -1, -1, -1,
    ...
    case ALN_RES_WITH_ORF_POS_WITHOUT_BT_COL_CNT:
      return Matcher::result_t(
          targetId, score, qCov, dbCov, seqId, eval, alnLength, qStart, qEnd,
          qLen, dbStart, dbEnd, dbLen, Util::fast_atoi<int>(entry[10]),
          Util::fast_atoi<int>(entry[11]), Util::fast_atoi<int>(entry[12]),
          Util::fast_atoi<int>(entry[13]), "");
...
size_t Matcher::resultToBuffer(char *buff1, const result_t &result,
                               bool addBacktrace, bool compress,
                               bool addOrfPosition) {
  char *basePos = buff1;
    ...
    if (compress) {
      *(tmpBuff - 1) = '\t';
      std::string compressedCigar =
          Matcher::compressAlignment(result.backtrace);
      tmpBuff =
      ...
      *(tmpBuff - 1) = '\t';
      tmpBuff =
          strncpy(tmpBuff, result.backtrace.c_str(), result.backtrace.length());
...
void Matcher::updateResultByRescoringBacktrace(
    const char *querySeq, const char *targetSeq, const char **subMat,
  ...
  for (unsigned int pos = 0; pos < result.backtrace.size(); pos++) {
    ...
    int curr;
    if (letter == 'M') {
      curr = subMat[static_cast<int>(querySeq[queryPos])]
                   [static_cast<int>(targetSeq[targetPos])];
  ...
  result.seqId =
      static_cast<float>(maxIdAaCnt) / static_cast<float>(result.alnLength);
