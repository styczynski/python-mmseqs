#ifndef BIOSNAKE_EVALUE_COMPUTATION_H
#define BIOSNAKE_EVALUE_COMPUTATION_H
...
class EvalueComputation {
 public:
  EvalueComputation(biosnake_output* output, size_t dbResCount, BaseMatrix *subMat)
      : out(output), dbResCount(dbResCount) {
    init(subMat, 0, 0, false);
  }
  EvalueComputation(biosnake_output* output, size_t dbResCount, BaseMatrix *subMat, int gapOpen,
                    int gapExtend)
      : out(output), dbResCount(dbResCount) {
  ...
  int minScore(double evalue, size_t qL) {
    ...
    double s =
        (std::log(evaluer.parameters().K * area(60, qL)) - std::log(evalue)) /
  ...
  inline double computeLogEvalue(double score, double seqLength) {
    double eval = std::max(computeEvalue(score, seqLength),
  ...
  void init(BaseMatrix *subMat, int gapOpen, int gapExtend, bool isGapped) {
    ...
    const static EvalueParameters defaultParameter[] = {
        {"nucleotide.out",
         ...
         1,
         true,
         {1.0960171987681839, 0.33538787507026158, 2.0290734315292083,
          -0.46514786408422282, 2.0290734315292083, -0.46514786408422282,
          5.0543294182155085, 15.130999712620039, 5.0543294182155085,
          15.130999712620039, 5.0543962679167036, 15.129930117400917}},
        ...
        {"blosum62.out",
         ...
         {0.27359865037097330642, 0.044620920658722244834,
          1.5938724404943873658, -19.959867650284412122, 1.5938724404943873658,
          -19.959867650284412122, 30.455610143099914211, -622.28684628915891608,
          30.455610143099914211, -622.28684628915891608, 29.602444874818868215,
          -601.81087985041381216}},
        {"blosum62.out",
         0,
         0,
         false,
         {0.3207378152604042354, 0.13904657125294345166, 0.76221128839920349041,
          0, 0.76221128839920349041, 0, 4.5269915477182944841, 0,
          4.5269915477182944841, 0, 4.5269915477182944841, 0}}};
    ...
    if (par != NULL) {
      ...
      if (isGapped) {
        //-1 to avoid X
        evaluer.initGapped(subMat->alphabetSize - 1,
                           (const long *const *)tmpMat, subMat->pBack,
                           subMat->pBack, gapOpen, gapExtend, gapOpen,
                           gapExtend, false, lambdaTolerance, kTolerance,
        ...
        evaluer.initGapless(subMat->alphabetSize - 1,
                            (const long *const *)tmpMat, subMat->pBack,
                            subMat->pBack, maxSeconds);
        //                std::cout << std::setprecision(20) <<
        //                          evaluer.parameters().lambda <<"\t" <<
        //                          evaluer.parameters().K <<"\t" <<
        //                          evaluer.parameters().a_J<<"\t" <<
...
#endif  // BIOSNAKE_EVALUE_COMPUTATION_H
