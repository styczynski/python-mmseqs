...
//
#include <biosnake/alignment/bandedNucleotideAligner.h>
...
BandedNucleotideAligner::BandedNucleotideAligner(biosnake_output* output, BaseMatrix *subMat,
...
void BandedNucleotideAligner::initQuery(Sequence *query) {
  ...
  if (query->L >= querySeqRevDataLen) {
    ...
    queryRevCompSeq =
        static_cast<uint8_t *>(realloc(queryRevCompSeq, query->L + 1));
    queryRevCompCharSeq =
        static_cast<char *>(realloc(queryRevCompCharSeq, query->L + 1));
    queryRevCompSeqRev =
        static_cast<uint8_t *>(realloc(queryRevCompSeqRev, query->L + 1));
    querySeqRevDataLen = query->L;
  }
  SmithWaterman::seq_reverse((int8_t *)querySeqRev, (int8_t *)querySeq,
                             query->L);
  ...
  for (int pos = query->L - 1; pos > -1; pos--) {
    ...
    queryRevCompCharSeq[(query->L - 1) - pos] =
        subMat->num2aa[subMat->reverseResidue(res)];
  }
  SmithWaterman::seq_reverse((int8_t *)queryRevCompSeqRev,
                             (int8_t *)queryRevCompSeq, query->L);
}

s_align BandedNucleotideAligner::align(Sequence *targetSeqObj, int diagonal,
                                       bool reverse, std::string &backtrace,
                                       int &aaIds, EvalueComputation *evaluer,
                                       bool wrappedScoring) {
  ...
  if (targetSeqObj->L >= targetSeqRevDataLen) {
    targetSeqRev =
        static_cast<uint8_t *>(realloc(targetSeqRev, targetSeqObj->L + 1));
    targetSeqRevDataLen = targetSeqObj->L;
  }
  SmithWaterman::seq_reverse((int8_t *)targetSeqRev, (int8_t *)targetSeq,
                             targetSeqObj->L);
  ...
  if (wrappedScoring) {
    alignment = DistanceCalculator::computeUngappedWrappedAlignment(
        queryCharSeqAlign, querySeqObj->L, targetSeqObj->getSeqData(),
        targetSeqObj->L, diagonal, fastMatrix.matrix,
        Parameters::RESCORE_MODE_ALIGNMENT);
  ...
  if (qUngappedEndPos - qUngappedStartPos == origQueryLen - 1 &&
    ...
    result.qCov = SmithWaterman::computeCov(result.qStartPos1, result.qEndPos1,
                                            querySeqObj->L);
    if (wrappedScoring) result.qCov = std::min(1.0f, result.qCov * 2);
    result.tCov = SmithWaterman::computeCov(result.dbStartPos1,
                                            result.dbEndPos1, targetSeqObj->L);
    result.evalue = evaluer->computeEvalue(result.score1, origQueryLen);
    for (int i = qUngappedStartPos; i <= qUngappedEndPos; i++) {
      aaIds += (querySeqAlign[i] ==
                targetSeq[dbUngappedStartPos + (i - qUngappedStartPos)])
                   ? 1
                   : 0;
    }
  ...
  //    printf("%d\t%d\t%d\n", alignment.score,  alignment.startPos,
  //    alignment.endPos);
  ...
  ksw_extz2_sse(0, queryRevLenToAlign, querySeqRevAlign + qStartRev,
                targetSeqObj->L - tStartRev, targetSeqRev + tStartRev, 5, mat,
                gapo, gape, 64, zdrop, flag, &ez);
  ...
  ksw_extz2_sse(0, queryLenToAlign, querySeqAlign + qStartPos,
                targetSeqObj->L - tStartPos, targetSeq + tStartPos, 5, mat,
                gapo, gape, 64, zdrop, alignFlag, &ezAlign);
  ...
  result.tCov = SmithWaterman::computeCov(result.dbStartPos1, result.dbEndPos1,
                                          targetSeqObj->L);
  result.evalue = evaluer->computeEvalue(result.score1, origQueryLen);
  if (result.cigar) {
