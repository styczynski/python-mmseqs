...
PSSMCalculator::PSSMCalculator(biosnake_output* output, BaseMatrix *subMat, size_t maxSeqLength,
  ...
  this->matchWeight = (float *)malloc_simd_float(
      Sequence::PROFILE_AA_SIZE * (maxSeqLength + 1) * sizeof(float));
  this->pseudocountsWeight = (float *)malloc_simd_float(
      Sequence::PROFILE_AA_SIZE * (maxSeqLength + 1) * sizeof(float));
  ...
  unsigned int NAA_ALIGNSIZE =
      ...
      sizeof(float);
  NAA_ALIGNSIZE =
      ((NAA_ALIGNSIZE + ALIGN_FLOAT - 1) / ALIGN_FLOAT) * ALIGN_FLOAT;
  w_contrib = new float *[maxSeqLength + 1];
  w_contrib_backing = (unsigned char *)mem_align(
      ALIGN_FLOAT, NAA_ALIGNSIZE * (maxSeqLength + 1));
  ...
  n_backing =
      (unsigned char *)mem_align(ALIGN_INT, NAA_ALIGNSIZE * (maxSeqLength + 2));
...
PSSMCalculator::Profile PSSMCalculator::computePSSMFromMSA(size_t setSize,
  ...
  if (wg == false) {
    ...
    computeContextSpecificWeights(matchWeight, seqWeight, Neff_M, queryLength,
                                  setSize, msaSeqs);
  } else {
    // compute matchWeight based on sequence weight
    computeMatchWeights(matchWeight, seqWeight, setSize, queryLength, msaSeqs);
    // compute NEFF_M
    computeNeff_M(matchWeight, seqWeight, Neff_M, queryLength, setSize,
                  msaSeqs);
  }
  // compute consensus sequence
  std::string consensusSequence = computeConsensusSequence(
      matchWeight, queryLength, subMat->pBack, subMat->num2aa);
  if (pca > 0.0) {
    // add pseudocounts (compute the scalar product between matchWeight and
    // substitution matrix with pseudo counts)
    preparePseudoCounts(matchWeight, pseudocountsWeight,
                        Sequence::PROFILE_AA_SIZE, queryLength,
                        (const float **)subMat->subMatrixPseudoCounts);
    //    SubstitutionMatrix::print(subMat->subMatrixPseudoCounts,
    //    subMat->num2aa, 20 );
    computePseudoCounts(profile, matchWeight, pseudocountsWeight,
                        Sequence::PROFILE_AA_SIZE, Neff_M, queryLength, pca,
                        pcb);
  } else {
    for (size_t pos = 0; pos < queryLength; pos++) {
      for (size_t aa = 0; aa < Sequence::PROFILE_AA_SIZE; ++aa) {
        profile[pos * Sequence::PROFILE_AA_SIZE + aa] =
            matchWeight[pos * Sequence::PROFILE_AA_SIZE + aa];
        ;
      }
    }
  }
...
void PSSMCalculator::printPSSM(size_t queryLength) {
  ...
  for (size_t i = 0; i < queryLength; i++) {
    ...
    for (size_t aa = 0; aa < Sequence::PROFILE_AA_SIZE; aa++) {
      ...
      //            -128) ? 0 : pssm[i * Sequence::PROFILE_AA_SIZE + aa]  ;
      char pssmVal = pssm[i * Sequence::PROFILE_AA_SIZE + aa];
...
void PSSMCalculator::computeLogPSSM(char *pssm, const float *profile,
                                    float bitFactor, size_t queryLength,
                                    float scoreBias) {
  for (size_t pos = 0; pos < queryLength; pos++) {
    for (size_t aa = 0; aa < Sequence::PROFILE_AA_SIZE; aa++) {
      ...
      pssm[idx] =
          static_cast<char>((pssmVal < 0.0) ? pssmVal - 0.5 : pssmVal + 0.5);
      float truncPssmVal = std::min(pssmVal, 127.0f);
      truncPssmVal = std::max(-128.0f, truncPssmVal);
      pssm[idx] = static_cast<char>((truncPssmVal < 0.0) ? truncPssmVal - 0.5
                                                         : truncPssmVal + 0.5);
    }
  }
}

void PSSMCalculator::preparePseudoCounts(float *frequency,
                                         float *frequency_with_pseudocounts,
                                         size_t entrySize, size_t queryLength,
                                         float const **R) {
  for (size_t pos = 0; pos < queryLength; pos++) {
    for (size_t aa = 0; aa < Sequence::PROFILE_AA_SIZE; aa++) {
      frequency_with_pseudocounts[pos * entrySize + aa] =
          ScalarProd20(R[aa], &frequency[pos * entrySize]);
    }
  }
}
void PSSMCalculator::computeNeff_M(float *frequency, float *seqWeight,
                                   float *Neff_M, size_t queryLength,
                                   size_t setSize, char const **msaSeqs) {
  float Neff_HMM = 0.0f;
  for (size_t pos = 0; pos < queryLength; pos++) {
    float sum = 0.0f;
    for (size_t aa = 0; aa < Sequence::PROFILE_AA_SIZE; ++aa) {
  ...
  float scale = MathUtil::flog2(
      (Nlim - Neff_HMM) / (Nlim - 1.0));  // for calculating Neff for those seqs
                                          // with inserts at specific pos
  for (size_t pos = 0; pos < queryLength; pos++) {
    ...
    Neff_M[pos] =
        (w_M < 0) ? 1.0 : Nlim - (Nlim - 1.0) * MathUtil::fpow2(scale * w_M);
    //        fprintf(stderr,"M  i=%3i  ncol=---  Neff_M=%5.2f  Nlim=%5.2f
    //        w_M=%5.3f  Neff_M=%5.2f\n",pos,Neff_HMM,Nlim,w_M,Neff_M[pos]);
  }
}

void PSSMCalculator::computeSequenceWeights(float *seqWeight,
                                            size_t queryLength, size_t setSize,
                                            const char **msaSeqs) {
  ...
  for (size_t pos = 0; pos < queryLength; pos++) {
    int nl[Sequence::PROFILE_AA_SIZE];  // nl[a] = number of seq's with amino
                                        // acid a at position l
    ...
    for (size_t k = 0; k < setSize; ++k) {
      if (msaSeqs[k][pos] != MultipleAlignment::GAP) {
        ...
        // residue of a long sequence: contribution is proportional to one over
        // sequence length nres[k] plus 30.
      }
    }
  }
  delete[] number_res;
...
void PSSMCalculator::computePseudoCounts(float *profile, float *frequency,
                                         float *frequency_with_pseudocounts,
                                         size_t entrySize, float *Neff_M,
                                         size_t queryLength, float pca,
                                         float pcb) {
  for (size_t pos = 0; pos < queryLength; pos++) {
...
void PSSMCalculator::computeMatchWeights(float *matchWeight, float *seqWeight,
                                         size_t setSize, size_t queryLength,
                                         const char **msaSeqs) {
  for (size_t pos = 0; pos < queryLength; pos++) {
    ...
    MathUtil::NormalizeTo1(&matchWeight[pos * Sequence::PROFILE_AA_SIZE],
                           Sequence::PROFILE_AA_SIZE, subMat->pBack);
  }
}

void PSSMCalculator::computeContextSpecificWeights(float *matchWeight,
                                                   float *wg, float *Neff_M,
                                                   size_t queryLength,
                                                   size_t setSize,
                                                   const char **X) {
  // For weighting: include only columns into subalignment i that have a max
  // fraction of seqs with endgap
  const float MAXENDGAPFRAC = 0.1;
  const int NCOLMIN = 20;  // min number of cols in subalignment for calculating
                           // pos-specific weights w[k][i]
  const int ENDGAP = 22;   // Important to distinguish because end gaps do not
  ...
  // insert endgaps
  for (size_t k = 0; k < setSize; ++k) {
    for (size_t i = 0; i < queryLength && X[k][i] == MultipleAlignment::GAP;
         ++i)
      ((char **)X)[k][i] = ENDGAP;
    for (int i = queryLength - 1; i >= 0 && X[k][i] == MultipleAlignment::GAP;
         i--)
      ((char **)X)[k][i] = ENDGAP;
  }
  //////////////////////////////////////////////////////////////////////////////////////////////
  // Main loop through alignment columns
  for (size_t i = 0; i < queryLength;
       i++)  // Calculate wi[k] at position i as well as Neff[i]
  {
    bool change = false;
    // Check all sequences k and update n[j][a] and ri[j] if necessary
    for (size_t k = 0; k < setSize; ++k) {
      // Update amino acid and GAP / ENDGAP counts for sequences with AA in i-1
      // and GAP/ENDGAP in i or vice versa
      //            printf("%d %d %d\n", k, i, (int) X[k][i - 1]);
      if ((i == 0 && X[k][i] < MultipleAlignment::ANY) ||
          (i != 0 && X[k][i - 1] >= MultipleAlignment::ANY &&
           X[k][i] < MultipleAlignment::ANY)) {  // ... if sequence k was NOT
                                                 // included in i-1 and has to
                                                 // be included for column i
        change = true;
        nseqi++;
        for (size_t j = 0; j < queryLength; ++j) {
          n[j][(int)X[k][j]]++;
        }
      } else if (i != 0 && X[k][i - 1] < MultipleAlignment::ANY &&
                 X[k][i] >=
                     MultipleAlignment::ANY) {  // ... if sequence k WAS
                                                // included in i-1 and has to be
                                                // thrown out for column i
        change = true;
        nseqi--;
        for (size_t j = 0; j < queryLength; ++j) n[j][(int)X[k][j]]--;
      }
    ...
    if (change) {
      ...
      // Find min and max borders between which > fraction MAXENDGAPFRAC of
      // sequences in subalignment contain an aa
      int jmin;
      int jmax;
      for (jmin = 0; jmin < static_cast<int>(queryLength) &&
                     n[jmin][ENDGAP] > MAXENDGAPFRAC * nseqi;
           ++jmin) {
      };
      // TODO maybe wrong jmax >= 0
      for (jmax = queryLength - 1;
      ...
      if (ncol < NCOLMIN) {
        ...
        for (int j = jmin; j <= jmax; ++j) {
          ...
          const int aa_size =
              (MultipleAlignment::ANY + VECSIZE_INT - 1) / VECSIZE_INT;
          for (int a = 0; a < aa_size; ++a) {
            ...
            simdf32_store(w_contrib[j] + (a * VECSIZE_INT),
                          simdf32_sub(simdf32_add(rcp, rcp), mul));
          }
          for (int a = MultipleAlignment::ANY; a < MultipleAlignment::NAA + 3;
               ++a)
            w_contrib[j][a] = 0.0f;  // set non-amino acid values to 0 to avoid
                                     // checking in next loop for X[k][j]<ANY
        }

        // Compute pos-specific weights wi[k]
        for (size_t k = 0; k < setSize; ++k) {
          if (X[k][i] >= MultipleAlignment::ANY) continue;
          for (int j = jmin; j <= jmax;
               ++j)  // innermost, time-critical loop; O(L*setSize*L)
      ...
      for (int j = jmin; j <= jmax; ++j) {
        ...
        for (int a = 0; a < 20; ++a)
          if (f[j][a] > 1E-10) Neff_M[i] -= f[j][a] * MathUtil::flog2(f[j][a]);
      }
    ...
    MathUtil::NormalizeTo1((matchWeight + i * Sequence::PROFILE_AA_SIZE),
                           MultipleAlignment::NAA, subMat->pBack);
  }
  // remove end gaps
  for (size_t k = 0; k < setSize; ++k) {
...
std::string PSSMCalculator::computeConsensusSequence(float *frequency,
                                                     size_t queryLength,
                                                     double *pBack,
                                                     char *num2aa) {
  std::string consens;
...
void PSSMCalculator::Profile::toBuffer(Sequence &centerSequence,
                                       BaseMatrix &subMat,
                                       std::string &result) {
  toBuffer(centerSequence.numSequence, centerSequence.L, subMat, result);
}

void PSSMCalculator::Profile::toBuffer(const unsigned char *centerSequence,
                                       size_t centerSeqLen, BaseMatrix &subMat,
                                       std::string &result) {
  for (size_t pos = 0; pos < centerSeqLen; pos++) {
    for (size_t aa = 0; aa < Sequence::PROFILE_AA_SIZE; aa++) {
      result.push_back(
          Sequence::scoreMask(prob[pos * Sequence::PROFILE_AA_SIZE + aa]));
    }
    ...
    result.push_back(static_cast<unsigned char>(
        subMat.aa2num[static_cast<int>(consensus[pos])]));
    result.push_back(
        static_cast<unsigned char>(MathUtil::convertNeffToChar(neffM[pos])));
  }
}
