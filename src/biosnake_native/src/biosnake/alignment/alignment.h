...
class Alignment {
 public:
  Alignment(biosnake_output* output, const std::string &querySeqDB, const std::string &targetSeqDB,
  ...
  static bool checkCriteria(Matcher::result_t &res, bool isIdentity,
                            double evalThr, double seqIdThr, int alnLenThr,
                            int covMode, float covThr);

  static unsigned int initSWMode(unsigned int alignmentMode, float covThr,
                                 float seqIdThr);
 ...
 private:
  ...
  // keeps state of the SW alignment mode (ALIGNMENT_MODE_SCORE_ONLY,
  // ALIGNMENT_MODE_SCORE_COV or ALIGNMENT_MODE_SCORE_COV_SEQID)
  unsigned int swMode;
  ...
  void computeAlternativeAlignment(unsigned int queryDbKey, Sequence &dbSeq,
                                   std::vector<Matcher::result_t> &vector,
                                   Matcher &matcher, float covThr,
