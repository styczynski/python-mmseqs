...
#ifndef BIOSNAKE_MULTIPLEALIGNMENT_H
#define BIOSNAKE_MULTIPLEALIGNMENT_H
...
class MultipleAlignment {
 public:
  enum alignment_element {
    ...
    ENDGAP = 22  // number representing a ignored gaps (for some calculations
                 // like gap percentage)
  };
  ...
  struct MSAResult {
    ...
    MSAResult(size_t msaSequenceLength, size_t centerLength, size_t setSize,
              char **msa)
        : msaSequenceLength(msaSequenceLength),
          centerLength(centerLength),
          setSize(setSize),
          msaSequence(msa) {}
  };

  MultipleAlignment(biosnake_output* output, size_t maxSeqLen, SubstitutionMatrix *subMat);

  ~MultipleAlignment();

  MSAResult computeMSA(Sequence *centerSeq,
 ...
 private:
  ...
  void computeQueryGaps(unsigned int *queryGaps, Sequence *centerSeq,
                        size_t edges,
                        const std::vector<Matcher::result_t> &alignmentResults);

  size_t updateGapsInCenterSequence(char **msaSequence, Sequence *centerSeq,
                                    bool noDeletionMSA);

  void updateGapsInSequenceSet(
...
#endif  // BIOSNAKE_MULTIPLEALIGNMENT_H
