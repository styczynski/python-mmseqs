...
void CompressedA3M::extractMatcherResults(
    unsigned int &key, std::vector<Matcher::result_t> &results,
    const char *data, size_t dataSize, DBReader<unsigned int> &sequenceReader,
    bool skipFirst) {
  // read stuff till compressed part
  char lastChar = '\0';
  size_t index = 0;
...
//#define CA3M_DEBUG
void CompressedA3M::hitToBuffer(unsigned int targetId,
                                const Matcher::result_t &hit,
                                std::string &buffer) {
#ifndef CA3M_DEBUG
  buffer.append(reinterpret_cast<const char *>(&targetId),
                sizeof(unsigned int));
#else
...
#ifndef CA3M_DEBUG
  buffer.append(reinterpret_cast<const char *>(&startPos),
  ...
  for (size_t btIndex = 0; btIndex < hit.backtrace.size();) {
    ...
    while (btIndex < hit.backtrace.size() && hit.backtrace[btIndex] == 'M' &&
           matchLen < 255) {
      btIndex++;
      matchLen++;
    }
...
#ifndef CA3M_DEBUG
    buffer.append(reinterpret_cast<const char *>(&matchLen),
    ...
    while (btIndex < hit.backtrace.size() &&
           hit.backtrace[btIndex] == inOrDel && indelLen < 127) {
      btIndex++;
      indelLen++;
    }

    // deletions must be counted backwards
    // deletion states are insertion states from the perspective of the aligned
    // sequence
...
#ifndef CA3M_DEBUG
  buffer.replace(countPos, sizeof(unsigned short),
                 reinterpret_cast<const char *>(&nbOfBlocks),
                 sizeof(unsigned short));
#else
