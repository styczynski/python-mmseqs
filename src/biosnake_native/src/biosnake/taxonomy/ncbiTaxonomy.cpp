...
NcbiTaxonomy::NcbiTaxonomy(biosnake_output* output, const std::string& namesFile,
  ...
  for (std::vector<TaxonNode>::const_iterator it = tmpNodes.begin();
       it != tmpNodes.end(); ++it) {
...
std::vector<std::string> splitByDelimiter(const std::string& s,
...
std::pair<int, std::string> parseName(biosnake_output* out, const std::string& line) {
...
void NcbiTaxonomy::elh(std::vector<std::vector<TaxID>> const& children,
                       TaxID taxId, int level, std::vector<int>& tmpE,
                       std::vector<int>& tmpL) {
  assert(taxId > 0);
  int id = nodeId(taxId);
  ...
  for (std::vector<TaxID>::const_iterator child_it = children[id].begin();
       child_it != children[id].end(); ++child_it) {
...
// AtRanks returns a slice of slices having the taxons at the specified
// taxonomic levels
std::vector<std::string> NcbiTaxonomy::AtRanks(
    TaxonNode const* node, const std::vector<std::string>& levels) const {
  std::vector<std::string> result;
  ...
  for (std::vector<std::string>::const_iterator it = levels.begin();
    ...
    // ii) No taxon for the LCA at the required level -- give the first known
    // upstream
...
std::vector<std::string> NcbiTaxonomy::parseRanks(biosnake_output* out, const std::string& ranks) {
...
std::map<std::string, std::string> NcbiTaxonomy::AllRanks(
    TaxonNode const* node) const {
...
std::unordered_map<TaxID, TaxonCounts> NcbiTaxonomy::getCladeCounts(
  ...
  for (std::unordered_map<TaxID, unsigned int>::const_iterator it =
    ...
    if (nodeExists(it->first)) {
      ...
      while (taxon->parentTaxId != taxon->taxId &&
             nodeExists(taxon->parentTaxId)) {
  ...
  for (size_t i = 0; i < maxNodes; ++i) {
    ...
    if (tn.parentTaxId != tn.taxId && cladeCounts.count(tn.taxId)) {
      std::unordered_map<TaxID, TaxonCounts>::iterator itp =
          cladeCounts.find(tn.parentTaxId);
...
NcbiTaxonomy* NcbiTaxonomy::openTaxonomy(biosnake_output* out, const std::string& database) {
...
WeightedTaxHit::WeightedTaxHit(biosnake_output* output, const TaxID taxon, const float evalue,
...
WeightedTaxResult NcbiTaxonomy::weightedMajorityLCA(
    const std::vector<WeightedTaxHit>& setTaxa, const float majorityCutoff) {
  ...
  for (size_t i = 0; i < setTaxa.size(); ++i) {
    ...
    // iterate all ancestors up to root (including). add currWeight and
    // candidate status to each
    TaxID currParentTaxId = node->parentTaxId;
    while (currParentTaxId != currTaxId) {
      if ((it = ancTaxIdsCounts.find(currParentTaxId)) !=
          ancTaxIdsCounts.end()) {
  ...
  if (totalAssignedSeqsWeights == 0) {
    return WeightedTaxResult(selctedTaxon, assignedSeqs, unassignedSeqs, 0,
                             0.0);
  }
  ...
  for (std::map<TaxID, TaxNode>::iterator it = ancTaxIdsCounts.begin();
       it != ancTaxIdsCounts.end(); it++) {
    ...
    if (currPercent >= majorityCutoff) {
      // iterate all ancestors to find lineage min rank (the candidate is a
      // descendant of a node with this rank)
      ...
      if ((currMinRank < minRank) ||
          ((currMinRank == minRank) && (currPercent > selectedPercent))) {
  ...
  // count the number of seqs who have selectedTaxon in their ancestors (agree
  // with selection):
  if (selctedTaxon == ROOT_TAXID) {
    // all agree with "root"
    return WeightedTaxResult(selctedTaxon, assignedSeqs, unassignedSeqs,
                             assignedSeqs, selectedPercent);
  }
  if (selctedTaxon == 0) {
    // nothing informative
    return WeightedTaxResult(selctedTaxon, assignedSeqs, unassignedSeqs, 0,
  ...
  return WeightedTaxResult(selctedTaxon, assignedSeqs, unassignedSeqs,
                           seqsAgreeWithSelectedTaxon, selectedPercent);
}
