...
#ifndef BIOSNAKE_NCBITAXONOMY_H
#define BIOSNAKE_NCBITAXONOMY_H

#include <biosnake/output.h>
#include <biosnake/commons/stringBlock.h>
...
struct TaxonNode {
 public:
  ...
  TaxonNode(int id, TaxID taxId, TaxID parentTaxId, size_t rankIdx,
            size_t nameIdx)
      : id(id),
        taxId(taxId),
...
struct WeightedTaxHit {
  WeightedTaxHit(biosnake_output* output, const TaxID taxon, const float evalue,
...
struct WeightedTaxResult {
  WeightedTaxResult(TaxID taxon, size_t assignedSeqs, size_t unassignedSeqs,
                    size_t seqsAgreeWithSelectedTaxon, double selectedPercent)
      : taxon(taxon),
        assignedSeqs(assignedSeqs),
        unassignedSeqs(unassignedSeqs),
        seqsAgreeWithSelectedTaxon(seqsAgreeWithSelectedTaxon),
...
struct TaxonCounts {
  ...
  unsigned int
      cladeCount;  // number of reads/sequences matching to taxa or its children
...
static const std::map<std::string, char> NcbiShortRanks = {
    {"species", 's'}, {"genus", 'g'},  {"family", 'f'},  {"order", 'o'},
    {"class", 'c'},   {"phylum", 'p'}, {"kingdom", 'k'}, {"superkingdom", 'd'}};

class NcbiTaxonomy {
 public:
  static NcbiTaxonomy *openTaxonomy(biosnake_output* out, const std::string &database);
  NcbiTaxonomy(biosnake_output* output, const std::string &namesFile, const std::string &nodesFile,
  ...
  static std::vector<std::string> parseRanks(biosnake_output* out, const std::string &ranks);
  ...
  std::unordered_map<TaxID, TaxonCounts> getCladeCounts(
      std::unordered_map<TaxID, unsigned int> &taxonCounts) const;

  WeightedTaxResult weightedMajorityLCA(
 ...
 private:
  ...
  NcbiTaxonomy(TaxonNode *taxonNodes, size_t maxNodes, int maxTaxID, int *D,
               int *E, int *L, int *H, int **M,
