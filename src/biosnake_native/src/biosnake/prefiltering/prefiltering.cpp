...
#include <biosnake/commons/patternCompiler.h>
...
Prefiltering::Prefiltering(biosnake_output *out, const std::string &queryDB,
...
void Prefiltering::setupSplit(biosnake_output* out, DBReader<unsigned int> &tdbr,
  ...
  optimalNumSplits = std::max(
      static_cast<size_t>(std::max(BiosnakeMPI::numProc, 1)), optimalNumSplits);
#endif
...
ScoreMatrix Prefiltering::getScoreMatrix(biosnake_output* out, const BaseMatrix &matrix,
...
void Prefiltering::getIndexTable(biosnake_output *out, int split, size_t dbFrom,
                                 size_t dbSize) {
  if (templateDBIsIndex == true) {
    indexTable =
...
bool Prefiltering::isSameQTDB(biosnake_output* out) {
...
void Prefiltering::runAllSplits(biosnake_output *out, const std::string &resultDB,
                                const std::string &resultDBIndex) {
  runSplits(out, resultDB, resultDBIndex, 0, splits, false);
}

#ifdef HAVE_MPI
void Prefiltering::runMpiSplits(biosnake_output *out, const std::string &resultDB,
                                const std::string &resultDBIndex,
  ...
  size_t splitCount = splitCntPerProc[BiosnakeMPI::rank];
  ...
  std::pair<std::string, std::string> result = Util::createTmpFileNames(
      procTmpResultDB, procTmpResultDBIndex, BiosnakeMPI::rank + runRandomId);
  ...
  if (localTmpPath != "") {
    std::pair<std::string, std::string> resultShared =
        Util::createTmpFileNames(resultDB, resultDBIndex, BiosnakeMPI::rank);
  ...
  if (BiosnakeMPI::isMaster()) {
    results = new int[BiosnakeMPI::numProc]();
  }

  MPI_Gather(&hasResult, 1, MPI_INT, results, 1, MPI_INT, BiosnakeMPI::MASTER,
             MPI_COMM_WORLD);
  if (BiosnakeMPI::isMaster()) {
    // gather does not write the result of the master into the array
    results[BiosnakeMPI::MASTER] = hasResult;
...
int Prefiltering::runSplits(biosnake_output *out, const std::string &resultDB,
...
bool Prefiltering::runSplit(biosnake_output *out, const std::string &resultDB,
...
void Prefiltering::printStatistics(biosnake_output* out, const statistics_t &stats,
...
int Prefiltering::getKmerThreshold(biosnake_output* out, const float sensitivity,
