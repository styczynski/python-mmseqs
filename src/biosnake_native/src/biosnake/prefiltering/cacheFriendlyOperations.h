...
class CacheFriendlyOperations {
 public:
  ...
  CacheFriendlyOperations(biosnake_output* output, size_t maxElement, size_t initBinSize);
  ...
  // merge elements in CounterResult assuming that each element
  // (diagonalMatcher.id) exist at most twice
  size_t mergeElementsByScore(CounterResult *inputOutputArray, const size_t N);

  // merge elements in CounterResult by diagonal, combines elements with same
  // ids that occur after each other
  size_t mergeElementsByDiagonal(CounterResult *inputOutputArray,
                                 const size_t N);

  size_t keepMaxScoreElementOnly(CounterResult *inputOutputArray,
                                 const size_t N);
 ...
 private:
  ...
  void hashIndexEntry(unsigned short position_i, IndexEntryLocal *inputArray,
                      size_t N, CounterResult *lastPosition);

  // detect duplicates in diagonal
  size_t findDuplicates(CounterResult *output, size_t outputSize,
