...
//
// Written by Martin Steinegger martin.steinegger@snu.ac.kr and Maria Hauser
// mhauser@genzentrum.lmu.de
//
// Abstract: Index table stores the list of DB sequences containing a certain
// k-mer, for each k-mer.
//
...
struct __attribute__((__packed__)) IndexEntryLocal {
  ...
  static bool comapreByIdAndPos(IndexEntryLocal first, IndexEntryLocal second) {
    if (first.seqId < second.seqId) return true;
    if (second.seqId < first.seqId) return false;
    if (first.position_j < second.position_j) return true;
    if (second.position_j < first.position_j) return false;
    return false;
  }
};
...
struct __attribute__((__packed__)) IndexEntryLocalTmp {
  ...
  IndexEntryLocalTmp(unsigned int kmer, unsigned int seqId,
                     unsigned short position_j)
      : kmer(kmer), seqId(seqId), position_j(position_j) {}

  IndexEntryLocalTmp() {}

  static bool comapreByIdAndPos(IndexEntryLocalTmp first,
                                IndexEntryLocalTmp second) {
...
class IndexTable {
 ...
 public:
  IndexTable(biosnake_output* output, int alphabetSize, int kmerSize, bool externalData)
      : out(output),
  ...
  // count k-mers in the sequence, so enough memory for the sequence lists can
  // be allocated in the end
  size_t addSimilarKmerCount(Sequence *s, KmerGenerator *kmerGenerator) {
    ...
    while (s->hasNextKmer()) {
      ...
      const std::pair<size_t *, size_t> kmerList =
  ...
  size_t addKmerCount(Sequence *s, Indexer *idxer,
                      unsigned int *seqKmerPosBuffer, int threshold,
                      char *diagonalScore) {
    s->resetCurrPos();
    size_t countKmer = 0;
    bool removeX = (Parameters::isEqualDbtype(s->getSequenceType(),
...
#pragma omp parallel for
    for (size_t i = 0; i < tableSize; i++) {
      ...
      SORT_SERIAL(entries, entries + entrySize,
                  IndexEntryLocal::comapreByIdAndPos);
    }
  }

  // get pointer to entries array
  IndexEntryLocal *getEntries() { return entries; }

  inline size_t getOffset(size_t kmer) { return offsets[kmer]; }

  size_t *getOffsets() { return offsets; }
  ...
  void init() {
    // set the pointers in the index table to the start of the list for a
    // certain k-mer
    size_t offset = 0;
    for (size_t i = 0; i < tableSize; i++) {
  ...
  void initTableByExternalDataCopy(size_t sequenceCount, size_t tableEntriesNum,
  ...
  void printStatistics(biosnake_output *out, char *num2aa) {
    ...
    for (size_t i = 0; i < tableSize; i++) {
      ...
      if (((size_t)size) < topElements[top_N - 1].first) continue;
      for (size_t j = 0; j < top_N; j++) {
    ...
    out->output_string("INDEX_TABLE_DB_SIZE",
                       std::to_string((entrySize * sizeof(IndexEntryLocal) +
  ...
  void addSimilarSequence(Sequence *s, KmerGenerator *kmerGenerator,
                          IndexEntryLocalTmp **buffer, size_t &bufferSize,
                          Indexer *idxer) {
    // iterate over all k-mers of the sequence and add the id of s to the
    // sequence list of the k-mer (tableDummy)
    s->resetCurrPos();
    idxer->reset();
    size_t kmerPos = 0;
    while (s->hasNextKmer()) {
      const unsigned char *kmer = s->nextKmer();
      std::pair<size_t *, size_t> scoreMatrix =
          kmerGenerator->generateKmerList(kmer);
      if (kmerPos + scoreMatrix.second >= bufferSize) {
        *buffer = static_cast<IndexEntryLocalTmp *>(
            realloc(*buffer, sizeof(IndexEntryLocalTmp) * bufferSize * 2));
      ...
      for (size_t i = 0; i < scoreMatrix.second; i++) {
        ...
        if (offsets[kmerIdx + 1] - offsets[kmerIdx] == 0) continue;
    ...
    if (kmerPos > 1) {
      SORT_SERIAL(*buffer, *buffer + kmerPos,
                  IndexEntryLocalTmp::comapreByIdAndPos);
    }
  ...
  // add k-mers of the sequence to the index table
  void addSequence(Sequence *s, Indexer *idxer, IndexEntryLocalTmp **buffer,
                   size_t bufferSize, int threshold, char *diagonalScore) {
    // iterate over all k-mers of the sequence and add the id of s to the
    // sequence list of the k-mer (tableDummy)
    s->resetCurrPos();
    idxer->reset();
...
//        }
  ...
  static int computeKmerSize(biosnake_output* out, size_t aaSize) {
    return aaSize < getUpperBoundAACountForKmerSize(out, 6) ? 6 : 7;
  }

  static size_t getUpperBoundAACountForKmerSize(biosnake_output* out, int kmerSize) {
    switch (kmerSize) {
      case 6:
        return 3350000000;
      case 7:
  ...
  static size_t getUpperBoundNucCountForKmerSize(biosnake_output* out, int kmerSize) {
    switch (kmerSize) {
      case 14:
        return 3350000000;
      case 15:
 ...
 protected:
  ...
  // Index table entries: ids of sequences containing a certain k-mer, stored
