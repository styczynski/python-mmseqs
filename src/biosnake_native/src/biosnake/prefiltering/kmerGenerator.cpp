...
KmerGenerator::KmerGenerator(biosnake_output* output, size_t kmerSize, size_t alphabetSize,
...
void KmerGenerator::initDataStructure() {
  ...
  for (size_t i = 0; i < 2; i++) {
    outputScoreArray[i] =
        (short *)mem_align(ALIGN_INT, MAX_KMER_RESULT_SIZE * sizeof(short));
    outputIndexArray[i] =
        (size_t *)mem_align(ALIGN_INT, MAX_KMER_RESULT_SIZE * sizeof(size_t));
  }
}

std::pair<size_t *, size_t> KmerGenerator::generateKmerList(
    const unsigned char *int_seq, bool addIdentity) {
  int dividerBefore = 0;
  // pre compute phase
  ...
  for (size_t i = 0; i < this->divideStepCount; i++) {
    ...
    const unsigned int index = this->indexer->int2index(
        int_seq, dividerBefore, dividerBefore + divider);
  ...
  const short *inputScoreArray =
      &inputScoreMatrix->score[index * inputScoreMatrix->rowSize];
  for (size_t pos = 0;
       pos < inputScoreMatrix->rowSize && inputScoreArray[pos] >= cutoff1;
       pos++) {
    outputIndexArray[1][pos] =
        inputScoreMatrix->index[index * inputScoreMatrix->rowSize + pos];
  }
  ...
  for (i = 0; i < this->divideStepCount - 1; i++) {
    ...
    const short *nextScoreArray =
        &nextScoreMatrix->score[index * nextScoreMatrix->rowSize];
    const unsigned int *nextIndexArray =
        &nextScoreMatrix->index[index * nextScoreMatrix->rowSize];

    const size_t lastElm = calculateArrayProduct(
        inputScoreArray, inputIndexArray, sizeInputMatrix, nextScoreArray,
        nextIndexArray, nextScoreMatrix->elementSize, outputScoreArray[i % 2],
        outputIndexArray[i % 2], cutoff1, possibleRest[i + 1],
  ...
  if (addIdentity && sizeInputMatrix == 0) {
    ...
    for (unsigned int z = 0; z < this->divideStepCount; z++) {
      ...
      outputIndexArray[0][0] +=
          static_cast<size_t>(nextIndexArray[0]) * stepMultiplicator[z];
    }
...
size_t KmerGenerator::calculateArrayProduct(
    const short *__restrict scoreArray1, const size_t *__restrict indexArray1,
    const size_t array1Size, const short *__restrict scoreArray2,
    const unsigned int *__restrict indexArray2, const size_t array2Size,
    short *__restrict outputScoreArray, size_t *__restrict outputIndexArray,
    const short cutoff1, const short possibleRest, const size_t pow) {
  size_t counter = 0;
  for (size_t i = 0; i < array1Size; i++) {
    const short score_i = scoreArray1[i];
    const size_t kmer_i = indexArray1[i];
    if (score_i < cutoff1) break;
    const short cutoff2 = this->threshold - score_i - possibleRest;
    for (size_t j = 0;
         j < array2Size && (counter + 1 < (int)MAX_KMER_RESULT_SIZE) &&
         (scoreArray2[j] >= cutoff2);
         j++) {
