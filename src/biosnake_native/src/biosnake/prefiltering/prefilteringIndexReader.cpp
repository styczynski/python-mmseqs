...
#include <biosnake/prefiltering/indexBuilder.h>
#include <biosnake/commons/parameters.h>
#include <biosnake/prefiltering/prefiltering.h>
...
bool PrefilteringIndexReader::checkIfIndexFile(DBReader<unsigned int> *reader) {
  ...
  return (strncmp(version, CURRENT_VERSION, strlen(CURRENT_VERSION)) == 0)
             ? true
             : false;
}
...
void PrefilteringIndexReader::createIndexFile(
    biosnake_output *out, const std::string &outDB, DBReader<unsigned int> *dbr1,
    DBReader<unsigned int> *dbr2, DBReader<unsigned int> *hdbr1,
    ...
    int maskLowerCase, int kmerThr, int splits) {
  ...
  out->info("Write VERSION ({})", VERSION);
  writer.writeData((char *)CURRENT_VERSION,
                   strlen(CURRENT_VERSION) * sizeof(char), VERSION, SPLIT_META);
  writer.alignToPageSize(SPLIT_META);

  out->info("Write META ({})", META);
  ...
  int metadata[] = {maxSeqLen,  kmerSize,   biasCorr, alphabetSize,
                    mask,       spacedKmer, kmerThr,  seqType,
                    srcSeqType, headers1,   headers2, splits};
  ...
  if (Parameters::isEqualDbtype(seqType, Parameters::DBTYPE_HMM_PROFILE) ==
    ...
    out->info("Write SCOREMATRIX3MER ({})", SCOREMATRIX3MER);
    writer.writeData(serialized3mer, ScoreMatrix::size(s3), SCOREMATRIX3MER,
                     SPLIT_META);
    writer.alignToPageSize(SPLIT_META);
    ...
    out->info("Write SCOREMATRIX2MER ({})", SCOREMATRIX2MER);
    writer.writeData(serialized2mer, ScoreMatrix::size(s2), SCOREMATRIX2MER,
                     SPLIT_META);
    writer.alignToPageSize(SPLIT_META);
  ...
  out->info("Write SCOREMATRIXNAME ({})", SCOREMATRIXNAME);
  ...
  writer.writeData(
      ...
      SCOREMATRIXNAME, SPLIT_META);
  writer.alignToPageSize(SPLIT_META);
  free(subData);

  if (spacedKmerPattern.empty() != false) {
    out->info("Write SPACEDPATTERN ({})", SPACEDPATTERN);
    writer.writeData(spacedKmerPattern.c_str(), spacedKmerPattern.length(),
                     SPACEDPATTERN, SPLIT_META);
    writer.alignToPageSize(SPLIT_META);
  }

  out->info("Write GENERATOR ({})", GENERATOR);
  writer.writeData(version, strlen(version), GENERATOR, SPLIT_META);
  writer.alignToPageSize(SPLIT_META);

  out->info("Write DBR1INDEX ({})", DBR1INDEX);
  char *data = DBReader<unsigned int>::serialize(*dbr1);
  size_t offsetIndex = writer.getOffset(SPLIT_SEQS);
  writer.writeData(data, DBReader<unsigned int>::indexMemorySize(*dbr1),
                   DBR1INDEX, SPLIT_SEQS);
  writer.alignToPageSize(SPLIT_SEQS);

  out->info("Write DBR1DATA ({})", DBR1DATA);
  ...
  for (size_t fileIdx = 0; fileIdx < dbr1->getDataFileCnt(); fileIdx++) {
    writer.writeAdd(dbr1->getDataForFile(fileIdx),
                    dbr1->getDataSizeForFile(fileIdx), SPLIT_SEQS);
  }
  ...
  if (dbr2 == NULL) {
    writer.writeIndexEntry(DBR2INDEX, offsetIndex,
                           DBReader<unsigned int>::indexMemorySize(*dbr1) + 1,
                           SPLIT_SEQS);
    writer.writeIndexEntry(DBR2DATA, offsetData, dbr1->getTotalDataSize() + 1,
                           SPLIT_SEQS);
  } else {
    out->info("Write DBR2INDEX ({})", DBR2INDEX);
    data = DBReader<unsigned int>::serialize(*dbr2);
    writer.writeData(data, DBReader<unsigned int>::indexMemorySize(*dbr2),
                     DBR2INDEX, SPLIT_SEQS);
    writer.alignToPageSize(SPLIT_SEQS);
    out->info("Write DBR2DATA ({})", DBR2DATA);
    writer.writeStart(SPLIT_SEQS);
    for (size_t fileIdx = 0; fileIdx < dbr2->getDataFileCnt(); fileIdx++) {
      writer.writeAdd(dbr2->getDataForFile(fileIdx),
                      dbr2->getDataSizeForFile(fileIdx), SPLIT_SEQS);
    }
  ...
  if (hdbr1 != NULL) {
    out->info("Write HDR1INDEX ({})", HDR1INDEX);
    data = DBReader<unsigned int>::serialize(*hdbr1);
    size_t offsetIndex = writer.getOffset(SPLIT_SEQS);
    writer.writeData(data, DBReader<unsigned int>::indexMemorySize(*hdbr1),
                     HDR1INDEX, SPLIT_SEQS);
    writer.alignToPageSize(SPLIT_SEQS);

    out->info("Write HDR1DATA ({})", HDR1DATA);
    size_t offsetData = writer.getOffset(SPLIT_SEQS);
    writer.writeStart(SPLIT_SEQS);
    for (size_t fileIdx = 0; fileIdx < hdbr1->getDataFileCnt(); fileIdx++) {
      writer.writeAdd(hdbr1->getDataForFile(fileIdx),
                      hdbr1->getDataSizeForFile(fileIdx), SPLIT_SEQS);
    }
    writer.writeEnd(HDR1DATA, SPLIT_SEQS);
    writer.alignToPageSize(SPLIT_SEQS);
    free(data);
    if (hdbr2 == NULL) {
      writer.writeIndexEntry(
          HDR2INDEX, offsetIndex,
          DBReader<unsigned int>::indexMemorySize(*hdbr1) + 1, SPLIT_SEQS);
      writer.writeIndexEntry(HDR2DATA, offsetData,
                             hdbr1->getTotalDataSize() + 1, SPLIT_SEQS);
    }
  }
  if (hdbr2 != NULL) {
    out->info("Write HDR2INDEX ({})", HDR2INDEX);
    data = DBReader<unsigned int>::serialize(*hdbr2);
    writer.writeData(data, DBReader<unsigned int>::indexMemorySize(*hdbr2),
                     HDR2INDEX, SPLIT_SEQS);
    writer.alignToPageSize(SPLIT_SEQS);
    out->info("Write HDR2DATA ({})", HDR2DATA);
    writer.writeStart(SPLIT_SEQS);
    for (size_t fileIdx = 0; fileIdx < hdbr2->getDataFileCnt(); fileIdx++) {
      writer.writeAdd(hdbr2->getDataForFile(fileIdx),
                      hdbr2->getDataSizeForFile(fileIdx), SPLIT_SEQS);
    }
  ...
  const int adjustAlphabetSize =
      (Parameters::isEqualDbtype(seqType, Parameters::DBTYPE_NUCLEOTIDES) ||
       Parameters::isEqualDbtype(seqType, Parameters::DBTYPE_AMINO_ACIDS))
          ? alphabetSize - 1
          : alphabetSize;
  ...
  for (int s = 0; s < splits; s++) {
    ...
    IndexBuilder::fillDatabase(
        ...
        dbFrom + dbSize, kmerThr, maskMode, maskLowerCase);
    ...
    out->info("Write ENTRIES ({})", (keyOffset + ENTRIES));
    char *entries = (char *)indexTable.getEntries();
    size_t entriesSize =
        indexTable.getTableEntriesNum() * indexTable.getSizeOfEntry();
    writer.writeData(entries, entriesSize, (keyOffset + ENTRIES),
                     SPLIT_INDX + s);
    ...
    writer.writeData(seqindexDataSizePtr, 1 * sizeof(int64_t),
                     (keyOffset + SEQINDEXDATASIZE), SPLIT_INDX + s);
    writer.alignToPageSize(SPLIT_INDX + s);
    ...
    out->info("Write SEQINDEXDATA ({})", keyOffset + SEQINDEXDATA);
    ...
    // ENTRIESNUM
    out->info("Write ENTRIESNUM ({})", keyOffset + ENTRIESNUM);
    ...
    writer.writeData(entriesNumPtr, 1 * sizeof(uint64_t),
                     (keyOffset + ENTRIESNUM), SPLIT_INDX + s);
    writer.alignToPageSize(SPLIT_INDX + s);

    // SEQCOUNT
    out->info("Write SEQCOUNT ({})", (keyOffset + SEQCOUNT));
    size_t tablesize = indexTable.getSize();
    char *tablesizePtr = (char *)&tablesize;
    writer.writeData(tablesizePtr, 1 * sizeof(size_t), (keyOffset + SEQCOUNT),
                     SPLIT_INDX + s);
    writer.alignToPageSize(SPLIT_INDX + s);
  }
...
IndexTable *PrefilteringIndexReader::getIndexTable(biosnake_output* out, unsigned int split,
...
void PrefilteringIndexReader::printSummary(biosnake_output* out, DBReader<unsigned int> *dbr) {
  out->info("Index version: {}", dbr->getDataByDBKey(VERSION, 0)
                    );

  size_t id;
  if ((id = dbr->getId(GENERATOR)) != UINT_MAX) {
    out->info("Generated by:  {}", dbr->getDataUncompressed(id)
                      );
  }
  ...
  while (subMatData[pos] != '\0') {
    if (subMatData[pos] == '.' && subMatData[pos + 1] == 'o' &&
        subMatData[pos + 2] == 'u' && subMatData[pos + 3] == 't' &&
        subMatData[pos + 4] == ':') {
      break;
    }
    pos++;
  }
  out->info("ScoreMatrix:  {}", std::string(subMatData, pos + 4)
                    );
}

void PrefilteringIndexReader::printMeta(biosnake_output* out, int *metadata_tmp) {
...
PrefilteringIndexData PrefilteringIndexReader::getMetadata(
  ...
  // Keep compatible to index version 15, where meta[11] would have been zero
  // due to the alignment padding
  data.splits = meta[11] == 0 ? 1 : meta[11];

  return data;
}

std::string PrefilteringIndexReader::getSubstitutionMatrixName(
    DBReader<unsigned int> *dbr) {
  ...
  bool found = false;
  for (size_t pos = 0; pos < std::max(len, (size_t)4) - 4 && found == false;
       pos++) {
    if (data[pos] == '.' && data[pos + 1] == 'o' && data[pos + 2] == 'u' &&
        data[pos + 3] == 't' && data[pos + 4] == ':') {
...
std::string PrefilteringIndexReader::getSubstitutionMatrix(
    DBReader<unsigned int> *dbr) {
  return std::string(dbr->getDataByDBKey(SCOREMATRIXNAME, 0));
}

std::string PrefilteringIndexReader::getSpacedPattern(
    DBReader<unsigned int> *dbr) {
...
ScoreMatrix PrefilteringIndexReader::get2MerScoreMatrix(
    DBReader<unsigned int> *dbr, int preloadMode) {
...
ScoreMatrix PrefilteringIndexReader::get3MerScoreMatrix(
    DBReader<unsigned int> *dbr, int preloadMode) {
...
std::string PrefilteringIndexReader::dbPathWithoutIndex(std::string &dbname) {
  ...
  if (linidxlastpos != std::string::npos &&
      dbname.size() - linidxlastpos == 7) {
