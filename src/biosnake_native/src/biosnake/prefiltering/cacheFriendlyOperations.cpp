#include <biosnake/prefiltering/cacheFriendlyOperations.h>
#include <biosnake/commons/util.h>

#include <cmath>

template <unsigned int BINSIZE>
CacheFriendlyOperations<BINSIZE>::CacheFriendlyOperations(biosnake_output* output, size_t maxElement,
                                                          size_t initBinSize): out(output) {
  // find nearest upper power of 2^(x)
  size_t size = pow(2, ceil(log(maxElement) / log(2)));
  size =
      std::max(size >> MASK_0_5_BIT, (size_t)1);  // space needed in bit array
...
template <unsigned int BINSIZE>
size_t CacheFriendlyOperations<BINSIZE>::findDuplicates(
    IndexEntryLocal **input, CounterResult *output, size_t outputSize,
    unsigned short indexFrom, unsigned short indexTo, bool computeTotalScore) {
  do {
    setupBinPointer();
  ...
  } while (checkForOverflowAndResizeArray(true) ==
           true);  // overflowed occurred
  return findDuplicates(output, outputSize, computeTotalScore);
}

template <unsigned int BINSIZE>
size_t CacheFriendlyOperations<BINSIZE>::mergeElementsByScore(
    CounterResult *inputOutputArray, const size_t N) {
  do {
    setupBinPointer();
    hashElements(inputOutputArray, N);
  } while (checkForOverflowAndResizeArray(false) ==
           true);  // overflowed occurred
  return mergeScoreDuplicates(inputOutputArray);
}

template <unsigned int BINSIZE>
size_t CacheFriendlyOperations<BINSIZE>::mergeElementsByDiagonal(
    CounterResult *inputOutputArray, const size_t N) {
  do {
    setupBinPointer();
    hashElements(inputOutputArray, N);
...
template <unsigned int BINSIZE>
size_t CacheFriendlyOperations<BINSIZE>::keepMaxScoreElementOnly(
    CounterResult *inputOutputArray, const size_t N) {
  do {
    setupBinPointer();
    hashElements(inputOutputArray, N);
...
template <unsigned int BINSIZE>
size_t CacheFriendlyOperations<BINSIZE>::mergeDiagonalDuplicates(
  ...
  for (size_t bin = 0; bin < BINCOUNT; bin++) {
    ...
    while (n != static_cast<size_t>(-1)) {
      ...
      duplicateBitArray[element] =
          static_cast<unsigned char>(binStartPos[n].diagonal) + 1;
      --n;
    }
    // combine diagonals
    for (size_t n = 0; n < currBinSize; n++) {
      ...
      //            std::cout << output[doubleElementCount].id << " " <<
      //            (int)output[doubleElementCount].count << " " <<
      //            (int)static_cast<unsigned
      //            char>(output[doubleElementCount].diagonal) << std::endl;
      // memory overflow can not happen since input array = output array
      doubleElementCount +=
          (duplicateBitArray[hashBinElement] !=
           static_cast<unsigned char>(binStartPos[n].diagonal))
              ? 1
              : 0;

      duplicateBitArray[hashBinElement] =
          static_cast<unsigned char>(element.diagonal);
    }
  }
...
template <unsigned int BINSIZE>
size_t CacheFriendlyOperations<BINSIZE>::mergeScoreDuplicates(
    CounterResult *output) {
  size_t doubleElementCount = 0;
  const CounterResult *bin_ref_pointer = binDataFrame;
  // duplicateBitArray is already zero'd from findDuplicates
  ...
  for (size_t bin = 0; bin < BINCOUNT; bin++) {
    ...
    for (size_t n = 0; n < currBinSize; n++) {
      ...
      const unsigned char newScore =
          (currScore > 0xFF - dbScore) ? 0xFF : dbScore + currScore;
    ...
    for (size_t n = 0; n < currBinSize; n++) {
      ...
      // memory overflow can not happen since input array = output array
      doubleElementCount +=
          (UNLIKELY(duplicateBitArray[hashBinElement] != 0)) ? 1 : 0;
      duplicateBitArray[hashBinElement] =
          static_cast<unsigned char>(binStartPos[n].diagonal);
    }
  }
...
template <unsigned int BINSIZE>
size_t CacheFriendlyOperations<BINSIZE>::findDuplicates(
    CounterResult *output, size_t outputSize, bool computeTotalScore) {
  memset(duplicateBitArray, 0, duplicateBitArraySize * sizeof(unsigned char));
  size_t doubleElementCount = 0;
  const CounterResult *bin_ref_pointer = binDataFrame;
  for (size_t bin = 0; bin < BINCOUNT; bin++) {
    ...
    for (size_t n = 0; n < currBinSize; n++) {
      const CounterResult element = binStartPos[n];
      const unsigned int hashBinElement = element.id >> (MASK_0_5_BIT);
      // const unsigned int byteArrayPos = hashBinElement >> 3; // equal to
      // hashBinElement / 8 const unsigned char bitPosMask = 1 <<
      // (hashBinElement & 7); // 7 = 00000111 check if duplicate element was
      // found before
    ...
    if (computeTotalScore) {
      ...
      for (size_t n = 0; n < elementCount; n++) {
        const unsigned int element = tmpElementBuffer[n].id >> (MASK_0_5_BIT);
        duplicateBitArray[element] +=
            (duplicateBitArray[element] < 255) ? 1 : 0;
      }
      // extract results
      for (size_t n = 0; n < elementCount; n++) {
      ...
      // so (duplicateBitArray[hashBinElement] != tmpElementBuffer[n].diagonal)
      // is true
      size_t n = elementCount - 1;
      while (n != static_cast<size_t>(-1)) {
        const unsigned int element = tmpElementBuffer[n].id >> (MASK_0_5_BIT);
        duplicateBitArray[element] =
            static_cast<unsigned char>(tmpElementBuffer[n].diagonal) + 1;
        --n;
      }
      ...
      for (size_t n = 0; n < elementCount; n++) {
        ...
        output[doubleElementCount].diagonal = tmpElementBuffer[n].diagonal;
        //            const unsigned char diagonal = static_cast<unsigned
        //            char>(tmpElementBuffer[n].diagonal);
        // memory overflow can not happen since input array = output array
        //            if(duplicateBitArray[hashBinElement] !=
        //            tmpElementBuffer[n].diagonal){
        //                std::cout << "seq="<< output[doubleElementCount].id <<
        //                "\tDiag=" << (int) output[doubleElementCount].diagonal
        //                << " dup.Array=" <<
        //                (int)duplicateBitArray[hashBinElement] << "
        //                tmp.Arr="<< (int)tmpElementBuffer[n].diagonal <<
        //                std::endl;
        //            }
        doubleElementCount +=
            (duplicateBitArray[hashBinElement] !=
             static_cast<unsigned char>(tmpElementBuffer[n].diagonal))
                ? 1
                : 0;
        duplicateBitArray[hashBinElement] =
            static_cast<unsigned char>(tmpElementBuffer[n].diagonal);
    ...
    if (currBinSize < duplicateBitArraySize / 16) {
      ...
      memset(duplicateBitArray, 0,
             duplicateBitArraySize * sizeof(unsigned char));
    }
  }
  return doubleElementCount;
}
...
template <unsigned int BINSIZE>
bool CacheFriendlyOperations<BINSIZE>::checkForOverflowAndResizeArray(
    bool includeTmpResult) {
  const CounterResult *bin_ref_pointer = binDataFrame;
  CounterResult *lastPosition = (binDataFrame + BINCOUNT * binSize) - 1;
  for (size_t bin = 0; bin < BINCOUNT; bin++) {
    const CounterResult *binStartPos = (bin_ref_pointer + bin * binSize);
...
template <unsigned int BINSIZE>
void CacheFriendlyOperations<BINSIZE>::hashElements(CounterResult *inputArray,
                                                    size_t N) {
  CounterResult *lastPosition = (binDataFrame + BINCOUNT * binSize) - 1;
...
template <unsigned int BINSIZE>
void CacheFriendlyOperations<BINSIZE>::hashIndexEntry(
    unsigned short position_i, IndexEntryLocal *inputArray, size_t N,
    CounterResult *lastPosition) {
  for (size_t n = 0; n < N; n++) {
    ...
    // do not write over boundary of the data frame
    // std::cout << bins[bin]->id << " " << position_i << " " <<
    // element.position_j << " " << bins[bin]->diagonal << " " << position_i -
    // element.position_j << std::endl;
    bins[bin] += (bins[bin] >= lastPosition) ? 0 : 1;
  }
}
...
size_t CacheFriendlyOperations<BINSIZE>::keepMaxElement(CounterResult *output) {
  ...
  for (size_t bin = 0; bin < BINCOUNT; bin++) {
    ...
    for (size_t n = 0; n < currBinSize; n++) {
      ...
      const unsigned char currScore = element.count;
      const unsigned char dbScore = duplicateBitArray[hashBinElement];
      const unsigned char maxScore =
          (currScore > dbScore) ? currScore : dbScore;
    ...
    for (size_t n = 0; n < currBinSize; n++) {
      ...
      // memory overflow can not happen since input array = output array
      bool found =
          (UNLIKELY(duplicateBitArray[hashBinElement] == element.count)) ? 1
                                                                         : 0;
      doubleElementCount += found;
      duplicateBitArray[hashBinElement] =
          duplicateBitArray[hashBinElement] * (1 - found);
    }
  }
  return doubleElementCount;
}
