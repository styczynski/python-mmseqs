...
#define FOR_EACH(action, ...)                                              \
  GET_MACRO(__VA_ARGS__, FE_11, FE_10, FE_9, FE_8, FE_7, FE_6, FE_5, FE_4, \
            FE_3, FE_2, FE_1)                                              \
  (action, __VA_ARGS__)

QueryMatcher::QueryMatcher(biosnake_output* output, IndexTable *indexTable,
                           SequenceLookup *sequenceLookup,
                           BaseMatrix *kmerSubMat,
                           BaseMatrix *ungappedAlignmentSubMat, short kmerThr,
                           int kmerSize, size_t dbSize, unsigned int maxSeqLen,
  ...
  if (diagonalScoring) {
    ungappedAlignment = new UngappedAlignment(
        maxSeqLen, ungappedAlignmentSubMat, sequenceLookup);
  }
...
std::pair<hit_t *, size_t> QueryMatcher::matchQuery(Sequence *querySeq,
  ...
  if (aaBiasCorrection == true) {
    if (Parameters::isEqualDbtype(querySeq->getSeqType(),
                                  Parameters::DBTYPE_AMINO_ACIDS)) {
      SubstitutionMatrix::calcLocalAaBiasCorrection(
          kmerSubMat, querySeq->numSequence, querySeq->L, compositionBias);
    } else {
  ...
  if (diagonalScoring) {
    ...
    ungappedAlignment->processQuery(querySeq, compositionBias, foundDiagonals,
                                    resultSize);
    memset(scoreSizes, 0, SCORE_RANGE * sizeof(unsigned int));
    ...
    unsigned int diagonalThr =
        computeScoreThreshold(scoreSizes, this->maxHitsPerQuery);
    ...
    if (resultSize < foundDiagonalsSize / 2) {
      unsigned int maxDiagonalScoreThr =
          (UCHAR_MAX - ungappedAlignment->getQueryBias());
      bool scoreIsTruncated =
          (diagonalThr >= maxDiagonalScoreThr) ? true : false;
      size_t elementsCntAboveDiagonalThr =
          radixSortByScoreSize(scoreSizes, foundDiagonals + resultSize,
                               diagonalThr, foundDiagonals, resultSize);
      if (scoreIsTruncated == true) {
        memset(scoreSizes, 0, SCORE_RANGE * sizeof(unsigned int));
        std::pair<size_t, unsigned int> rescoreResult =
            rescoreHits(querySeq, scoreSizes, foundDiagonals + resultSize,
                        elementsCntAboveDiagonalThr, ungappedAlignment,
                        maxDiagonalScoreThr);
        size_t newResultSize = rescoreResult.first;
        unsigned int maxSelfScoreMinusDiag = rescoreResult.second;
        elementsCntAboveDiagonalThr =
            radixSortByScoreSize(scoreSizes, foundDiagonals, 0,
                                 foundDiagonals + resultSize, newResultSize);
        queryResult = getResult<UNGAPPED_DIAGONAL_SCORE>(
            foundDiagonals, elementsCntAboveDiagonalThr, identityId, 0,
            ungappedAlignment, maxSelfScoreMinusDiag);
      } else {
        queryResult = getResult<UNGAPPED_DIAGONAL_SCORE>(
            foundDiagonals + resultSize, elementsCntAboveDiagonalThr,
            identityId, diagonalThr, ungappedAlignment, false);
      }
      stats->truncated = 0;
    } else {
    ...
    if (resultSize < foundDiagonalsSize / 2) {
      int elementsCntAboveDiagonalThr =
          radixSortByScoreSize(scoreSizes, foundDiagonals + resultSize, thr,
                               foundDiagonals, resultSize);
      queryResult = getResult<KMER_SCORE>(
          foundDiagonals + resultSize, elementsCntAboveDiagonalThr, identityId,
  ...
  if (queryResult.second > 1) {
    if (identityId != UINT_MAX) {
      SORT_SERIAL(resList + 1, resList + queryResult.second,
                  hit_t::compareHitsByScoreAndId);
    } else {
      SORT_SERIAL(resList, resList + queryResult.second,
                  hit_t::compareHitsByScoreAndId);
    }
  }
  return queryResult;
...
size_t QueryMatcher::match(Sequence *seq, float *compositionBias) {
  ...
  while (seq->hasNextKmer()) {
    ...
    short bias = static_cast<short>(
        (biasCorrection < 0.0) ? biasCorrection - 0.5 : biasCorrection + 0.5);
    ...
    if (takeOnlyBestKmer) {
      ...
      std::pair<size_t *, size_t> kmerList =
          kmerGenerator->generateKmerList(kmer);
    ...
    for (unsigned int kmerPos = 0; kmerPos < kmerElementSize; kmerPos++) {
      const IndexEntryLocal *entries =
          indexTable->getDBSeqList(index[kmerPos], &seqListSize);
      // DEBUG
      ...
      if ((sequenceHits + seqListSize) >= lastSequenceHit) {
        ...
        const size_t hitCount =
            findDuplicates(indexPointer, foundDiagonals + overflowHitCount,
                           foundDiagonalsSize - overflowHitCount, indexStart,
                           current_i, (diagonalScoring == false));

        if (overflowHitCount != 0) {
          // merge lists, hitCount is max. dbSize so there can be no overflow in
          // mergeElements
          overflowHitCount =
              mergeElements(foundDiagonals, hitCount + overflowHitCount);
        } else {
...
outer:
  ...
  size_t hitCount =
      findDuplicates(indexPointer, foundDiagonals + overflowHitCount,
                     foundDiagonalsSize - overflowHitCount, indexStart, indexTo,
                     (diagonalScoring == false));
  if (overflowHitCount != 0) {
    // overflow occurred
...
std::pair<hit_t *, size_t> QueryMatcher::getResult(CounterResult *results,
  ...
  for (size_t i = 0; i < resultSize && currentHits < maxHitsPerQuery; i++) {
    ...
    // std::cout << i << "\t" << results[i].id << "\t" << (int)results[i].count
    // << "\t" << results[i].diagonal << std::endl;
    if (aboveThreshold && isNotQueryId) {
      ...
      // printf("%d\t%d\t%f\t%f\t%f\t%f\t%f\n", result->seqId, scoreCurr,
      // seqLens[seqIdCurr], mu, logMatchProb, logScoreFactorial[scoreCurr]);
      if (TYPE == KMER_SCORE) {
        ...
        } else if (static_cast<int>(scoreCurr) >=
                   (UCHAR_MAX - align->getQueryBias())) {
          unsigned int newScore =
              align->scoreSingelSequenceByCounterResult(results[i]);
          result->prefScore = newScore;
        }
      }
      currentHits++;
...
void QueryMatcher::initDiagonalMatcher(size_t dbsize,
...
#define DELETE_CASE(x)         \
  case x:                      \
    delete cachedOperation##x; \
    break;
...
#define COUNT_CASE(x)                                        \
  case x:                                                    \
    localResultSize += cachedOperation##x->findDuplicates(   \
        hitsByIndex, output, outputSize, indexFrom, indexTo, \
        computeTotalScore);                                  \
    break;
  switch (activeCounter) {
...
size_t QueryMatcher::mergeElements(CounterResult *foundDiagonals,
                                   size_t hitCounter) {
  size_t overflowHitCount = 0;
#define MERGE_CASE(x)                                                     \
  case x:                                                                 \
    overflowHitCount = diagonalScoring                                    \
                           ? cachedOperation##x->mergeElementsByDiagonal( \
                                 foundDiagonals, hitCounter)              \
                           : cachedOperation##x->mergeElementsByScore(    \
                                 foundDiagonals, hitCounter);             \
    break;
...
size_t QueryMatcher::keepMaxScoreElementOnly(CounterResult *foundDiagonals,
                                             size_t resultSize) {
  size_t retSize = 0;
#define MAX_CASE(x)                                                       \
  case x:                                                                 \
    retSize = cachedOperation##x->keepMaxScoreElementOnly(foundDiagonals, \
                                                          resultSize);    \
    break;
  switch (activeCounter) {
...
std::pair<size_t, unsigned int> QueryMatcher::rescoreHits(
    Sequence *querySeq, unsigned int *scoreSizes, CounterResult *results,
    size_t resultSize, UngappedAlignment *align, int lowerBoundScore) {
  size_t elements = 0;
  const unsigned char *query = querySeq->numSequence;
  int maxSelfScore =
      align->scoreSingleSequence(std::make_pair(query, querySeq->L), 0, 0);
  ...
  for (size_t i = 0; i < resultSize && results[i].count >= lowerBoundScore;
       i++) {
    unsigned int newScore =
        align->scoreSingelSequenceByCounterResult(results[i]);
    newScore -= lowerBoundScore;
    float score = static_cast<float>(
        std::min(newScore, static_cast<unsigned int>(USHRT_MAX)));
    results[i].count = static_cast<unsigned char>(
        (score / fltMaxSelfScore) * static_cast<float>(UCHAR_MAX) + 0.5);
...
template std::pair<hit_t *, size_t> QueryMatcher::getResult<0>(
    CounterResult *results, size_t resultSize, const unsigned int id,
    const unsigned short thr, UngappedAlignment *align, const int rescaleScore);
template std::pair<hit_t *, size_t> QueryMatcher::getResult<1>(
    CounterResult *results, size_t resultSize, const unsigned int id,
    const unsigned short thr, UngappedAlignment *align, const int rescaleScore);
