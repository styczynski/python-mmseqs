...
#ifndef BIOSNAKE_QUERYTEMPLATEMATCHEREXACTMATCH_H
#define BIOSNAKE_QUERYTEMPLATEMATCHEREXACTMATCH_H
...
struct statistics_t {
  ...
  statistics_t()
      : kmersPerPos(0.0),
        ...
        querySeqLen(0),
        diagonalOverflow(0),
        resultsPassedPrefPerSeq(0),
        truncated(0){};
  statistics_t(double kmersPerPos, size_t dbMatches, size_t doubleMatches,
               size_t querySeqLen, size_t diagonalOverflow,
               size_t resultsPassedPrefPerSeq, size_t truncated)
      : kmersPerPos(kmersPerPos),
        dbMatches(dbMatches),
...
struct hit_t {
  ...
  static bool compareHitsByScoreAndId(const hit_t &first, const hit_t &second) {
    if (abs(first.prefScore) > abs(second.prefScore)) return true;
    if (abs(second.prefScore) > abs(first.prefScore)) return false;
...
class QueryMatcher {
 public:
  QueryMatcher(biosnake_output* output, IndexTable *indexTable, SequenceLookup *sequenceLookup,
               ...
               bool aaBiasCorrection, bool diagonalScoringMode,
               unsigned int minDiagScoreThr, bool takeOnlyBestKmer);
  ~QueryMatcher();

  // returns result for the sequence
  // identityId is the id of the identitical sequence in the target database if
  // there is any, UINT_MAX otherwise
  std::pair<hit_t *, size_t> matchQuery(Sequence *querySeq,
  ...
  static hit_t parsePrefilterHit(biosnake_output* out, char *data) {
    hit_t result;
  ...
  static std::vector<hit_t> parsePrefilterHits(biosnake_output* out, char *data) {
  ...
  static void parsePrefilterHits(biosnake_output* out, char *data, std::vector<hit_t> &entries) {
    while (*data != '\0') {
      hit_t result = parsePrefilterHit(out, data);
 ...
 protected:
  ...
  static unsigned int computeScoreThreshold(unsigned int *scoreSizes,
                                            size_t maxHitsPerQuery) {
    size_t foundHits = 0;
    size_t scoreThr = 0;
  ...
  std::pair<hit_t *, size_t> getResult(CounterResult *results,
                                       size_t resultSize, const unsigned int id,
  ...
  std::pair<size_t, unsigned int> rescoreHits(
...
#define CacheFriendlyOperations(x) \
  CacheFriendlyOperations<x> *cachedOperation##x
...
#undef CacheFriendlyOperations
  ...
  size_t findDuplicates(IndexEntryLocal **hitsByIndex, CounterResult *output,
                        size_t outputSize, unsigned short indexFrom,
                        unsigned short indexTo, bool computeTotalScore);

  size_t mergeElements(CounterResult *foundDiagonals, size_t hitCounter);

  size_t keepMaxScoreElementOnly(CounterResult *foundDiagonals,
                                 size_t resultSize);
};

#endif  // BIOSNAKE_QUERYTEMPLATEMATCHEREXACTMATCH_H
