...
#include <biosnake/prefiltering/ungappedAlignment.h>
...
UngappedAlignment::UngappedAlignment(const unsigned int maxSeqLen,
  ...
  vectorSequence =
      (unsigned char *)malloc_simd_int(VECSIZE_INT * 4 * maxSeqLen);
...
void UngappedAlignment::processQuery(Sequence *seq, float *biasCorrection,
                                     CounterResult *results,
                                     size_t resultSize) {
  short bias = createProfile(seq, biasCorrection, subMatrix->subMatrix,
                             subMatrix->alphabetSize);
  this->bias = bias;
  queryLen = seq->L;
...
int UngappedAlignment::scalarDiagonalScoring(const char *profile,
  ...
  for (unsigned int pos = 0; pos < seqLen; pos++) {
    ...
    //        std::cout << (int) dbSeq[pos] << "\t" << curr << "\t" << max <<
    //        "\t" << score <<  "\t" << (curr - bias) << std::endl;
...
inline __m256i Shuffle(const __m256i &value, const __m256i &shuffle) {
  const __m256i K0 = _mm256_setr_epi8(
      (char)0x70, (char)0x70, (char)0x70, (char)0x70, (char)0x70, (char)0x70,
      (char)0x70, (char)0x70, (char)0x70, (char)0x70, (char)0x70, (char)0x70,
      (char)0x70, (char)0x70, (char)0x70, (char)0x70, (char)0xF0, (char)0xF0,
      (char)0xF0, (char)0xF0, (char)0xF0, (char)0xF0, (char)0xF0, (char)0xF0,
      (char)0xF0, (char)0xF0, (char)0xF0, (char)0xF0, (char)0xF0, (char)0xF0,
      (char)0xF0, (char)0xF0);
  const __m256i K1 = _mm256_setr_epi8(
      (char)0xF0, (char)0xF0, (char)0xF0, (char)0xF0, (char)0xF0, (char)0xF0,
      (char)0xF0, (char)0xF0, (char)0xF0, (char)0xF0, (char)0xF0, (char)0xF0,
      (char)0xF0, (char)0xF0, (char)0xF0, (char)0xF0, (char)0x70, (char)0x70,
      (char)0x70, (char)0x70, (char)0x70, (char)0x70, (char)0x70, (char)0x70,
      (char)0x70, (char)0x70, (char)0x70, (char)0x70, (char)0x70, (char)0x70,
      (char)0x70, (char)0x70);
  return _mm256_or_si256(
      _mm256_shuffle_epi8(value, _mm256_add_epi8(shuffle, K0)),
      _mm256_shuffle_epi8(_mm256_permute4x64_epi64(value, 0x4E),
                          _mm256_add_epi8(shuffle, K1)));
}
#endif
...
simd_int UngappedAlignment::vectorDiagonalScoring(const char *profile,
  ...
  for (unsigned int pos = 0; pos < seqLen; pos++) {
...
#ifdef AVX2
    __m256i score_matrix_vec01 =
        _mm256_load_si256((simd_int *)&profile[pos * PROFILESIZE]);
    __m256i score_vec_8bit = Shuffle(score_matrix_vec01, template01);
    //        __m256i score_vec_8bit = _mm256_shuffle_epi8(score_matrix_vec01,
    //        template01);
    //        __m256i lookup_mask01  = _mm256_cmpgt_epi8(sixten, template01); //
    //        16 > t score_vec_8bit = _mm256_and_si256(score_vec_8bit,
    //        lookup_mask01);
#else
    ...
    __m128i score_matrix_vec16 =
        _mm_load_si128((__m128i *)&profile[pos * 32 + 16]);
    ...
    //        std::cout << (int)((char *)&template01)[0] << "\t" <<  SSTR(((char
    //        *)&score_vec_8bit)[0]) << "\t" << SSTR(((char *)&vMaxScore)[0]) <<
    //        "\t" << SSTR(((char *)&vscore)[0]) << std::endl;
    vMaxScore = simdui8_max(vMaxScore, vscore);
  }
  return vMaxScore;
}

std::pair<unsigned char *, unsigned int> UngappedAlignment::mapSequences(
    std::pair<unsigned char *, unsigned int> *seqs, unsigned int seqCount) {
...
void UngappedAlignment::scoreDiagonalAndUpdateHits(
    const char *queryProfile, const unsigned int queryLen, const short diagonal,
    CounterResult **hits, const unsigned int hitSize, const short bias) {
  //    unsigned char minDistToDiagonal = distanceFromDiagonal(diagonal);
  //    unsigned char maxDistToDiagonal = (minDistToDiagonal == 0) ? 0 :
  //    (DIAGONALCOUNT - minDistToDiagonal); unsigned int i_splits =
  //    computeSplit(queryLen, minDistToDiagonal);
  ...
  if (queryLen >= 32768) {
    for (size_t hitIdx = 0; hitIdx < hitSize; hitIdx++) {
      ...
      std::pair<const unsigned char *, const unsigned int> dbSeq =
          sequenceLookup->getSequence(seqId);
  ...
  if (hitSize > (VECSIZE_INT * 4) / 16) {
    ...
    for (unsigned int seqIdx = 0; seqIdx < hitSize; seqIdx++) {
      std::pair<const unsigned char *, const unsigned int> tmp =
          sequenceLookup->getSequence(hits[seqIdx]->id);
      if (tmp.second >= 32768) {
        // hack to avoid too long sequences
        // this sequences will be processed by computeLongScore later
        seqs[seqIdx] =
            std::make_pair((unsigned char *)tmp.first, (unsigned int)1);
      } else {
        seqs[seqIdx] = std::make_pair((unsigned char *)tmp.first,
                                      (unsigned int)tmp.second);
      }
    }
    ...
    if (diagonal >= 0 && minDistToDiagonal < queryLen) {
      unsigned int minSeqLen =
          std::min(seq.second, queryLen - minDistToDiagonal);
      simd_int ret = vectorDiagonalScoring(
          queryProfile + (minDistToDiagonal * PROFILESIZE), bias, minSeqLen,
          seq.first);
      vMaxScore = simdui8_max(ret, vMaxScore);
    } else if (diagonal < 0 && minDistToDiagonal < seq.second) {
      unsigned int minSeqLen =
          std::min(seq.second - minDistToDiagonal, queryLen);
      simd_int ret = vectorDiagonalScoring(
          queryProfile, bias, minSeqLen,
    ...
    for (size_t hitIdx = 0; hitIdx < hitSize; hitIdx++) {
      ...
      if (seqs[hitIdx].second == 1) {
        std::pair<const unsigned char *, const unsigned int> dbSeq =
            sequenceLookup->getSequence(hits[hitIdx]->id);
        if (dbSeq.second >= 32768) {
          int max =
              computeLongScore(queryProfile, queryLen, dbSeq, diagonal, bias);
          hits[hitIdx]->count =
              static_cast<unsigned char>(std::min(255 - bias, max));
        }
      }
    }
  } else {
    for (size_t hitIdx = 0; hitIdx < hitSize; hitIdx++) {
      ...
      if (dbSeq.second >= 32768) {
        ...
        max = computeSingelSequenceScores(queryProfile, queryLen, dbSeq,
                                          diagonal, minDistToDiagonal, bias);
      }
      hits[hitIdx]->count =
          static_cast<unsigned char>(std::min(255 - bias, max));
    }
...
int UngappedAlignment::computeLongScore(
    const char *queryProfile, unsigned int queryLen,
    std::pair<const unsigned char *, const unsigned int> &dbSeq,
    unsigned short diagonal, short bias) {
  int totalMax = 0;
  for (unsigned int devisions = 1; devisions <= 1 + dbSeq.second / 32768;
       devisions++) {
    int realDiagonal = (-devisions * 65536 + diagonal);
    int minDistToDiagonal = abs(realDiagonal);
    int max = computeSingelSequenceScores(
        queryProfile, queryLen, dbSeq, realDiagonal, minDistToDiagonal, bias);
...
void UngappedAlignment::computeScores(const char *queryProfile,
  ...
  for (size_t i = 0; i < resultSize; i++) {
    ...
    diagonalMatches[currDiag * (VECSIZE_INT * 4) + diagonalCounter[currDiag]] =
        &results[i];
    diagonalCounter[currDiag]++;
    if (diagonalCounter[currDiag] >= (VECSIZE_INT * 4)) {
      scoreDiagonalAndUpdateHits(queryProfile, queryLen,
                                 static_cast<short>(currDiag),
                                 &diagonalMatches[currDiag * (VECSIZE_INT * 4)],
                                 diagonalCounter[currDiag], bias);
  ...
  for (size_t i = 0; i < DIAGONALCOUNT; i++) {
    if (diagonalCounter[i] > 0) {
      scoreDiagonalAndUpdateHits(queryProfile, queryLen, static_cast<short>(i),
                                 &diagonalMatches[i * (VECSIZE_INT * 4)],
                                 diagonalCounter[i], bias);
    }
    diagonalCounter[i] = 0;
  }
}

unsigned short UngappedAlignment::distanceFromDiagonal(
...
short UngappedAlignment::createProfile(Sequence *seq, float *biasCorrection,
                                       short **subMat, int alphabetSize) {
  short bias = 0;
  int aaBias = 0;
  if (Parameters::isEqualDbtype(seq->getSequenceType(),
    ...
    for (int pos = 0; pos < seq->L; pos++) {
      ...
      aaCorrBias =
          (aaCorrBias < 0.0) ? aaCorrBias / 4 - 0.5 : aaCorrBias / 4 + 0.5;
      aaCorrectionScore[pos] = static_cast<char>(aaCorrBias);
      aaBias =
          (aaCorrectionScore[pos] < aaBias) ? aaCorrectionScore[pos] : aaBias;
    }
  ...
  if (Parameters::isEqualDbtype(seq->getSequenceType(),
    ...
    for (int pos = 0; pos < seq->L; pos++) {
      if (Parameters::isEqualDbtype(seq->getSequenceType(),
        ...
        for (int aa_num = 0; aa_num < alphabetSize; aa_num++) {
          queryProfile[pos * PROFILESIZE + aa_num] =
              (profile_aln[aa_num * seq->L + pos]) + bias;
        }
      } else {
    ...
    for (int pos = 0; pos < seq->L; pos++) {
      ...
      for (int i = 0; i < subMatrix->alphabetSize; i++) {
        queryProfile[pos * PROFILESIZE + i] =
            (subMat[aaIdx][i] + aaCorrectionScore[pos] + bias);
      }
    }
  }
  return bias;
...
unsigned int UngappedAlignment::diagonalLength(const short diagonal,
                                               const unsigned int queryLen,
...
int UngappedAlignment::computeSingelSequenceScores(
    const char *queryProfile, const unsigned int queryLen,
    std::pair<const unsigned char *, const unsigned int> &dbSeq, int diagonal,
    unsigned int minDistToDiagonal, short bias) {
  int max = 0;
  if (diagonal >= 0 && minDistToDiagonal < queryLen) {
    unsigned int minSeqLen =
        std::min(dbSeq.second, queryLen - minDistToDiagonal);
    int scores =
        scalarDiagonalScoring(queryProfile + (minDistToDiagonal * PROFILESIZE),
                              bias, minSeqLen, dbSeq.first);
    max = std::max(scores, max);
  } else if (diagonal < 0 && minDistToDiagonal < dbSeq.second) {
    unsigned int minSeqLen =
        std::min(dbSeq.second - minDistToDiagonal, queryLen);
    int scores = scalarDiagonalScoring(queryProfile, bias, minSeqLen,
                                       dbSeq.first + minDistToDiagonal);
    max = std::max(scores, max);
  }
  return max;
}

int UngappedAlignment::scoreSingelSequenceByCounterResult(
    CounterResult &result) {
  std::pair<const unsigned char *, const unsigned int> dbSeq =
      sequenceLookup->getSequence(result.id);
...
int UngappedAlignment::scoreSingleSequence(
    std::pair<const unsigned char *, const unsigned int> dbSeq,
    unsigned short diagonal, unsigned short minDistToDiagonal) {
  if (queryLen >= 32768 || dbSeq.second >= 32768) {
    return computeLongScore(queryProfile, queryLen, dbSeq, diagonal, bias);
  } else {
    return computeSingelSequenceScores(queryProfile, queryLen, dbSeq,
                                       static_cast<short>(diagonal),
                                       minDistToDiagonal, bias);
  }
}
