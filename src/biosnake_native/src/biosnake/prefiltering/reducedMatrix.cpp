...
ReducedMatrix::ReducedMatrix(biosnake_output* output, double** probMatrix, float** rMatrix,
  ...
  for (size_t step = 0; step < reduce_steps; step++) {
    ...
    std::pair<int, int> reduce_bases = coupleWithBestInfo(
        this->probMatrix, probMatrix_new, rMatrix, origAlphabetSize - 1 - step);
  ...
  for (int i = 0; i < this->alphabetSize; i++) {
    ...
    double Pab = probMatrix[oldIndex][origAlphabetSize - 1] /
                 (origpBack[oldIndex] * origpBack[origAlphabetSize - 1]);
    probMatrix_new[alphabetSize - 1][i] =
        Pab * pBack[i] * pBack[alphabetSize - 1];
    probMatrix_new[i][alphabetSize - 1] =
        Pab * pBack[alphabetSize - 1] * pBack[i];
  }
  delete[] origpBack;
  generateSubMatrix(probMatrix_new, rMatrix, this->subMatrix, alphabetSize,
                    true, bitFactor, 0.0);
...
double ReducedMatrix::calculateMutualInformation(double** pMatrix,
                                                 double** subMatrix,
                                                 size_t size) {
  double mutualInfo = 0;
...
void ReducedMatrix::coupleBases(double** input, double** output, size_t size,
                                size_t base1, size_t base2) {
...
std::pair<size_t, size_t> ReducedMatrix::coupleWithBestInfo(double** pinput,
                                                            double** pMatrix,
                                                            float** rMatrix,
                                                            size_t size) {
  double bestInfo = 0;
...
void ReducedMatrix::addTwoColumns(double** input, double** output, size_t size,
  ...
  for (size_t i = 0; i < size; i++) {
    ...
    for (size_t i = 0; i < size; i++) output[i][size - 1] = 0.0;
  }
}

void ReducedMatrix::addTwoRows(double** input, double** output, size_t size,
                               size_t row1, size_t row2) {
  // copy the same data until row2 (excluding)
  for (size_t i = 0; i < row2; i++) {
    for (size_t j = 0; j < size; j++) output[i][j] = input[i][j];
  }
  ...
  for (size_t i = row2; i < size - 1; i++) {
    for (size_t j = 0; j < size; j++) output[i][j] = input[i + 1][j];
  }

  for (size_t j = 0; j < size; j++) output[size - 1][j] = 0.0;
}
