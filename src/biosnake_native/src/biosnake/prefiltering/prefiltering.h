...
class Prefiltering {
 public:
  Prefiltering(biosnake_output *out, const std::string &queryDB,
  ...
  void runAllSplits(biosnake_output *out, const std::string &resultDB,
                    const std::string &resultDBIndex);

#ifdef HAVE_MPI
  void runMpiSplits(biosnake_output *out, const std::string &resultDB,
                    const std::string &resultDBIndex,
                    const std::string &localTmpPath, const int runRandomId);
#endif

  int runSplits(biosnake_output *out, const std::string &resultDB,
  ...
  static void setupSplit(biosnake_output* out, DBReader<unsigned int> &dbr, const int alphabetSize,
  ...
  static int getKmerThreshold(biosnake_output* out, const float sensitivity, const bool isProfile,
 ...
 private:
  ...
  bool runSplit(biosnake_output *out, const std::string &resultDB,
                const std::string &resultDBIndex, size_t split, bool merge);

  // compute kmer size and split size for index table
  static std::pair<int, int> optimizeSplit(biosnake_output* out, size_t totalMemoryInByte,
                                           DBReader<unsigned int> *tdbr,
  ...
  ScoreMatrix getScoreMatrix(biosnake_output* out, const BaseMatrix &matrix, const size_t kmerSize);

  // needed for index lookup
  void getIndexTable(biosnake_output *out, int split, size_t dbFrom,
                     size_t dbSize);

  void printStatistics(biosnake_output* out, const statistics_t &stats, std::list<int> **reslens,
                       unsigned int resLensSize, size_t empty,
                       size_t maxResults);

  bool isSameQTDB(biosnake_output* out);
};
