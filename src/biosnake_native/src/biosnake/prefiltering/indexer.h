...
// Written by Maria Hauser mhauser@genzentrum.lmu.de, Martin Steinegger
// martin.steinegger@snu.ac.kr
//
// Manages the conversion of the int coded k-mer into a int index and vice
// versa.
//
...
class Indexer {
 public:
  Indexer(biosnake_output* output, const size_t alphabetSize, const int maxKmerSize);
  ~Indexer();

  // get the index of the k-mer, beginning at "begin" in the int_seq and ending
  // at "end"
  size_t int2index(const unsigned char* int_seq, const int begin,
                   const int end) {
