...
int doswap(biosnake_output* out, Parameters &par, bool isGeneralMode) {
...
#pragma omp for schedule(dynamic, 100)
  ...
  memoryLimit = (memoryLimit > bytesForTargetElements)
                    ? (memoryLimit - bytesForTargetElements)
                    : 0;
        ...
        *(tmpBuff) = '\0';
        ...
        while (*data != '\0') {
          ...
          if (dbKey >= prevDbKeyToWrite && dbKey <= dbKeyToWrite) {
            ...
            memcpy(&tmpData[offset + queryKeyLen], data + targetKeyLen,
                   oldLineLen - targetKeyLen);
          }
          data = nextLine;
        }
      }
      ...
      for (size_t i = prevDbKeyToWrite; i <= dbKeyToWrite; ++i) {
        ...
        while (dataSize > 0) {
          if (isAlignmentResult) {
            ...
            curRes.emplace_back(hit.seqId, hit.prefScore, 0, 0, 0,
                                -static_cast<float>(hit.prefScore),
                                hit.diagonal, 0, 0, 0, 0, 0, 0, "");
          }
          char *nextLine = Util::skipLine(data);
          size_t lineLen = nextLine - data;
          dataSize -= lineLen;
        ...
        if (curRes.empty() == false) {
          ...
          for (size_t j = 0; j < curRes.size(); j++) {
            ...
            if (isAlignmentResult) {
              size_t len =
                  Matcher::resultToBuffer(buffer, res, hasBacktrace, false);
...
int swapdb(biosnake_output *out, Parameters &par) {
  //    Parameters &par = Parameters::getInstance();
  //    par.parseParameters(argc, argv, command, true, 0, 0);
  return doswap(out, par, true);
}

int swapresults(biosnake_output *out, Parameters &par) {
