...
// For PSSMs: Biosnake just stores the position specific score in 1 byte
...
struct compareFirstEntry {
  bool operator()(const std::pair<std::string, unsigned int> &lhs,
...
int diffseqdbs(biosnake_output *out, Parameters &par) {
  ...
  // key pairs contain (headerID, key) where key is the DB key corresponding to
  // the header
...
#pragma omp parallel for schedule(dynamic, 10)
  for (size_t id = 0; id < indexSizeOld; ++id) {
    ...
    std::pair<std::string, unsigned int> *mappedKey = std::lower_bound(
        keysNew, keysNew + indexSizeNew, keyToSearch, compareKeyToFirstEntry());

    if (mappedKey != (keysNew + indexSizeNew) &&
        keyToSearch.compare(mappedKey->first) == 0) {
      // Found
  ...
  for (size_t id = 0; id < indexSizeNew; ++id) {
    if (checkedNew[id]) {
      keptSeqDBWriter << keysOld[mappedIds[id]].second << "\t"
                      << keysNew[id].second << std::endl;
    } else {
