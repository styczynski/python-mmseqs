...
int transitivealign(biosnake_output *out, Parameters &par) {
  ...
  size_t iterations =
      static_cast<int>(ceil(static_cast<double>(alnReader.getSize()) /
      ...
      for (size_t id = start; id < (start + bucketSize); id++) {
        ...
        for (size_t entryIdx_i = 0; entryIdx_i < results.size(); entryIdx_i++) {
          const unsigned int queryId =
              sequenceDbr.getId(results[entryIdx_i].dbKey);
          ...
          *(tmpBuff - 1) = '\t';
          ...
          if (queryKey == alnKey) {
            for (size_t aliId = 0; aliId < results.size(); aliId++) {
              size_t len =
                  Matcher::resultToBuffer(tmpBuff, results[aliId], true, true);
              resultWriter.writeAdd(buffer, queryIdLen + len, thread_idx);
            }
            continue;
          }

          for (size_t entryIdx_j = 0; entryIdx_j < results.size();
               entryIdx_j++) {
            const unsigned int targetId =
                sequenceDbr.getId(results[entryIdx_j].dbKey);
            char *targetSeq = sequenceDbr.getData(targetId, thread_idx);

            if (Util::canBeCovered(par.covThr, par.covMode, swappedResult.qLen,
                                   results[entryIdx_j].dbLen) == false) {
              continue;
            }
            const bool isIdentity =
                (queryId == targetId && par.includeIdentity) ? true : false;
            Matcher::result_t result;
            if (results[entryIdx_i].dbKey == results[entryIdx_j].dbKey) {
              unsigned int score =
                  DistanceCalculator::computeSubstitutionDistance(
                      querySeq, targetSeq, results[entryIdx_i].dbLen,
                      fastMatrix.matrix);
              double bitScore = evaluer.computeBitScore(score);
              double evalue =
                  evaluer.computeEvalue(score, results[entryIdx_j].dbLen);
              ...
              btTranslate.translateResult(swappedResult, results[entryIdx_j],
                                          result);
              Matcher::updateResultByRescoringBacktrace(
                  querySeq, targetSeq, fastMatrix.matrix, evaluer,
                  par.gapOpen.aminoacids, par.gapExtend.aminoacids, result);
            }
            // checkCriteria and Util::canBeCovered always work together
            if (Alignment::checkCriteria(result, isIdentity, par.evalThr,
                                         par.seqIdThr, par.alnLenThr,
                                         par.covMode, par.covThr)) {
              outputResults.push_back(result);
            }
          }
          SORT_SERIAL(outputResults.begin(), outputResults.end(),
                      Matcher::compareHits);
          for (size_t aliId = 0; aliId < outputResults.size(); aliId++) {
            size_t len = Matcher::resultToBuffer(tmpBuff, outputResults[aliId],
                                                 true, true);
            resultWriter.writeAdd(buffer, queryIdLen + len, thread_idx);
          }
          outputResults.clear();
...
#pragma omp for schedule(dynamic, 100)
  ...
  for (size_t i = 0; i <= maxTargetId; i++) {
    ...
    if (bytesToWrite > memoryLimit) {
      splits.push_back(
          std::make_pair(i - 1, bytesToWrite - targetElementSize[i]));
      bytesToWrite = targetElementSize[i];
    }
  }
      ...
      for (size_t i = 0; i < resultSize; ++i) {
        ...
        *(tmpBuff) = '\0';
        ...
        while (*data != '\0') {
          ...
          size_t offset =
              __sync_fetch_and_add(&(targetElementSize[dbKey]), newLineLen) -
              prevBytesToWrite;
          if (dbKey >= prevDbKeyToWrite && dbKey <= dbKeyToWrite) {
            // memcpy(&tmpData[offset], queryKeyStr, queryKeyLen);
            memcpy(&tmpData[offset], data + (targetKeyLen + 1),
                   oldLineLen - (targetKeyLen + 1));
          }
          data = nextLine;
        }
      }
