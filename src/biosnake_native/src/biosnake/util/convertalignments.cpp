...
void printSeqBasedOnAln(biosnake_output *mmout, std::string &out, const char *seq,
                        unsigned int offset, const std::string &bt,
                        bool reverse, bool isReverseStrand,
                        bool translateSequence,
                        const TranslateNucl &translateNucl) {
  unsigned int seqPos = 0;
  char codon[3];
  for (uint32_t i = 0; i < bt.size(); ++i) {
    char seqChar = (isReverseStrand == true)
                       ? Orf::complement(seq[offset - seqPos])
                       : seq[offset + seqPos];
    if (translateSequence) {
      codon[0] = (isReverseStrand == true)
                     ? Orf::complement(seq[offset - seqPos])
                     : seq[offset + seqPos];
      codon[1] = (isReverseStrand == true)
                     ? Orf::complement(seq[offset - (seqPos + 1)])
                     : seq[offset + (seqPos + 1)];
      codon[2] = (isReverseStrand == true)
                     ? Orf::complement(seq[offset - (seqPos + 2)])
                     : seq[offset + (seqPos + 2)];
...
bits        Bit score
cigar       Alignment as string M=letter pair, D=delete (gap in query), I=insert
(gap in target) qseq        Full-length query sequence tseq        Full-length
target sequence qheader     Header of Query sequence theader     Header of
Target sequence qaln        Aligned query sequence with gaps taln        Aligned
target sequence with gaps qframe      Query frame (-3 to +3) tframe      Target
frame (-3 to +3) mismatch    Number of mismatches qcov        Fraction of query
sequence covered by alignment tcov        Fraction of target sequence covered by
alignment qset        Query set tset        Target set
 */

std::map<unsigned int, unsigned int> readKeyToSet(biosnake_output* out, const std::string &file) {
  ...
  while (*data != '\0') {
    ...
    mapping.emplace(Util::fast_atoi<unsigned int>(entry[0]),
                    Util::fast_atoi<unsigned int>(entry[2]));
...
std::map<unsigned int, std::string> readSetToSource(biosnake_output* out, const std::string &file) {
...
int convertalignments(biosnake_output *out, Parameters &par) {
  ...
  int dbaccessMode = needSequenceDB ? (DBReader<unsigned int>::USE_INDEX |
  ...
  bool queryNucs = Parameters::isEqualDbtype(qDbr.sequenceReader->getDbtype(),
                                             Parameters::DBTYPE_NUCLEOTIDES);
  bool targetNucs = Parameters::isEqualDbtype(tDbr->sequenceReader->getDbtype(),
                                              Parameters::DBTYPE_NUCLEOTIDES);
  if (needSequenceDB) {
    // try to figure out if search was translated. This is can not be solved
    // perfectly.
    ...
    if ((targetNucs == true && queryNucs == false) ||
        (targetNucs == false && queryNucs == true) ||
  ...
  if (needSequenceDB) {
    queryProfile = Parameters::isEqualDbtype(qDbr.sequenceReader->getDbtype(),
...
#ifdef OPENMP
  localThreads =
      std::min((unsigned int)par.threads, (unsigned int)alnDbr.getSize());
#endif
  ...
  if (format == Parameters::FORMAT_ALIGNMENT_SAM) {
    ...
    for (size_t i = 0; i < alnDbr.getSize(); i++) {
      ...
      while (*data != '\0') {
        ...
        if (headerWritten[dbKey] == false) {
          ...
          const char *tHeader =
              tDbrHeader->sequenceReader->getData(tHeaderId, 0);
    ...
    size_t dstSize = ZSTD_findDecompressedSize(result_viz_prelude_html_zst,
                                               result_viz_prelude_html_zst_len);
    char *dst = (char *)malloc(sizeof(char) * dstSize);
    size_t realSize = ZSTD_decompress(dst, dstSize, result_viz_prelude_html_zst,
                                      result_viz_prelude_html_zst_len);
    resultWriter.writeData(dst, realSize, 0, 0, false, false);
    const char *scriptBlock = "<script>render([";
    resultWriter.writeData(scriptBlock, strlen(scriptBlock), 0, 0, false,
                           false);
    free(dst);
  }
...
#pragma omp for schedule(dynamic, 10)
    for (size_t i = 0; i < alnDbr.getSize(); i++) {
      ...
      if (needSequenceDB) {
        ...
        if (queryProfile) {
          Sequence::extractProfileConsensus(querySeqData, *subMat,
                                            queryProfData);
        }
      }

      std::vector<biosnake_blast_tab_record> results_row;
      ...
      while (*data != '\0') {
        biosnake_blast_tab_record record;
        Matcher::result_t res = Matcher::parseAlignmentRecord(out, data, true);
        data = Util::skipLine(data);

        if (res.backtrace.empty() && needBacktrace == true) {
          out->failure("Backtrace cigar is missing in the alignment result. Please recompute the alignment with the -a flag. Command: biosnake align {} {} {} alnNew -a", par.db1, par.db2, par.db3);
        }

        size_t tHeaderId = tDbrHeader->sequenceReader->getId(res.dbKey);
        const char *tHeader =
            tDbrHeader->sequenceReader->getData(tHeaderId, thread_idx);
        ...
        if (res.backtrace.empty() == false) {
          ...
          //                res.seqId = X / alnLen;
          identical = static_cast<unsigned int>(
              res.seqId * static_cast<float>(alnLen) + 0.5);
          ...
          const float bestMatchEstimate =
              static_cast<float>(std::min(abs(res.qEndPos - adjustQstart),
                                          abs(res.dbEndPos - adjustDBstart)));
          missMatchCount = static_cast<unsigned int>(
              bestMatchEstimate * (1.0f - res.seqId) + 0.5);
        }
        ...
        switch (format) {
          case Parameters::FORMAT_ALIGNMENT_BLAST_TAB: {
            if (outcodes.empty()) {
              int count = snprintf(
                  ...
                  missMatchCount, gapOpenCount, res.qStartPos + 1,
              ...
              if (needTaxonomy || needTaxonomyMapping) {
                ...
                std::vector<std::pair<unsigned int, unsigned int>>::iterator
                    mappingIt;
                mappingIt = std::upper_bound(mapping.begin(), mapping.end(),
                                             val, compareToFirstInt);
                if (mappingIt == mapping.end() ||
                    mappingIt->first != val.first) {
                  taxon = 0;
                  taxonNode = NULL;
                } else {
              ...
              if (needSequenceDB) {
                ...
                if (targetProfile) {
                  Sequence::extractProfileConsensus(targetSeqData, *subMat,
                                                    targetProfData);
                }
              }
              for (size_t i = 0; i < outcodes.size(); i++) {
                switch (outcodes[i]) {
                  ...
                  case Parameters::OUTFMT_RAW:
                    // results_row.push_back(SSTR(static_cast<int>(evaluer->computeRawScoreFromBitScore(res.score)
                    // + 0.5)));
                    break;
                  ...
                  case Parameters::OUTFMT_CIGAR:
                    if (isTranslatedSearch == true && targetNucs == true &&
                        queryNucs == true) {
                      Matcher::result_t::protein2nucl(res.backtrace,
                                                      newBacktrace);
                  ...
                  case Parameters::OUTFMT_TAXNAME:
                    // results_row.push_back((taxonNode != NULL) ?
                    // t->getString(taxonNode->nameIdx) : "unclassified");
                    break;
                  case Parameters::OUTFMT_TAXLIN:
                    // results_row.push_back((taxonNode != NULL) ?
                    // t->taxLineage(taxonNode, true) : "unclassified");
                    break;
          ...
          case Parameters::FORMAT_ALIGNMENT_BLAST_WITH_LEN: {
            int count = snprintf(
                ...
                queryId.c_str(), targetId.c_str(), res.seqId, alnLen,
                missMatchCount, gapOpenCount, res.qStartPos + 1,
                res.qEndPos + 1, res.dbStartPos + 1, res.dbEndPos + 1, res.eval,
          ...
          case Parameters::FORMAT_ALIGNMENT_SAM: {
            ...
            int count = snprintf(buffer, sizeof(buffer), "%s\t%d\t%s\t%d\t%d\t",
          ...
          case Parameters::FORMAT_ALIGNMENT_HTML: {
            const char *jsAln =
                "{\"target\": \"%s\", \"seqId\": %1.3f, \"alnLen\": %d, "
                "\"mismatch\": %d, \"gapopen\": %d, \"qStartPos\": %d, "
                "\"qEndPos\": %d, \"dbStartPos\": %d, \"dbEndPos\": %d, "
                "\"eval\": %.2E, \"score\": %d, \"qLen\": %d, \"dbLen\": %d, "
                "\"qAln\": \"";
            int count = snprintf(
                buffer, sizeof(buffer), jsAln, targetId.c_str(), res.seqId,
                alnLen, missMatchCount, gapOpenCount, res.qStartPos + 1,
                res.qEndPos + 1, res.dbStartPos + 1, res.dbEndPos + 1, res.eval,
                res.score, res.qLen, res.dbLen);
            ...
            size_t tId = tDbr->sequenceReader->getId(res.dbKey);
            char *targetSeqData =
                tDbr->sequenceReader->getData(tId, thread_idx);
            if (targetProfile) {
              Sequence::extractProfileConsensus(targetSeqData, *subMat,
                                                targetProfData);
              printSeqBasedOnAln(
      ...
      resultWriter.writeData(result.c_str(), result.size(), queryKey,
                             thread_idx, isDb);
      result.clear();
    }
  }
  if (format == Parameters::FORMAT_ALIGNMENT_HTML) {
    ...
    resultWriter.writeData(endBlock, strlen(endBlock), 0, localThreads - 1,
                           false, false);
  }
  // tsv output
