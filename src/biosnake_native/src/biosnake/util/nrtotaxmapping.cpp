...
static bool compareToFirstString(const std::pair<std::string, TaxID>& lhs,
                                 const std::pair<std::string, TaxID>& rhs) {
  return (lhs.first <= rhs.first);
}

static bool sortMappingByDbKey(const std::pair<unsigned int, TaxID>& lhs,
                               const std::pair<unsigned int, TaxID>& rhs) {
  return (lhs.first <= rhs.first);
}

static bool sortByFirstString(const std::pair<std::string, TaxID>& lhs,
                              const std::pair<std::string, TaxID>& rhs) {
  return (lhs.first < rhs.first);
}
...
struct SortByName {
  ...
  bool operator()(const TaxonNode& lhs, const TaxonNode& rhs) const {
    return strcmp(taxonomy->getString(lhs.nameIdx),
                  taxonomy->getString(rhs.nameIdx)) < 0;
  }
  const NcbiTaxonomy* taxonomy;
};

TaxID lookupTaxID(const std::vector<std::pair<std::string, TaxID>>& mapping,
                  const std::string& value) {
  std::pair<std::string, TaxID> val;
  val.first = value;
  std::vector<std::pair<std::string, TaxID>>::const_iterator mappingIt =
      std::upper_bound(mapping.begin(), mapping.end(), val,
                       compareToFirstString);
...
int nrtotaxmapping(biosnake_output* out, Parameters& par) {
...
#else
      out->failure("Biosnake2 was not compiled with zlib support. Cannot read compressed input");
#endif
    } else {
  ...
  std::vector<TaxonNode> nodesCopy(taxonomy->taxonNodes,
                                   taxonomy->taxonNodes + taxonomy->maxNodes);
  ...
  if (nodesSize >= 2 && taxonomy->getString(nodesCopy[0].nameIdx) !=
                            taxonomy->getString(nodesCopy[1].nameIdx)) {
    uniqueNames.emplace_back(taxonomy->getString(nodesCopy[0].nameIdx),
                             nodesCopy[0].taxId);
  }
  for (size_t i = 1; i < (nodesSize - 1); ++i) {
    if ((taxonomy->getString(nodesCopy[i - 1].nameIdx) !=
         taxonomy->getString(nodesCopy[i].nameIdx)) &&
        (taxonomy->getString(nodesCopy[i].nameIdx) !=
         taxonomy->getString(nodesCopy[i + 1].nameIdx))) {
      uniqueNames.emplace_back(taxonomy->getString(nodesCopy[i].nameIdx),
                               nodesCopy[i].taxId);
    }
  }
  if (nodesSize > 2 &&
      (taxonomy->getString(nodesCopy[nodesSize - 1].nameIdx) !=
       taxonomy->getString(nodesCopy[nodesSize - 2].nameIdx))) {
    uniqueNames.emplace_back(
        taxonomy->getString(nodesCopy[nodesSize - 1].nameIdx),
        nodesCopy[nodesSize - 1].taxId);
  }
  nodesCopy.clear();
