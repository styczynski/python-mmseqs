...
int masksequence(biosnake_output *out, Parameters &par) {
...
#pragma omp for schedule(dynamic, 1)
    for (size_t id = 0; id < reader.getSize(); ++id) {
      ...
      while (seqData[seqLen] != '\0') {
        charSequence[seqLen] =
            (char)subMat->aa2num[static_cast<int>(seqData[seqLen])];
        seqLen++;
      }
      tantan::maskSequences(
          charSequence, charSequence + seqLen, 50 /*options.maxCycleLength*/,
          probMatrix.probMatrixPointers, 0.005 /*options.repeatProb*/,
          0.05 /*options.repeatEndProb*/, 0.9 /*options.repeatOffsetProbDecay*/,
          0, 0, 0.5 /*options.minMaskProb*/, probMatrix.hardMaskTable);

      for (unsigned int pos = 0; pos < seqLen; pos++) {
        char aa = seqData[pos];
        charSequence[pos] = (charSequence[pos] == probMatrix.hardMaskTable[0])
                                ? tolower(aa)
                                : toupper(aa);
      }
