...
int proteinaln2nucl(biosnake_output *out, Parameters &par) {
  ...
  if (Parameters::isEqualDbtype(qdbr_nuc->getDbtype(),
      ...
      Parameters::isEqualDbtype(tdbr_nuc->getDbtype(),
                                Parameters::DBTYPE_NUCLEOTIDES) == false ||
      Parameters::isEqualDbtype(qdbr_aa->getDbtype(),
...
#pragma omp for schedule(dynamic, 10)
    for (size_t i = 0; i < alnDbr.getSize(); i++) {
          ...
          switch (res.backtrace[pos]) {
            case 'M':
              for (int bt = 0; bt < cnt * 3; bt++) {
                ...
                score += fastMatrix.matrix[(int)nuclQuerySeq[qPos]]
                                          [(int)nuclTargetSeq[tPos]];
                tPos++;
                qPos++;
              }
              update = true;
      ...
      resultWriter.writeData(result.c_str(), result.length(), alnKey,
                             thread_idx);
      result.clear();
    }
  }
