...
int createseqfiledb(biosnake_output *out, Parameters &par) {
...
#pragma omp for schedule(dynamic, 100)
    for (size_t i = 0; i < resultDb.getSize(); ++i) {
      ...
      if (entries < (unsigned int)par.minSequences ||
          entries > (unsigned int)par.maxSequences) {
        continue;
      }
      ...
      while (*data != '\0') {
        ...
        if (entries_num == 1 && par.hhFormat) {
          ...
          result.append(1, '>');
          result.append(headerDb.getData(headerId, thread_idx),
                        headerDb.getEntryLen(headerId) - 1);
          result.append(seqDb.getData(headerId, thread_idx),
                        seqDb.getEntryLen(headerId) - 1);
        }
      }
