...
int msa2result(biosnake_output *out, Parameters &par) {
  //    return EXIT_FAILURE;
  //}
  ...
  unsigned int *setSizes =
      (unsigned int *)calloc((msaReader.getSize() + 1), sizeof(unsigned int));
#pragma omp parallel
  {
...
#pragma omp for schedule(dynamic, 10) reduction(max : maxSeqLength, maxSetSize)
  ...
  // the validity of the evalue is questionable since we give the number of
...
#pragma omp parallel
  {
    ...
    char *msaContent = (char *)mem_align(
        ALIGN_INT, sizeof(char) * (maxSeqLength + 1) * maxSetSize);
    ...
    char **msaSequences =
        (char **)mem_align(ALIGN_INT, sizeof(char *) * maxSetSize);
...
#pragma omp for schedule(dynamic, 1)
    for (size_t id = 0; id < msaReader.getSize(); ++id) {
      ...
      std::string msa;
      if (par.msaType == 0) {
        msa = CompressedA3M::extractA3M(entryData, entryLength - 2,
                                        *sequenceReader, *headerReader,
                                        thread_idx);
      ...
      while (kseq_read(seq) >= 0) {
        ...
        if ((par.msaType == 0 || par.msaType == 1) &&
            strncmp("ss_", seq->name.s, strlen("ss_")) == 0) {
          continue;
        }
        ...
        if (setSize == 0) {
          ...
          //                    if (maskByFirst == true) {
          //                        for (size_t i = 0; i < centerLengthWithGaps;
          //                        ++i) {
          //                            if (seq->seq.s[i] == '-') {
        ...
        for (size_t i = 0; i < centerLengthWithGaps; ++i) {
          ...
          msaContent[msaPos++] = (seq->seq.s[i] == '-')
                                     ? (int)MultipleAlignment::GAP
                                     : sequence.numSequence[i];
        }
      ...
      if (maskByFirst == false) {
        PSSMCalculator::computeSequenceWeights(
            seqWeight, centerLengthWithGaps, setSize,
            const_cast<const char **>(msaSequences));
        ...
        for (unsigned int k = 0; k < setSize; ++k) {
          for (unsigned int i = 0; i < centerLengthWithGaps &&
                                   msaSequences[k][i] == MultipleAlignment::GAP;
               ++i)
            msaSequences[k][i] = MultipleAlignment::ENDGAP;
          for (unsigned int i = centerLengthWithGaps - 1;
               msaSequences[k][i] == MultipleAlignment::GAP; i--)
            msaSequences[k][i] = MultipleAlignment::ENDGAP;
        }
      ...
      if (par.filterMsa == 1) {
        filteredSetSize = filter.filter(
            setSize, centerLength, static_cast<int>(par.covMSAThr * 100),
      ...
      PSSMCalculator::Profile pssmRes = calculator.computePSSMFromMSA(
          filteredSetSize, centerLength, (const char **)msaSequences, par.wg);
      ...
      for (size_t i = 0; i < setSize; ++i) {
        ...
        for (size_t j = 0; j < centerLengthWithGaps; ++j) {
          ...
          char seqRes = currSeq[takeFromEnd ? (centerLength + (currentMask - 1))
                                            : currentCol - 1];
          seqRes = seqRes == MultipleAlignment::GAP
                       ? '-'
                       : subMat.num2aa[(int)seqRes];
        ...
        //                    int aa = currSeq[pos];
        //                    std::cout << (char)((aa != MultipleAlignment::GAP)
        ...
        Matcher::result_t res(key, 0, 1.0, 1.0, seqId, 0, bt.length(), 0,
                              consSeqNoGaps.size() - 1, consSeqNoGaps.size(), 0,
                              currSeqNoGaps.size() - 1, currSeqNoGaps.size(),
                              bt);

        // and update them and compute the score
        Matcher::updateResultByRescoringBacktrace(
            consSeqNoGaps.c_str(), currSeqNoGaps.c_str(), fastMatrix.matrix,
            evaluer, par.gapOpen.aminoacids, par.gapExtend.aminoacids, res);
      ...
      for (size_t i = 0; i < setSize; ++i) {
        unsigned int len =
            Matcher::resultToBuffer(buffer, results[i], true, true);
