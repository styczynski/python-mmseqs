...
#include <biosnake/commons/expressionParser.h>
...
int filterdb(biosnake_output *out, Parameters &par) {
  ...
  // positiveFilter = true => outDB = inDB \intersect filter ; othw : outDB =
  // inDB - filter
  ...
  if (par.sortEntries != 0) {
    ...
    for (size_t i = 0; i < filenames.size(); i++) {
      ...
      size_t offset = 0;
      bool inKey = true;
      // parse first column in each line without tripping over additional null
      // bytes as we allow database data files as input
...
#pragma omp parallel
  {
    ...
    if (mode == EXPRESSION_FILTERING) {
      ...
      if (parser->isOk() == false) {
        out->failure("Error in expression {}", par.filterExpression);
      }
    ...
    for (size_t id = 0; id < reader.getSize(); ++id) {
      ...
      while (*data != '\0') {
        ...
        if (mode != GET_FIRST_LINES || trimToOneColumn) {
          ...
          columnValue[Util::getLastNonWhitespace(columnValue, colStrLen)] =
              '\0';
        }
        ...
        if (mode == GET_FIRST_LINES) {
          ...
          if (newId != UINT_MAX) {
            ...
            if (columnToTake == -1) {
              ...
              std::vector<std::string> splittedLine =
                  Util::split(fullLine, "\t");
              char *newValue =
                  const_cast<char *>(splittedLine[columnToTake].c_str());
          ...
          std::vector<std::string>::iterator it = std::upper_bound(
              filter.begin(), filter.end(), toSearch, compareString());
          ...
          std::vector<std::pair<std::string, std::string>>::iterator it =
              std::lower_bound(mapping.begin(), mapping.end(), toSearch,
          ...
          *newLineBuffer = '\0';
          ...
          while (it != mapping.end() && toSearch.compare(it->first) == 0) {
            ...
            memcpy(newLineBuffer + newLineBufferIndex, (it->second).c_str(),
                   (it->second).length());
            newLineBufferIndex += (it->second).length();

            // copy the next columns
            if (foundElements > column) {
              memcpy(newLineBuffer + newLineBufferIndex,
                     columnPointer[column - 1] + fieldLength,
