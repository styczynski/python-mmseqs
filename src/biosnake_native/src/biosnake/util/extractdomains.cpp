...
#include <biosnake/commons/domain.h>
...
std::vector<Domain> getEntries(const std::string &entry) {
  ...
  while (std::getline(iss, line)) {
    ...
    unsigned int qStart =
        static_cast<unsigned int>(strtoul(fields[2].c_str(), NULL, 10));
    unsigned int qEnd =
        static_cast<unsigned int>(strtoul(fields[3].c_str(), NULL, 10));
    unsigned int qLength =
        static_cast<unsigned int>(strtoul(fields[4].c_str(), NULL, 10));

    unsigned int tStart =
        static_cast<unsigned int>(strtoul(fields[5].c_str(), NULL, 10));
    unsigned int tEnd =
        static_cast<unsigned int>(strtoul(fields[6].c_str(), NULL, 10));
    unsigned int tLength =
        static_cast<unsigned int>(strtoul(fields[7].c_str(), NULL, 10));
    double eValue = strtod(fields[8].c_str(), NULL);

    result.emplace_back(fields[0], qStart, qEnd, qLength, fields[1], tStart,
                        tEnd, tLength, eValue);
  }
...
int scoreSubAlignment(biosnake_output* out, std::string query, std::string target,
  ...
  for (unsigned int i = 0; i < (qEnd - qStart); ++i) {
    ...
    if (query[qPos] == '-') {
      ...
      //            std::cout << "qGap\t"  << query[qPos] << "\t" <<
      //            target[tPos] << "\t" << rawScore << "\t" << rawScore << "\t"
      //            << maxScore << std::endl;
    }
    // skip gaps and lower letters (since they are insertions)
    if (target[tPos] == '-' || (islower(target[tPos]))) {
      ...
      //            std::cout << "Matc\t"  << queryAA << "\t" << targetAA <<
      //            "\t" << matchScore << "\t" << rawScore << "\t" << maxScore
      //            << std::endl;
...
std::vector<Domain> mapMsa(biosnake_output* out, const std::string &msa,
  ...
  while (kseq_read(seq) >= 0) {
    ...
    if (Util::startWith("consensus_", fullName) ||
        Util::endsWith("_consensus", fullName)) {
      continue;
    }
    ...
    for (std::vector<Domain>::const_iterator it = domains.begin();
         it != domains.end(); ++it) {
      const Domain &domain = *it;
      unsigned int length = static_cast<unsigned int>(
          std::count_if(sequence.begin(), sequence.end(), isalpha));
      ...
      for (size_t aa_pos = 0; aa_pos < sequence.length(); aa_pos++) {
        ...
        if (posWithoutInsertion == domain.qEnd && foundStart == true) {
          foundStart = false;
          unsigned int domainEnd =
              std::min(static_cast<unsigned int>(aa_pos), length - 1);
          float domainCov =
...
int doExtract(biosnake_output* out, Parameters &par, DBReader<unsigned int> &blastTabReader,
  ...
  std::string msaHeaderDataName, msaHeaderIndexName, msaSequenceDataName,
      msaSequenceIndexName;
...
#pragma omp for schedule(dynamic, 100)
    for (size_t i = dbFrom; i < dbFrom + dbSize; ++i) {
      ...
      switch (par.msaType) {
        case 0: {
          msa = CompressedA3M::extractA3M(data, entryLength, *sequenceReader,
                                          *headerReader, thread_idx);
          break;
        }
        case 1:
        case 2: {
...
int doExtract(biosnake_output* out, Parameters &par, const unsigned int mpiRank,
...
int doExtract(biosnake_output* out, Parameters &par) {
  size_t resultSize;
...
int extractdomains(biosnake_output *out, Parameters &par) {
...
#ifdef HAVE_MPI
  int status = doExtract(par, BiosnakeMPI::rank, BiosnakeMPI::numProc);
#else
