...
int apply(biosnake_output* out, Parameters& par) {
...
int apply_by_entry(char *data, size_t size, unsigned int key, DBWriter &writer,
  ...
  snprintf(environ[0], 64, "BIOSNAKE_ENTRY_NAME=%d", key);
...
char **local_environment() {
  ...
  for (size_t i = 0; environ[i] != NULL; ++i) {
    if (prefix("BIOSNAKE_ENTRY_NAME=", environ[i]) == 0) {
...
int apply(biosnake_output *out, Parameters &par) {
  ...
  for (int thread = 0; thread < par.threads; ++thread) {
    switch (fork()) {
      ...
      case 0: {
        ...
        if (thread != 0 || mpiRank != BiosnakeMPI::MASTER) {
          //Log::setLogLevel(0);
        }
...
#ifdef HAVE_MPI
  shared_memory->mpiRank = BiosnakeMPI::rank;
  shared_memory->mpiProc = BiosnakeMPI::numProc;
  ...
  if (BiosnakeMPI::rank == BiosnakeMPI::MASTER) {
