...
int result2rbh(biosnake_output *out, Parameters &par) {
    ...
    for (size_t id = 0; id < resultReader.getSize(); id++) {
      ...
      while (*results != '\0') {
        ...
        if (bestAtoBbitScore == 0) {
          ...
          // current B->A has bestAtoBbitScore - we retain this line for
          // writing!
          startRbhB = results;
          results = Util::skipLine(results);
          endRbhB = results;
      ...
      dbw.writeData(rbhBsString.c_str(), rbhBsString.length(), AdbID,
                    thread_idx);
      rbhBsString.clear();
    }
  }
