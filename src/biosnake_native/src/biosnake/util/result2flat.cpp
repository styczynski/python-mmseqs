...
int result2flat(biosnake_output *out, Parameters &par) {
  ...
  for (size_t i = 0; i < dbr_data.getSize(); i++) {
    ...
    while (*data != '\0') {
      ...
      for (size_t i = 0; i < DbValidator::resultDb.size(); i++) {
        if (Parameters::isEqualDbtype(dbr_data.getDbtype(),
                                      DbValidator::resultDb[i])) {
          keyLen = (words[1] - words[0]);
          dbKeyBuffer.size();
          dbKeyBuffer.append(words[0], keyLen);
          const unsigned int dbKey =
              (unsigned int)strtoul(dbKeyBuffer.c_str(), NULL, 10);
