...
int alignall(biosnake_output *out, Parameters &par) {
...
#pragma omp for schedule(dynamic, 1)
      for (size_t id = start; id < (start + bucketSize); id++) {
        ...
        for (size_t entryIdx1 = 0; entryIdx1 < results.size(); entryIdx1++) {
          ...
          query.mapSequence(queryId, queryKey, querySeq,
                            tdbr.getSeqLen(queryId));
          ...
          *(tmpBuff - 1) = '\t';
          ...
          for (size_t entryIdx = 0; entryIdx < results.size(); entryIdx++) {
            ...
            Matcher::result_t result = matcher.getSWResult(
                &target, INT_MAX, false, par.covMode, par.covThr, par.evalThr,
            ...
            if (Alignment::checkCriteria(result, isIdentity, par.evalThr,
              ...
              size_t len =
                  Matcher::resultToBuffer(tmpBuff, result, par.addBacktrace);
