...
int msa2profile(biosnake_output *out, Parameters &par) {
...
#pragma omp for schedule(dynamic, 1)
    for (size_t id = 0; id < qDbr.getSize(); ++id) {
      ...
      while (kseq_read(seq) >= 0) {
        ...
        if (setSize == 0) {
          ...
          if ((mode & DBReader<unsigned int>::USE_LOOKUP) == 0) {
            ...
            header.append("\n");
            headerWriter.writeData(header.c_str(), header.size(), queryKey,
                                   thread_idx);
          }
        }
      ...
      pssmRes.toBuffer((const unsigned char *)msaSequences[0], centerLength,
                       subMat, result);
      ...
      if (mode & DBReader<unsigned int>::USE_LOOKUP) {
        ...
        header.append(1, '\n');
        headerWriter.writeData(header.c_str(), header.length(), queryKey,
                               thread_idx);
      }
