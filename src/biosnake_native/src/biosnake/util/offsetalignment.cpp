#include <biosnake/clustering/alignmentSymmetry.h>
...
void chainAlignmentHits(std::vector<Matcher::result_t> &results,
                        std::vector<Matcher::result_t> &tmp) {
  if (results.size() > 1) {
    std::stable_sort(results.begin(), results.end(),
                     Matcher::compareHitsByPosAndStrand);
    ...
    for (size_t resIdx = 0; resIdx < results.size(); resIdx++) {
      bool currQueryStrang =
          (results[resIdx].qStartPos > results[resIdx].qEndPos);
      int qStartPos =
          std::min(results[resIdx].qStartPos, results[resIdx].qEndPos);
      int qEndPos =
          std::max(results[resIdx].qStartPos, results[resIdx].qEndPos);
      bool currTargetStrand =
          (results[resIdx].dbStartPos > results[resIdx].dbEndPos);
      int dbStartPos =
          std::min(results[resIdx].dbStartPos, results[resIdx].dbEndPos);
      int dbEndPos =
          std::max(results[resIdx].dbStartPos, results[resIdx].dbEndPos);
      std::cout << results[resIdx].dbKey << "\t" << qStartPos << "\t" << qEndPos
                << "\t" << dbStartPos << "\t" << dbEndPos << "\t" << std::endl;
      ...
      if (isLastElement == false) {
        int nextqStartPos = std::min(results[resIdx + 1].qStartPos,
                                     results[resIdx + 1].qEndPos);
        int nextdbStartPos = std::min(results[resIdx + 1].dbStartPos,
                                      results[resIdx + 1].dbEndPos);
        nextDiagonal = nextqStartPos - nextdbStartPos;
        nextQueryStrand =
            (results[resIdx + 1].qStartPos > results[resIdx + 1].qEndPos);
        nextTargetStrand =
            (results[resIdx + 1].dbStartPos > results[resIdx + 1].dbEndPos);
      }
      const bool queryIsOverlapping =
          currRegion.qEndPos >= qStartPos && currRegion.qEndPos <= qEndPos;
      const bool targetIsOverlapping =
          currRegion.dbEndPos >= dbStartPos && currRegion.dbEndPos <= dbEndPos;
      const bool sameNextDiagonal = (currDiagonal == nextDiagonal);
      const bool samePrevDiagonal = (currDiagonal == prevDiagonal);
      if ((sameNextDiagonal || samePrevDiagonal) && queryIsOverlapping &&
          targetIsOverlapping) {
      ...
      bool isDifferentStrand = (nextQueryStrand != currQueryStrang) ||
                               (nextTargetStrand != currTargetStrand);
...
// query update
//   Nucl/Prot
void updateOffset(biosnake_output* out, char *data, std::vector<Matcher::result_t> &results,
  ...
  for (size_t i = startPos; i < endPos; i++) {
    ...
    if (targetNeedsUpdate == true || qloc == NULL) {
      ...
      size_t from =
          (tloc.id != UINT_MAX)
              ? tloc.from
              : (tloc.strand == Orf::STRAND_MINUS) ? res.dbLen - 1 : 0;
    ...
    if (qloc != NULL) {
      ...
      size_t from =
          (qloc->id != UINT_MAX)
              ? qloc->from
              : (qloc->strand == Orf::STRAND_MINUS) ? 0 : res.qLen - 1;
...
void updateLengths(std::vector<Matcher::result_t> &results,
                   unsigned int qSourceLen, IndexReader *tSourceDbr) {
...
int offsetalignment(biosnake_output *out, Parameters &par) {
  ...
  if (targetNucl) {
    ...
    if (Parameters::isEqualDbtype(tSourceDbr->getDbtype(),
      ...
      isTransNucTransNucSearch = Parameters::isEqualDbtype(
...
#pragma omp for schedule(dynamic, 10)
    ...
    out->info("Computing contig offsets");
...
#pragma omp parallel for schedule(static) num_threads(localThreads)
    ...
    out->info("Computing contig lookup");
...
#pragma omp parallel for schedule(static) num_threads(localThreads)
    ...
    contigOffsets[0] = 0;
    out->info("Time for contig lookup: {}", timer.lap());
  }

  out->info("Writing results to: {}", par.db6);
    ...
    for (size_t i = 0; i < entryCount; ++i) {
      ...
      if (Parameters::isEqualDbtype(queryDbType,
        ...
        for (unsigned int j = 0; j < orfCount; ++j) {
          ...
          // this is needed when alnDbr does not contain all identifier of the
          // queryDB
          if (orfId == UINT_MAX) {
            continue;
          }
          ...
          if (par.mergeQuery == false) {
            for (size_t i = 0; i < results.size(); i++) {
              ...
              if (isTransNuclAln == true && isNuclNuclSearch == false &&
                  isTransNucTransNucSearch == true && hasBacktrace) {
              ...
              size_t len = Matcher::resultToBuffer(buffer, res, hasBacktrace,
                                                   false, true);
              ss.append(buffer, len);
            }
            resultWriter.writeData(ss.c_str(), ss.length(), queryKey,
                                   thread_idx);
            results.clear();
            ss.clear();
            tmp.clear();
      ...
      } else if (Parameters::isEqualDbtype(targetDbType,
                                           Parameters::DBTYPE_NUCLEOTIDES)) {
      ...
      if (par.mergeQuery == true) {
        ...
        if (par.chainAlignment == false) {
          ...
          for (size_t i = 0; i < results.size(); i++) {
            ...
            size_t len =
                Matcher::resultToBuffer(buffer, res, hasBacktrace, false, true);
            ss.append(buffer, len);
          }
