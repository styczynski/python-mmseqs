...
static inline float getOverlap(const std::vector<bool> &covered,
...
std::vector<Domain> mapDomains(biosnake_output* out, const std::vector<Domain> &input, float overlap,
  ...
  for (size_t i = 0; i < input.size(); i++) {
    ...
    float targetCov =
        MathUtil::getCoverage(domain.tStart, domain.tEnd, domain.tLength);
    if (percentageOverlap <= overlap && targetCov > minCoverage &&
        domain.eValue < eValThreshold) {
...
std::map<std::string, unsigned int> readLength(biosnake_output* out, const std::string &file) {
  ...
  while (std::getline(mappingStream, line)) {
    ...
    unsigned int length =
        static_cast<unsigned int>(strtoul(split[1].c_str(), NULL, 10));
...
std::vector<Domain> getEntries(
  ...
  while (std::getline(iss, line)) {
    ...
    unsigned int qStart =
        static_cast<unsigned int>(strtoul(fields[6].c_str(), NULL, 10)) - 1;
    unsigned int qEnd =
        static_cast<unsigned int>(strtoul(fields[7].c_str(), NULL, 10)) - 1;

    std::map<std::string, unsigned int>::const_iterator it =
        lengths.lower_bound(query);
    ...
    unsigned int tStart =
        static_cast<unsigned int>(strtoul(fields[8].c_str(), NULL, 10)) - 1;
    unsigned int tEnd =
        static_cast<unsigned int>(strtoul(fields[9].c_str(), NULL, 10)) - 1;
    ...
    result.push_back(Domain(query, qStart, qEnd, qLength, fields[1], tStart,
                            tEnd, tLength, eValue));
  }
...
int doAnnotate(biosnake_output* out, Parameters &par, DBReader<unsigned int> &blastTabReader,
...
int doAnnotate(biosnake_output* out, Parameters &par, const unsigned int mpiRank,
...
int doAnnotate(biosnake_output* out, Parameters &par) {
  DBReader<unsigned int> reader(
      out, par.db1.c_str(), par.db1Index.c_str(), par.threads,
      DBReader<unsigned int>::USE_INDEX | DBReader<unsigned int>::USE_DATA);
  reader.open(DBReader<unsigned int>::LINEAR_ACCCESS);
...
int summarizetabs(biosnake_output *out, Parameters &par) {
...
#ifdef HAVE_MPI
  int status = doAnnotate(out, par, BiosnakeMPI::rank, BiosnakeMPI::numProc);
#else
