...
int alignbykmer(biosnake_output *out, Parameters &par) {
  ...
  struct KmerPos {
    ...
    static bool compareKmerPos(const KmerPos &first, const KmerPos &second) {
      ...
      if (first.ij < second.ij) return true;
      if (second.ij < first.ij) return false;
  ...
  struct Stretche {
    ...
    static bool compareStretche(const Stretche &first, const Stretche &second) {
      // return (first.eval < second.eval);
      if (first.i_start < second.i_start) return true;
      if (first.i_start > second.i_start) return false;
      if (first.i_end > second.i_end) return true;
      if (first.i_end < second.i_end) return false;
      return false;
    }
  };
  ...
  size_t iterations = static_cast<int>(ceil(
      static_cast<double>(dbr_res.getSize()) / static_cast<double>(flushSize)));
...
#pragma omp for schedule(dynamic, 1)
      for (size_t id = start; id < (start + bucketSize); id++) {
        ...
        while (query.hasNextKmer()) {
          ...
          //                    kmerGenerator.generateKmerList(kmer); for
          //                    (size_t pos = 0; pos <  scoreMat.elementSize;
          //                    pos++) {
          //                        if (queryPosLookup[scoreMat.index[pos]] ==
          //                        USHRT_MAX) {
          //                            queryPosLookup[scoreMat.index[pos]] =
          //                            pos;
          //                        }
          //                    }
        }
        ...
        while (*data != '\0') {
          ...
          const unsigned int dbKey =
              (unsigned int)strtoul(dbKeyBuffer, NULL, 10);
          unsigned int targetId = tdbr->getId(dbKey);
          char *targetSeq = tdbr->getData(targetId, thread_idx);
          const bool isIdentity =
              (queryId == targetId && (par.includeIdentity || sameDB)) ? true
                                                                       : false;
          ...
          SORT_SERIAL(kmerPosVec, kmerPosVec + kmerPosSize,
                      KmerPos::compareKmerPos);
          ...
          if (kmerPosSize > 1) {
            ...
            for (size_t kmerIdx = 0; kmerIdx < kmerPosSize; kmerIdx++) {
              unsigned int currDiagonal =
                  static_cast<unsigned int>(kmerPosVec[kmerIdx].i) -
                  static_cast<unsigned int>(kmerPosVec[kmerIdx].j);
              ...
              if (kmerIdx < (kmerPosSize - 1)) {
                nextDiagonal =
                    static_cast<unsigned int>(kmerPosVec[kmerIdx + 1].i) -
                    static_cast<unsigned int>(kmerPosVec[kmerIdx + 1].j);
              }
              // skip single hits
              if (currDiagonal != nextDiagonal &&
                  currDiagonal != prevDiagonal) {
                continue;
              }

              if ((nextDiagonal == currDiagonal ||
                   prevDiagonal == currDiagonal) &&
                  prev_i <= curr_i && prev_j <= curr_j) {
                //                std::cout << "ij: " << kmerPosVec[kmerIdx].ij
                //                << " i: " << kmerPosVec[kmerIdx].i
                //                      << " j:" << kmerPosVec[kmerIdx].j << "
                //                      Diag: " <<  kmerPosVec[kmerIdx].i -
                //                      kmerPosVec[kmerIdx].j << std::endl;
              ...
              prev_j = curr_j;
              if (nextDiagonal != currDiagonal ||
                  kmerIdx == (kmerPosSize - 1)) {
                //                                std::cout << region_min_i <<
                //                                "\t" << region_max_i << "\t"
                //                                << region_min_j << "\t"  <<
                //                                region_max_j << "\t" <<
                //                                region_min_i - region_min_j <<
                //                                std::endl;
          ...
          SORT_SERIAL(stretcheVec, stretcheVec + stretcheSize,
                      Stretche::compareStretche);
          ...
          for (size_t currStretche = 0; currStretche < stretcheSize;
               ++currStretche) {
            for (size_t prevPotentialStretche = 0;
                 prevPotentialStretche < currStretche;
                 ++prevPotentialStretche) {
              // check the first one does not contain the second one:
              if (stretcheVec[currStretche].i_start >
                      stretcheVec[prevPotentialStretche].i_end &&
                  stretcheVec[currStretche].j_start >
                      stretcheVec[prevPotentialStretche].i_end) {
                int bestScorePathPrevIsLast =
                    dpMatrixRow[prevPotentialStretche].pathScore;
                int distance =
                    gapOpen + (stretcheVec[prevPotentialStretche].i_end -
                               stretcheVec[currStretche].i_start) *
                                  gapExtend;
                int costOfPrevToCurrTransition = distance;
                int currScore =
                    stretcheVec[currStretche].kmerCnt * par.kmerSize * 2;
                int currScoreWithPrev = bestScorePathPrevIsLast +
                                        costOfPrevToCurrTransition + currScore;

                // update row of currPotentialExon in case of improvement:
                if (currScoreWithPrev > dpMatrixRow[currStretche].pathScore) {
                  dpMatrixRow[currStretche].prevPotentialId =
                      prevPotentialStretche;
          ...
          for (size_t stretch = (strechtPath.size() - 1); stretch > 0;
               stretch--) {
            int score = 0;
            int pos = 0;
            //                        query.print();
            ...
            //                        for (int i = strechtPath[stretch].i_end, j
            //                        = strechtPath[stretch].j_end;
            //                             i < strechtPath[stretch - 1].i_start;
            //                             i++, j++) {
            //                            std::cout <<
            //                            subMat->num2aa[query.sequence[i]];
            //                        }
            //                        std::cout << std::endl;
            //
            //                        for (int i = strechtPath[stretch].i_end, j
            ...
            //                             i++, j++) {
            //                            std::cout <<
            //                            subMat->num2aa[target.sequence[j]];
            //                        }
            //                        std::cout << std::endl;

            for (int i = strechtPath[stretch].i_end,
                     j = strechtPath[stretch].j_end;
                 i < strechtPath[stretch - 1].i_start &&
                 j < strechtPath[stretch - 1].j_start;
                 i++, j++) {
              int curr =
                  subMat
                      ->subMatrix[query.numSequence[i]][target.numSequence[j]];
              score = curr + score;
            ...
            scores[pos] = 0;
            score = 0;
            for (int i = strechtPath[stretch - 1].i_start,
                     j = strechtPath[stretch - 1].j_start;
                 i > strechtPath[stretch].i_end &&
                 j > strechtPath[stretch].j_end;
                 i--, j--) {
              int curr =
                  subMat
            ...
            strechtPath[stretch - 1].i_start =
                strechtPath[stretch - 1].i_start - maxRevPos;
            strechtPath[stretch - 1].j_start =
                strechtPath[stretch - 1].j_start - maxRevPos;
          ...
          // find correct start and end
          {
            int maxScore = 0;
            int score = 0;
            for (int i = strechtPath[strechtPath.size() - 1].i_start,
                     j = strechtPath[strechtPath.size() - 1].j_start;
                 i > -1 && j > -1; i--, j--) {
              int curr =
                  subMat
                      ->subMatrix[query.numSequence[i]][target.numSequence[j]];
            ...
            for (int i = strechtPath[0].i_end, j = strechtPath[0].j_end;
                 i < query.L && j < target.L; i++, j++) {
              int curr =
                  subMat
                      ->subMatrix[query.numSequence[i]][target.numSequence[j]];
              score = curr + score;
          ...
          //                    for(size_t stretch = 0; stretch <
          //                    strechtPath.size() ; stretch++) {
          //                        std::cout << stretcheVec[stretch].i_start <<
          //                        "\t" << stretcheVec[stretch].i_end << "\t";
          //                    }
          //                    std::cout << std::endl;
          ...
          for (int stretch = strechtPath.size() - 1; stretch > -1; stretch--) {
            for (size_t i = strechtPath[stretch].i_start,
                        j = strechtPath[stretch].j_start;
            ...
            if (stretch > 0) {
              score -= gapOpen;
              if (strechtPath[stretch - 1].i_start ==
                  strechtPath[stretch].i_end) {
                for (size_t pos = strechtPath[stretch].j_end;
                     pos < strechtPath[stretch - 1].j_start; pos++) {
                  //                                    querystr.push_back('-');
                ...
                for (size_t pos = strechtPath[stretch].i_end;
                     pos < strechtPath[stretch - 1].i_start; pos++) {
          ...
          float queryCov = SmithWaterman::computeCov(
              strechtPath[strechtPath.size() - 1].i_start, strechtPath[0].i_end,
              query.L);
          float targetCov = SmithWaterman::computeCov(
              strechtPath[strechtPath.size() - 1].j_start, strechtPath[0].j_end,
              target.L);
          int alnLen = bt.size();

          const float seqId =
              static_cast<float>(ids) / static_cast<float>(alnLen);
          ...
          const bool hasCov =
              Util::hasCoverage(par.covThr, par.covMode, queryCov, targetCov);
          // --min-seq-id
          const bool hasSeqId =
          ...
          if (isIdentity || (hasCov && hasSeqId && hasEvalue)) {
            Matcher::result_t result = Matcher::result_t(
                dbKey, bitScore, queryCov, targetCov, seqId, evalue, alnLen,
                strechtPath[strechtPath.size() - 1].i_start,
                strechtPath[0].i_end, query.L,
                strechtPath[strechtPath.size() - 1].j_start,
                strechtPath[0].j_end, target.L, bt);
