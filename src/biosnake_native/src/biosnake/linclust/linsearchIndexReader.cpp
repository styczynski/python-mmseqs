...
#include <biosnake/linclust/kmersearch.h>
#ifndef SIZE_T_MAX
#define SIZE_T_MAX ((size_t)-1)
#endif

template <int TYPE>
size_t LinsearchIndexReader::pickCenterKmer(biosnake_output* out, KmerPosition<short> *hashSeqPair,
...
template size_t LinsearchIndexReader::pickCenterKmer<0>(
    biosnake_output* out, KmerPosition<short> *hashSeqPair, size_t splitKmerCount);
template size_t LinsearchIndexReader::pickCenterKmer<1>(
    biosnake_output* out, KmerPosition<short> *hashSeqPair, size_t splitKmerCount);

template <int TYPE>
void LinsearchIndexReader::mergeAndWriteIndex(biosnake_output* out, DBWriter &dbw,
...
template void LinsearchIndexReader::mergeAndWriteIndex<0>(
    biosnake_output* out, DBWriter &dbw, std::vector<std::string> tmpFiles, int alphSize,
    int kmerSize);
...
template <int TYPE>
void LinsearchIndexReader::writeIndex(biosnake_output* out, DBWriter &dbw,
...
template void LinsearchIndexReader::writeIndex<0>(
    biosnake_output* out, DBWriter &dbw, KmerPosition<short> *hashSeqPair, size_t totalKmers,
    int alphSize, int kmerSize);
template void LinsearchIndexReader::writeIndex<1>(
    biosnake_output* out, DBWriter &dbw, KmerPosition<short> *hashSeqPair, size_t totalKmers,
    int alphSize, int kmerSize);

std::string LinsearchIndexReader::indexName(biosnake_output* out, std::string baseName) {
  std::string result(baseName);
  result.append(".").append("linidx");
  return result;
}

bool LinsearchIndexReader::checkIfIndexFile(biosnake_output* out, DBReader<unsigned int> *pReader) {
...
void LinsearchIndexReader::writeKmerIndexToDisk(biosnake_output* out, std::string fileName,
...
std::string LinsearchIndexReader::findIncompatibleParameter(
    biosnake_output* out, DBReader<unsigned int> &index, Parameters &par, int dbtype) {
...
std::string LinsearchIndexReader::searchForIndex(biosnake_output* out, const std::string &dbName) {
