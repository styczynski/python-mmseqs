...
KmerSearch::ExtractKmerAndSortResult KmerSearch::extractKmerAndSort(
    biosnake_output* out, size_t totalKmers, size_t hashStartRange, size_t hashEndRange,
...
int kmersearch(biosnake_output *out, Parameters &par) {
  ...
  if (Parameters::isEqualDbtype(FileUtil::parseDbType(out, par.db2.c_str()),
    ...
    out->failure("Create index before calling kmersearch with biosnake createlinindex");
  }
