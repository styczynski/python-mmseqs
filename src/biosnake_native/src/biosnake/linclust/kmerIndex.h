#ifndef BIOSNAKE_KMERINDEX_H
#define BIOSNAKE_KMERINDEX_H
...
class KmerIndex {
 ...
 public:
  struct KmerEntry {
    ...
    unsigned short pos;
    unsigned short seqLen;
    KmerEntry(size_t kmer, unsigned int id, unsigned short pos,
              unsigned short seqLen)
  ...
  KmerIndex(biosnake_output* output): out(output) {}
  ...
  void init(const size_t alphabetSize, const size_t kmerSize,
    ...
    this->entryCount = 0;
    this->indexGridSize = MathUtil::ceilIntDivision(
        MathUtil::ipow<size_t>(alphabetSize, kmerSize), indexGridResolution);
  ...
  KmerIndex(biosnake_output* output, const size_t alphabetSize, const size_t kmerSize)
  ...
  KmerEntry getNextEntry() {
    iteratorPos++;
    while (iteratorPos >=
           static_cast<long long>(entryOffsets[entryOffsetPos + 1])) {
      entryOffsetPos++;
    }
    size_t kmer;
    ...
    if (TYPE == Parameters::DBTYPE_NUCLEOTIDES) {
      ...
      //            std::cout << entryOffsetPos <<  "\t" << iteratorPos << "\t"
      //            << entryOffsets[entryOffsetPos] <<  "\t" <<
      //            entries[iteratorPos].kmerOffset << std::endl;
      kmer = entryOffsetPos * indexGridResolution +
             static_cast<size_t>(entries[iteratorPos].kmerOffset);
    }
    return KmerEntry(kmer, entries[iteratorPos].id, entries[iteratorPos].pos,
                     entries[iteratorPos].seqLen);
  }

  size_t getGridPosition(size_t kmer) { return kmer / indexGridResolution; }

  size_t getKmerStartRange(size_t kmer) {
    return (kmer / indexGridResolution) * indexGridResolution;
  }

  size_t getGridResolution() { return indexGridResolution; }
  ...
  void flush(DBWriter &writer) {
    writer.writeAdd((char *)entries,
                    writingPosition * sizeof(KmerEntryRelative), 0);
    writingPosition = 0;
  }

  void addElementSorted(const size_t kmer, unsigned int id, unsigned short pos,
                        unsigned short seqLen, bool isReverse) {
    const size_t gridPosition = getGridPosition(kmer);
    size_t kmerStartRange = getKmerStartRange(kmer);
    ...
    entries[writingPosition].kmerOffset =
        (isReverse) ? BIT_SET(entries[writingPosition].kmerOffset, 15)
                    : entries[writingPosition].kmerOffset;
  ...
  size_t getOffsetsSize() { return indexGridSize; }

  uint64_t getTableEntriesNum() { return entryCount; }

  KmerIndex(int alphabetSize, int kmerSize, char *entriesData,
            char *entriesOffetData, size_t entryCount, size_t gridResolution) {
    ...
    this->indexGridSize = MathUtil::ceilIntDivision(
        MathUtil::ipow<size_t>(alphabetSize, kmerSize), gridResolution);
