...
template <typename T>
KmerPosition<T> *initKmerPositionMemory(biosnake_output* out, size_t size) {
  KmerPosition<T> *hashSeqPair = new (std::nothrow) KmerPosition<T>[size + 1];
  Util::checkAllocation(out, hashSeqPair, "Can not allocate memory");
...
void maskSequence(int maskMode, int maskLowerCase, Sequence &seq,
  ...
  if (maskLowerCase == 1 &&
    ...
    for (int i = 0; i < seq.L; i++) {
      seq.numSequence[i] =
          (islower(charSeq[i])) ? maskLetter : seq.numSequence[i];
    }
  }
}
...
std::pair<size_t, size_t> fillKmerPositionArray(
  ...
  if (TYPE == Parameters::DBTYPE_HMM_PROFILE) {
    two = ExtendedSubstitutionMatrix::calcScoreMatrix(out, *subMat, 2);
    three = ExtendedSubstitutionMatrix::calcScoreMatrix(out, *subMat, 3);
  }
...
#pragma omp parallel
  {
    ...
    const int adjustedKmerSize =
        (par.adjustKmerLength) ? std::min(par.kmerSize + 5, 23) : par.kmerSize;
    Sequence seq(out, par.maxSeqLen, querySeqType, subMat, adjustedKmerSize,
                 par.spacedKmer, false, true, par.spacedKmerPattern);
    KmerGenerator *generator;
    if (TYPE == Parameters::DBTYPE_HMM_PROFILE) {
      generator = new KmerGenerator(out, par.kmerSize, subMat->alphabetSize, 150);
      generator->setDivideStrategy(&three, &two);
    }
    Indexer idxer(out, subMat->alphabetSize - 1, par.kmerSize);
    const unsigned int BUFFER_SIZE = 1048576;
    size_t bufferPos = 0;
    KmerPosition<T> *threadKmerBuffer = new KmerPosition<T>[BUFFER_SIZE];
    SequencePosition *kmers = (SequencePosition *)malloc(
        (par.pickNbest * (par.maxSeqLen + 1) + 1) * sizeof(SequencePosition));
    size_t kmersArraySize = par.maxSeqLen;
    const size_t flushSize = 100000000;
    size_t iterations =
        static_cast<size_t>(ceil(static_cast<double>(seqDbr.getSize()) /
                                 static_cast<double>(flushSize)));
    for (size_t i = 0; i < iterations; i++) {
      size_t start = (i * flushSize);
      size_t bucketSize =
          std::min(seqDbr.getSize() - (i * flushSize), flushSize);
...
#pragma omp for schedule(dynamic, 100)
      for (size_t id = start; id < (start + bucketSize); id++) {
        ...
        maskSequence(par.maskMode, par.maskLowerCaseMode, seq,
                     subMat->aa2num[static_cast<int>('X')], probMatrix);
        ...
        while (seq.hasNextKmer()) {
          ...
          if (TYPE == Parameters::DBTYPE_NUCLEOTIDES) {
            ...
            const unsigned short hash =
                static_cast<unsigned short>(XXH64(&kmerIdx, 8, par.hashShift));

            if (par.adjustKmerLength) {
              unsigned char revKmer[32];
              unsigned char *kmerToHash = kmer;
              if (pickReverseKmer) {
                for (int pos = static_cast<int>(adjustedKmerSize) - 1; pos > -1;
                     pos--) {
                  revKmer[(adjustedKmerSize - 1) - pos] =
                      nuclMatrix->reverseResidue(kmer[pos]);
                }
                kmerToHash = revKmer;
              }
              kmerLen = MarkovKmerScore::adjustedLength(
                  kmerToHash, adjustedKmerSize,
                  (par.kmerSize - MarkovScores::MARKOV_ORDER) *
                      MarkovScores::MEDIAN_SCORE);
              longestKmer = std::max(kmerLen, longestKmer);
              kmerIdx = Indexer::computeKmerIdx(kmerToHash, kmerLen);
            }

            // set signed bit for normal kmers to make the  SIZE_T_MAX logic
            // easier reversed kmers do not have a signed bit
            size_t kmerRev = (pickReverseKmer) ? BIT_CLEAR(kmerIdx, 63)
                                               : BIT_SET(kmerIdx, 63);
            (kmers + seqKmerCount)->kmer = kmerRev;
            int pos = seq.getCurrentPosition();
            (kmers + seqKmerCount)->pos =
                (pickReverseKmer) ? (seq.L) - pos - kmerLen : pos;
            (kmers + seqKmerCount)->score = hash;
            scoreDist[hash]++;
            hierarchicalScoreDist[hash >> 9]++;
            seqKmerCount++;
          } else if (TYPE == Parameters::DBTYPE_HMM_PROFILE) {
            std::pair<size_t *, size_t> scoreMat =
                generator->generateKmerList(kmer, true);
            //                        std::cout << scoreMat.elementSize <<
            //                        std::endl;
            for (size_t kmerPos = 0;
                 kmerPos < scoreMat.second &&
                 kmerPos < static_cast<size_t>(par.pickNbest);
                 kmerPos++) {
              (kmers + seqKmerCount)->kmer = scoreMat.first[kmerPos];
              (kmers + seqKmerCount)->pos = seq.getCurrentPosition();
              const unsigned short hash = static_cast<unsigned short>(
                  XXH64(&(kmers + seqKmerCount)->kmer, 8, par.hashShift));
              (kmers + seqKmerCount)->score = hash;
              scoreDist[hash]++;
              hierarchicalScoreDist[hash >> 9]++;
              seqKmerCount++;
            ...
            (kmers + seqKmerCount)->pos = seq.getCurrentPosition();
            const unsigned short hash = static_cast<unsigned short>(
                XXH64(&kmerIdx, sizeof(kmerIdx), par.hashShift));
            //                        (kmers + seqKmerCount)->score = hash;
            //                        const unsigned short hash =
            //                        circ_hash(kmer, par.kmerSize, 5);
            (kmers + seqKmerCount)->score = hash;
            scoreDist[hash]++;
            hierarchicalScoreDist[hash >> 9]++;
            //                        std::cout << seqId << "\t" << (kmers +
            //                        seqKmerCount)->score << "\t" << (kmers +
            //                        seqKmerCount)->pos << std::endl;

            seqKmerCount++;
          }
          if (seqKmerCount >= kmersArraySize) {
            kmersArraySize = seq.getMaxLen();
            kmers = (SequencePosition *)realloc(
                kmers, (par.pickNbest * (kmersArraySize + 1) + 1) *
                           sizeof(SequencePosition));
          }
        }
        ...
        size_t kmerConsidered =
            std::min(static_cast<size_t>(par.kmersPerSequence - 1 +
                                         (kmersPerSequenceScale * seq.L)),
                     seqKmerCount);
        ...
        if (seqKmerCount > 0) {
          ...
          for (hierarchicaThreshold = 0;
               hierarchicaThreshold < 128 && kmerInBins < kmerConsidered;
               hierarchicaThreshold++) {
            kmerInBins += hierarchicalScoreDist[hierarchicaThreshold];
          }
          hierarchicaThreshold -= (hierarchicaThreshold > 0) ? 1 : 0;
          kmerInBins -= hierarchicalScoreDist[hierarchicaThreshold];
          for (threshold = hierarchicaThreshold * 512;
               threshold <= USHRT_MAX && kmerInBins < kmerConsidered;
               threshold++) {
        ...
        if (static_cast<unsigned short>(seqHash) >= hashStartRange &&
            static_cast<unsigned short>(seqHash) <= hashEndRange) {
          ...
          if (hashDistribution != NULL) {
            __sync_fetch_and_add(
                &hashDistribution[static_cast<unsigned short>(seqHash)], 1);
          }
          bufferPos++;
        ...
        if (par.ignoreMultiKmer) {
          if (TYPE == Parameters::DBTYPE_NUCLEOTIDES) {
            SORT_SERIAL(kmers, kmers + seqKmerCount,
                        SequencePosition::compareByScoreReverse);
          } else {
            SORT_SERIAL(kmers, kmers + seqKmerCount,
                        SequencePosition::compareByScore);
          }
        }
        size_t selectedKmer = 0;
        for (size_t kmerIdx = 0;
             kmerIdx < seqKmerCount && selectedKmer < kmerConsidered;
             kmerIdx++) {
          ...
          if ((kmers + kmerIdx)->score < threshold) {
            // this if is needed to avoid extracting too much elements in the
            // last bin
            if ((kmers + kmerIdx)->score == (threshold - 1) &&
            ...
            //                        std::cout << seqId << "\t" << (kmers +
            //                        kmerIdx)->score << "\t" << (kmers +
            //                        kmerIdx)->pos << std::endl;

            selectedKmer++;
            if ((kmers + kmerIdx)->score >= hashStartRange &&
                (kmers + kmerIdx)->score <= hashEndRange) {
              //                            {
              //                                size_t tmpKmerIdx= (kmers +
              //                                kmerIdx)->kmer;
              //                                tmpKmerIdx=BIT_CLEAR(tmpKmerIdx,
              //                                63); std::cout << seqId << "\t"
              //                                << (kmers + kmerIdx)->score <<
              //                                "\t" << tmpKmerIdx << std::endl;
              //                            }
              ...
              if (hashDistribution != NULL) {
                __sync_fetch_and_add(
                    &hashDistribution[(kmers + kmerIdx)->score], 1);
              }
...
template <typename T>
KmerPosition<T> *doComputation(biosnake_output* out, size_t totalKmers, size_t hashStartRange,
  ...
  if (Parameters::isEqualDbtype(seqDbr.getDbtype(),
    ...
    std::pair<size_t, size_t> ret =
        fillKmerPositionArray<Parameters::DBTYPE_NUCLEOTIDES, T>(
  ...
  if (Parameters::isEqualDbtype(seqDbr.getDbtype(),
    ...
    SORT_PARALLEL(hashSeqPair, hashSeqPair + elementsToSort,
                  KmerPosition<T>::compareRepSequenceAndIdAndPosReverse);
  } else {
    SORT_PARALLEL(hashSeqPair, hashSeqPair + elementsToSort,
  ...
  if (Parameters::isEqualDbtype(seqDbr.getDbtype(),
                                Parameters::DBTYPE_NUCLEOTIDES)) {
    writePos = assignGroup<Parameters::DBTYPE_NUCLEOTIDES, T>(
        hashSeqPair, totalKmers, par.includeOnlyExtendable, par.covMode,
        par.covThr);
  } else {
    writePos = assignGroup<Parameters::DBTYPE_AMINO_ACIDS, T>(
        hashSeqPair, totalKmers, par.includeOnlyExtendable, par.covMode,
        par.covThr);
  }
  ...
  if (Parameters::isEqualDbtype(seqDbr.getDbtype(),
    ...
    SORT_PARALLEL(hashSeqPair, hashSeqPair + writePos,
                  KmerPosition<T>::compareRepSequenceAndIdAndDiagReverse);
  } else {
    SORT_PARALLEL(hashSeqPair, hashSeqPair + writePos,
...
template <int TYPE, typename T>
size_t assignGroup(KmerPosition<T> *hashSeqPair, size_t splitKmerCount,
    ...
    if (prevHash != currKmer) {
      for (size_t i = prevHashStart; i < elementIdx; i++) {
        ...
        size_t rId = (kmer != SIZE_T_MAX)
                         ? ((prevSetSize == 1) ? SIZE_T_MAX : repSeqId)
                         : SIZE_T_MAX;
        ...
        if (rId != SIZE_T_MAX) {
          ...
          if (TYPE == Parameters::DBTYPE_NUCLEOTIDES) {
            ...
            //  10 Same here, we can revert query to match the not inverted
            //  target 11 Both are reverted so no problem! So we need just 1 bit
            //  of information to encode all four states
            bool targetIsReverse =
                (BIT_CHECK(hashSeqPair[i].kmer, 63) == false);
            ...
            if (repIsReverse == true && targetIsReverse == false) {
              ...
              // instead of reverting the target, we revert the query and offset
              // the the query/target position
          ...
          //                    std::cout << diagonal << "\t" << repSeq_i_pos <<
          //                    "\t" << hashSeqPair[i].pos << std::endl;

          bool canBeExtended =
              diagonal < 0 || (diagonal > (queryLen - hashSeqPair[i].seqLen));
          bool canBecovered =
              Util::canBeCovered(covThr, covMode, static_cast<float>(queryLen),
        ...
        hashSeqPair[i].kmer =
            (i != writePos - 1) ? SIZE_T_MAX : hashSeqPair[i].kmer;
      }
      prevSetSize = 0;
...
template size_t assignGroup<0, short>(KmerPosition<short> *kmers,
                                      size_t splitKmerCount,
                                      bool includeOnlyExtendable, int covMode,
                                      float covThr);
template size_t assignGroup<0, int>(KmerPosition<int> *kmers,
                                    size_t splitKmerCount,
                                    bool includeOnlyExtendable, int covMode,
                                    float covThr);
template size_t assignGroup<1, short>(KmerPosition<short> *kmers,
                                      size_t splitKmerCount,
                                      bool includeOnlyExtendable, int covMode,
                                      float covThr);
template size_t assignGroup<1, int>(KmerPosition<int> *kmers,
                                    size_t splitKmerCount,
                                    bool includeOnlyExtendable, int covMode,
                                    float covThr);
...
size_t computeKmerCount(DBReader<unsigned int> &reader, size_t KMER_SIZE,
                        size_t chooseTopKmer, float chooseTopKmerScale) {
  size_t totalKmers = 0;
  for (size_t id = 0; id < reader.getSize(); id++) {
    int seqLen = static_cast<int>(reader.getSeqLen(id));
    // we need one for the sequence hash
    int kmerAdjustedSeqLen =
        std::max(1, seqLen - static_cast<int>(KMER_SIZE) + 2);
    totalKmers += std::min(
        kmerAdjustedSeqLen,
        static_cast<int>(chooseTopKmer + (chooseTopKmerScale * seqLen)));
  }
  return totalKmers;
}
...
template <typename T>
int kmermatcherInner(biosnake_output* out, Parameters &par, DBReader<unsigned int> &seqDbr) {
  int querySeqType = seqDbr.getDbtype();
  BaseMatrix *subMat;
  if (Parameters::isEqualDbtype(querySeqType, Parameters::DBTYPE_NUCLEOTIDES)) {
    ...
    if (par.alphabetSize.aminoacids == 21) {
      ...
      SubstitutionMatrix sMat(out, par.scoringMatrixFile.aminoacids, 8.0, -0.2f);
  ...
  size_t fromSplit = 0;
  size_t splitCount = 1;
  mpiRank = BiosnakeMPI::rank;
  // if split size is great than nodes than we have to
  // distribute all splits equally over all nodes
  unsigned int *splitCntPerProc = new unsigned int[BiosnakeMPI::numProc];
  memset(splitCntPerProc, 0, sizeof(unsigned int) * BiosnakeMPI::numProc);
  for (size_t i = 0; i < splits; i++) {
    splitCntPerProc[i % BiosnakeMPI::numProc] += 1;
  }
  for (int i = 0; i < BiosnakeMPI::rank; i++) {
    fromSplit += splitCntPerProc[i];
  }
  splitCount = splitCntPerProc[BiosnakeMPI::rank];
  ...
  for (size_t split = fromSplit; split < fromSplit + splitCount; split++) {
    ...
    hashSeqPair = doComputation<T>(out, totalKmers, hashRanges[split].first,
                                   hashRanges[split].second, splitFileName,
                                   seqDbr, par, subMat);
  }
  ...
  for (size_t split = 0; split < hashRanges.size(); split++) {
    ...
    if (FileUtil::fileExists(out, splitFileNameDone.c_str()) == false) {
      hashSeqPair = doComputation<T>(
          out, totalKmersPerSplit, hashRanges[split].first, hashRanges[split].second,
  ...
  if (mpiRank == 0) {
    ...
    // write result
    DBWriter dbw(out, par.db2.c_str(), par.db2Index.c_str(), 1, par.compressed,
                 (Parameters::isEqualDbtype(seqDbr.getDbtype(),
                                            Parameters::DBTYPE_NUCLEOTIDES))
    ...
    if (splits > 1) {
      ...
      if (Parameters::isEqualDbtype(seqDbr.getDbtype(),
                                    Parameters::DBTYPE_NUCLEOTIDES)) {
        writeKmerMatcherResult<Parameters::DBTYPE_NUCLEOTIDES>(
            dbw, hashSeqPair, totalKmersPerSplit, repSequence, 1);
      } else {
        writeKmerMatcherResult<Parameters::DBTYPE_AMINO_ACIDS>(
            dbw, hashSeqPair, totalKmersPerSplit, repSequence, 1);
      }
    }
...
int kmermatcher(biosnake_output *out, Parameters &par) {
  ...
  //    std::vector<BiosnakeParameter *> *params = command.params;
  ...
  if (seqDbr.getMaxSeqLen() < SHRT_MAX) {
    kmermatcherInner<short>(out, par, seqDbr);
  } else {
    kmermatcherInner<int>(out, par, seqDbr);
  }
...
template <int TYPE, typename T>
void writeKmerMatcherResult(DBWriter &dbw, KmerPosition<T> *hashSeqPair,
                            size_t totalKmers, std::vector<char> &repSequence,
                            size_t threads) {
...
#pragma omp parallel for schedule(dynamic, 1) num_threads(threads)
  for (size_t thread = 0; thread < threads; thread++) {
    ...
    for (kmerPos = threadOffsets[thread];
         kmerPos < threadOffsets[thread + 1] &&
      ...
      if (repSeqId != currKmer) {
        if (writeSets > 0) {
          ...
          dbw.writeData(prefResultsOutString.c_str(),
                        prefResultsOutString.length(), repSeqId, thread);
        } else {
      ...
      while (lastTargetId != targetId &&
        ...
        if (diagonalCnt >= maxDiagonal) {
          ...
          if (TYPE == Parameters::DBTYPE_NUCLEOTIDES) {
            bestReverMask =
                BIT_CHECK(hashSeqPair[kmerPos + kmerOffset].kmer, 63) == false;
          }
        }
...
template <int TYPE, typename T>
size_t queueNextEntry(KmerPositionQueue &queue, int file, size_t offsetPos,
                      T *entries, size_t entrySize) {
  ...
  while (entries[offsetPos + pos].seqId != UINT_MAX) {
    if (TYPE == Parameters::DBTYPE_NUCLEOTIDES) {
      queue.push(FileKmerPosition(repSeqId, entries[offsetPos + pos].seqId,
                                  entries[offsetPos + pos].diagonal,
                                  entries[offsetPos + pos].score,
                                  entries[offsetPos + pos].getRev(), file));
    } else {
      queue.push(FileKmerPosition(repSeqId, entries[offsetPos + pos].seqId,
                                  entries[offsetPos + pos].diagonal,
                                  entries[offsetPos + pos].score, file));
    }
    pos++;
  }
...
template <int TYPE, typename T>
void mergeKmerFilesAndOutput(biosnake_output* out, DBWriter &dbw, std::vector<std::string> tmpFiles,
  ...
  for (int file = 0; file < fileCnt; file++) {
    offsetPos[file] = queueNextEntry<TYPE, T>(queue, file, 0, entries[file],
                                              entrySizes[file]);
  }
  std::string prefResultsOutString;
  prefResultsOutString.reserve(100000000);
  ...
  while (queue.empty() == false) {
    ...
    if (res.id == UINT_MAX) {
      offsetPos[res.file] =
          queueNextEntry<TYPE, T>(queue, res.file, offsetPos[res.file],
    ...
    do {
      prevHitId = res.id;
      diagonalScore = (diagonalScore == 0 || prevDiagonal != res.pos)
                          ? res.score
    ...
    } while (hitId == prevHitId && res.repSeq == currRepSeq &&
             hitId != UINT_MAX);
...
void writeKmersToDisk(biosnake_output* out, std::string tmpFile,
                      KmerPosition<seqLenType> *hashSeqPair,
                      size_t totalKmers) {
  size_t repSeqId = SIZE_T_MAX;
  size_t lastTargetId = SIZE_T_MAX;
  ...
  for (size_t kmerPos = 0;
       kmerPos < totalKmers && hashSeqPair[kmerPos].kmer != SIZE_T_MAX;
       kmerPos++) {
    size_t currKmer = hashSeqPair[kmerPos].kmer;
    if (TYPE == Parameters::DBTYPE_NUCLEOTIDES) {
      currKmer = BIT_CLEAR(currKmer, 63);
    ...
    // find diagonal score
    do {
      diagonalScore += (diagonalScore == 0 ||
                        (lastTargetId == targetId && lastDiagonal == diagonal));
      ...
      kmerPos++;
             ...
             hashSeqPair[kmerPos].pos == diagonal && kmerPos < totalKmers &&
             hashSeqPair[kmerPos].kmer != SIZE_T_MAX);
    kmerPos--;
...
void setKmerLengthAndAlphabet(Parameters &parameters, size_t aaDbSize,
                              int seqTyp) {
  if (Parameters::isEqualDbtype(seqTyp, Parameters::DBTYPE_NUCLEOTIDES)) {
    if (parameters.kmerSize == 0) {
      parameters.kmerSize = std::max(
          17, static_cast<int>(log(static_cast<float>(aaDbSize)) / log(4)));
    ...
    if (parameters.kmerSize == 0) {
      if ((parameters.seqIdThr + 0.001) >= 0.99) {
        ...
        parameters.kmerSize = std::max(
            10, static_cast<int>(log(static_cast<float>(aaDbSize)) / log(8.7)));
        parameters.alphabetSize.aminoacids = 13;
      }
    }
    if (parameters.kmersPerSequence == 0) {
...
template std::pair<size_t, size_t> fillKmerPositionArray<0, short>(
    biosnake_output* out,
    KmerPosition<short> *kmerArray, size_t kmerArraySize,
    DBReader<unsigned int> &seqDbr, Parameters &par, BaseMatrix *subMat,
    bool hashWholeSequence, size_t hashStartRange, size_t hashEndRange,
    size_t *hashDistribution);
template std::pair<size_t, size_t> fillKmerPositionArray<1, short>(
    biosnake_output* out,
    KmerPosition<short> *kmerArray, size_t kmerArraySize,
    DBReader<unsigned int> &seqDbr, Parameters &par, BaseMatrix *subMat,
    bool hashWholeSequence, size_t hashStartRange, size_t hashEndRange,
    size_t *hashDistribution);
template std::pair<size_t, size_t> fillKmerPositionArray<2, short>(
    biosnake_output* out,
    KmerPosition<short> *kmerArray, size_t kmerArraySize,
    DBReader<unsigned int> &seqDbr, Parameters &par, BaseMatrix *subMat,
    bool hashWholeSequence, size_t hashStartRange, size_t hashEndRange,
    size_t *hashDistribution);
template std::pair<size_t, size_t> fillKmerPositionArray<0, int>(
    biosnake_output* out,
    KmerPosition<int> *kmerArray, size_t kmerArraySize,
    DBReader<unsigned int> &seqDbr, Parameters &par, BaseMatrix *subMat,
    bool hashWholeSequence, size_t hashStartRange, size_t hashEndRange,
    size_t *hashDistribution);
template std::pair<size_t, size_t> fillKmerPositionArray<1, int>(
    biosnake_output* out,
    KmerPosition<int> *kmerArray, size_t kmerArraySize,
    DBReader<unsigned int> &seqDbr, Parameters &par, BaseMatrix *subMat,
    bool hashWholeSequence, size_t hashStartRange, size_t hashEndRange,
    size_t *hashDistribution);
template std::pair<size_t, size_t> fillKmerPositionArray<2, int>(
    biosnake_output* out,
    KmerPosition<int> *kmerArray, size_t kmerArraySize,
    DBReader<unsigned int> &seqDbr, Parameters &par, BaseMatrix *subMat,
    bool hashWholeSequence, size_t hashStartRange, size_t hashEndRange,
...
template KmerPosition<short> *initKmerPositionMemory(biosnake_output *out, size_t size);
template KmerPosition<int> *initKmerPositionMemory(biosnake_output *out,size_t size);
...
template std::vector<std::pair<size_t, size_t>> setupKmerSplits<short>(
    biosnake_output* out,
    Parameters &par, BaseMatrix *subMat, DBReader<unsigned int> &seqDbr,
    size_t totalKmers, size_t splits);
template std::vector<std::pair<size_t, size_t>> setupKmerSplits<int>(
    biosnake_output* out, Parameters &par, BaseMatrix *subMat, DBReader<unsigned int> &seqDbr,
    size_t totalKmers, size_t splits);
