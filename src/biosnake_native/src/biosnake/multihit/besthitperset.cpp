...
class BestHitBySetFilter : public Aggregation {
 public:
  BestHitBySetFilter(biosnake_output* output, const std::string &targetDbName,
  ...
  std::string aggregateEntry(
      std::vector<std::vector<std::string>> &dataToAggregate, unsigned int,
...
int besthitperset(biosnake_output *out, Parameters &par) {
