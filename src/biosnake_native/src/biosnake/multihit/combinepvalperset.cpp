#include <biosnake/multihit/aggregation.h>
...
void precomputeLogB(const unsigned int orfCount, const double pvalThreshold,
                    double *lGammaLookup, double *logB) {
  ...
  for (int i = (orfCount - 2); i >= 0; i--) {
    int k = i + 1;
    double log_newTerm = LBinCoeff(lGammaLookup, orfCount, k) + k * logPvalThr +
                         (orfCount - k) * log1MinusPvalThr;
...
class PvalueAggregator : public Aggregation {
 public:
  PvalueAggregator(biosnake_output* output, std::string queryDbName, std::string targetDbName,
    ...
    for (size_t i = 0; i < querySizeReader->getSize(); ++i) {
      unsigned int currentCount =
          Util::fast_atoi<unsigned int>(querySizeReader->getData(i, 0));
  ...
  void prepareInput(unsigned int querySetKey, unsigned int thread_idx) {
    unsigned int orfCount = Util::fast_atoi<unsigned int>(
        querySizeReader->getDataByDBKey(querySetKey, thread_idx));
    precomputeLogB(orfCount, alpha / (orfCount + 1), lGammaLookup,
                   logBiLookup[thread_idx]);
  }

  // Get all result of a single Query Set VS a Single Target Set and return the
  // multiple-match p-value for it
  std::string aggregateEntry(
    ...
    if (aggregationMode == Parameters::AGGREGATION_MODE_MULTIHIT) {
      ...
      for (size_t i = 0; i < orfCount; ++i) {
        truncatedFisherPval +=
            exp(i * logR - lGammaLookup[i + 1] + logBiLookup[thread_idx][i]);
      }
    ...
    // 1) the minimum of all P-values(as a baseline)
      ...
      double minLogPval = 0;
      for (size_t i = 0; i < dataToAggregate.size(); ++i) {
        double currentLogPval =
            std::strtod(dataToAggregate[i][1].c_str(), NULL);
    ...
    // 3) the P-values of the (modified) truncated product method
             ...
             Parameters::AGGREGATION_MODE_TRUNCATED_PRODUCT) {
      // new theory: taking the best hit regardless of threshold and (from
      // second hit on)sum of how much it surpassed threshold
      unsigned int orfCount = Util::fast_atoi<unsigned int>(
          querySizeReader->getDataByDBKey(querySetKey, thread_idx));
      ...
      for (size_t i = 0; i < dataToAggregate.size(); ++i) {
        ...
        if (logPvalue < logPvalThreshold) {
          // sum up the part exceeding logThreshold, add a minus to make score
          // positive
...
int combinepvalperset(biosnake_output *out, Parameters &par) {
