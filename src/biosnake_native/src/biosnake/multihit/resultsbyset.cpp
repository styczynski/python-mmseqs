...
class SetSummaryAggregator : public Aggregation {
 public:
  SetSummaryAggregator(const std::string &queryDbName,
                       const std::string &targetDbName,
                       const std::string &resultDbName,
                       const std::string &outputDbName, bool shortOutput,
    ...
    querySizeReader = new DBReader<unsigned int>(
        data.c_str(), index.c_str(), threads,
    ...
    targetSourceReader = new DBReader<unsigned int>(
        data.c_str(), index.c_str(), threads,
        DBReader<unsigned int>::USE_DATA | DBReader<unsigned int>::USE_INDEX);
  ...
  std::string aggregateEntry(
    ...
    double targetGeneCount = std::strtod(
        targetSizeReader->getDataByDBKey(targetSetKey, thread_idx), NULL);
    ...
    double genomeSize = (targetSourceReader->getSeqLen(
        targetSourceReader->getId(targetSetKey)));
    ...
    if (hitsUnderThreshold > 1) {
      ...
      for (size_t i = 0; i < hitsUnderThreshold - 1; i++) {
        size_t currentInterGenePosition =
            genesPositions[i + 1].first - genesPositions[i].second;
      ...
      if (shortOutput == false) {
        ...
        buffer.append("\t");
        buffer.append(std::to_string(spreadPval(
            interGeneSpaces[interSpaceIndex], rate, hitsUnderThreshold)));
        buffer.append("\t");
...
int resultsbyset(biosnake_output *out, Parameters &par) {
  //    Parameters &par = Parameters::getInstance();
  //    par.parseParameters(argc, argv, command, true, 0, 0);

  SetSummaryAggregator aggregation(par.db1, par.db2, par.db3, par.db4,
                                   par.shortOutput, par.alpha,
