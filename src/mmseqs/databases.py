...
from .convert import GenericSequences, GenericSequencesFilePathConverter
    ...
    def dataframe(self):
        return pd.DataFrame(
            [
                ...
                for alignments in self.records
    ...
    def _compute(self):
        ...
        if self._operation == "merge":
            ...
            with self._base.settings.meta_db.open() as meta_db:
                path_in_a = os.path.join(
                    self._base.settings.seq_storage_directory,
                    self._args[0].name,
                )
                path_out = os.path.join(
                    self._base.settings.seq_storage_directory, self._name
                )
                path_in_b = os.path.join(
                    self._base.settings.seq_storage_directory,
                    self._args[1].name,
                )
                ...
                new_db = Database(
                    ...
                    input_files=[
                        *self._args[0].input_files,
                        *self._args[1].input_files,
                    ],
...
class Database(ObjectWithBaseRef):
    ...
    def name(self, name):
        ...
        with self._base.settings.meta_db.open() as meta_db:
            path_in = os.path.join(
                self._base.settings.seq_storage_directory, original_name
            )
            path_out = os.path.join(
                self._base.settings.seq_storage_directory, name
            )
            self._base._execute_cli(
                "mvdb",
                dict(db1=path_in, db2=path_out, filenames=[path_in, path_out]),
            )
            self._name = name
            meta_db.list_replace(
                "databases",
                self._base,
                self,
                lambda x: x.name == original_name,
                save_back=True,
            )
    ...
    def records(self):
        ...
        with open(f"{db_path}", "r") as db_file, open(
            f"{db_path}_h", "r"
        ) as h_file:
            for seq_contents, header in zip(db_file, h_file):
                if len(seq_contents) > 1:
                    yield SeqRecord(
                        Seq(seq_contents[:-1].lstrip("\x00")),
                        id=header[:-1].lstrip("\x00"),
                    )

    def to_fasta(self, output_path):
        # convert2fasta
        db_path = os.path.join(
            self._base.settings.seq_storage_directory, self.name
        )
        self._base._execute_cli(
            "convert2fasta",
            dict(
    ...
    def copy(self, name):
        with self._base.settings.meta_db.open() as meta_db:
            src_dbs, _ = meta_db.list_filter(
                "databases", self._base, lambda db: db.name != self.name
            )
            src_db = src_dbs[0]
            db_path = os.path.join(
                self._base.settings.seq_storage_directory, src_db.name
            )
            db_copy_path = os.path.join(
                self._base.settings.seq_storage_directory, name
            )
            self._base._execute_cli(
                "cpdb", dict(db1=db_path, db2=db_copy_path)
            )
            new_db = Database(
                name=name,
    ...
    def remove(self):
        with self._base.settings.meta_db.open() as meta_db:
            removed_dbs, left_dbs = meta_db.list_filter(
                "databases", self._base, lambda db: db.name != self._name
            )
            for db in removed_dbs:
                db_path = os.path.join(
                    self._base.settings.seq_storage_directory, db.name
                )
    ...
    def search(
        ...
        with GenericSequencesFilePathConverter(
            search_input
        ) as input_file_paths:
            tmp_dir = "tmp"
            results_path = os.path.join(
                self._base.settings.seq_results_directory,
                f"{self._name}.query_results.m8",
            )
            seq_db_path = os.path.join(
                self._base.settings.seq_storage_directory, self._name
            )
            out = self._base._execute_cli(
                "easy-search",
                dict(
                    filenames=[
                        *input_file_paths,
            ...
            self._execute_cli(
                ...
                dict(
                    filenames=[
                        *input_files,
                        os.path.join(
                            self.settings.seq_storage_directory, name
                        ),
                    ],
