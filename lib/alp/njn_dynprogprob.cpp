...
void DynProgProb::init(
    size_t arrayCapacity_)  // range for d_array_p [0,1][0...arrayCapacity_ - 1]
{
  if (arrayCapacity_ > 0) {
...
void DynProgProb::clear(
    long int valueBegin_,   // lower limit for long int values in the array (an
                            // offset)
...
void DynProgProb::clear(   // initializes the "probabilities" with non-negative
                           // weights
    long int valueLower_,  // lower long int value corresponding to the
                           // "probability" array
    long int valueUpper_,  // one beyond present upper long int value
                           // corresponding to the "probability" array
    const double *prob_)   // "probabilities" prob [valueLower_, valueUpper_)
                           // corresponding to the long ints
{
  assert(
      (!prob_ && valueLower_ <= 0 && 0 <= valueUpper_) ||
      /*sls added "("*/ (prob_ && valueLower_ < valueUpper_) /*sls added ")"*/);

  if (prob_) {
    for (size_t i = 0; i < static_cast<size_t>(valueUpper_ - valueLower_);
         i++) {
      assert(0.0 <= prob_[i]);
    }
...
void DynProgProb::copy(
    ...
    long int
        valueUpper_,  // one beyond present upper long int value in the array
                               ...
                               // [0...dimStateProb - 1]
{
  ...
  for (size_t i = 0; i < 2; i++) {
    if (getArrayCapacity() > 0)
      MemUtil::memCpy(d_array_p[i], array_[i],
                      sizeof(double) * getArrayCapacity());
  }
                               ...
                               // [0...dimStateProb - 1]
{
  ...
  if (getDimInputProb() > 0)
    MemUtil::memCpy(d_inputProb_p, inputProb_,
                    sizeof(double) * getDimInputProb());
}
  ...
  for (i = getValueLower(); i < getValueUpper(); i++) {
    ...
    for (j = 0; j < getDimInputProb(); j++) {
      ...
      while (value < getValueBegin() || getValueEnd() <= value) {
        ...
        if (value < getValueBegin())
          valueBegin -= (ARRAY_FAC - 1) * getArrayCapacity();
...
// assert (offSet < getArrayCapacity ());
{
  ...
  for (size_t i = 0; i < 2; i++) {
    ...
    MemUtil::memCpy(d_array_p[i] + offSet, array,
                    sizeof(double) * (getArrayCapacity() - offSet));
  }
