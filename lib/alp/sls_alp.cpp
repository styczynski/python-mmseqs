    ...
    for (i = 0; i <= d_nalp; i++) {
      if (i <= d_alp_states->d_dim) {
        if (d_alp_states->d_elem[i]) {
          for (i = 0; i <= d_nalp; i++) {
            release_and_calculate_memory(
                d_alp_states->d_elem[i]->d_HS_i_const_next,
                d_alp_states->d_elem[i]->d_H_matr_len);

            release_and_calculate_memory(
                d_alp_states->d_elem[i]->d_HI_i_const_next,
                d_alp_states->d_elem[i]->d_H_matr_len);

            release_and_calculate_memory(
                d_alp_states->d_elem[i]->d_HD_i_const_next,
                d_alp_states->d_elem[i]->d_H_matr_len);

            release_and_calculate_memory(
                d_alp_states->d_elem[i]->d_H_i_const_next,
                d_alp_states->d_elem[i]->d_H_matr_len);

            release_and_calculate_memory(
                d_alp_states->d_elem[i]->d_HS_j_const_next,
                d_alp_states->d_elem[i]->d_H_matr_len);

            release_and_calculate_memory(
                d_alp_states->d_elem[i]->d_HI_j_const_next,
                d_alp_states->d_elem[i]->d_H_matr_len);

            release_and_calculate_memory(
                d_alp_states->d_elem[i]->d_HD_j_const_next,
                d_alp_states->d_elem[i]->d_H_matr_len);

            release_and_calculate_memory(
                d_alp_states->d_elem[i]->d_H_j_const_next,
                d_alp_states->d_elem[i]->d_H_matr_len);

            release_and_calculate_memory(
                d_alp_states->d_elem[i]->d_cells_counts);
...
long int alp::random_AA1() {
  return d_alp_data->random_long(d_alp_data->ran2(), d_alp_data->d_number_of_AA,
...
bool alp::one_step_of_importance_sampling_without_weight_calculation(
    long int d_dim1_, long int d_dim2_) {
  ...
  if (length1_ == 0 && length2_ == 0) {
    state_ = alp_data::random_long(las_object_->ran2(), 3, d_is_->d_for_S,
  ...
  if (state_ == 'D') {
    ...
    state_ = alp_data::random_long(las_object_->ran2(), 3, d_is_->d_for_D,
  ...
  if (state_ == 'I') {
    ...
    state_ = alp_data::random_long(las_object_->ran2(), 2, d_is_->d_for_I,
  ...
  if (state_ == 'S') {
    ...
    q_elem pair = alp_data::random_long(
        ...
        d_is_->d_elements_values, d_is_->d_elements);
...
void alp::increment_sequences() {
  ...
  try {
    ...
    d_alp_data->d_memory_size_in_MB +=
        (double)(sizeof(long int) * d_a_step * 2) / mb_bytes;
...
void alp::increment_W_matrix() {
  ...
  try {
    ...
    delete[] d_WS_i_const_pred;
    d_WS_i_const_pred = NULL;
    delete[] d_WI_i_const_pred;
    d_WI_i_const_pred = NULL;
    delete[] d_WD_i_const_pred;
    d_WD_i_const_pred = NULL;

    delete[] d_WS_i_const_next;
    d_WS_i_const_next = NULL;
    delete[] d_WI_i_const_next;
    d_WI_i_const_next = NULL;
    delete[] d_WD_i_const_next;
    d_WD_i_const_next = NULL;

    delete[] d_WS_j_const_pred;
    d_WS_j_const_pred = NULL;
    delete[] d_WI_j_const_pred;
    d_WI_j_const_pred = NULL;
    delete[] d_WD_j_const_pred;
    d_WD_j_const_pred = NULL;

    delete[] d_WS_j_const_next;
    d_WS_j_const_next = NULL;
    delete[] d_WI_j_const_next;
    d_WI_j_const_next = NULL;
    delete[] d_WD_j_const_next;
    d_WD_j_const_next = NULL;

    d_alp_data->d_memory_size_in_MB +=
        (double)(sizeof(double) * d_a_step * 12) / mb_bytes;

    d_WS_i_const_pred = d_WS_i_const_pred_new;
    d_WS_i_const_pred_new = NULL;
    d_WI_i_const_pred = d_WI_i_const_pred_new;
    d_WI_i_const_pred_new = NULL;
    d_WD_i_const_pred = d_WD_i_const_pred_new;
    d_WD_i_const_pred_new = NULL;

    d_WS_i_const_next = d_WS_i_const_next_new;
    d_WS_i_const_next_new = NULL;
    d_WI_i_const_next = d_WI_i_const_next_new;
    d_WI_i_const_next_new = NULL;
    d_WD_i_const_next = d_WD_i_const_next_new;
    d_WD_i_const_next_new = NULL;

    d_WS_j_const_pred = d_WS_j_const_pred_new;
    d_WS_j_const_pred_new = NULL;
    d_WI_j_const_pred = d_WI_j_const_pred_new;
    d_WI_j_const_pred_new = NULL;
    d_WD_j_const_pred = d_WD_j_const_pred_new;
    d_WD_j_const_pred_new = NULL;

    d_WS_j_const_next = d_WS_j_const_next_new;
    d_WS_j_const_next_new = NULL;
    d_WI_j_const_next = d_WI_j_const_next_new;
    d_WI_j_const_next_new = NULL;
    d_WD_j_const_next = d_WD_j_const_next_new;
    d_WD_j_const_next_new = NULL;

  } catch (...) {
    delete[] d_WS_i_const_pred_new;
    d_WS_i_const_pred_new = NULL;
    delete[] d_WI_i_const_pred_new;
    d_WI_i_const_pred_new = NULL;
    delete[] d_WD_i_const_pred_new;
    d_WD_i_const_pred_new = NULL;

    delete[] d_WS_i_const_next_new;
    d_WS_i_const_next_new = NULL;
    delete[] d_WI_i_const_next_new;
    d_WI_i_const_next_new = NULL;
    delete[] d_WD_i_const_next_new;
    d_WD_i_const_next_new = NULL;

    delete[] d_WS_j_const_pred_new;
    d_WS_j_const_pred_new = NULL;
    delete[] d_WI_j_const_pred_new;
    d_WI_j_const_pred_new = NULL;
    delete[] d_WD_j_const_pred_new;
    d_WD_j_const_pred_new = NULL;

    delete[] d_WS_j_const_next_new;
    d_WS_j_const_next_new = NULL;
    delete[] d_WI_j_const_next_new;
    d_WI_j_const_next_new = NULL;
    delete[] d_WD_j_const_next_new;
    d_WD_j_const_next_new = NULL;
    throw;
  };
}
...
void alp::increment_H_matrix() {
  ...
  try {
    ...
    delete[] d_HS_i_const_pred;
    d_HS_i_const_pred = NULL;
    delete[] d_HI_i_const_pred;
    d_HI_i_const_pred = NULL;
    delete[] d_HD_i_const_pred;
    d_HD_i_const_pred = NULL;
    delete[] d_H_i_const_pred;
    d_H_i_const_pred = NULL;

    delete[] d_HS_i_const_next;
    d_HS_i_const_next = NULL;
    delete[] d_HI_i_const_next;
    d_HI_i_const_next = NULL;
    delete[] d_HD_i_const_next;
    d_HD_i_const_next = NULL;
    delete[] d_H_i_const_next;
    d_H_i_const_next = NULL;

    delete[] d_HS_j_const_pred;
    d_HS_j_const_pred = NULL;
    delete[] d_HI_j_const_pred;
    d_HI_j_const_pred = NULL;
    delete[] d_HD_j_const_pred;
    d_HD_j_const_pred = NULL;
    delete[] d_H_j_const_pred;
    d_H_j_const_pred = NULL;

    delete[] d_HS_j_const_next;
    d_HS_j_const_next = NULL;
    delete[] d_HI_j_const_next;
    d_HI_j_const_next = NULL;
    delete[] d_HD_j_const_next;
    d_HD_j_const_next = NULL;
    delete[] d_H_j_const_next;
    d_H_j_const_next = NULL;
    ...
    d_alp_data->d_memory_size_in_MB +=
        (double)(sizeof(long int) * d_a_step * 17) / mb_bytes;

    d_HS_i_const_pred = d_HS_i_const_pred_new;
    d_HS_i_const_pred_new = NULL;
    d_HI_i_const_pred = d_HI_i_const_pred_new;
    d_HI_i_const_pred_new = NULL;
    d_HD_i_const_pred = d_HD_i_const_pred_new;
    d_HD_i_const_pred_new = NULL;
    d_H_i_const_pred = d_H_i_const_pred_new;
    d_H_i_const_pred_new = NULL;

    d_HS_i_const_next = d_HS_i_const_next_new;
    d_HS_i_const_next_new = NULL;
    d_HI_i_const_next = d_HI_i_const_next_new;
    d_HI_i_const_next_new = NULL;
    d_HD_i_const_next = d_HD_i_const_next_new;
    d_HD_i_const_next_new = NULL;
    d_H_i_const_next = d_H_i_const_next_new;
    d_H_i_const_next_new = NULL;

    d_HS_j_const_pred = d_HS_j_const_pred_new;
    d_HS_j_const_pred_new = NULL;
    d_HI_j_const_pred = d_HI_j_const_pred_new;
    d_HI_j_const_pred_new = NULL;
    d_HD_j_const_pred = d_HD_j_const_pred_new;
    d_HD_j_const_pred_new = NULL;
    d_H_j_const_pred = d_H_j_const_pred_new;
    d_H_j_const_pred_new = NULL;

    d_HS_j_const_next = d_HS_j_const_next_new;
    d_HS_j_const_next_new = NULL;
    d_HI_j_const_next = d_HI_j_const_next_new;
    d_HI_j_const_next_new = NULL;
    d_HD_j_const_next = d_HD_j_const_next_new;
    d_HD_j_const_next_new = NULL;
    d_H_j_const_next = d_H_j_const_next_new;
    d_H_j_const_next_new = NULL;

    d_H_edge_max = d_H_edge_max_new;
    d_H_edge_max_new = NULL;

  } catch (...) {
    delete[] d_HS_i_const_pred_new;
    d_HS_i_const_pred_new = NULL;
    delete[] d_HI_i_const_pred_new;
    d_HI_i_const_pred_new = NULL;
    delete[] d_HD_i_const_pred_new;
    d_HD_i_const_pred_new = NULL;
    delete[] d_H_i_const_pred_new;
    d_H_i_const_pred_new = NULL;

    delete[] d_HS_i_const_next_new;
    d_HS_i_const_next_new = NULL;
    delete[] d_HI_i_const_next_new;
    d_HI_i_const_next_new = NULL;
    delete[] d_HD_i_const_next_new;
    d_HD_i_const_next_new = NULL;
    delete[] d_H_i_const_next_new;
    d_H_i_const_next_new = NULL;

    delete[] d_HS_j_const_pred_new;
    d_HS_j_const_pred_new = NULL;
    delete[] d_HI_j_const_pred_new;
    d_HI_j_const_pred_new = NULL;
    delete[] d_HD_j_const_pred_new;
    d_HD_j_const_pred_new = NULL;
    delete[] d_H_j_const_pred_new;
    d_H_j_const_pred_new = NULL;

    delete[] d_HS_j_const_next_new;
    d_HS_j_const_next_new = NULL;
    delete[] d_HI_j_const_next_new;
    d_HI_j_const_next_new = NULL;
    delete[] d_HD_j_const_next_new;
    d_HD_j_const_next_new = NULL;
    delete[] d_H_j_const_next_new;
    d_H_j_const_next_new = NULL;

    delete[] d_H_edge_max_new;
    d_H_edge_max_new = NULL;
    throw;
  };
}
...
// assumes that letters are defined for d_W_matr_len
{
  ...
  for (i = d_W_matr_len_2; i >= 1; i--) {
    d_WS_i_const_next[i] =
        d_is_tmp->d_exp_s[d_seqi[d_W_matr_len_1]][d_seqj[d_W_matr_len_2 - i]] *
        (d_is_tmp->d_eta * d_WS_i_const_pred[i] +
         d_is_tmp->d_mu_SI * d_WI_i_const_pred[i] +
         d_is_tmp->d_mu_SD * d_WD_i_const_pred[i]);
    d_WI_i_const_next[i] = d_is_tmp->d_mu_IS * d_WS_i_const_next[i + 1] +
                           d_is_tmp->d_nu * d_WI_i_const_next[i + 1] +
                           d_is_tmp->d_mu_ID * d_WD_i_const_next[i + 1];
    d_WD_i_const_next[i] = d_is_tmp->d_mu_DS * d_WS_i_const_pred[i - 1] +
                           d_is_tmp->d_nu * d_WD_i_const_pred[i - 1];

    d_WS_j_const_next[i] =
        d_is_tmp->d_exp_s[d_seqi[d_W_matr_len_2 - i]][d_seqj[d_W_matr_len_1]] *
        (d_is_tmp->d_eta * d_WS_j_const_pred[i] +
         d_is_tmp->d_mu_SI * d_WI_j_const_pred[i] +
         d_is_tmp->d_mu_SD * d_WD_j_const_pred[i]);
    d_WI_j_const_next[i] = d_is_tmp->d_mu_IS * d_WS_j_const_pred[i - 1] +
                           d_is_tmp->d_nu * d_WI_j_const_pred[i - 1] +
                           d_is_tmp->d_mu_ID * d_WD_j_const_pred[i - 1];
    d_WD_j_const_next[i] = d_is_tmp->d_mu_DS * d_WS_j_const_next[i + 1] +
                           d_is_tmp->d_nu * d_WD_j_const_next[i + 1];
  };
  ...
  if (d_W_matr_len > 1) {
    ...
    d_WI_i_const_next[i] = d_is_tmp->d_mu_IS * d_WS_i_const_next[i + 1] +
                           ...
                           d_is_tmp->d_mu_ID * d_WD_i_const_next[i + 1];
    d_WD_i_const_next[i] =
        d_is_tmp->d_mu_DS * d_WS_ij_pred + d_is_tmp->d_nu * d_WD_ij_pred;
    ...
    d_WS_j_const_next[i] =
        ...
        (d_is_tmp->d_eta * d_WS_j_const_pred[i] +
         d_is_tmp->d_mu_SI * d_WI_j_const_pred[i] +
         d_is_tmp->d_mu_SD * d_WD_j_const_pred[i]);
    d_WI_j_const_next[i] = d_is_tmp->d_mu_IS * d_WS_ij_pred +
                           d_is_tmp->d_nu * d_WI_ij_pred +
                           d_is_tmp->d_mu_ID * d_WD_ij_pred;
    d_WD_j_const_next[i] = d_is_tmp->d_mu_DS * d_WS_j_const_next[i + 1] +
                           d_is_tmp->d_nu * d_WD_j_const_next[i + 1];
  };

  d_WS_ij_next =
      d_is_tmp->d_exp_s[d_seqi[d_W_matr_len_1]][d_seqj[d_W_matr_len_1]] *
      (d_is_tmp->d_eta * d_WS_ij_pred + d_is_tmp->d_mu_SI * d_WI_ij_pred +
       d_is_tmp->d_mu_SD * d_WD_ij_pred);
  d_WI_ij_next = d_is_tmp->d_mu_IS * d_WS_i_const_next[0] +
                 d_is_tmp->d_nu * d_WI_i_const_next[0] +
                 d_is_tmp->d_mu_ID * d_WD_i_const_next[0];
  d_WD_ij_next = d_is_tmp->d_mu_DS * d_WS_j_const_next[0] +
                 d_is_tmp->d_nu * d_WD_j_const_next[0];
}
...
// assumes that letters are defined for d_H_matr_len
{
  ...
  long int gap_tmp1 =
      -d_alp_data->d_open1 - d_H_matr_len_1 * d_alp_data->d_epen1;
  long int gap_tmp2 =
      -d_alp_data->d_open2 - d_H_matr_len_1 * d_alp_data->d_epen2;
  ...
  for (i = d_H_matr_len_2; i >= 1; i--) {
    d_HS_i_const_next[i] =
        d_alp_data
            ->d_smatr[d_seqi[d_H_matr_len_1]][d_seqj[d_H_matr_len_2 - i]] +
        d_H_i_const_pred[i];
    d_HI_i_const_next[i] =
        alp_data::Tmax(d_HS_i_const_next[i + 1] - d_alp_data->d_open2,
                       d_HI_i_const_next[i + 1] - d_alp_data->d_epen2);
    d_HD_i_const_next[i] =
        alp_data::Tmax(d_HS_i_const_pred[i - 1] - d_alp_data->d_open1,
                       d_HD_i_const_pred[i - 1] - d_alp_data->d_epen1);
    d_H_i_const_next[i] = alp_data::Tmax(
        d_HS_i_const_next[i], d_HI_i_const_next[i], d_HD_i_const_next[i]);

    d_HS_j_const_next[i] =
        d_alp_data
            ->d_smatr[d_seqi[d_H_matr_len_2 - i]][d_seqj[d_H_matr_len_1]] +
        d_H_j_const_pred[i];
    d_HI_j_const_next[i] =
        alp_data::Tmax(d_HS_j_const_pred[i - 1] - d_alp_data->d_open2,
                       d_HI_j_const_pred[i - 1] - d_alp_data->d_epen2);
    d_HD_j_const_next[i] =
        alp_data::Tmax(d_HS_j_const_next[i + 1] - d_alp_data->d_open1,
                       d_HD_j_const_next[i + 1] - d_alp_data->d_epen1);
    d_H_j_const_next[i] = alp_data::Tmax(
        d_HS_j_const_next[i], d_HI_j_const_next[i], d_HD_j_const_next[i]);
  };
  ...
  if (d_H_matr_len > 1) {
    ...
    d_HI_i_const_next[i] =
        alp_data::Tmax(d_HS_i_const_next[i + 1] - d_alp_data->d_open2,
                       d_HI_i_const_next[i + 1] - d_alp_data->d_epen2);
    d_HD_i_const_next[i] = alp_data::Tmax(d_HS_ij_pred - d_alp_data->d_open1,
                                          d_HD_ij_pred - d_alp_data->d_epen1);
    d_H_i_const_next[i] = alp_data::Tmax(
        d_HS_i_const_next[i], d_HI_i_const_next[i], d_HD_i_const_next[i]);
    ...
    d_HS_j_const_next[i] =
        d_alp_data
            ->d_smatr[d_seqi[d_H_matr_len_2 - i]][d_seqj[d_H_matr_len_1]] +
        d_H_j_const_pred[i];
    d_HI_j_const_next[i] = alp_data::Tmax(d_HS_ij_pred - d_alp_data->d_open2,
                                          d_HI_ij_pred - d_alp_data->d_epen2);
    d_HD_j_const_next[i] =
        alp_data::Tmax(d_HS_j_const_next[i + 1] - d_alp_data->d_open1,
                       d_HD_j_const_next[i + 1] - d_alp_data->d_epen1);
    d_H_j_const_next[i] = alp_data::Tmax(
  ...
  d_HS_ij_next =
      d_alp_data->d_smatr[d_seqi[d_H_matr_len_1]][d_seqj[d_H_matr_len_1]] +
      d_H_ij_pred;
  d_HI_ij_next = alp_data::Tmax(d_HS_i_const_next[0] - d_alp_data->d_open2,
                                d_HI_i_const_next[0] - d_alp_data->d_epen2);
  d_HD_ij_next = alp_data::Tmax(d_HS_j_const_next[0] - d_alp_data->d_open1,
                                d_HD_j_const_next[0] - d_alp_data->d_epen1);
...
// assumes that letters are defined for d_H_matr_len
{
  ...
  for (i = d_H_matr_len_2; i >= 1; i--) {
    ...
    d_HI_i_const_next[i] =
        alp_data::Tmax(d_HS_i_const_next[i + 1] - d_alp_data->d_open2,
                       d_HI_i_const_next[i + 1] - d_alp_data->d_epen2,
                       d_HD_i_const_next[i + 1] - d_alp_data->d_open2);
    d_HD_i_const_next[i] =
        alp_data::Tmax(d_HS_i_const_pred[i - 1] - d_alp_data->d_open1,
                       d_HD_i_const_pred[i - 1] - d_alp_data->d_epen1);
    d_H_i_const_next[i] = alp_data::Tmax(
    ...
    d_HI_j_const_next[i] =
        alp_data::Tmax(d_HS_j_const_pred[i - 1] - d_alp_data->d_open2,
                       d_HI_j_const_pred[i - 1] - d_alp_data->d_epen2,
                       d_HD_j_const_pred[i - 1] - d_alp_data->d_open2);
    d_HD_j_const_next[i] =
        alp_data::Tmax(d_HS_j_const_next[i + 1] - d_alp_data->d_open1,
                       d_HD_j_const_next[i + 1] - d_alp_data->d_epen1);
    d_H_j_const_next[i] = alp_data::Tmax(
  ...
  if (d_H_matr_len > 1) {
    ...
    d_HI_j_const_next[i] = alp_data::Tmax(d_HS_ij_pred - d_alp_data->d_open2,
                                          d_HI_ij_pred - d_alp_data->d_epen2,
                                          d_HD_ij_pred - d_alp_data->d_open2);
    d_HD_j_const_next[i] =
        alp_data::Tmax(d_HS_j_const_next[i + 1] - d_alp_data->d_open1,
                       d_HD_j_const_next[i + 1] - d_alp_data->d_epen1);
    d_H_j_const_next[i] = alp_data::Tmax(
  ...
  d_HI_ij_next = alp_data::Tmax(d_HS_i_const_next[0] - d_alp_data->d_open2,
                                d_HI_i_const_next[0] - d_alp_data->d_epen2,
                                d_HD_i_const_next[0] - d_alp_data->d_open2);
  d_HD_ij_next = alp_data::Tmax(d_HS_j_const_next[0] - d_alp_data->d_open1,
                                d_HD_j_const_next[0] - d_alp_data->d_epen1);
  d_H_ij_next = alp_data::Tmax(d_HS_ij_next, d_HI_ij_next, d_HD_ij_next);
...
void alp::check_time_function() {
  ...
  if (d_alp_data->d_max_time <= 0 &&
      d_alp_data->d_max_time_with_computation_parameters > 0) {
    double time_after3;
    alp_data::get_current_time(time_after3);

    if ((time_after3 - d_alp_data->d_time_before1) >
        d_alp_data->d_max_time_with_computation_parameters) {
      throw error(
          "Error - you have exceeded the calculation time or memory "
          "limit.\nThe error might indicate that the regime is linear or too "
          "close to linear to permit efficient computation.\nPossible "
...
void alp::increment_H_weights_with_sentinels(long int diff_opt_)
// the function calculates alignment scores for d_H_matr_len increased by 1
// assumes that letters are defined for d_H_matr_len
// uses sentinels
{
  if (d_alp_data->d_insertions_after_deletions) {
    increment_H_weights_with_sentinels_with_insertions_after_deletions(
        diff_opt_);
  } else {
    increment_H_weights_with_sentinels_without_insertions_after_deletions(
        diff_opt_);
  };
}
...
// uses sentinels
{
  ...
  // boundary conditions
  long int sentinel_i_boundary =
      alp_data::Tmin(d_sentinel_i_pred + (long int)2, d_H_matr_len_1);
  long int sentinel_j_boundary =
      alp_data::Tmin(d_sentinel_j_pred + (long int)2, d_H_matr_len_1);
...
void alp::restore_state(state *&state_) {
  d_M = state_->d_M;
...
void alp::save_state(state *&state_) {
  if (d_H_matr_len < 0) {
    throw error("Unexpected error\n", 4);
  };
  ...
  if (d_H_matr_len == 0) {
    ...
    d_alp_data->d_memory_size_in_MB +=
        8.0 * (double)(d_H_matr_len * sizeof(long int)) / mb_bytes;
...
void alp::kill_upto_level(long int M_min_, long int M_level_,
    ...
    long int length_)  // calculation of weigths for the importance sampling
{
  ...
  for (j = 1; j <= length_ - 1; j++) {
    double US_next = d_alp_data->d_r_i_dot[d_seqi[j - 1]] *
                         ((d_is_tmp->d_eta) * US + (d_is_tmp->d_mu_SI) * UI +
                          (d_is_tmp->d_mu_SD) * UD) +
                     d_WS_j_const_next[d_W_matr_len_1 - j];
    double UD_next = ((d_is_tmp->d_mu_DS) * US + (d_is_tmp->d_nu) * UD);
    double UI_next =
        ((d_is_tmp->d_mu_IS) * US_next + (d_is_tmp->d_mu_ID) * UD_next +
         d_WI_j_const_next[d_W_matr_len_1 - j]) /
        (1 - (d_is_tmp->d_nu));

    double VS_next = d_alp_data->d_r_dot_j[d_seqj[j - 1]] *
                         ((d_is_tmp->d_eta) * VS + (d_is_tmp->d_mu_SI) * VI +
                          (d_is_tmp->d_mu_SD) * VD) +
                     d_WS_i_const_next[d_W_matr_len_1 - j];
    double VI_next = ((d_is_tmp->d_mu_IS) * VS + (d_is_tmp->d_mu_ID) * VD +
                      (d_is_tmp->d_nu) * VI);
    double VD_next = ((d_is_tmp->d_mu_DS) * VS_next +
                      d_WD_i_const_next[d_W_matr_len_1 - j]) /
                     (1 - (d_is_tmp->d_nu));
  ...
  double US_next = d_alp_data->d_r_i_dot[d_seqi[j - 1]] *
                   ...
                   d_WS_ij_next;
  double UD_next = ((d_is_tmp->d_mu_DS) * US + (d_is_tmp->d_nu) * UD);
  double UI_next = ((d_is_tmp->d_mu_IS) * US_next +
                    (d_is_tmp->d_mu_ID) * UD_next + d_WI_ij_next) /
                   (1 - (d_is_tmp->d_nu));
  ...
  double VI_next = ((d_is_tmp->d_mu_IS) * VS + (d_is_tmp->d_mu_ID) * VD +
                    (d_is_tmp->d_nu) * VI);
  double VD_next =
      ((d_is_tmp->d_mu_DS) * VS_next + d_WD_ij_next) / (1 - (d_is_tmp->d_nu));
...
void alp::simulate_next_alp()  // simulates next ALP
{
  ...
  if (!d_is_now) {
    throw error(
        "Unexpected error - ALP can be generated only in the importance "
        "sampling mode\n",
        4);
  };
  ...
  while (d_nalp < target_nalp) {
    ...
    while (alp_data::Tmin(d_seqi_len, d_seqj_len) != k + 1) {
      bool success = one_step_of_importance_sampling_without_weight_calculation(
          d_alp_data->d_dim1_tmp, d_alp_data->d_dim2_tmp);
  ...
  double weight =
      John2_weight_calculation(alp_data::Tmin(d_seqi_len, d_seqj_len));
  if (weight <= 0) {
    throw error("Unexpected error\n", 4);
  };
...
void alp::simulate_alp_upto_the_given_number(  // simulates ALP upto the given
                                               // number nalp_ including
    long int nalp_) {
...
void alp::simulate_alp_upto_the_given_level(  // simulates ALP upto the given
                                              // level M_min_ including
    long int M_min_) {
  d_sentinels_flag = false;
  while (d_alp->d_elem[d_nalp] < M_min_) {
    simulate_next_alp();
