...
void LocalMaxStatUtil::flatten(  // allocates memory for linear probabilities
                                 // and scores
    size_t dimension_,           // dimension of equilProb_
    const long int *const *scoreMatrix_,  // packed scoring matrix
                                          // [0...dimension_)[0...dimension2_)
    const double *const *prob_,  // prob_ [0...dimension_)[0...dimension2_) :
                                 // distribution of scores sum to 1.0
...
double LocalMaxStatUtil::lambda(
    ...
    const long int *const *
        scoreMatrix_,  // packed scoring matrix [0...dimension_)[0...dimension_)
    const double
        *q_)  // q_ [0...dimension_) : distribution of independent letters
{
  size_t i = 0;
  size_t j = 0;
...
long int n_entry = 0;  // n_entry = 0 : weak descending ladder epoch ; n_entry =
                       // -1 : strict descending ladder epoch
...
void n_setParameters(
    ...
    long int entry_ = 0)  // entry_ = 0 : weak descending ladder epoch ; entry_
                          // = -1 : strict descending ladder epoch
{
...
double n_meanAssoc(double x_) { return n_meanPowerAssoc(x_); }

void n_bracket(double *p_, double *q_) {
  const double FACTOR = 0.5;
  *p_ = -log(n_prob[n_dimension - 1]) /
        static_cast<double>(n_score[n_dimension - 1]);
...
long int LocalMaxStatUtil::delta(  // theta [minus delta] for ungapped sequence
                                   // comparison
    size_t dimension_,  // #(distinct values) of scores & probabilities (which
                        // are paired)
    const long int *score_)  // scores
...
double LocalMaxStatUtil::thetaMinusDelta(  // theta [minus delta] for ungapped
                                           // sequence comparison
...
void LocalMaxStatUtil::descendingLadderEpochRepeat(
    ...
    double
        *eOneMinusExpSumAlphaW_,  // expectation [1.0 - exp (sum [alpha]); alpha
                                  // = w] : eOneMinusExpSumAlphaW_ [0, wEnd)
    ...
    double muAssoc0_,  // mean of associated flattened distribution (avoid
                       // recomputation)
    double
        thetaMin0_,  // thetaMin of flattened distribution (avoid recomputation)
    ...
    bool *terminated_)  // ? Was the dynamic programming computation terminated
                        // prematurely ?
// assumes logarithmic regime
{
  ...
  assert(mu0 < 0.0);
  double lambda0 =
      0.0 == lambda0_ ? lambda(dimension_, score_, prob_) : lambda0_;
  assert(0.0 < lambda0);
  if (lambda_ == 0.0) lambda_ = lambda0;
  assert(0.0 < lambda_);
  double muAssoc0 = 0.0 == muAssoc0_
                        ? muAssoc(dimension_, score_, prob_, lambda0)
                        : muAssoc0_;
  assert(0.0 < muAssoc0);
  double thetaMin0 = 0.0 == thetaMin0_
                         ? thetaMin(dimension_, score_, prob_, lambda0)
                         : thetaMin0_;
  assert(0.0 < thetaMin0);
  double rMin0 = 0.0 == rMin0_
                     ? rMin(dimension_, score_, prob_, lambda0, thetaMin0)
                     : rMin0_;
  assert(0.0 < rMin0 && rMin0 < 1.0);

  const long int ITER_MIN =
      static_cast<long int>((log(REL_TOL * (1.0 - rMin0)) / log(rMin0)));
  assert(0 < ITER_MIN);
  const long int ITER = static_cast<long int>(endW_) < ITER_MIN
                            ? ITER_MIN
                            : static_cast<long int>(endW_);
  assert(0 < ITER);
  ...
  for (size_t w = 1; w < static_cast<size_t>(ITER); w++) {
    if (w < endW_) {  // sum pAlphaW_ [w] and eOneMinusExpSumAlphaW_ [w]
      ...
      for (value = score_[0]; value <= entry; value++) {
        ...
        if (eOneMinusExpSumAlphaW_)
          eOneMinusExpSumAlphaW_[w] +=
    ...
    for (value = score_[0]; value <= entry; value++) {
      if (eSumAlpha_)
        *eSumAlpha_ += dynProgProb.getProb(value) * static_cast<double>(value);
      if (eOneMinusExpSumAlpha_)
...
void LocalMaxStatUtil::descendingLadderEpoch(
    ...
    bool *terminated_)  // ? Was the dynamic programming computation terminated
                        // prematurely ?
{
  descendingLadderEpochRepeat(dimension_, score_, prob_, eSumAlpha_,
                              eOneMinusExpSumAlpha_, isStrict_, 0.0, 0, 0, 0,
                              lambda0_, mu0_, muAssoc0_, thetaMin0_, rMin0_,
                              time_, terminated_);
}
