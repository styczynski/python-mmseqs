...
void pvalues::get_appr_tail_prob_with_cov(const ALP_set_of_parameters &par_,
                                          bool blast_, double y_, double m_,
                                          double n_,

                                          double &P_, double &P_error_,

                                          double &E_, double &E_error_,
  ...
  m_li_y_error =
      alp_data::error_of_the_sum(fabs(y_) * ai_hat_error_, bi_hat_error_);
  m_li_y = m_ - tmp;

  double vi_y_error = 0;
  double vi_y = 0;

  vi_y_error = alp_data::error_of_the_sum(fabs(y_) * alphai_hat_error_,
                                          betai_hat_error_);
  ...
  if (sqrt_vi_y == 0.0 || blast_) {
    ...
    m_F_error = alp_data::error_of_the_ratio(m_li_y, m_li_y_error, sqrt_vi_y,
                                             sqrt_vi_y_error);
  ...
  n_lj_y_error =
      alp_data::error_of_the_sum(fabs(y_) * aj_hat_error_, bj_hat_error_);
  n_lj_y = n_ - tmp;

  double vj_y_error = 0;
  double vj_y = 0;

  vj_y_error = alp_data::error_of_the_sum(fabs(y_) * alphaj_hat_error_,
                                          betaj_hat_error_);
  ...
  if (sqrt_vj_y == 0.0 || blast_) {
    ...
    n_F_error = alp_data::error_of_the_ratio(n_lj_y, n_lj_y_error, sqrt_vj_y,
                                             sqrt_vj_y_error);
  ...
  double P_m_F_P_n_F_error =
      alp_data::error_of_the_product(P_m_F, P_m_F_error, P_n_F, P_n_F_error);
  double P_m_F_P_n_F = P_m_F * P_n_F;

  double c_y_P_m_F_P_n_F_error = alp_data::error_of_the_product(
      c_y, c_y_error, P_m_F_P_n_F, P_m_F_P_n_F_error);
  double c_y_P_m_F_P_n_F = c_y * P_m_F_P_n_F;

  double p1_p2_error =
      alp_data::error_of_the_product(p1, p1_error, p2, p2_error);
  double p1_p2 = p1 * p2;

  double area_error =
      alp_data::error_of_the_sum(p1_p2_error, c_y_P_m_F_P_n_F_error);
  ...
  double k_exp_lambda_y_error = alp_data::error_of_the_product(
      k_, k_error_, exp_lambda_y, exp_lambda_y_error);
  double k_exp_lambda_y = k_ * exp_lambda_y;

  double area_k_exp_lambda_y_error = alp_data::error_of_the_product(
      area, area_error, k_exp_lambda_y, k_exp_lambda_y_error);
...
void pvalues::compute_intercepts(ALP_set_of_parameters &par_) {
  if (!par_.d_params_flag) {
    throw error(
        "Unexpected error: pvalues::compute_intercepts is called for undefined "
        "parameters\n",
        1);
  };
  ...
  par_.b_I_error =
      2.0 * par_.G *
      alp_data::error_of_the_sum(par_.gapless_a_error, par_.a_I_error);
  ...
  par_.b_I_error =
      2.0 * par_.G *
      alp_data::error_of_the_sum(par_.gapless_a_error, par_.a_J_error);
  ...
  for (i = 0; i < vector_size; i++) {
    ...
    par_.m_BetaISbs[i] =
        2.0 * par_.G * (par_.gapless_alpha - par_.m_AlphaISbs[i]);

    par_.m_BJSbs[i] = 2.0 * par_.G * (par_.gapless_a - par_.m_AJSbs[i]);
    par_.m_BetaJSbs[i] =
        2.0 * par_.G * (par_.gapless_alpha - par_.m_AlphaJSbs[i]);
...
void pvalues::compute_tmp_values(ALP_set_of_parameters &par_) {
  ...
  if (par_.lambda > 0) {
    par_.vi_y_thr =
        alp_data::Tmax(nat_cut_off_in_max * par_.alpha_I / par_.lambda, 0.0);
    par_.vj_y_thr =
        alp_data::Tmax(nat_cut_off_in_max * par_.alpha_J / par_.lambda, 0.0);
    par_.c_y_thr =
        alp_data::Tmax(nat_cut_off_in_max * par_.sigma / par_.lambda, 0.0);
  } else {
    par_.vi_y_thr = 0;
    par_.vj_y_thr = 0;
    par_.c_y_thr = 0;
...
void pvalues::get_appr_tail_prob_with_cov_without_errors(
    const ALP_set_of_parameters &par_, bool blast_, double y_, double m_,
    double n_,
    ...
    double &area_, bool &area_is_1_flag_, bool compute_only_area_) {
  // to optimize performance
  blast_ = false;
...
void pvalues::get_P_error_using_splitting_method(
  ...
  for (i = 0; i < dim; i++) {
    ...
    get_appr_tail_prob_with_cov_without_errors(par_tmp, blast_, y_, m_, n_,

                                               P_tmp,

                                               E_tmp,

                                               area_tmp, area_is_1_flag_);
  ...
  if (P_ < 1e-4) {
    ...
    P_error_ = exp_E_values_aver *
               alp_reg::sqrt_for_errors(exp_E_values_error / (double)dim);
  };
...
void pvalues::calculate_P_values(long int Score1, long int Score2,
                                 double Seq1Len, double Seq2Len,
  ...
  for (y = Score1; y <= Score2; y++) {
    calculate_P_values(y, Seq1Len, Seq2Len, ParametersSet, P_values[y - Score1],
                       P_values_errors[y - Score1], E_values[y - Score1],
                       E_values_errors[y - Score1]);
  };
}

void pvalues::calculate_P_values(double Score, double Seq1Len, double Seq2Len,
                                 const ALP_set_of_parameters &ParametersSet,
                                 double &P_value, double &P_value_error,
                                 double &E_value, double &E_value_error,
  ...
  if (read_Sbs_par_flag) {
    get_appr_tail_prob_with_cov_without_errors(ParametersSet, blast, Score,
                                               Seq1Len, Seq2Len,
    ...
    if (ParametersSet.m_LambdaSbs.size() > 0) {
      get_P_error_using_splitting_method(ParametersSet, blast, Score, Seq1Len,
                                         Seq2Len,
    ...
    get_appr_tail_prob_with_cov(ParametersSet, blast, Score, Seq1Len, Seq2Len,
...
std::ostream &operator<<(std::ostream &s_,
  ...
  s_ << "Lambda\tLambda error\tK\tK error\tC\tC error\ta\ta error\ta_1\ta_1 "
        "error\ta_2\ta_2 error\tsigma\tsigma error\talpha\talpha "
        "error\talpha_1\talpha_1 error\talpha_2\talpha_2 error\tGapless "
        "a\tGapless a error\tGapless alpha\tGapless alpha "
        "error\tG\tCalculation time\tArrays for error calculation\n";
  s_.precision(17);
  s_ << gumbel_params_.lambda << "\t" << gumbel_params_.lambda_error << "\t"
     ...
     << "\t" << gumbel_params_.gapless_alpha << "\t"
     << gumbel_params_.gapless_alpha_error << "\t" << gumbel_params_.G << "\t"
...
std::istream &operator>>(std::istream &s_,
  ...
  try {
    ...
    getline(s_, st);
    s_ >> gumbel_params_.lambda >> gumbel_params_.lambda_error >>
        gumbel_params_.K >> gumbel_params_.K_error >> gumbel_params_.C >>
        gumbel_params_.C_error >> gumbel_params_.a >> gumbel_params_.a_error >>
        gumbel_params_.a_J >> gumbel_params_.a_J_error >> gumbel_params_.a_I >>
        gumbel_params_.a_I_error >> gumbel_params_.sigma >>
        gumbel_params_.sigma_error >> gumbel_params_.alpha >>
        gumbel_params_.alpha_error >> gumbel_params_.alpha_J >>
        gumbel_params_.alpha_J_error >> gumbel_params_.alpha_I >>
        gumbel_params_.alpha_I_error >> gumbel_params_.gapless_a >>
        gumbel_params_.gapless_a_error >> gumbel_params_.gapless_alpha >>
        gumbel_params_.gapless_alpha_error >> gumbel_params_.G >>
        gumbel_params_.m_CalcTime;
...
// returns "true" if the Gumbel parameters are properly defined and "false"
// otherwise
bool pvalues::assert_Gumbel_parameters(
    const ALP_set_of_parameters &par_)  // a set of Gumbel parameters
{
  if (!(par_.lambda > 0) || par_.lambda_error < 0 ||

      // the parameters C and K_C are not necessary for the P-value calculation
      // par_.C<0||
      // par_.C_error<0||

      !(par_.K > 0) || par_.K_error < 0 ||

      par_.a_I < 0 || par_.a_I_error < 0 ||

      par_.a_J < 0 || par_.a_J_error < 0 ||

      par_.sigma < 0 || par_.sigma_error < 0 ||

      par_.alpha_I < 0 || par_.alpha_I_error < 0 ||

      par_.alpha_J < 0 || par_.alpha_J_error < 0 ||

      par_.gapless_a < 0 || par_.gapless_a_error < 0 ||

      par_.gapless_alpha < 0 || par_.gapless_alpha_error < 0 ||

      par_.G < 0 || par_.G1 < 0 || par_.G2 < 0 ||
  ...
  if (par_.m_KSbs.size() != size_tmp ||
      ...
      par_.m_AISbs.size() != size_tmp || par_.m_AJSbs.size() != size_tmp ||

      par_.m_BISbs.size() != size_tmp || par_.m_BJSbs.size() != size_tmp ||
