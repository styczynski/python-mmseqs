...
void flatten(           // allocates memory for linear probabilities and scores
    ...
    const double *const *prob_,  // prob_ [0...dimension_)[0...dimension_) :
    ...
    size_t dimension2_ =
        0);  // dimension2 of equilProb_ : defaults to dimension_
// asserts (sum (p_) == 1.0);
...
void descendingLadderEpoch(
    ...
    double lambda0_ =
        0.0,  // lambda for flattened distribution (avoid recomputation)
    double mu0_ = 0.0,  // mean of flattened distribution (avoid recomputation)
    double muAssoc0_ =
        0.0,  // mean of associated flattened distribution (avoid recomputation)
    double thetaMin0_ =
        0.0,  // thetaMin of flattened distribution (avoid recomputation)
    double rMin0_ =
        0.0,             // rMin of flattened distribution (avoid recomputation)
    double time_ = 0.0,  // get time for the dynamic programming computation
    bool *terminated_ = 0);  // ? Was the dynamic programming computation
...
void descendingLadderEpochRepeat(
    ...
    double *eOneMinusExpSumAlphaW_ =
        0,  // expectation [1.0 - exp (sum [alpha]); alpha = w] :
            // eOneMinusExpSumAlphaW_ [0, wEnd)
    double lambda0_ =
        0.0,  // lambda for flattened distribution (avoid recomputation)
    double mu0_ = 0.0,  // mean of flattened distribution (avoid recomputation)
    double muAssoc0_ =
...
bool isProbDist(size_t dimension_,     // #(distinct values) of scores &
                                       // probabilities (which are paired)
                const double *prob_);  // corresponding probabilities

bool isScoreIncreasing(size_t dimension_,        // #(distinct values)
                       const long int *score_);  // scores in increasing order

bool isLogarithmic(size_t dimension_,       // #(distinct values)
                   const long int *score_,  // scores in increasing order
                   const double *prob_);  // probability of corresponding value
