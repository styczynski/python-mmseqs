...
class Vector {
 public:
  ...
  virtual inline const T *getVector() const {
    return d_vector_p;
  }  // the vector
...
template <typename T>
Vector<T>::Vector() : d_m(0), d_vector_p(0) {}

template <typename T>
Vector<T>::Vector(const T *begin_, const T *end_) : d_m(0), d_vector_p(0) {
  copy(begin_, end_);
}
...
Vector<T> &Vector<T>::operator=(const Vector<T> &vector_) {
  if (this != &vector_)
    copy(vector_.getVector(), vector_.getVector() + vector_.getM());
  return *this;
}
...
template <typename T>
void Vector<T>::copy(const T *begin_, const T *end_) {
  assert(begin_ <= end_);
...
template <typename T>
void Vector<T>::copy(size_t m_,    // m
                     const T &a_)  // the vector elements
{
  if (m_ != getM()) {
    free2();
