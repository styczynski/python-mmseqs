...
class AlignmentEvaluer {
  ...
  friend std::ostream &operator<<(std::ostream &s_, const AlignmentEvaluer &g_);

  // Read the parameters:
  friend std::istream &operator>>(std::istream &s_, AlignmentEvaluer &g_);
 ...
 public:
  ...
  // sequence #1 if max_time_<=0 then d_gapped_computation_parameters will be
  // used; d_gapped_computation_parameters.d_parameters_flag must be true in this
  // case every execution of initGapped with max_time_>0 rewrites
  ...
  void initParameters(
      const AlignmentEvaluerParameters
          &parameters_);  // parameters_ must be defined by the user

  void initParameters(
      const AlignmentEvaluerParametersWithErrors
          &parameters_);  // parameters_ must be defined by the user
  ...
  // returns "true" if the set of parameters "d_params" is fully defined for
  // P-value calculation
  bool isGood() const { return d_params.d_params_flag; }
  ...
  const gapped_computation_parameters_struct &gapped_computation_parameters()
      const {
    return d_gapped_computation_parameters;
  }

  // simplified setting of the computation parameters
  void set_gapped_computation_parameters_simplified(
      double max_time_ =
          -1.0,  // maximum allowed time in seconds for the whole computation
      long number_of_samples_ =
          1000,  // number of realization for the main stage
      long number_of_samples_for_preliminary_stages_ =
          200);  // number of realization for the preliminary stages

  void set_gapped_computation_parameters(
      const gapped_computation_parameters_struct
          &gapped_computation_parameters_) {
 ...
 private:
  ...
  void assert_Gapless_input_parameters(
      ...
      const double
          *letterFreqs2_,  // background frequencies of letters in sequence #2
      double *&letterFreqs1_normalized_,  // normalized background frequencies
                                          // of letters in sequence #1
      double *&letterFreqs2_normalized_,  // normalized background frequencies
                                          // of letters in sequence #2
      const std::string
          function_name_);  //"assert_Gapless_input_parameters" is called from
