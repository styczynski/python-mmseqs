...
template <typename T>
double
huntExtreme(  // finds root f_(x_) = y_ in (p_, q_) by looking until mesh = tol_
    double y_,                        // f_ (x_) = y_
    double (*f_)(double, const T &),  // function : param_ contains parameters
    const T &param_,                  // parameters for function
    double p_,                        // end-point
...
inline double
newtonRaphson(  // finds root f_(x_) = y_ in [p_, q_] with derivative y_'
    double y_,  // f_ (x_) = y_
...
inline double
hunt(  // finds root f_ (x_) = y_ in (p_, q_) by looking until mesh = tol_
    double y_,              // f_ (x_) = y_
    ...
    long int *itmax_)  // maximum # of permitted iterations : default = 0
{
  ...
  if (fp * fq > 0.0) IoUtil::abort("Root::newtonRaphson : root not bracketed");
  ...
  for (; 0 < *itmax; --*itmax) {
    ...
    if ((dfx * (x - p_) - fx) * (dfx * (x - q_) - fx) >= 0.0 ||
      ...
      if (fabs(dxold) < tol_ || fabs(dxold) < rtol_ * fabs(x)) {
        if (((*f_)((x - Function::signum(dxold) * tol_), param_) - y_) * fx <
            0.0)
          return x;
      }
    }
    ...
    long int *itmax_)  // maximum # of permitted iterations : default = 0
{
  ...
  double fp = (*f_)(p_, param_) - y_;
  double fq = (*f_)(q_, param_) - y_;
  if (fp * fq > 0.0) IoUtil::abort("Root::bisection : root not bracketed");
    ...
    long int *itmax_)  // maximum # of permitted iterations : default = 0
{
  ...
  while (tol_ <= del) {
    ...
    for (size_t i = 0; i < pts && 0 < *itmax; i++, --*itmax) {
      fx = (*f_)(x, param_) - y_;
      if (fx * fx0 < 0.0)
        return bisection<T>(y_, f_, param_, x, x0, tol_, rtol_, itmax);
      x += del;
    }
