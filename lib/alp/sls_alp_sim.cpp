...
alp_sim::alp_sim(  // constructor
  ...
  try {
    ...
    d_alp_data->d_memory_size_in_MB +=
        (double)(sizeof(array_positive<alp *>)) / mb_bytes;
    ...
    quick_test(quick_tests_trials_number,
    ...
    do {
      ...
      if (d_alp_data->d_rand_flag) {
        ...
        rand_i++;
        number_tmp =
            d_alp_data->d_rand_all
                ->d_first_stage_preliminary_realizations_numbers_ALP[rand_i];
      } else {
        number_tmp = alp_data::Tmin(
            maximum_number_of_realizations_for_preliminary_simulation - 1,
            d_n_alp_obj +
                sim_number * d_alp_data->d_minimum_realizations_number - 1);
      };

      get_minimal_simulation(0, number_tmp, M_min, nalp, nalp_lambda,
                             C_calculation, check_time_flag);

      if (!d_alp_data->d_rand_flag) {
        d_alp_data->d_rand_all
            ->d_first_stage_preliminary_realizations_numbers_ALP.push_back(
                number_tmp);
      };
      ...
      if (d_lambda_tmp->d_elem[nalp] >= 0) {
        if (d_lambda_tmp_errors->d_elem[nalp] / d_lambda_tmp->d_elem[nalp] <
            alp_data_->d_eps_lambda) {
      ...
      if (d_alp_data->d_rand_flag) {
        loop_break_flag =
            (rand_i >=
             (long int)(d_alp_data->d_rand_all
        ...
        loop_break_flag = !(
            ...
            ((time_after_tmp - time_before1) <= 0 ||
             ((time_after_tmp - time_before1) < 0.01 * d_alp_data->d_max_time &&
              d_alp_data->d_memory_size_in_MB < d_alp_data->d_max_mem)));
      };
    ...
    long int limit_by_memory = (long int)alp_data::Tmin(
        (double)LONG_MAX - 1,
        alp_data::round(d_alp_data->d_max_mem / 2.0 /
                        (memory_after1 - memory_before1) * d_n_alp_obj));

    if (d_alp_data->d_memory_size_in_MB > d_alp_data->d_max_mem) {
      // cout<<"\nWarning: memory limit is reached.\nTo get the requested
      // accuracy please\nincrease maximum allowed amount of memory if
      // possible\n\n";
    };
    ...
    if (time_after1 <= time_before1) {
      ...
      limit_by_time = (long int)alp_data::Tmin(
          (double)LONG_MAX - 1,
          alp_data::round(d_alp_data->d_max_time / 3.0 / 4.0 /
                          (time_after1 - time_before1) * d_n_alp_obj));
    };

    long int realizations_number2 = alp_data::Tmin(
        (long int)alp_data::round(limit_by_time) - 1, limit_by_memory - 1);

    realizations_number2 = alp_data::Tmin(
        maximum_number_of_realizations_for_preliminary_simulation - (long int)1,
        realizations_number2);

    realizations_number2 =
        alp_data::Tmax(d_n_alp_obj - (long int)1, realizations_number2);

    delete d_lambda_tmp;
    d_lambda_tmp = NULL;
    delete d_lambda_tmp_errors;
    d_lambda_tmp_errors = NULL;

    delete d_C_tmp;
    d_C_tmp = NULL;
    delete d_C_tmp_errors;
    d_C_tmp_errors = NULL;
    ...
    long int number_of_realizations_with_ALP = alp_data::Tmin(
        realizations_number2,
        d_n_alp_obj - (long int)1 + d_alp_data->d_minimum_realizations_number);
    ...
    do {
      ...
      if (d_alp_data->d_rand_flag) {
        ...
        number_of_realizations_with_ALP =
            d_alp_data->d_rand_all
                ->d_preliminary_realizations_numbers_ALP[rand_i];
      };

      get_minimal_simulation(0, number_of_realizations_with_ALP, M_min, nalp,
                             nalp_lambda, C_calculation, check_time_flag);

      if (!d_alp_data->d_rand_flag) {
        d_alp_data->d_rand_all->d_preliminary_realizations_numbers_ALP
            .push_back(number_of_realizations_with_ALP);
      };
      ...
      if (d_lambda_tmp->d_elem[nalp] > 0) {
        tmp_lambda =
            ((d_lambda_tmp_errors->d_elem[nalp] / d_lambda_tmp->d_elem[nalp]) /
             d_alp_data->d_eps_lambda);
      };
      ...
      if (d_alp_data->d_rand_flag) {
        loop_break_flag =
            (rand_i >=
             (long int)(d_alp_data->d_rand_all
                            ->d_preliminary_realizations_numbers_ALP.size() -
                        1));
        if (loop_break_flag) {
          break;
        };
      ...
      if (time_during_ALP - time_before1 >= d_alp_data->d_max_time * 0.25 ||
      ...
      if (time_during_ALP <= time_before_ALP) {
        number_of_realizations_with_ALP =
            alp_data::Tmin(realizations_number2,
                           number_of_realizations_with_ALP +
                               d_alp_data->d_minimum_realizations_number);
      } else {
        long int max_number_of_realizations = (long int)floor(
            number_of_realizations_with_ALP *
            (d_alp_data->d_max_time * 0.35 - (time_before_ALP - time_before1)) /
            (time_during_ALP - time_before_ALP));
        number_of_realizations_with_ALP = alp_data::Tmin(
            realizations_number2,
            (long int)floor(0.5 * number_of_realizations_with_ALP +
                            0.5 * max_number_of_realizations));
        if (number_of_realizations_with_ALP >= max_number_of_realizations) {
          number_of_realizations_with_ALP =
              d_alp_data->Tmin(realizations_number2,
                               number_of_realizations_with_ALP +
                                   d_alp_data->d_minimum_realizations_number);
        };

        if ((double)(number_of_realizations_with_ALP -
                     number_of_realizations_with_ALP_pred) /
                (double)number_of_realizations_with_ALP_pred <
            0.005) {
          number_of_realizations_with_ALP =
              number_of_realizations_with_ALP_pred;
          if (!d_alp_data->d_rand_flag) {
    ...
    long int number_of_realizations_with_killing = d_alp_data->Tmin(
        realizations_number2,
        d_alp_data->d_minimum_realizations_number - (long int)1);
    ...
    do {
      ...
      if (d_alp_data->d_rand_flag) {
        ...
        number_of_realizations_with_killing =
            d_alp_data->d_rand_all
                ->d_preliminary_realizations_numbers_killing[rand_i];
      };

      kill(check_time_flag, 0, number_of_realizations_with_killing, M_min,
           lambda, eps_K, K_C, K_C_error, level, diff_opt);

      if (!d_alp_data->d_rand_flag) {
        d_alp_data->d_rand_all->d_preliminary_realizations_numbers_killing
            .push_back(number_of_realizations_with_killing);
      };
      ...
      if (time_during_kill - time_before1 >= d_alp_data->d_max_time ||
      ...
      if (time_during_kill <= time_before_kill) {
        ...
        long int max_number_of_realizations = (long int)floor(
            number_of_realizations_with_killing *
            (d_alp_data->d_max_time - (time_before_kill - time_before1)) /
            (time_during_kill - time_before_kill));
        number_of_realizations_with_killing = d_alp_data->Tmin(
            realizations_number2,
            (long int)floor(0.5 * number_of_realizations_with_killing +
                            0.5 * max_number_of_realizations));
        ...
        if ((double)(number_of_realizations_with_killing -
                     number_of_realizations_with_killing_pred) /
                (double)number_of_realizations_with_killing_pred <
            0.005) {
          number_of_realizations_with_killing =
              number_of_realizations_with_killing_pred;
          if (!d_alp_data->d_rand_flag) {
    ...
    if (K_C <= 0) {
      throw error(
          "Error - you have exceeded the calculation time or memory "
          "limit.\nThe error might indicate that the regime is linear or too "
          "close to linear to permit efficient computation.\nPossible "
          "solutions include changing the randomization seed, or increasing "
          "the allowed calculation time and the memory limit.\n",
          3);
    };

    double tmp = ((K_C_error / K_C) / d_alp_data->d_eps_K);
    tmp = alp_data::Tmin(ceil((realizations_number2_K + 1) * tmp * tmp),
                         (double)LONG_MAX);
    long int realizations_number_killing = (long int)tmp;
    tmp = ((d_lambda_tmp_errors->d_elem[nalp] / d_lambda_tmp->d_elem[nalp]) /
           d_alp_data->d_eps_lambda);
    tmp = alp_data::Tmin(ceil((realizations_number2_lambda + 1) * tmp * tmp),
                         (double)LONG_MAX);
    ...
    bool kill_flag =
        (realizations_number_killing > realizations_number2_K + 1 + j);
    bool lambda_flag =
        (realizations_number_lambda > realizations_number2_lambda + 1 + j);
    ...
    if (d_alp_data->d_rand_flag) {
      lambda_flag =
          (d_alp_data->d_rand_all->d_total_realizations_number_with_ALP >
           realizations_number2_K + j - 1);
      kill_flag =
          (d_alp_data->d_rand_all->d_total_realizations_number_with_killing >
           realizations_number2_K + j - 1);
    };
    ...
    if (kill_flag || lambda_flag) {
      ...
      while (d_n_alp_obj < LONG_MAX - realizations_number2_lambda - j) {
        ...
        lambda_flag =
            (realizations_number_lambda > realizations_number2_lambda + j);

        if (d_alp_data->d_rand_flag) {
          lambda_flag =
              (d_alp_data->d_rand_all->d_total_realizations_number_with_ALP >
               realizations_number2_K + j - 1);
          kill_flag = (d_alp_data->d_rand_all
        ...
        if (!(kill_flag || lambda_flag)) {
          if (!d_alp_data->d_rand_flag) {
            // cout<<"\nThe parameters were calculated with the required
            // accuracy\n\n";
            break;
          } else {
            break;
        ...
        try {
          ...
          while (!sucess_flag) {
            ...
            get_single_realization(check_time_flag, M_min, nalp_for_simulation,
                                   kill_flag, level, diff_opt, obj, sucess_flag,
                                   eps_tmp);
            ...
            if (!sucess_flag) {
              ...
              //	d_n_alp_obj--;
              //};
              // throw error("Error - you have exceeded the calculation time or
              // memory limit.\nThe error might indicate that the regime is
              // linear or too close to linear to permit efficient
              // computation.\nPossible solutions include changing the
              // randomization seed, or increasing the allowed calculation time
          ...
          // accuracy please\nincrease the allowed calculation time if
          // possible\n\n";
          if (realizations_number2_K + j > realizations_number2_lambda) {
            delete obj;
            obj = NULL;
        ...
        d_alp_obj->d_elem[realizations_number2_K + j]
            ->partially_release_memory();
        ...
        if (d_alp_data->d_memory_size_in_MB > d_alp_data->d_max_mem) {
          if (!d_alp_data->d_rand_flag) {
            // cout<<"\nWarning: memory limit is reached.\nTo get the requested
            // accuracy please\nincrease the allowed amount of memory if
            // possible\n\n";
            break;
          };
        };
    ...
    long int final_realizations_number_killing =
        kill_j + realizations_number2_K + 1;
    long int final_realizations_number_lambda = alp_data::Tmax(
    ...
    // seconds\n"; cout<<"Actual memory usage is
    // "<<d_alp_data->Tmax(memory_after2,memory_after3)<<" MBs\n\n";

    if (!d_alp_data->d_rand_flag) {
      d_alp_data->d_rand_all->d_total_realizations_number_with_ALP =
          final_realizations_number_lambda - 1;
      d_alp_data->d_rand_all->d_total_realizations_number_with_killing =
    ...
    if (!d_alp_data->d_rand_flag && d_alp_data->d_randout != "") {
      // string
      // rand_st="rand_"+alp_data::long_to_string(d_alp_data->d_rand_all->d_random_factor)+".out";
      ...
      frand << d_alp_data->d_rand_all
                   ->d_first_stage_preliminary_realizations_numbers_ALP.size()
            << "\t";
      for (i = 0;
           i < (long int)d_alp_data->d_rand_all
                   ->d_first_stage_preliminary_realizations_numbers_ALP.size();
           i++) {
        frand << d_alp_data->d_rand_all
                     ->d_first_stage_preliminary_realizations_numbers_ALP[i]
              << "\t";
      };
      frand << endl;

      frand << d_alp_data->d_rand_all->d_preliminary_realizations_numbers_ALP
                   .size()
            << "\t";
      for (i = 0; i < (long int)d_alp_data->d_rand_all
                          ->d_preliminary_realizations_numbers_ALP.size();
           i++) {
        frand
            << d_alp_data->d_rand_all->d_preliminary_realizations_numbers_ALP[i]
            << "\t";
      };
      frand << endl;

      frand << d_alp_data->d_rand_all
                   ->d_preliminary_realizations_numbers_killing.size()
            << "\t";
      for (i = 0; i < (long int)d_alp_data->d_rand_all
                          ->d_preliminary_realizations_numbers_killing.size();
           i++) {
        frand << d_alp_data->d_rand_all
                     ->d_preliminary_realizations_numbers_killing[i]
              << "\t";
      };
      frand << endl;

      frand << d_alp_data->d_rand_all->d_total_realizations_number_with_ALP
            << endl;
      frand << d_alp_data->d_rand_all->d_total_realizations_number_with_killing
            << endl;
    ...
    // cout<<"A number of
    // realizations\t"<<d_alp_data->d_rand_all->d_total_realizations_number_with_ALP<<endl;
    ...
    output_main_parameters2m_new(
        nalp_for_simulation, level, inside_simulation_flag,
        final_realizations_number_lambda, final_realizations_number_killing);
...
void alp_sim::symmetric_parameters_for_symmetric_scheme() {
  ...
  if (symmetric_flag) {
    if (d_alp_data->d_epen1 != d_alp_data->d_epen2 ||
        d_alp_data->d_open1 != d_alp_data->d_open2) {
      symmetric_flag = false;
    };
  };
...
void alp_sim::randomize_realizations(
  ...
  randomize_realizations_ind(final_realizations_number_killing_,
                             final_realizations_number_lambda_ - 1);
}

void alp_sim::randomize_realizations_ind(long int ind1_, long int ind2_) {
  alp **array_ind = NULL;
  long int *perm = NULL;
...
void alp_sim::generate_random_permulation(long int *perm_, long int dim_) {
  long int i;
  for (i = 0; i < dim_; i++) {
    perm_[i] = i;
  };
...
void alp_sim::output_main_parameters2m_new(
    long int nalp_for_lambda_simulation, long int level,
    bool &inside_simulation_flag, long int final_realizations_number_lambda_,
    long int final_realizations_number_killing_) {
  double lambda;
  double lambda_error;
  ...
  do {
    calculate_main_parameters2m(
        final_realizations_number_lambda_, final_realizations_number_killing_,
        nalp_for_lambda_simulation, level, inside_simulation_flag, lambda,
        lambda_error, test_difference, test_difference_error, C, C_error, K_C,
        K_C_error, a_I, a_I_error, a_J, a_J_error, sigma, sigma_error, alpha_I,
        alpha_I_error, alpha_J, alpha_J_error, K, K_error, flag);

    number_of_trials++;

    if (!flag) {
      randomize_realizations(final_realizations_number_lambda_,
  ...
  if (!flag) {
    throw error(
        ...
        "error might indicate that the regime is linear or too close to linear "
        "to permit efficient computation.\nPossible solutions include changing "
        "the randomization seed, or increasing the allowed calculation time "
        "and the memory limit.\n",
        3);
  };
}

double alp_sim::round_double(double val_, long int digits_) {
  long int i;
  for (i = 0; i < digits_; i++) {
    val_ *= 10;
  };
...
double alp_sim::relative_error_in_percents(double val_, double val_error_) {
  if (val_ == 0) {
    return DBL_MAX;
  };
...
long int alp_sim::get_number_of_subsimulations(
  ...
  if (number_of_realizations_ < min_number_of_realizations_for_subsimulation *
                                    min_number_of_subsimulations) {
    throw error(
        "Error - you have exceeded the calculation time or memory limit.\nThe "
        "error might indicate that the regime is linear or too close to linear "
        "to permit efficient computation.\nPossible solutions include changing "
  ...
  long int res_subsimulations =
      (long int)ceil(sqrt((double)number_of_realizations_));
  res_subsimulations =
...
void alp_sim::memory_release_for_calculate_main_parameters2m(
    long int nalp_for_lambda_simulation, long int *&d_mult_realizations,
    long int *&d_mult_K_realizations,

    double *&lambda_mult, double *&lambda_mult_error,

    double *&C_mult, double *&C_mult_error,

    double *&a_I_mult, double *&a_I_mult_error,

    double *&a_J_mult, double *&a_J_mult_error,

    double *&sigma_mult, double *&sigma_mult_error,

    double *&alpha_I_mult, double *&alpha_I_mult_error,

    double *&alpha_J_mult, double *&alpha_J_mult_error,

    double *&K_C_mult, double *&K_C_mult_error,

    double *&K_mult, double *&K_mult_error,

    double *&Sc_mult, double *&Sc_mult_error,

    void **&alp_distr, void **&alp_distr_errors,

    void ***&alp_mult_distr, void ***&alp_mult_distr_errors) {
  if (alp_distr) {
    long int j;
    for (j = 1; j <= nalp_for_lambda_simulation; j++) {
      delete (array_positive<double> *)alp_distr[j];
      alp_distr[j] = NULL;
    };
  ...
  if (alp_distr_errors) {
    long int j;
    for (j = 1; j <= nalp_for_lambda_simulation; j++) {
      delete (array_positive<double> *)alp_distr_errors[j];
      alp_distr_errors[j] = NULL;
    };

    delete[] alp_distr_errors;
  ...
  if (alp_mult_distr) {
    ...
    for (k = 1; k <= d_mult_number; k++) {
      if (alp_mult_distr[k]) {
        for (j = 1; j <= nalp_for_lambda_simulation; j++) {
          delete (array_positive<double> *)alp_mult_distr[k][j];
          alp_mult_distr[k][j] = NULL;
        };

        delete[] alp_mult_distr[k];
        alp_mult_distr[k] = NULL;
      };
    };

    delete[] alp_mult_distr;
    alp_mult_distr = NULL;
  };
  ...
  if (alp_mult_distr_errors) {
    ...
    for (k = 1; k <= d_mult_number; k++) {
      if (alp_mult_distr_errors[k]) {
        for (j = 1; j <= nalp_for_lambda_simulation; j++) {
          delete (array_positive<double> *)alp_mult_distr_errors[k][j];
          alp_mult_distr_errors[k][j] = NULL;
        };

        delete[] alp_mult_distr_errors[k];
        alp_mult_distr_errors[k] = NULL;
      };
    };

    delete[] alp_mult_distr_errors;
    alp_mult_distr_errors = NULL;
  };

  delete[] d_mult_realizations;
  d_mult_realizations = NULL;
  delete[] d_mult_K_realizations;
  d_mult_K_realizations = NULL;

  delete[] lambda_mult;
  lambda_mult = NULL;
  delete[] lambda_mult_error;
  lambda_mult_error = NULL;
  ...
  delete[] a_I_mult;
  a_I_mult = NULL;
  delete[] a_I_mult_error;
  a_I_mult_error = NULL;
  delete[] a_J_mult;
  a_J_mult = NULL;
  delete[] a_J_mult_error;
  a_J_mult_error = NULL;
  delete[] sigma_mult;
  sigma_mult = NULL;
  delete[] sigma_mult_error;
  sigma_mult_error = NULL;
  delete[] alpha_I_mult;
  alpha_I_mult = NULL;
  delete[] alpha_I_mult_error;
  alpha_I_mult_error = NULL;
  delete[] alpha_J_mult;
  alpha_J_mult = NULL;
  delete[] alpha_J_mult_error;
  alpha_J_mult_error = NULL;

  delete[] K_C_mult;
  K_C_mult = NULL;
  delete[] K_C_mult_error;
  K_C_mult_error = NULL;
...
void alp_sim::calculate_main_parameters2m(
    ...
    long int final_realizations_number_killing_,
    long int nalp_for_lambda_simulation, long int level,
    bool &inside_simulation_flag, double &lambda, double &lambda_error,
    double &test_difference, double &test_difference_error, double &C,
    double &C_error, double &K_C, double &K_C_error, double &a_I,
    double &a_I_error, double &a_J, double &a_J_error, double &sigma,
    double &sigma_error, double &alpha_I, double &alpha_I_error,
    double &alpha_J, double &alpha_J_error, double &K, double &K_error,
    bool &flag_) {
  ...
  try {
    ...
    if (final_realizations_number_killing_ >
        final_realizations_number_lambda_) {
      throw error("Unexpected error\n", 4);
    };

    long int mult_number_lambda = get_number_of_subsimulations(d_n_alp_obj);
    long int mult_number_K =
        get_number_of_subsimulations(final_realizations_number_killing_);
    ...
    for (j = 0; j <= nalp_for_lambda_simulation; j++) {
      get_and_allocate_alp_distribution(0, d_n_alp_obj - 1, alp_distr,
                                        alp_distr_errors, j);
    };
    ...
    long int real_number = (long int)floor(
        (double)final_realizations_number_lambda_ / (double)d_mult_number);
    ...
    for (k = 1; k <= d_mult_number; k++) {
      ...
      long int j;
      for (j = 0; j <= nalp_for_lambda_simulation; j++) {
        get_and_allocate_alp_distribution(nr_tmp - d_mult_realizations[k],
                                          nr_tmp - 1, alp_mult_distr[k],
                                          alp_mult_distr_errors[k], j);
      };
    };
    ...
    for (k = 1; k <= d_mult_number; k++) {
      ...
      calculate_lambda(
          false, nalp_for_lambda_simulation, nalp_tmp, inside_simulation_flag,
          alp_mult_distr[k], alp_mult_distr_errors[k], lambda_mult[k],
          lambda_mult_error[k], test_difference, test_difference_error);
    ...
    calculate_lambda(false, nalp_for_lambda_simulation, nalp_tmp,
                     inside_simulation_flag, alp_distr, alp_distr_errors,
                     lambda, lambda_error, test_difference,
    ...
    for (k = 1; k <= d_mult_number; k++) {
      nr_tmp += d_mult_realizations[k];
      calculate_C(0, nalp_for_lambda_simulation, alp_mult_distr[k],
                  alp_mult_distr_errors[k], lambda_mult[k],
                  lambda_mult_error[k], C_mult[k], C_mult_error[k], Sc_mult[k],
                  Sc_mult_error[k]);
    ...
    calculate_C(0, nalp_for_lambda_simulation, alp_distr, alp_distr_errors,
                lambda, lambda_error, C, C_error, Sc, Sc_error);
    ...
    for (k = 1; k <= d_mult_number; k++) {
      nr_tmp += d_mult_realizations[k];
      calculate_FSC(nalp_for_lambda_simulation, nr_tmp - d_mult_realizations[k],
                    nr_tmp - 1, alp_mult_distr[k], lambda_mult[k], Sc_mult[k],
                    // Sc_mult_error[k],

                    a_I_mult[k], a_I_mult_error[k], a_J_mult[k],
                    a_J_mult_error[k], sigma_mult[k], sigma_mult_error[k],
                    alpha_I_mult[k], alpha_I_mult_error[k], alpha_J_mult[k],
    ...
    calculate_FSC(nalp_for_lambda_simulation, 0,
                  final_realizations_number_lambda_ - 1, alp_distr, lambda, Sc,
                  // Sc_error,

                  a_I, a_I_error, a_J, a_J_error, sigma, sigma_error, alpha_I,
                  alpha_I_error, alpha_J, alpha_J_error);
    ...
    real_number = (long int)floor((double)final_realizations_number_killing_ /
                                  (double)d_mult_number);
    ...
    for (k = 1; k <= d_mult_number; k++) {
      ...
      check_K_criterion_during_killing(
          nr_tmp - d_mult_K_realizations[k], nr_tmp - 1, lambda_mult[k],
          d_alp_data->d_eps_K, level, recommended_level, diff_opt, K_C_mult[k],
          K_C_mult_error[k]);

      K_mult[k] = C_mult[k] * K_C_mult[k];
      K_mult_error[k] = alp_data::error_of_the_product(
          C_mult[k], C_mult_error[k], K_C_mult[k], K_C_mult_error[k]);
    ...
    check_K_criterion_during_killing(
        0, final_realizations_number_killing_ - 1, lambda, d_alp_data->d_eps_K,
        level, recommended_level, diff_opt, K_C, K_C_error);
    ...
    mult_number_double_lambda =
        (double)final_realizations_number_lambda_ / (double)real_number;
    mult_number_double_K =
        (double)final_realizations_number_killing_ / (double)real_number;

    lambda_mult2_error = alp_reg::sqrt_for_errors(lambda_mult2_error -
                                                  lambda_mult2 * lambda_mult2) /
                         sqrt((double)mult_number_double_lambda);
    C_mult2_error =
        alp_reg::sqrt_for_errors(C_mult2_error - C_mult2 * C_mult2) /
        sqrt((double)mult_number_double_lambda);
    K_C_mult2_error =
        alp_reg::sqrt_for_errors(K_C_mult2_error - K_C_mult2 * K_C_mult2) /
        sqrt((double)mult_number_double_K);
    a_I_mult2_error =
        alp_reg::sqrt_for_errors(a_I_mult2_error - a_I_mult2 * a_I_mult2) /
        sqrt((double)mult_number_double_lambda);
    a_J_mult2_error =
        alp_reg::sqrt_for_errors(a_J_mult2_error - a_J_mult2 * a_J_mult2) /
        sqrt((double)mult_number_double_lambda);
    sigma_mult2_error = alp_reg::sqrt_for_errors(sigma_mult2_error -
                                                 sigma_mult2 * sigma_mult2) /
                        sqrt((double)mult_number_double_lambda);
    alpha_I_mult2_error =
        alp_reg::sqrt_for_errors(alpha_I_mult2_error -
                                 alpha_I_mult2 * alpha_I_mult2) /
        sqrt((double)mult_number_double_lambda);
    alpha_J_mult2_error =
        alp_reg::sqrt_for_errors(alpha_J_mult2_error -
                                 alpha_J_mult2 * alpha_J_mult2) /
        sqrt((double)mult_number_double_lambda);
    K_mult2_error =
        alp_reg::sqrt_for_errors(K_mult2_error - K_mult2 * K_mult2) /
        sqrt((double)alp_data::Tmin(mult_number_double_lambda,
                                    mult_number_double_K));

    error_in_calculate_main_parameters2m(lambda, lambda_error, lambda_mult2,
                                         lambda_mult2_error);

    error_in_calculate_main_parameters2m(C, C_error, C_mult2, C_mult2_error);

    error_in_calculate_main_parameters2m(K_C, K_C_error, K_C_mult2,
                                         K_C_mult2_error);

    error_in_calculate_main_parameters2m(a_I, a_I_error, a_I_mult2,
                                         a_I_mult2_error);

    error_in_calculate_main_parameters2m(a_J, a_J_error, a_J_mult2,
                                         a_J_mult2_error);

    error_in_calculate_main_parameters2m(sigma, sigma_error, sigma_mult2,
                                         sigma_mult2_error);

    error_in_calculate_main_parameters2m(alpha_I, alpha_I_error, alpha_I_mult2,
                                         alpha_I_mult2_error);

    error_in_calculate_main_parameters2m(alpha_J, alpha_J_error, alpha_J_mult2,
                                         alpha_J_mult2_error);

    error_in_calculate_main_parameters2m(K, K_error, K_mult2, K_mult2_error);
  ...
  label1:;
    ...
    memory_release_for_calculate_main_parameters2m(
        nalp_for_lambda_simulation, d_mult_realizations, d_mult_K_realizations,

        lambda_mult, lambda_mult_error,

        C_mult, C_mult_error,

        a_I_mult, a_I_mult_error,

        a_J_mult, a_J_mult_error,

        sigma_mult, sigma_mult_error,

        alpha_I_mult, alpha_I_mult_error,

        alpha_J_mult, alpha_J_mult_error,

        K_C_mult, K_C_mult_error,

        K_mult, K_mult_error,

        Sc_mult, Sc_mult_error,

        alp_distr, alp_distr_errors,

        alp_mult_distr, alp_mult_distr_errors);
    throw;
  };
...
void alp_sim::error_in_calculate_main_parameters2m(double C, double &C_error,
                                                   double C_mult2,
...
alp_sim::~alp_sim()  // destructor
{
  ...
  if (d_alp_obj) {
    for (i = 0; i < d_n_alp_obj; i++) {
      delete d_alp_obj->d_elem[i];
      d_alp_obj->d_elem[i] = NULL;
    };
...
void alp_sim::kill(bool check_time_, long int ind1_, long int ind2_,
                   long int M_min_, double lambda_, double eps_K_, double &K_C_,
                   double &K_C_error_, long int &level_, long int &diff_opt_) {
  bool flag = false;
  ...
  for (i = ind1_; i <= ind2_; i++) {
    ...
    if (i - ind1_ + 1 >
        alp_obj_tmp->d_alp_data->d_minimum_realizations_number) {
  ...
  do {
    ...
    for (i = ind1_; i <= ind2_; i++) {
      ...
      while (!flag) {
        ...
        if (!alp_obj_tmp->d_success) {
          ...
          //{
          // throw error("Error - you have exceeded the calculation time or
          // memory limit.\nThe error might indicate that the regime is linear or
          // too close to linear to permit efficient computation.\nPossible
          // solutions include changing the randomization seed, or increasing the
          // allowed calculation time and the memory limit.\n",3);
          //};
          delete alp_obj_tmp;
          alp_obj_tmp = NULL;
          ...
          while (!flag) {
            ...
            //	{
            // throw error("Error - you have exceeded the calculation time or
            // memory limit.\nThe error might indicate that the regime is linear
            // or too close to linear to permit efficient computation.\nPossible
            // solutions include changing the randomization seed, or increasing
            // the allowed calculation time and the memory limit.\n",3);
            //	};
            //};
    ...
    flag = check_K_criterion_during_killing(ind1_, ind2_, lambda_, eps_K_,
                                            current_level, recommended_level,
                                            diff_opt_, K_C_, K_C_error_);
...
void alp_sim::get_single_realization(bool check_time_, long int M_min_,
                                     long int nalp_, bool killing_flag_,
                                     long int level_, long int diff_opt_,
                                     alp *&obj_, bool &sucess_flag_,
                                     double &d_eps_) {
  if (!obj_) {
...
void alp_sim::quick_test(long int trials_number_, double max_time_) {
  if (trials_number_ <= 0) {
  ...
  long int score_diff =
      (long int)alp_data::round(-log(p_thres) / lambda_ungapped);

  long int i;

  long int max_number_of_unsuccessful_objects2 = (long int)floor(
      /*5+*/ 0.5 * trials_number_ *
      (d_alp_obj->d_alp_data->d_eps_K + d_alp_obj->d_alp_data->d_eps_lambda));
  ...
  for (i = 0; i < trials_number_; i++) {
    ...
    while (!success3) {
      ...
      if (!success3) {
        ...
        if (number_of_unsuccessful_objects2 >
            max_number_of_unsuccessful_objects2) {
          throw error(
              "Error - you have exceeded the calculation time or memory "
              "limit.\nThe error might indicate that the regime is linear or "
              "too close to linear to permit efficient computation.\nPossible "
              "solutions include changing the randomization seed, or "
              "increasing the allowed calculation time and the memory limit.\n",
              3);
        };
      };
    };
    ...
    alp_obj_tmp->d_sentinels_flag = false;
    alp_obj_tmp->kill_upto_level(last_alp, last_alp - score_diff,
                                 &M_upper_level);
    if (!alp_obj_tmp->d_success) {
      number_of_unsuccessful_objects2++;
      if (number_of_unsuccessful_objects2 >
...
void alp_sim::get_minimal_simulation(long int ind1_, long int ind2_,
                                     long int &M_min_, long int &nalp_,
  ...
  try {
    ...
    do {
          ...
          if (!alp_obj_tmp->d_success) {
            ...
            while (!success2) {
              ...
              if (!success2) {
                ...
                // throw error("Error - you have exceeded the calculation time
                // or memory limit.\nThe error might indicate that the regime is
                // linear or too close to linear to permit efficient
                // computation.\nPossible solutions include changing the
                // randomization seed, or increasing the allowed calculation time
      ...
      criterion_flag =
          the_criterion(alp_number, nalp_lambda_, 0, ind2_, alp_distr,
                        alp_distr_errors, M_min_, M_min_flag, nalp_flag,
                        inside_simulation_flag, C_calculation_, &lambda);
      ...
      if (!inside_simulation_flag) {
        ...
        if (alp_distr) {
          for (i = 1; i <= alp_number; i++) {
            delete (array_positive<double> *)alp_distr[i];
            alp_distr[i] = NULL;
          };
        ...
        if (alp_distr_errors) {
          for (i = 1; i <= alp_number; i++) {
            delete (array_positive<double> *)alp_distr_errors[i];
            alp_distr_errors[i] = NULL;
          };
      ...
      if (criterion_flag) {
        ...
        if (criterion_flag) {
          criterion_flag = check_K_criterion(alp_number, ind1_, ind2_, lambda,
                                             d_alp_data->d_eps_K, M_min_);
        };
    ...
    memory_release_for_get_minimal_simulation(nalp_, alp_distr,
                                              alp_distr_errors);

    throw;
  };

  memory_release_for_get_minimal_simulation(nalp_, alp_distr, alp_distr_errors);
}

void alp_sim::memory_release_for_get_minimal_simulation(
    long int nalp_, void **&alp_distr, void **&alp_distr_errors) {
  // memory release
  if (alp_distr) {
    long int i;
    for (i = 1; i <= nalp_; i++) {
...
bool alp_sim::the_criterion(  // criteria of stopping of the simulating ALP
    ...
    // sets the flags M_min_flag_ and nalp_flag_ checking the corresponding
    // condition
    long int upto_nalp_, long int &nalp_for_lambda_simulation_, long int ind1_,
    long int ind2_, void **&alp_distr, void **&alp_distr_errors,
    long int &M_min_, bool &M_min_flag_, bool &nalp_flag_,
    bool &inside_simulation_flag_, bool C_calculation_, double *lambda_,
  ...
  get_and_allocate_alp_distribution(ind1_, ind2_, alp_distr, alp_distr_errors,
                                    nalp);

  calculate_lambda(true, upto_nalp_, nalp_for_lambda_simulation_,
                   inside_simulation_flag_, alp_distr, alp_distr_errors, lambda,
                   lambda_error, test_difference, test_difference_error);
  ...
  if (C_calculation_) {
    ...
    calculate_C(0, upto_nalp_, alp_distr, alp_distr_errors, lambda,
                lambda_error, C, C_error, Sc, Sc_error);
...
double alp_sim::lambda_exp(long int &i_, double *&exp_array_) {
  if (exp_array_[i_] == -1) {
    throw error(
        "The program is not able to calculate the parameters; rescaling "
        "penalties and scoring matrix might help\n",
        3);
  };
...
void alp_sim::memory_release_for_calculate_FSC(
    ...
    double *&delta_E, double *&delta_E_error,

    double *&delta_E_E, double *&delta_E_E_error,

    double *&delta_I, double *&delta_I_error,

    double *&delta_J, double *&delta_J_error,

    double *&delta_I_I, double *&delta_I_I_error,

    double *&delta_I_J, double *&delta_I_J_error,

    double *&delta_J_J, double *&delta_J_J_error,

    double *&cov_J_J, double *&cov_J_J_error,

    double *&cov_I_J, double *&cov_I_J_error,

    double *&cov_I_I, double *&cov_I_I_error,

    double *&cov_E_E, double *&cov_E_E_error) {
  // memory release
  delete[] exp_array;
  exp_array = NULL;
  ...
  delete[] delta_E;
  delta_E = NULL;
  delete[] delta_E_error;
  delta_E_error = NULL;
  delete[] delta_E_E;
  delta_E_E = NULL;
  delete[] delta_E_E_error;
  delta_E_E_error = NULL;

  delete[] delta_I;
  delta_I = NULL;
  delete[] delta_I_error;
  delta_I_error = NULL;
  delete[] delta_J;
  delta_J = NULL;
  delete[] delta_J_error;
  delta_J_error = NULL;

  delete[] delta_I_J;
  delta_I_J = NULL;
  delete[] delta_I_J_error;
  delta_I_J_error = NULL;
  delete[] delta_J_J;
  delta_J_J = NULL;
  delete[] delta_J_J_error;
  delta_J_J_error = NULL;
  delete[] delta_I_I;
  delta_I_I = NULL;
  delete[] delta_I_I_error;
  delta_I_I_error = NULL;

  delete[] cov_I_J;
  cov_I_J = NULL;
  delete[] cov_I_J_error;
  cov_I_J_error = NULL;
  delete[] cov_J_J;
  cov_J_J = NULL;
  delete[] cov_J_J_error;
  cov_J_J_error = NULL;
  delete[] cov_I_I;
  cov_I_I = NULL;
  delete[] cov_I_I_error;
  cov_I_I_error = NULL;

  delete[] cov_E_E;
  cov_E_E = NULL;
  delete[] cov_E_E_error;
  cov_E_E_error = NULL;
}

void alp_sim::calculate_FSC(long int nalp_, long int ind1_, long int ind2_,
                            void **alp_distr, double lambda_, double Sc_,
                            // double Sc_error_,
                            double &a_I_, double &a_I_error_, double &a_J_,
                            double &a_J_error_, double &sigma_,
                            double &sigma_error_, double &alpha_I_,
                            double &alpha_I_error_, double &alpha_J_,
  ...
  try {
    ...
    for (i = ind1_; i <= ind2_; i++) {
      ...
      for (j = 1; j <= nalp_; j++) {
        ...
        double delta_E_E_tmp =
            (E_j - E_j_1) * (E_j - E_j_1) * exp_tmp * weight_j;
    ...
    for (j = 0; j < nalp_; j++) {
      ...
      cov_I_J_error[j] = alp_data::error_of_the_product(
          delta_I[j], delta_I_error[j], delta_J[j], delta_J_error[j]);
      cov_I_J_error[j] = alp_reg::sqrt_for_errors(
          delta_I_J_error[j] + cov_I_J_error[j] * cov_I_J_error[j]);

      cov_I_I_error[j] = alp_data::error_of_the_product(
          delta_I[j], delta_I_error[j], delta_I[j], delta_I_error[j]);
      cov_I_I_error[j] = alp_reg::sqrt_for_errors(
          delta_I_I_error[j] + cov_I_I_error[j] * cov_I_I_error[j]);

      cov_J_J_error[j] = alp_data::error_of_the_product(
          delta_J[j], delta_J_error[j], delta_J[j], delta_J_error[j]);
      cov_J_J_error[j] = alp_reg::sqrt_for_errors(
          delta_J_J_error[j] + cov_J_J_error[j] * cov_J_J_error[j]);

      cov_E_E_error[j] = alp_data::error_of_the_product(
          delta_E[j], delta_E_error[j], delta_E[j], delta_E_error[j]);
      cov_E_E_error[j] = alp_reg::sqrt_for_errors(
          delta_E_E_error[j] + cov_E_E_error[j] * cov_E_E_error[j]);
    };
    ...
    alp_reg::robust_regression_sum_with_cut_LSM_beta1_is_defined(
        0, number_of_elements, delta_I, delta_I_error, cut_left_tail,
        cut_right_tail, y, delta_I_aver, beta1, delta_I_aver_error, beta1_error,
        k1_opt, k2_opt, res_was_calculated);
    ...
    alp_reg::robust_regression_sum_with_cut_LSM_beta1_is_defined(
        0, number_of_elements, delta_J, delta_J_error, cut_left_tail,
        cut_right_tail, y, delta_J_aver, beta1, delta_J_aver_error, beta1_error,
        k1_opt, k2_opt, res_was_calculated);
    ...
    alp_reg::robust_regression_sum_with_cut_LSM_beta1_is_defined(
        0, number_of_elements, delta_E, delta_E_error, cut_left_tail,
        cut_right_tail, y, delta_E_aver, beta1, delta_E_aver_error, beta1_error,
        k1_opt, k2_opt, res_was_calculated);
    ...
    alp_reg::robust_regression_sum_with_cut_LSM_beta1_is_defined(
        0, number_of_elements, cov_I_J, cov_I_J_error, cut_left_tail,
        cut_right_tail, y, cov_I_J_aver, beta1, cov_I_J_aver_error, beta1_error,
        k1_opt, k2_opt, res_was_calculated);
    ...
    alp_reg::robust_regression_sum_with_cut_LSM_beta1_is_defined(
        0, number_of_elements, cov_I_I, cov_I_I_error, cut_left_tail,
        cut_right_tail, y, cov_I_I_aver, beta1, cov_I_I_aver_error, beta1_error,
        k1_opt, k2_opt, res_was_calculated);
    ...
    alp_reg::robust_regression_sum_with_cut_LSM_beta1_is_defined(
        0, number_of_elements, cov_J_J, cov_J_J_error, cut_left_tail,
        cut_right_tail, y, cov_J_J_aver, beta1, cov_J_J_aver_error, beta1_error,
        k1_opt, k2_opt, res_was_calculated);
    ...
    alp_reg::robust_regression_sum_with_cut_LSM_beta1_is_defined(
        0, number_of_elements, cov_E_E, cov_E_E_error, cut_left_tail,
        cut_right_tail, y, cov_E_E_aver, beta1, cov_E_E_aver_error, beta1_error,
        k1_opt, k2_opt, res_was_calculated);
    ...
    a_I_error_ = alp_data::error_of_the_ratio(delta_I_aver, delta_I_aver_error,
                                              delta_E_aver, delta_E_aver_error);
    a_J_ = delta_J_aver / delta_E_aver;
    a_J_error_ = alp_data::error_of_the_ratio(delta_J_aver, delta_J_aver_error,
                                              delta_E_aver, delta_E_aver_error);

    sigma_calculation(delta_I_aver, delta_I_aver_error, delta_J_aver,
                      delta_J_aver_error, delta_E_aver, delta_E_aver_error,
                      cov_E_E_aver, cov_E_E_aver_error, cov_I_J_aver,
                      cov_I_J_aver_error, sigma_, sigma_error_);

    sigma_calculation(delta_I_aver, delta_I_aver_error, delta_I_aver,
                      delta_I_aver_error, delta_E_aver, delta_E_aver_error,
                      cov_E_E_aver, cov_E_E_aver_error, cov_I_I_aver,
                      cov_I_I_aver_error, alpha_I_, alpha_I_error_);

    sigma_calculation(delta_J_aver, delta_J_aver_error, delta_J_aver,
                      delta_J_aver_error, delta_E_aver, delta_E_aver_error,
                      cov_E_E_aver, cov_E_E_aver_error, cov_J_J_aver,
                      cov_J_J_aver_error, alpha_J_, alpha_J_error_);
    ...
    memory_release_for_calculate_FSC(exp_array,

                                     delta_E, delta_E_error,

                                     delta_E_E, delta_E_E_error,

                                     delta_I, delta_I_error,

                                     delta_J, delta_J_error,

                                     delta_I_I, delta_I_I_error,

                                     delta_I_J, delta_I_J_error,

                                     delta_J_J, delta_J_J_error,

                                     cov_J_J, cov_J_J_error,

                                     cov_I_J, cov_I_J_error,

                                     cov_I_I, cov_I_I_error,

                                     cov_E_E, cov_E_E_error);
    throw;
  };
...
void alp_sim::sigma_calculation(
    double delta_I_aver_, double delta_I_aver_error_, double delta_J_aver_,
    double delta_J_aver_error_, double delta_E_aver_,
    double delta_E_aver_error_, double cov_E_E_aver_,
    double cov_E_E_aver_error_, double cov_I_J_aver_,
    double cov_I_J_aver_error_, double &sigma_, double &sigma_error_) {
  ...
  double nom1_sigma_error = alp_data::error_of_the_product(
      delta_I_aver_, delta_I_aver_error_, delta_J_aver_, delta_J_aver_error_);
  nom1_sigma_error = alp_data::error_of_the_product(
      nom1_1, nom1_sigma_error, cov_E_E_aver_, cov_E_E_aver_error_);

  double nom2_sigma_error_2 = alp_data::error_of_the_product(
      delta_E_aver_, delta_E_aver_error_, delta_E_aver_, delta_E_aver_error_);
  double nom2_sigma_error = alp_data::error_of_the_product(
      nom2_2, nom2_sigma_error_2, cov_I_J_aver_, cov_I_J_aver_error_);

  double den_sigma_error = alp_data::error_of_the_product(
      nom2_2, nom2_sigma_error_2, delta_E_aver_, delta_E_aver_error_);

  double nom_sigma_error =
      alp_data::error_of_the_sum(nom1_sigma_error, nom2_sigma_error);

  sigma_error_ = alp_data::error_of_the_ratio(nom1 + nom2, nom_sigma_error, den,
                                              den_sigma_error);
}

void alp_sim::calculate_C(long int starting_point, long int nalp_,
                          void **alp_distr, void **alp_distr_errors,
                          double lambda_, double lambda_error_, double &C_,
                          double &C_error_, double &Sc_, double &Sc_error_) {
  double *P = NULL;
  ...
  try {
    ...
    for (j = 1; j <= total_number_of_ALP; j++) {
      ...
      array_positive<double> *tmp_errors =
          ((array_positive<double> *)alp_distr_errors[j]);
    ...
    for (j = 0; j < total_number_of_ALP; j++) {
      ...
      errors_P_ratio[j] = alp_data::error_of_the_ratio(
          P[j + 1], P_errors[j + 1], P[j], P_errors[j]);
    };
    ...
    alp_reg::robust_regression_sum_with_cut_LSM_beta1_is_defined(
        0, number_of_elements - starting_point, values_P_ratio + starting_point,
        errors_P_ratio + starting_point, cut_left_tail, cut_right_tail, y,
        P_beta_inf, beta1, P_beta_inf_error, beta1_error, k1_opt, k2_opt,
        res_was_calculated);
    ...
    if (total_number_of_ALP == 1) {
      ...
      alp_reg::robust_regression_sum_with_cut_LSM_beta1_is_defined(
          0, number_of_elements - starting_point, E + 1 + starting_point,
          E_errors + 1 + starting_point, cut_left_tail, cut_right_tail, y,
          E_aver, beta1, E_aver_error, beta1_error, k1_opt, k2_opt,
          res_was_calculated);
      ...
      alp_reg::robust_regression_sum_with_cut_LSM(
          0, number_of_elements - starting_point, E_T_beta + 1 + starting_point,
          E_T_beta_errors + 1 + starting_point, cut_left_tail, cut_right_tail,
          y, beta0, beta1, beta0_error, beta1_error, k1_opt, k2_opt,
          res_was_calculated);
    ...
    double den_error = alp_data::error_of_the_product(
        E_T_beta_diff_aver, E_T_beta_diff_aver_error, exp_lambda,
        exp_lambda_error);
    ...
    if (calculate_C_S_constant_flag) {
      ...
      nom_error = alp_data::error_of_the_product(P_beta_inf, P_beta_inf_error,
                                                 E_aver, E_aver_error);
      nom = P_beta_inf * E_aver;
    } else {
      double E_aver_sqr_error = alp_data::error_of_the_product(
          E_aver, E_aver_error, E_aver, E_aver_error);
      double E_aver_sqr = E_aver * E_aver;

      nom_error = alp_data::error_of_the_product(P_beta_inf, P_beta_inf_error,
                                                 E_aver_sqr, E_aver_sqr_error);
    ...
    delete[] values_P_ratio;
    values_P_ratio = NULL;
    delete[] errors_P_ratio;
    errors_P_ratio = NULL;
    ...
    delete[] E_errors;
    E_errors = NULL;
    delete[] E_T_beta_errors;
    E_T_beta_errors = NULL;
    throw;
  };
...
void alp_sim::get_and_allocate_alp_distribution(long int ind1_, long int ind2_,
                                                void **&alp_distr,
  ...
  try {
    ...
    alp_distr = alp_distr_tmp;
    alp_distr_tmp = NULL;
    alp_distr_errors = alp_distr_errors_tmp;
    alp_distr_errors_tmp = NULL;
    ...
    for (i = ind1_; i <= ind2_; i++) {
      ...
      ((array_positive<double> *)alp_distr[k])
          ->increase_elem_by_x(alp_tmp, weight_tmp);
      ((array_positive<double> *)alp_distr_errors[k])
          ->increase_elem_by_x(alp_tmp, weight_tmp * weight_tmp);
    };
    ...
    if (alp_distr_tmp) {
      for (i = 0; i <= allocation_dim_tmp; i++) {
        delete (array_positive<double> *)alp_distr_tmp[i];
        alp_distr_tmp[i] = NULL;
      };
      delete[] alp_distr_tmp;
      alp_distr_tmp = NULL;
    };

    if (alp_distr_errors_tmp) {
      for (i = 0; i <= allocation_dim_tmp; i++) {
        delete (array_positive<double> *)alp_distr_errors_tmp[i];
        alp_distr_errors_tmp[i] = NULL;
      };
      delete[] alp_distr_errors_tmp;
      alp_distr_errors_tmp = NULL;
    };
...
bool alp_sim::check_K_criterion(long int nalp_, long int ind1_, long int ind2_,
                                double lambda_, double eps_K_,
                                long int &M_min_) {
  if (nalp_ <= 0) {
    throw error("Unexpected error\n", 4);
  };
  ...
  try {
    ...
    for (i = ind1_; i <= ind2_; i++) {
      ...
      long int k;
      for (k = cells_counts->d_ind0;
           k <= alp_data::Tmin(alp_tmp, cells_counts->d_dim_plus_d_ind0); k++) {
        diff->increase_elem_by_x(
            alp_tmp - k,
            cells_counts->d_elem[k - cells_counts->d_ind0] * weight_tmp);
      };
    };
...
bool alp_sim::check_K_criterion_during_killing(
    long int ind1_, long int ind2_, double lambda_, double eps_K_,
    long int current_level_, long int &recommended_level_, long int &diff_opt_,
    double &K_C_, double &K_C_error_) {
  if (ind1_ > ind2_) {
    throw error("Unexpected error\n", 4);
  };
  ...
  try {
    ...
    for (i = ind1_; i <= ind2_; i++) {
      ...
      double &weight_tmp =
          alp_obj_tmp->d_alp_weights->d_elem[alp_obj_tmp->d_nalp_killing];
      sum_of_weights += weight_tmp;
    ...
    K_C_error_ = alp_data::error_of_the_ratio(
        sum_of_weights, sum_of_weights_error, den, den_error);
...
void alp_sim::calculate_lambda(bool check_the_criteria_, long int nalp_,
                               long int &nalp_thr_,
                               bool &inside_simulation_flag_, void **alp_distr,
                               void **alp_distr_errors, double &lambda_,
                               double &lambda_error_, double &test_difference_,
  ...
  double f1 = func(
      lambda_,
      func_pointer);  // cout<<ind1_<<"\t"<<ind2_<<"\t"<<tmp_struct.d_last_sum<<"\t"<<tmp_struct.d_last_sum_error<<endl;
...
double alp_sim::get_root(const std::vector<double> &res_tmp_, double point_) {
  if (res_tmp_.size() == 0) {
    // throw error("Error in alp_sim::get_root - the equation does not have
    // roots\n",2);
    throw error(
        "Error - you have exceeded the calculation time or memory limit.\nThe "
        "error might indicate that the regime is linear or too close to linear "
...
double alp_sim::function_for_lambda_calculation(double lambda_, void *data_) {
  double *expect = NULL;
  double *expect_errors = NULL;

  try {
    struct_for_lambda_calculation *tmp_struct =
        (struct_for_lambda_calculation *)data_;
    ...
    alp_reg::robust_regression_sum_with_cut_LSM(
        min_length, number_of_elements, expect, expect_errors, cut_left_tail,
        cut_right_tail, y, beta0, beta1, beta0_error, beta1_error, k1_opt,
        k2_opt, res_was_calculated);
