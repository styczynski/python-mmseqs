...
struct ALP_set_of_parameters {
  ...
  bool d_params_flag;  // if true, then the parameters are defined and P-values
                       // can be calculated
...
class pvalues {
 ...
 public:
  static void get_appr_tail_prob_with_cov(const ALP_set_of_parameters &par_,
                                          ...
                                          double &area_, bool &area_is_1_flag_);
  ...
  static void get_appr_tail_prob_with_cov_without_errors(
      ...
      double &area_, bool &area_is_1_flag_, bool compute_only_area_ = false);
 ...
 public:
  static void compute_intercepts(ALP_set_of_parameters &par_);

  void calculate_P_values(long int Score1, long int Score2, double Seq1Len,
                          double Seq2Len,
  ...
  void calculate_P_values(double Score, double Seq1Len, double Seq2Len,
                          const ALP_set_of_parameters &ParametersSet,
  ...
  static inline double standard_normal()  // generates standard normal random
                                          // value using the Box–Muller transform
  {
    double r1 = 0;
    while (r1 == 0) {
      r1 = ran3();
  ...
  static  // returns "true" if the Gumbel parameters are properly defined and
          // "false" otherwise
      bool
      assert_Gumbel_parameters(
