...
Format IoUtil::getFormat() { return format1; }

void IoUtil::setFormat(Format format_) { format1 = format_; }

Format IoUtil::clearFormat() { return format1 = FORMAT; }

char IoUtil::getTerminator() { return terminator; }

void IoUtil::setTerminator(char t_) { terminator = t_; }

char IoUtil::clearTerminator() { return terminator = TERMINATOR; }

void IoUtil::abort() { exit(1); }
...
{  // behaves like getline (in_, str_) but throws away the string after first
  ...
  while (getline(in_, str_)) {
    for (pbuf = str_.c_str(); *pbuf != '\0' && isspace(*pbuf); pbuf++)
...
std::istream &IoUtil::in(std::istream &in_, double &x_) {
  string s;
  in_ >> s;

  for (string::iterator i = s.begin(); i != s.end(); i++)
    *i = /*sls added (char)*/ (char)tolower(*i);
