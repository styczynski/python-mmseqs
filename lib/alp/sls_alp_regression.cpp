...
//-------------------------Solution of arbitrary
//equations----------------------------
void alp_reg::find_tetta_general(
    function_type *func_, void *func_pointer_,
    double a_,  //[a,b] is the interval for search of equation solution
    double b_, long int n_partition_, double eps_, std::vector<double> &res_) {
  res_.resize(0);
  ...
  for (i = 0; i < (long int)intervals.size(); i++) {
    double sol = find_single_tetta_general(
        func_, func_pointer_,
        a_ + intervals[i] *
                 h,  //[a,b] is the interval for search of equation solution
        a_ + (1 + intervals[i]) * h, eps_);
    res_.push_back(sol);
  };
...
void alp_reg::correction_of_errors(double *errors_,
  ...
  for (i = 0; i < number_of_elements_; i++) {
    if (errors_[i] < 0) {
      throw error(
          "Error in alp_reg::correction_of_errors: input error in the "
          "regression model is less than 0\n",
          4);
    };
...
void alp_reg::robust_regression_sum_with_cut_LSM(
    long int min_length_, long int number_of_elements_, double *values_,
    double *errors_, bool cut_left_tail_, bool cut_right_tail_, double y_,
    double &beta0_, double &beta1_, double &beta0_error_, double &beta1_error_,
    long int &k1_opt_, long int &k2_opt_, bool &res_was_calculated_) {
  ...
  for (k1 = k1_start; k1 <= k1_end; k1++) {
    for (k2 = sls_basic::Tmax(k1 + (long int)1,
                              sls_basic::Tmax(k1, k2_start) + min_length_);
         k2 <= k2_end; k2++) {
      double beta0_opt_tmp, beta1_opt_tmp, beta0_opt_error_tmp,
          beta1_opt_error_tmp;
      bool res_was_calculated;

      double tmp = function_for_robust_regression_sum_with_cut_LSM(
          values_ + k1, errors_ + k1, k2 - k1 + 1, k1, c, beta0_opt_tmp,
          beta1_opt_tmp, beta0_opt_error_tmp, beta1_opt_error_tmp,
          res_was_calculated);
...
double alp_reg::function_for_robust_regression_sum_with_cut_LSM(
    double *values_, double *errors_, long int number_of_elements_,
    long int k_start_, double c_, double &beta0_, double &beta1_,
    double &beta0_error_, double &beta1_error_, bool &res_was_calculated_) {
  long int i;
  double a11 = 0;
  double a12 = 0;
  double a21 = 0;
  ...
  for (i = 0; i < number_of_elements_; i++) {
    if (errors_[i] != 0) {
      ...
      y2_error += (double)(k_start_ + i) * (double)(k_start_ + i) * tmp * tmp *
                  errors_[i] * errors_[i];
    };
  };
  ...
  beta0_error_ =
      sqrt(y1_error * y1_error * a22 * a22 + a12 * a12 * y2_error * y2_error) /
      den;
  beta1_error_ =
      sqrt(a11 * a11 * y2_error * y2_error + a21 * a21 * y1_error * y1_error) /
      den;
...
alp_reg::function_for_robust_regression_sum_with_cut_LSM_beta1_is_defined(
  ...
  for (i = 0; i < number_of_elements_; i++) {
    if (errors_[i] != 0) {
      ...
      double error_tmp = errors_[i] * errors_[i] +
                         (double)(k_start_ + i) * (double)(k_start_ + i) *
...
double alp_reg::median(long int dim_, double *array_) {
...
double alp_reg::robust_sum(double *values, long int dim, long int N_points,
                           bool *&remove_flag) {
  remove_flag = NULL;
  ...
  try {
    ...
    double med_val = alp_reg::median(dim, values);
