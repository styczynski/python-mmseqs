...
void LocalMaxStat::copy(
    ...
    double
        muAssoc_,  // n_step mean for associated random walk (relative entropy)
...
void LocalMaxStat::copy(size_t dimension_,  // #(distinct values) of scores &
                        ...
                        const double *prob_)     // corresponding probabilities
{
  ...
  for (i = 0; i < dimension_; i++) {
    d_sigma += static_cast<double>(score_[i]) * static_cast<double>(score_[i]) *
               prob_[i];
  }
  ...
  d_muAssoc = LocalMaxStatUtil::muAssoc(getDimension(), getScore(), getProb(),
                                        getLambda());
  d_sigmaAssoc = 0.0;

  for (i = 0; i < getDimension(); i++) {
    d_sigmaAssoc += static_cast<double>(getScore()[i]) *
                    static_cast<double>(getScore()[i]) * getProb()[i] *
                    exp(getLambda() * static_cast<double>(getScore()[i]));
  }

  d_sigmaAssoc -= getMuAssoc() * getMuAssoc();
  d_sigmaAssoc = Function::psqrt(d_sigmaAssoc);

  d_thetaMin = LocalMaxStatUtil::thetaMin(getDimension(), getScore(), getProb(),
                                          getLambda());
  d_rMin = LocalMaxStatUtil::rMin(getDimension(), getScore(), getProb(),
                                  getLambda(), getThetaMin());

  d_delta = LocalMaxStatUtil::delta(getDimension(), getScore());
  d_thetaMinusDelta = LocalMaxStatUtil::thetaMinusDelta(
      getLambda(), getDimension(), getScore());
...
// expected renewal length for weak ladder epochs
{
  ...
  LocalMaxStatUtil::descendingLadderEpoch(
      getDimension(), getScore(), getProb(), &eSumAlpha_,
      &eOneMinusExpSumAlpha_, false, getLambda(), getMu(), getMuAssoc(),
      getThetaMin(), getRMin(), getTime(), &d_terminated);
