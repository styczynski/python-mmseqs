...
std::ostream &operator<<(std::ostream &s_, const AlignmentEvaluer &g_) {
  if (!pvalues::assert_Gumbel_parameters(g_.d_params) || !g_.isGood()) {
    throw error(
        "Error - the Gumbel parameters are not defined properly in the "
...
// Read the parameters:
std::istream &operator>>(std::istream &s_, AlignmentEvaluer &g_) {
  try {
...
void AlignmentEvaluer::assert_Gapless_input_parameters(
    ...
    double *&letterFreqs1_normalized_,  // normalized background frequencies of
                                        // letters in sequence #1
    double *&letterFreqs2_normalized_,  // normalized background frequencies of
                                        // letters in sequence #2
    const string function_name_)  //"assert_Gapless_input_parameters" is called
                                  //from "function_name_" function
{
  if (!(alphabetSize_ > 0)) {
    d_params.d_params_flag = false;
    throw error("Error - the parameter \"alphabetSize_\" in the function \"" +
                    function_name_ + "\" must be positive\n",
                1);
  };
  ...
  for (i = 0; i < alphabetSize_; i++) {
    if (letterFreqs1_[i] < 0) {
      d_params.d_params_flag = false;
      throw error("Error - the value \"letterFreqs1_[" +
                      alp_data::long_to_string(i) + "]\" in the function \"" +
                      function_name_ + "\" must be non-negative\n",
                  1);
    };
  ...
  if (sum1 <= 0) {
    throw error(
        "Error - sum of the frequencies \"letterFreqs1_\" is non-positive in "
        "the function \"" +
            function_name_ + "\"\n",
        1);
  };
  ...
  for (i = 0; i < alphabetSize_; i++) {
    if (letterFreqs2_[i] < 0) {
      d_params.d_params_flag = false;
      throw error("Error - the value \"letterFreqs2_[" +
                      alp_data::long_to_string(i) + "]\" in the function \"" +
                      function_name_ + "\" must be non-negative\n",
                  1);
    };
  ...
  if (sum2 <= 0) {
    throw error(
        "Error - sum of the frequencies \"letterFreqs2_\" is non-positive in "
        "the function \"" +
            function_name_ + "\"\n",
        1);
  };
...
void AlignmentEvaluer::initGapless(
    long alphabetSize_,  // a number of letters in the alphabet
    ...
    double max_time_)    // maximum allowed calculation time in seconds
{
  try {
    ...
    double *letterFreqs1_normalized =
        NULL;  // normalized background frequencies of letters in sequence #1
    double *letterFreqs2_normalized =
        NULL;  // normalized background frequencies of letters in sequence #2

    assert_Gapless_input_parameters(
        alphabetSize_,  // a number of letters in the alphabet
        letterFreqs1_,  // background frequencies of letters in sequence #1
        letterFreqs2_,  // background frequencies of letters in sequence #2
        letterFreqs1_normalized,  // normalized background frequencies of
                                  // letters in sequence #1
        letterFreqs2_normalized,  // normalized background frequencies of
                                  // letters in sequence #2
        function_name);  //"assert_Gapless_input_parameters" is called from
    ...
    Njn::LocalMaxStatMatrix local_max_stat_matrix(
        alphabetSize_, substitutionScoreMatrix_, letterFreqs1_normalized,
        letterFreqs2_normalized, alphabetSize_, max_time_);
    ...
    d_params.a = (d_params.a_I + d_params.a_J) * 0.5;

    d_params.a_error = (d_params.a_I_error + d_params.a_J_error) * 0.5;

    d_params.alpha = (d_params.alpha_I + d_params.alpha_J) * 0.5;

    d_params.alpha_error =
        (d_params.alpha_I_error + d_params.alpha_J_error) * 0.5;
    ...
    if (!pvalues::assert_Gumbel_parameters(d_params) || !isGood()) {
      d_params.d_params_flag = false;
      throw error(
          "Error - computation of the Gumbel parameters is unsuccessful in the "
          "function \"void AlignmentEvaluer::initGapless\"\n",
          1);
    };
...
// For example, a gap of length k is penalized as gapOpen1_+k*gapEpen1_ for
// sequence #1 if max_time_<=0 then d_gapped_computation_parameters will be used;
// d_gapped_computation_parameters.d_parameters_flag must be true in this case
// every execution of initGapped with max_time_>0 rewrites
// d_gapped_computation_parameters by the actual values
void AlignmentEvaluer::initGapped(
    long alphabetSize_,  // a number of letters in the alphabet
    const long *const *substitutionScoreMatrix_,  // scoring matrix
    const double
        *letterFreqs1_,  // background frequencies of letters in sequence #1
    ...
    bool insertions_after_deletions_,  // if true, then insertions after
  ...
  try {
    ...
    if (!(gapEpen1_ > 0)) {
      d_params.d_params_flag = false;
      throw error("Error - the parameter \"gapEpen1_\" in the function \"" +
                      function_name + "\" must be positive\n",
                  1);
    };

    if (!(gapEpen2_ > 0)) {
      d_params.d_params_flag = false;
      throw error("Error - the parameter \"gapEpen2_\" in the function \"" +
                      function_name + "\" must be positive\n",
                  1);
    };
    ...
    if (!(eps_lambda_ > 0)) {
      d_params.d_params_flag = false;
      throw error("Error - the parameter \"eps_lambda_\" in the function \"" +
                      function_name + "\" must be positive\n",
                  1);
    };
    ...
    if (!(eps_K_ > 0)) {
      d_params.d_params_flag = false;
      throw error("Error - the parameter \"eps_K_\" in the function \"" +
                      function_name + "\" must be positive\n",
                  1);
    };
    ...
    if (!(max_mem_ > 0)) {
      d_params.d_params_flag = false;
      throw error("Error - the parameter \"max_mem_\" in the function \"" +
                      function_name + "\" must be positive\n",
                  1);
    };
    ...
    if (max_time_ <= 0) {
      GaplessCalculationTime =
          120;  // the time is set to 120 seconds if not set as an input
    };
    ...
    Njn::LocalMaxStatMatrix local_max_stat_matrix(
        alphabetSize_, substitutionScoreMatrix_, letterFreqs1_normalized,
        letterFreqs2_normalized, alphabetSize_, GaplessCalculationTime);
    ...
    double GaplessPreliminaryTime =
        CurrentTimeGaplessPreliminary - CurrentTime1;
    ...
    long int gapEpen = alp_data::Tmin(gapEpen1_, gapEpen2_);
    long int gapOpen =
        alp_data::Tmin(gapOpen1_ + gapEpen1_, gapOpen2_ + gapEpen2_) - gapEpen;
    ...
    if (max_time_ <= 0) {
      if (d_gapped_computation_parameters.d_parameters_flag) {
        ...
        throw error(
            "Error - d_gapped_computation_parameters must be defined before "
            "calling AlignmentEvaluer::initGapped with max_time_<=0\n",
            1);
      };
    };
    ...
    Sls::alp_data data_obj(        // constructor
        ...
        randomization_parameters,  // if not NULL, sets d_rand_flag to true and
                                   // initializes d_rand_all
        ...
        insertions_after_deletions_,  // if true, then insertions after
                                      // deletions are allowed
        d_gapped_computation_parameters
            .d_max_time_for_quick_tests,  // maximum allowed calculation time in
                                          // seconds for quick tests
        d_gapped_computation_parameters
            .d_max_time_with_computation_parameters);  // maximum allowed time
                                                       // in seconds for the
                                                       // whole computation

    data_obj.d_max_time =
        Sls::alp_data::Tmax((1.0 - GaplessTimePortion) * data_obj.d_max_time,
                            data_obj.d_max_time - GaplessPreliminaryTime);
    ...
    Sls::alp_data::get_current_time(CurrentTime2);
    d_params.m_CalcTime = CurrentTime2 - CurrentTime1;
    delete randomization_parameters;
    randomization_parameters = NULL;

    if (!pvalues::assert_Gumbel_parameters(d_params) || !isGood()) {
      d_params.d_params_flag = false;
      throw error(
          "Error - computation of the Gumbel parameters is unsuccessful in the "
          "function \"void AlignmentEvaluer::initGapped\"\n",
          1);
    };
...
void AlignmentEvaluer::initParameters(
  ...
  try {
    ...
    d_params.d_params_flag = true;  // if true, then the parameters are defined
    ...
    if (!pvalues::assert_Gumbel_parameters(d_params) || !isGood()) {
      ...
      throw error(
          "Error - computation of the Gumbel parameters is unsuccessful in the "
          "function \"void AlignmentEvaluer::initParameters\"\n",
          1);
    };
  } catch (...) {
    d_params.d_params_flag = false;
...
void AlignmentEvaluer::initParameters(
  ...
  try {
    ...
    d_params.alpha = 0.5 * (parameters_.d_alpha1 + parameters_.d_alpha2);
    d_params.alpha_error =
        0.5 * (parameters_.d_alpha1_error + parameters_.d_alpha2_error);
    ...
    for (i = 0; i < array_dim; i++) {
      d_params.m_LambdaSbs.push_back(
          d_params.lambda +
          d_params.lambda_error * pvalues::standard_normal() * sqrt_array_dim);
      d_params.m_KSbs.push_back(d_params.K + d_params.K_error *
                                                 pvalues::standard_normal() *
                                                 sqrt_array_dim);
      d_params.m_CSbs.push_back(0);

      d_params.m_SigmaSbs.push_back(
          d_params.sigma +
          d_params.sigma_error * pvalues::standard_normal() * sqrt_array_dim);

      d_params.m_AlphaISbs.push_back(
          d_params.alpha_I +
          d_params.alpha_I_error * pvalues::standard_normal() * sqrt_array_dim);
      d_params.m_AlphaJSbs.push_back(
          d_params.alpha_J +
          d_params.alpha_J_error * pvalues::standard_normal() * sqrt_array_dim);

      d_params.m_AISbs.push_back(d_params.a_I + d_params.a_I_error *
                                                    pvalues::standard_normal() *
                                                    sqrt_array_dim);
      d_params.m_AJSbs.push_back(d_params.a_J + d_params.a_J_error *
                                                    pvalues::standard_normal() *
                                                    sqrt_array_dim);

      d_params.m_BISbs.push_back(d_params.b_I + d_params.b_I_error *
                                                    pvalues::standard_normal() *
                                                    sqrt_array_dim);
      d_params.m_BJSbs.push_back(d_params.b_J + d_params.b_J_error *
                                                    pvalues::standard_normal() *
                                                    sqrt_array_dim);

      d_params.m_BetaISbs.push_back(
          d_params.beta_I +
          d_params.beta_I_error * pvalues::standard_normal() * sqrt_array_dim);
      d_params.m_BetaJSbs.push_back(
          d_params.beta_J +
          d_params.beta_J_error * pvalues::standard_normal() * sqrt_array_dim);

      d_params.m_TauSbs.push_back(
          d_params.tau +
          d_params.tau_error * pvalues::standard_normal() * sqrt_array_dim);
    };
                              ...
                              double seqlen2_) const  // length of sequence #2
{
  if (seqlen1_ <= 0 || seqlen2_ <= 0) {
    throw error(
        "Error - seqlen1_<=0 or seq2en1_<=0 in \"double "
        "AlignmentEvaluer::area\"\n",
        2);
  };

  if (!isGood()) {
    throw error(
        "Unexpected error - the Gumbel parameters are not defined properly in "
        "\"double AlignmentEvaluer::area\"\n",
        1);
  };
  ...
  pvalues_obj.get_appr_tail_prob_with_cov_without_errors(
      d_params, pvalues_obj.blast, score_, seqlen2_, seqlen1_,

      P,

      E,

      area_res, area_is_1_flag, compute_only_area);
                            ...
                            double &evalueErr_) const  // E-value error
{
  if (seqlen1_ <= 0 || seqlen2_ <= 0) {
    throw error(
        "Error - seqlen1_<=0 or seqlen2_<=0 in \"double "
        "AlignmentEvaluer::calc\"\n",
        2);
  };
  ...
  if (!isGood()) {
    throw error(
        "Unexpected error - the Gumbel parameters are not defined properly in "
        "\"double AlignmentEvaluer::calc\"\n",
        1);
  };

  static Sls::pvalues pvalues_obj;

  pvalues_obj.calculate_P_values(score_, seqlen2_, seqlen1_, d_params, pvalue_,
                                 pvalueErr_, evalue_, evalueErr_);
}
                            ...
                            double &evalue_) const  // resulted E-value
{
  ...
  if (!isGood()) {
    throw error(
        "Unexpected error - d_params is not defined in \"double "
        "AlignmentEvaluer::calc\"\n",
        1);
  };
...
void AlignmentEvaluer::set_gapped_computation_parameters_simplified(
    double
        max_time_,  // maximum allowed time in seconds for the whole computation
    long number_of_samples_,  // number of realization for the main stage
    long
        number_of_samples_for_preliminary_stages_)  // number of realization for
                                                    // the preliminary stages
{
  if (number_of_samples_ <= 0 ||
      number_of_samples_for_preliminary_stages_ <= 0) {
    throw error(
        "Error - number_of_samples_<=0 or "
        "number_of_samples_for_preliminary_stages_<=0 in \"void "
        "AlignmentEvaluer::set_gapped_computation_parameters_simplified\"\n",
        2);
  };

  d_gapped_computation_parameters
      .d_first_stage_preliminary_realizations_numbers_ALP.resize(1);
  d_gapped_computation_parameters
      .d_first_stage_preliminary_realizations_numbers_ALP[0] =
      number_of_samples_for_preliminary_stages_;
  d_gapped_computation_parameters.d_preliminary_realizations_numbers_ALP.resize(
      1);
  d_gapped_computation_parameters.d_preliminary_realizations_numbers_ALP[0] =
      number_of_samples_for_preliminary_stages_;
  d_gapped_computation_parameters.d_preliminary_realizations_numbers_killing
      .resize(1);
  d_gapped_computation_parameters
      .d_preliminary_realizations_numbers_killing[0] =
      number_of_samples_for_preliminary_stages_;
  d_gapped_computation_parameters.d_total_realizations_number_with_ALP =
      number_of_samples_;
  d_gapped_computation_parameters.d_total_realizations_number_with_killing =
      number_of_samples_;
  d_gapped_computation_parameters.d_parameters_flag = true;
  d_gapped_computation_parameters.d_max_time_with_computation_parameters =
      max_time_;
  if (max_time_ > 0) {
    d_gapped_computation_parameters.d_max_time_for_quick_tests =
        0.5 * max_time_ * (double)quick_tests_trials_number /
        (double)(quick_tests_trials_number + number_of_samples_ +
                 number_of_samples_for_preliminary_stages_);
  } else {
...
void AlignmentEvaluer::gapped_computation_parameters_clear() {
  d_gapped_computation_parameters
      .d_first_stage_preliminary_realizations_numbers_ALP.clear();
  d_gapped_computation_parameters.d_preliminary_realizations_numbers_ALP
      .clear();
  d_gapped_computation_parameters.d_preliminary_realizations_numbers_killing
      .clear();
