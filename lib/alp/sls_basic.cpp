...
void sls_basic::get_current_time(double &seconds_) {
...
double sls_basic::one_minus_exp_function(double y_) {
  if (fabs(y_) > 1e-3) {
    return 1.0 - exp(y_);
  } else {
...
double sls_basic::normal_probability(double x_, double eps_) {
  if (x_ == 0) {
    return 0.5;
  };
...
double sls_basic::normal_probability(double a_, double b_, double h_,
                                     long int N_, double *p_, double x_,
                                     double eps_) {
  if (x_ < a_ || x_ > b_) {
...
double sls_basic::ln_one_minus_val(double val_) {
  if (val_ > 1e-8) {
    return log(1 - val_);
  };
