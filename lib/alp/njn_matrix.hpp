...
class Matrix {
 public:
  static inline Matrix<T> *matrix(size_t k_, size_t m_, size_t n_,
                                  T a_ = static_cast<T>(0));
  // allocates & returns an  a_-initialized pointer to k_ matrices of dimension
  // m_ x n_ to deallocate :
  //     delete [] matrix; matrix = 0;

  static inline bool approx(const Matrix<T> &x_, const Matrix<T> &y_, T eps_);
  static inline bool relApprox(const Matrix<T> &x_, const Matrix<T> &y_,
                               T eps_);
  static inline bool absRelApprox(const Matrix<T> &x_, const Matrix<T> &y_,
                                  T tol_, T rtol_);
  static inline bool isSymmetric(const Matrix<T> &x_);

  inline Matrix();
  inline Matrix(const Matrix &matrix_);
  inline Matrix(size_t m_, size_t n_,
                const T *vector_);  // assumes vector_ contains m_ * n_ elements
  inline Matrix(
      size_t m_, size_t n_,
      T a_ = static_cast<T>(0));  // copies a_ into m_ x n_ matrix elements
...
inline std::ostream &operator<<(std::ostream &ostr_,
                                Njn::MatrixIO::Format format_);
inline std::istream &operator>>(std::istream &istr_,
                                Njn::MatrixIO::Format format_);
...
// allocates & returns an a_-initialized pointer to k_ matrices of dimension m_
// x n_ to deallocate :
//     delete [] matrix; matrix = 0;
{
  Matrix<T> *matrix = new Matrix<T>[k_];

  for (size_t i = 0; i < k_; i++) matrix[i].copy(m_, n_, a_);
...
template <typename T>
bool Matrix<T>::absRelApprox(const Matrix<T> &x_, const Matrix<T> &y_, T tol_,
                             T rtol_) {
  assert(x_.getM() == y_.getM());
  assert(x_.getN() == y_.getN());
...
template <typename T>
Matrix<T>::Matrix() : d_m(0), d_n(0), d_matrix_p(0), d_value(0) {}
...
std::ostream &Matrix<T>::out(std::ostream &ostr_) const {
  ...
  if (MatrixIO::getFormat() == MatrixIO::GENERAL) {
    ...
    if (!isSymmetric(*this))
      IoUtil::abort("Matrix::out : matrix is not symmetric");
    ...
    IoUtil::abort("Matrix::out : impossible MatrixIO::getFormat ()");
  ...
  if (MatrixIO::getFormat() == MatrixIO::GENERAL) {
    switch (IoUtil::getFormat()) {
      case IoUtil::MACHINE:
        ...
        if (!sstream.eof())
          IoUtil::abort("Njn::Matrix::in : bad value for the MACHINE");
        ...
        for (i = 0; i < this->getM(); i++) {
          ...
          for (j = 0; j < this->getN(); j++) {
            ...
            if (sstream.fail()) {
              ...
              sjstream << j;
              IoUtil::abort("Njn::Matrix::in : bad d_matrix_p [" +
                            sistream.str() + "][" + sjstream.str() + "]");
            }
          }
        }
    ...
    if (!sstream.eof())
      IoUtil::abort("Njn::Matrix::in : bad value for the MatrixIO::SYMMETRIC");

    m = n = v.size();
    if (n == 0)
      IoUtil::abort("Njn::Matrix::in : bad n for the MatrixIO::SYMMETRIC");
    ...
    if (m * (m + 1) / 2 != v.size())
      IoUtil::abort(
          "Njn::Matrix::in : rows for the MatrixIO::SYMMETRIC have incorrect "
          "lengths.");
    ...
    IoUtil::abort("Matrix::in : impossible MatrixIO::getFormat ()");
...
template <typename T>
void Matrix<T>::copy(size_t m_,                // m
                     size_t n_,                // n
...
void Matrix<T>::free2() {
  ...
  if (this->getM() > 0) delete[] d_matrix_p;
  d_matrix_p = 0;
