...
char clearTerminator();       // resets the terminator character to and returns
                              // default
...
std::istream &getLine(
    ...
    const char t_ =
        getTerminator());  // termination character (could be '\n'; default '!')
// behaves like std::getline (in_, str_) but throws away the string after first
// character t_ && skips lines of white space
...
std::istream &getString(
    ...
    const char t_ =
        getTerminator());  // termination character (could be '\n'; default '!')
// extracts the first non-white-space string from the result of getLine (in_,
// str_, t_) and throws everything else away
...
void abort(const char *s_) { abort(static_cast<const std::string>(s_)); }

template <typename T>
std::istream &getString(std::istream &in_,   // input filestream
                        T &value_,           // string before '!' from next line
                        std::string &line_,  // string before '!' from next line
                        const char t_)  // termination character (could be '\n')
{  // behaves like getline (in_, str_) but throws away the string after first
   // character t_ && skips lines of white space
