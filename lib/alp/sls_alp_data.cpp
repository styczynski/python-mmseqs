...
void alp_data::input_data_for_the_constructor(
    string randout_,  // if defined, then the program outputs complete
                      // randomization information into a file
    ...
    struct_for_randomization &rand_all_, bool &rand_flag_, long int &rand_,

    long int &alphabetSize_, long int **&substitutionScoreMatrix_,
    double *&letterFreqs1_, double *&letterFreqs2_) {
  ifstream frand;
  ...
  try {
    ...
    read_smatr(smatr_file_name_, d_smatr, number_of_AA_smatr);

    d_number_of_AA_smatr = number_of_AA_smatr;

    read_RR(RR1_file_name_, d_RR1, d_RR1_sum, d_RR1_sum_elements,
            number_of_AA_RR1);

    read_RR(RR2_file_name_, d_RR2, d_RR2_sum, d_RR2_sum_elements,
            number_of_AA_RR2);

    if (number_of_AA_RR1 == number_of_AA_smatr) {
      alphabetSize_ = number_of_AA_smatr;
    } else {
      throw error("Number of letters is different in the files " +
                      smatr_file_name_ + " and " + RR1_file_name_ + "\n",
                  3);
    };

    if (number_of_AA_RR2 != number_of_AA_smatr) {
      throw error("Number of letters is different in the files " +
                      smatr_file_name_ + " and " + RR2_file_name_ + "\n",
                  3);
    };
    ...
    if (randout_ != "") {
      ...
      if (!frand) {
        ...
        for (i = 0; i < size; i++) {
          long int tmp;
          frand >> tmp;
          rand_all_.d_first_stage_preliminary_realizations_numbers_ALP
              .push_back(tmp);
          if (tmp < 0) {
            throw error("File " + rand_st + " is not correct\n", 3);
          };
    ...
    if (!(alphabetSize_ > 0 && substitutionScoreMatrix_ && letterFreqs1_ &&
          letterFreqs2_)) {
                                       ...
                                       // deletions are allowed
{
  try {
    ...
    d_is = new importance_sampling(this, d_open,

                                   d_epen,

                                   temperature_, d_number_of_AA, d_smatr, d_RR1,
                                   d_RR2);
    ...
    d_memory_size_in_MB +=
        (double)(sizeof(double) * d_number_of_AA * 2.0) / mb_bytes;

    double tmp_size1 = LONG_MAX;

    double tmp_size = Tmin(
        (double)(tmp_size1),
        (

            (double)mb_bytes * d_max_mem /
            (double)d_minimum_realizations_number) /
...
alp_data::alp_data(  // constructor
    long int rand_,  // randomization number
    struct_for_randomization
        *randomization_parameters_,  // if not NULL, sets d_rand_flag to true
                                     // and initializes d_rand_all
    ...
    long alphabetSize_, const long *const *substitutionScoreMatrix_,
    const double *letterFreqs1_, const double *letterFreqs2_,
    ...
    bool insertions_after_deletions_,  // if true, then insertions after
                                       // deletions are allowed
    double max_time_for_quick_tests_,  // maximum allowed calculation time in
                                       // seconds for quick tests
    double max_time_with_computation_parameters_)  // maximum allowed time in
                                                   // seconds for the whole
                                                   // computation
{
  d_smatr = NULL;
  d_RR1 = NULL;
  ...
  try {
    ...
    if (randomization_parameters_) {
      ...
      d_rand_all->d_random_seed = randomization_parameters_->d_random_seed;
    ...
    init_main_class_members(
        rand_,    // randomization number
        randout,  // if defined, then the program outputs complete randomization
    ...
    calculate_RR_sum(d_RR1, alphabetSize_, d_RR1_sum, d_RR1_sum_elements);

    calculate_RR_sum(d_RR2, alphabetSize_, d_RR2_sum, d_RR2_sum_elements);
...
long int alp_data::random_long(double value_, long int dim_) {
...
void alp_data::release_memory() {
  ...
  d_memory_size_in_MB -= 2.0 *
                         (double)(2.0 * sizeof(double) + sizeof(long int)) *
                         (double)d_number_of_AA / mb_bytes;
...
void alp_data::check_out_file(string out_file_name_) {
  ifstream f;
  char *str_ch = NULL;
  ...
  try {
    ...
    if (!test_flag0) {
      throw error("The output file " + out_file_name_ +
                      " exists and does not have the correct format;\nplease "
    ...
    if (symmetric_case_flag) {
      if (!d_smatr_symmetric_flag) {
        throw error("The output file " + out_file_name_ +
                        ...
                        "current calculation uses non-symmetric "
    ...
    if (!symmetric_case_flag) {
      if (d_smatr_symmetric_flag) {
        throw error("The output file " + out_file_name_ +
                        ...
                        "current calculation uses symmetric "
                        "parameters;\nplease define another output file name\n",
                    3);
      };
    };
...
void alp_data::read_smatr(string smatr_file_name_, long int **&smatr_,
  ...
  try {
    ...
    if (number_of_AA_smatr_ <= 0) {
      throw error(
          "Error - number of letters in the scoring matrix file must be "
          "greater than 0\n",
          3);
    };
...
void alp_data::read_RR(string RR_file_name_, double *&RR_, double *&RR_sum_,
                       long int *&RR_sum_elements_,
                       long int &number_of_AA_RR_) {
  read_RR(RR_file_name_, RR_, number_of_AA_RR_);

  calculate_RR_sum(RR_, number_of_AA_RR_, RR_sum_, RR_sum_elements_);
}

void alp_data::check_RR_sum(double sum_tmp_, long int number_of_AA_RR_,
                            string RR_file_name_) {
  if (number_of_AA_RR_ <= 0) {
    throw error(
        "Error - number of letters in the probabilities file must be greater "
        "than 0\n",
        3);
  };
  ...
  if (diff_tmp > 0) {
    double lg_diff =
        -(log(diff_tmp) - log((double)number_of_AA_RR_)) / log(10.0);
    double lg_eps = -log(DBL_EPSILON) / log(10.0) - 1;
    if (lg_diff < lg_eps) {
      if (sum_tmp_ <= 0) {
        if (RR_file_name_ != "") {
          throw error("Error: the sum of the probabilities from the file " +
                          RR_file_name_ + " is non-positive\n",
                      3);
        } else {
      ...
      if (RR_file_name_ != "") {
        ...
        if (!flag_RR[RR_file_name_]) {
          cout << "\n!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"
                  "!!!!!!!!!!!!!!\n";
          cout << "Warning: the sum of the probabilities from the file "
               << RR_file_name_ << " is not equal to 1\n";
...
void alp_data::calculate_RR_sum(double *RR_, long int number_of_AA_RR_,
                                double *&RR_sum_, long int *&RR_sum_elements_) {
  RR_sum_ = NULL;
  ...
  try {
    ...
    check_RR_sum(sum_tmp, number_of_AA_RR_, "");
    ...
    delete[] RR_sum_;
    RR_sum_ = NULL;
    delete[] RR_sum_elements_;
    RR_sum_elements_ = NULL;
    throw;
  };
}

void alp_data::read_RR(string RR_file_name_, double *&RR_,
                       long int &number_of_AA_RR_) {
  ifstream f;
  RR_ = NULL;
  ...
  try {
    ...
    for (i = 0; i < number_of_AA_RR_; i++) {
      ...
      if (RR_[i] < 0) {
        throw error("Error - the frequencies defined in the file " +
                        RR_file_name_ + " must be non-negative\n",
                    3);
      };
    ...
    check_RR_sum(sum_tmp, number_of_AA_RR_, RR_file_name_);
...
importance_sampling::importance_sampling(alp_data *alp_data_, long int open_,

                                         long int epen_,

                                         double temperature_,
                                         long int number_of_AA_,
                                         long int **smatr_, double *RR1_,
                                         double *RR2_) {
  d_elements = NULL;
  ...
  try {
    {
      ...
      if (smatr_max <= 0) {
        throw error(
            "Error - at least one element of the scoring matrix must be "
            "positive\n",
            3);
      };
      ...
      alp_reg::find_tetta_general(importance_sampling::lambda_equation,
                                  (void *)(&tmp_ptr), a, b, n_partition, eps,
                                  res_lambda);

      sort(res_lambda.begin(), res_lambda.end());

      if (res_lambda.size() == 0) {
        // throw error("Error - the program is not able to find the ungapped
        // lambda. The program does not work with the input scoring scheme.
        // \n",3);
        throw error(
            "Error - you have exceeded the calculation time or memory "
            "limit.\nThe error might indicate that the regime is linear or too "
    ...
    for (ind = 1; ind < number_of_AA_ * number_of_AA_; ind++) {
      d_elements_values[ind] =
          d_elements_values[ind - 1] + d_elements_values[ind];
    };
    ...
    d_for_D[0] = d_nu;
    d_for_D_states[0] = 'D';
    d_for_D[1] = d_for_D[0] + d_mu_SD;
    d_for_D_states[1] = 'S';
    d_for_D[2] = d_for_D[1] + d_mu_ID;
    d_for_D_states[2] = 'I';

    d_for_I[0] = d_nu;
    d_for_I_states[0] = 'I';
    d_for_I[1] = d_for_I[0] + d_mu_SI;
    d_for_I_states[1] = 'S';

    d_for_S[0] = d_eta;
    d_for_S_states[0] = 'S';
    d_for_S[1] = d_for_S[0] + d_mu_DS;
    d_for_S_states[1] = 'D';
    d_for_S[2] = d_for_S[1] + d_mu_IS;
    d_for_S_states[2] = 'I';

    d_alp_data->d_memory_size_in_MB +=
        sizeof(double) * number_of_AA_ / mb_bytes;
    d_alp_data->d_memory_size_in_MB +=
        sizeof(q_elem) * number_of_AA_ / mb_bytes;
  } catch (...) {
...
importance_sampling::~importance_sampling() {
  delete[] d_elements;
  d_elements = NULL;
  delete[] d_elements_values;
  d_elements_values = NULL;

  if (d_alp_data) {
    d_alp_data->delete_memory_for_matrix(d_is_number_of_AA, d_exp_s);
    d_alp_data->d_memory_size_in_MB -=
        sizeof(double) * d_is_number_of_AA / mb_bytes;
    d_alp_data->d_memory_size_in_MB -=
        sizeof(q_elem) * d_is_number_of_AA / mb_bytes;
  };
}

double alp_data::error_of_the_sum(  // v1_+v2_
    double v1_error_, double v2_error_) {
...
double alp_data::error_of_the_product(  // v1_*v2_
    double v1_, double v1_error_, double v2_, double v2_error_) {
  if (v1_error_ >= 1e100 || v2_error_ >= 1e100) {
    return 1e100;
  };
...
double alp_data::error_of_the_lg(  // lg(v1_)
  ...
  return alp_data::Tmin(fabs(log(v1_) / log(10.0)),
                        v1_error_ / v1_ / log(10.0));
}

bool alp_data::the_value_is_double(string str_, double &val_) {
  if (str_ == "") {
    return false;
  };
...
bool alp_data::the_value_is_long(string str_, long int &val_) {
  if (str_.length() == 0) {
    return false;
  };
  ...
  if (((long int)str_.size() - start_digit > 10) ||
      ((long int)str_.size() - start_digit) <= 0) {
    return false;
  };
