...
struct error_for_single_realization  // struct to handle exceptions during
                                     // calclation of single realization
{
  std::string st;
...
class array_positive {
 public:
  ...
  inline void set_elem(long int ind_, T elem_) {
    if (ind_ > d_dim) {
  ...
  inline void increase_elem_by_1(long int ind_) {
    if (ind_ > d_dim) {
      increment_array(ind_);
    };
  ...
  inline void increase_elem_by_x(long int ind_, T x_) {
    if (ind_ > d_dim) {
      increment_array(ind_);
    };
...
class importance_sampling {
 public:
  importance_sampling(alp_data *alp_data_, long int open_,

                      long int epen_,

                      double temperature_, long int number_of_AA_,
                      long int **smatr_, double *RR1_, double *RR2_);
...
class alp_data : public sls_basic {
 public:
  alp_data(                  // constructor
      ...
      std::string randout_,  // if defined, then the program outputs complete
  ...
  static void read_smatr(std::string smatr_file_name_, long int **&smatr_,
                         long int &number_of_AA_smatr_);

  void check_out_file(std::string out_file_name_);
  ...
  static bool the_value_is_double(std::string str_, double &val_);

  static bool the_value_is_long(std::string str_, long int &val_);

  static void read_RR(std::string RR_file_name_, double *&RR_, double *&RR_sum_,
                      long int *&RR_sum_elements_, long int &number_of_AA_RR_);

  static void read_RR(std::string RR_file_name_, double *&RR_,
                      long int &number_of_AA_RR_);

  static void calculate_RR_sum(double *RR_, long int number_of_AA_RR_,
                               double *&RR_sum_, long int *&RR_sum_elements_);

  static void check_RR_sum(double sum_tmp_, long int number_of_AA_RR_,
                           std::string RR_file_name_);

  template <typename T>
  static void get_memory_for_matrix(long int dim_, T **&matr_,
    ...
    try {
      ...
      if (alp_data_) {
        alp_data_->d_memory_size_in_MB +=
            (double)sizeof(T) * (double)dim_ * (double)dim_ / mb_bytes;
      };
  ...
  template <typename T>
  static void delete_memory_for_matrix(long int dim_, T **&matr_,
                                       alp_data *alp_data_ = NULL) {
    long int i;
    if (matr_) {
      for (i = 0; i < dim_; i++) {
  ...
  static long int random_long(double value_, long int dim_);

  template <typename T>
  static T random_long(double value_, long int dim_, double *sum_distr_,
                       T *elements_)  // sum_distr_[dim_-1] must be equal to 1
  {
    ...
    if (elements_) {
      ...
      throw error(
          "Unexpected error in alp_data::random_long: the parameter elements_ "
          "must be defined\n",
          4);
    };
  }
 ...
 public:
  ...
  double d_max_time_for_quick_tests;  // maximum allowed calculation time in
                                      // seconds for quick tests
  double d_max_time_with_computation_parameters;  // maximum allowed time in
                                                  // seconds for the whole
                                                  // computation
  ...
  bool d_insertions_after_deletions;  // if true, then insertions after
                                      // deletions are allowed
  ...
  long int d_random_seed;
  std::string d_randout;  // if defined, then the program outputs complete
                          // randomization information into a file
...
array_positive<T>::~array_positive() {
  ...
  if (d_alp_data) {
    d_alp_data->d_memory_size_in_MB -=
        (double)sizeof(T) * (double)(d_dim + 1) / mb_bytes;
  };
}
...
void array_positive<T>::increment_array(long int ind_) {
  ...
  try {
    ...
    if (d_alp_data) {
      d_alp_data->d_memory_size_in_MB +=
          (double)sizeof(T) * (double)jump / mb_bytes;
    };
