...
class CPPFS_API LocalFileHandle : public AbstractFileHandleBackend {
 ...
 protected:
  std::shared_ptr<LocalFileSystem>
      m_fs;                  ///< File system that created this handle
  std::string m_path;        ///< Path to file or directory
  mutable void* m_fileInfo;  ///< Information about the current file (resolves
                             ///< links, created on demand)
  mutable void* m_linkInfo;  ///< Information about the current file (does not
                             ///< resolve links, created on demand)
};
