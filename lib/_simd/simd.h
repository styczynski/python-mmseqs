...
#define simdi16_slli(x, y) \
  _mm512_slli_epi16(x, y)  // shift integers in a left by y
#define simdi16_srli(x, y) \
  _mm512_srli_epi16(x, y)  // shift integers in a right by y
#define simdi32_slli(x, y) \
  _mm512_slli_epi32(x, y)  // shift integers in a left by y
#define simdi32_srli(x, y) \
  _mm512_srli_epi32(x, y)                     // shift integers in a right by y
...
#define simdi16_slli(x, y) \
  _mm256_slli_epi16(x, y)  // shift integers in a left by y
#define simdi16_srli(x, y) \
  _mm256_srli_epi16(x, y)  // shift integers in a right by y
#define simdi32_slli(x, y) \
  _mm256_slli_epi32(x, y)  // shift integers in a left by y
#define simdi32_srli(x, y) \
  _mm256_srli_epi32(x, y)                     // shift integers in a right by y
...
#define simdi16_slli(x, y) \
  _mm_slli_epi16(x, y)  // shift integers in a left by y
#define simdi16_srli(x, y) \
  _mm_srli_epi16(x, y)  // shift integers in a right by y
#define simdi32_slli(x, y) \
  _mm_slli_epi32(x, y)  // shift integers in a left by y
#define simdi32_srli(x, y) \
  _mm_srli_epi32(x, y)                     // shift integers in a right by y
...
#define ICEIL(x_int, fac_int) ((x_int + fac_int - 1) / fac_int) * fac_int
  ...
  T** matrix = (T**)mem_align(
      ALIGN_FLOAT, size_pointer_array + dim1 * dim2_padded * sizeof(T));
...
inline float ScalarProd20(const float* qi, const float* tj) {
  ...
  //  P = _mm256_permute2f128_ps(R, R, 0x01); // swap hi and lo 128 bits: 4, 5,
  //  6, 7, 0, 1, 2, 3 R = _mm256_add_ps(R,P);     // 0+4, 1+5, 2+6, 3+7, 0+4,
  //  1+5, 2+6, 3+7 R = _mm256_add_ps(R,S);     // 0+4+A, 1+5+B, 2+6+C, 3+7+D,
  //  ?, ?, ? ,? R = _mm256_hadd_ps(R,R);    // 04A15B, 26C37D, ?, ?, 04A15B,
  //  26C37D, ?, ? R = _mm256_hadd_ps(R,R);    // 01234567ABCD, ?, 01234567ABCD,
  //  ?, 01234567ABCD, ?, 01234567ABCD, ? _mm256_store_ps(&res, R); return res;
  //#else
  //
  //
  // TODO fix this
  ...
  //            + tj[8] * qi[8] + tj[9] * qi[9] + tj[10] * qi[10] + tj[11] *
  //            qi[11]
