...
TEST_SUBMODULE(builtin_casters, m) {
  ...
  m.def(
      "int_passthrough_noconvert", [](int arg) { return arg; },
      py::arg{}.noconvert());
  ...
  // [workaround(intel)] ICC 20/21 breaks with py::arg().stuff, using
  // py::arg{}.stuff works.
  ...
  // Test "bool_passthrough_noconvert" again, but using () instead of {} to
  // construct py::arg When compiled with the Intel compiler, this results in
  // segmentation faults when importing the module. Tested with icc (ICC) 2021.1
  // Beta 20200827, this should be tested again when a newer version of icc is
  // available.
  m.def(
      "bool_passthrough_noconvert2", [](bool arg) { return arg; },
      py::arg().noconvert());
#endif
  ...
  m.def("refwrap_usertype_const", [](std::reference_wrapper<const UserType> p) {
    return p.get().value();
  });
  ...
  m.def("takes_ref_wrap",
        [](std::reference_wrapper<ConstRefCasted> x) { return x.get().tag; });
