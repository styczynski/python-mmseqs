...
// Intel compiler requires a separate header file to support aligned new
// operators and does not set the __cpp_aligned_new feature macro. This header
// needs to be included before pybind11.
#include <aligned_new>
#endif
...
TEST_SUBMODULE(class_, m) {
  ...
  m.add_object("implicitly_convert_variable_fail", [&] {
    ...
    py::capsule def_capsule(
        def, [](void *ptr) { delete reinterpret_cast<PyMethodDef *>(ptr); });
  ...
  class PublicistB : public ProtectedB {
   public:
    ...
    // Removing or defaulting this destructor results in linking errors with the
    // Intel compiler (in Debug builds only, tested with icpc (ICC) 2021.1 Beta
    // 20200827)
