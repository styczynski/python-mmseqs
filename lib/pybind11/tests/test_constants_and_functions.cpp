...
// Make the f1 test basically the same as the f2 test in C++17 mode for the
// Intel compiler as it fails to compile with a plain noexcept (tested with icc
// (ICC) 2021.1 Beta 20200827).
...
TEST_SUBMODULE(constants_and_functions, m) {
  ...
  m.def("register_large_capture_with_invalid_arguments", [](py::module_ m) {
    LargeCapture
        capture;  // VS 2015's MSVC is acting up if we create the array here
    m.def(
        "should_raise", [capture](int) { return capture.zeros[9] + 33; },
        py::kw_only(), py::arg());
  });
  m.def("register_with_raising_repr", [](py::module_ m,
                                         py::object default_value) {
    m.def(
        "should_raise", [](int, int, py::object) { return 42; },
        "some docstring", py::arg_v("x", 42),
        py::arg_v("y", 42, "<the answer>"), py::arg_v("z", default_value));
  });
}
