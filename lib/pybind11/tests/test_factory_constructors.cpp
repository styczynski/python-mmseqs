...
#define MAKE_TAG_TYPE(Name)                                      \
  ...
  py::class_<TestFactory3, std::shared_ptr<TestFactory3>> pyTestFactory3(
      m, "TestFactory3");
  pyTestFactory3
      .def(py::init(
  ...
  ignoreOldStyleInitWarnings([&pyNoisyAlloc]() {
    pyNoisyAlloc.def("__init__", [](NoisyAlloc &a, double d, double) {
      new (&a) NoisyAlloc(d);
    });
  });
  ...
  ignoreOldStyleInitWarnings([&pyNoisyAlloc]() {
    pyNoisyAlloc.def("__init__", [](NoisyAlloc &a, int i, std::string) {
      new (&a) NoisyAlloc(i);
    });
  });
