...
class loader_life_support {
 public:
  ...
  ~loader_life_support() {
    ...
    if (stack.empty()) pybind11_fail("loader_life_support: internal error");
...
PYBIND11_NOINLINE inline void all_type_info_populate(
    PyTypeObject *t, std::vector<type_info *> &bases) {
...
PYBIND11_NOINLINE inline detail::type_info *get_type_info(
    const std::type_index &tp, bool throw_if_missing = false) {
  if (auto ltype = get_local_type_info(tp)) return ltype;
  if (auto gtype = get_global_type_info(tp)) return gtype;
...
PYBIND11_NOINLINE inline value_and_holder instance::get_value_and_holder(
  ...
  if (it != vhs.end()) return *it;

  if (!throw_if_missing) return value_and_holder();
...
PYBIND11_NOINLINE inline std::string error_string() {
  ...
  if (scope.value) errorString += (std::string)str(scope.value);
...
PYBIND11_NOINLINE inline handle get_object_handle(
    const void *ptr, const detail::type_info *type) {
  auto &instances = get_internals().registered_instances;
  auto range = instances.equal_range(ptr);
  for (auto it = range.first; it != range.second; ++it) {
    for (const auto &vh : values_and_holders(it->second)) {
      if (vh.type == type) return handle((PyObject *)it->second);
    }
  }
  return handle();
}
 ...
 public:
  ...
  bool try_direct_conversions(handle src) {
    for (auto &converter : *typeinfo->direct_conversions) {
      if (converter(src.ptr(), value)) return true;
    }
    return false;
  }
  ...
  PYBIND11_NOINLINE bool try_load_foreign_module_local(handle src) {
    ...
    if (!hasattr(pytype, local_key)) return false;
  ...
  PYBIND11_NOINLINE bool load_impl(handle src, bool convert) {
    ...
    // Case 2: We have a derived class
      ...
      if (this_.try_implicit_casts(src, convert)) return true;
    }
    ...
    if (convert) {
      ...
      if (this_.try_direct_conversions(src)) return true;
    }
 ...
 public:
  ...
  static std::pair<const void *, const type_info *> src_and_type(
    ...
    if (instance_type && !same_type(cast_type, *instance_type)) {
      ...
      if (const auto *tpi = get_type_info(*instance_type)) return {vsrc, tpi};
    }
 ...
 public:
  bool load(handle src, bool convert) {
    ...
    if (std::is_floating_point<T>::value) {
      ...
      return false;
    } else if (!convert && !index_check(src.ptr()) &&
...
struct string_caster {
  ...
  bool load(handle src, bool) {
    ...
    if (!src) {
      ...
      // returns false
      if (!PYBIND11_BYTES_CHECK(load_src.ptr())) return false;
    ...
    // alive:
    if (IsView) loader_life_support::add_patient(utfNbytes);
...
struct type_caster<CharT, enable_if_t<is_std_char_type<CharT>::value>> {
 ...
 public:
  ...
  operator CharT &() {
    if (none) throw value_error("Cannot convert None to a character");
...
class tuple_caster {
 ...
 protected:
  ...
  bool load_impl(const sequence &seq, bool convert, index_sequence<Is...>) {
    ...
    if ((... || !std::get<Is>(subcasters).load(seq[Is], convert))) return false;
#else
...
struct pyobject_caster {
  ...
  bool load(handle src, bool /* convert */) {
    if (!isinstance<type>(src)) return false;
...
#if defined(NDEBUG)
    throw cast_error(
        "Unable to cast Python instance to C++ rvalue: instance has multiple "
        "references"
        " (compile in debug mode for details)");
#else
...
template <typename T>
detail::enable_if_t<detail::move_if_unreferenced<T>::value, T> cast(
    object &&object) {
...
template <typename T>
enable_if_t<!cast_is_temporary_value_reference<T>::value, T> cast_safe(
    object &&o) {
...
#if defined(NDEBUG)
      throw cast_error(
          "make_tuple(): unable to convert arguments to Python object (compile "
...
class argument_loader {
  ...
  static_assert(args_kwargs_are_last,
                "py::args/py::kwargs are only permitted as the last "
...
class unpacking_collector {
 ...
 private:
  ...
  void process(list &args_list, detail::args_proxy ap) {
    for (auto a : ap) args_list.append(a);
  }
