...
struct set_caster {
  ...
  bool load(handle src, bool convert) {
    if (!isinstance<pybind11::set>(src)) return false;
    auto s = reinterpret_borrow<pybind11::set>(src);
    value.clear();
    for (auto entry : s) {
      key_conv conv;
      if (!conv.load(entry, convert)) return false;
  ...
  static handle cast(T &&src, return_value_policy policy, handle parent) {
    ...
    for (auto &&value : src) {
      ...
      if (!value_ || !s.add(value_)) return handle();
    }
    return s.release();
  }
...
struct array_caster {
 ...
 public:
  bool load(handle src, bool convert) {
    ...
    if (!require_size(l.size())) return false;
    size_t ctr = 0;
    for (auto it : l) {
      value_conv conv;
      if (!conv.load(it, convert)) return false;
