...
inline bool isinstance(handle obj, handle type) {
  ...
  if (result == -1) throw error_already_set();
  return result != 0;
}
...
inline bytes::bytes(const pybind11::str &s) {
  ...
  if (!obj) pybind11_fail("Could not allocate bytes object!");
...
inline str::str(const bytes &b) {
  ...
  if (!obj) pybind11_fail("Could not allocate string object!");
  m_ptr = obj.release().ptr();
}
...
class dict : public object {
 ...
 private:
  ...
  static PyObject *raw_dict(PyObject *op) {
    if (PyDict_Check(op)) return handle(op).inc_ref().ptr();
...
class function : public object {
 public:
  ...
  handle cpp_function() const {
    ...
    if (fun && PyCFunction_Check(fun.ptr())) return fun;
    return handle();
  }
...
class memoryview : public object {
 public:
  ...
  static memoryview from_memory(void *mem, ssize_t size,
    ...
    if (!ptr) pybind11_fail("Could not allocate memoryview object!");
...
inline size_t len(handle h) {
  ...
  if (result < 0) throw error_already_set();
  return (size_t)result;
}
...
bool object_api<D>::rich_compare(object_api const &other, int value) const {
  ...
  if (rv == -1) throw error_already_set();
  return rv == 1;
}
