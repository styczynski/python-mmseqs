...
struct type_hash {
  size_t operator()(const std::type_index &t) const {
    ...
    while (auto c = static_cast<unsigned char>(*ptr++)) hash = (hash * 33) ^ c;
    return hash;
  }
};
...
struct override_hash {
  inline size_t operator()(
      const std::pair<const PyObject *, const char *> &v) const {
...
struct internals {
  ...
  ~internals() {
    ...
    // happens when PyThread_tss_free is called. PYBIND11_TLS_FREE is
...
PYBIND11_NOINLINE inline internals &get_internals() {
  ...
  if (internals_pp && *internals_pp) return **internals_pp;
