...
struct type_record {
  ...
  PYBIND11_NOINLINE void add_base(const std::type_info &base,
    ...
    if (base_info->type->tp_dictoffset != 0) dynamic_attr = true;

    if (caster) base_info->implicit_casts.emplace_back(type, caster);
  }
};
...
inline void process_kw_only_arg(const arg &a, function_record *r) {
  if (!a.name || strlen(a.name) == 0)
    pybind11_fail(
        "arg(): cannot specify an unnamed argument after an kw_only() "
        "annotation");
  ++r->nargs_kw_only;
}
