...
#pragma warning(disable : 4996)  // warning C4996: The POSIX name for this item
                                 // is deprecated. Instead, use the ISO C and
                                 // C++ conformant name
#pragma warning(disable : 4702)  // warning C4702: unreachable code
#pragma warning( \
    disable : 4522)  // warning C4522: multiple assignment operators specified
#pragma warning( \
    disable : 4505)  // warning C4505: 'PySlice_GetIndicesEx': unreferenced
                     // local function has been removed (PyPy only)
...
#pragma GCC diagnostic pop
  ...
  void initialize_generic(unique_function_record &&unique_rec, const char *text,
    ...
    // the unique_ptr, we do not want this to destuct the pointer. `initialize`
    ...
    for (auto &a : rec->args) {
      if (a.name) a.name = guarded_strdup(a.name);
      if (a.descr)
    ...
    if (rec->sibling) {
      if (PyCFunction_Check(rec->sibling.ptr())) {
        ...
        if (!chain->scope.is(rec->scope)) chain = nullptr;
      }
    ...
    for (auto it = chain_start; it != nullptr; it = it->next) {
      if (options::show_function_signatures()) {
        ...
        if (chain) signatures += std::to_string(++index) + ". ";
        signatures += rec->name;
...
// If this is running on 3.9.0, we have to work around a bug.
    ...
    while (rec) {
      ...
      for (auto &arg : rec->args) arg.value.dec_ref();
      if (rec->def) {
...
// See https://github.com/python/cpython/pull/22670
  ...
  static PyObject *dispatcher(PyObject *self, PyObject *args_in,
    ...
    if (overloads->is_constructor) {
      if (!PyObject_TypeCheck(parent.ptr(),
    ...
    try {
      ...
      for (; it != nullptr; it = it->next) {
        ...
        size_t args_to_copy = (std::min)(
            pos_args, n_args_in);  // Protect std::min with parentheses
...
class module_ : public object {
 public:
  ...
  static module_ import(const char *name) {
    ...
    if (!obj) throw error_already_set();
 ...
 protected:
  ...
  void mark_parents_nonsimple(PyTypeObject *value) {
    ...
    for (handle h : t) {
      ...
      if (tinfo2) tinfo2->simple_type = false;
...
class class_ : public detail::generic_type {
 ...
 public:
  ...
  class_(handle scope, const char *name, const Extra &... extra) {
    ...
    // parameters
    static_assert(none_of<is_pyobject<Extra>...>::value ||  // no base class
                                                            // arguments, or:
                  ...
                  "Error: multiple inheritance bases must be specified via "
                  "class_ template options");
  ...
  class_ &def_buffer(Func &&func) {
    ...
    install_buffer_funcs(
        [](PyObject *obj, void *ptr) -> buffer_info * {
          ...
          if (!caster.load(obj, false)) return nullptr;
...
inline str enum_name(handle arg) {
  ...
  for (auto kv : entries) {
    if (handle(kv.second[int_(0)]).equal(arg)) return pybind11::str(kv.first);
  }
  return "???";
}
...
#define PYBIND11_ENUM_OP_STRICT(op, expr, strict_behavior)               \
  m_base.attr(op) = cpp_function(                                        \
      [](object a, object b) {                                           \
        if (!type::handle_of(a).is(type::handle_of(b))) strict_behavior; \
        return expr;                                                     \
      },                                                                 \
...
#undef PYBIND11_ENUM_OP_STRICT
  ...
  PYBIND11_NOINLINE void export_values() {
    dict entries = m_base.attr("__entries");
    for (auto kv : entries) m_parent.attr(kv.first) = kv.second[int_(0)];
  }
...
inline void keep_alive_impl(handle nurse, handle patient) {
  if (!nurse || !patient) pybind11_fail("Could not activate keep_alive!");
...
void implicitly_convertible() {
  ...
  auto implicit_caster = [](PyObject *obj, PyTypeObject *type) -> PyObject * {
    ...
    if (!detail::make_caster<InputType>().load(obj, false)) return nullptr;
    tuple args(1);
    args[0] = obj;
    PyObject *result = PyObject_Call((PyObject *)type, args.ptr(), nullptr);
    if (result == nullptr) PyErr_Clear();
    return result;
  };
...
class gil_scoped_acquire {
 public:
  ...
  PYBIND11_NOINLINE void dec_ref() {
    ...
    if (tstate->gilstate_counter == 0) {
#if !defined(NDEBUG)
      if (!release) pybind11_fail("scoped_acquire::dec_ref(): internal error!");
#endif
...
class gil_scoped_release {
 public:
  ...
  ~gil_scoped_release() {
    ...
    if (active) PyEval_RestoreThread(tstate);
    if (disassoc) {
...
inline function get_type_override(const void *this_ptr,
  ...
  if (cache.find(key) != cache.end()) return function();
  ...
  if (frame && (std::string)str(frame->f_code->co_name) == name &&
    ...
    if (self_caller == self.ptr()) return function();
  }
#else
