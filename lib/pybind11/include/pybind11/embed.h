...
struct embedded_module {
  ...
  embedded_module(const char *name, init_t init) {
    ...
    if (result == -1) pybind11_fail("Insufficient memory to add a new module");
  }
};
...
inline void initialize_interpreter(bool init_signal_handlers = true) {
  if (Py_IsInitialized()) pybind11_fail("The interpreter is already running");
...
class scoped_interpreter {
 public:
  ...
  ~scoped_interpreter() {
    if (is_valid) finalize_interpreter();
  }
