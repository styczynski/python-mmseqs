...
struct EigenProps {
  ...
  static EigenConformable<row_major> conformable(const array &a) {
    ...
    if (vector) {  // Eigen type is a compile-time vector
      if (fixed && size != n) return false;  // Vector size mismatch
...
struct type_caster<Type, enable_if_t<is_eigen_dense_plain<Type>::value>> {
  ...
  bool load(handle src, bool convert) {
    ...
    // type
    if (!convert && !isinstance<array_t<Scalar>>(src)) return false;
 ...
 public:
  bool load(handle src, bool convert) {
    ...
    if (need_copy) {
      ...
      if (!fits || !fits.template stride_compatible<props>()) return false;
...
struct type_caster<Type, enable_if_t<is_eigen_sparse<Type>::value>> {
  ...
  bool load(handle src, bool) {
    ...
    if (!values || !innerIndices || !outerIndices) return false;
