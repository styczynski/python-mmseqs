 ...
 */
#include <biosnake/commons/alloc.h>
#include <math.h>
#include <stdio.h>
#include <stdlib.h>
#include "cacode.h"
...
Lambda Find_JP(const double **mat_b, double la_min, double la_max, double **JP,
               double *p_in, double *q_in);

Sum Check_root(const double **mat_b, double **a, double lambda, double *p,
               double *q);

double Check_det(const double **mat_b, double **a, double lambda);

Sum Nail_lambda(const double **mat_b, int flag_sign, double lambda_min,
                double lambda_max, double *p, double *q, double *la_add);

double Nail_det(const double **mat_b, int flag_sign, double lambda_min,
                double lambda_max);

bool Check_range(const double **mat_b);

double *Locate_det_zero(
    const double **mat_b,
    int *);  // pointer to root list are returned with how many of them by int
...
double calculate_lambda(const double **mat_b, int alpha_size, double *p,
  ...
  if (N_root == 0) {
    ...
    for (k = 0; k <= N_root; k++) {
      ...
      Lambda_local =
          Find_JP(mat_b, Lambda_local.min, Lambda_local.max, JP, p, q);
      if (1 == Lambda_local.flag) {  // sensible solution found
        // free_dvector(p, 1,Alphsize);
        // free_dvector(q, 1,Alphsize);
        free(root_location);
        ...
        // printf("matrix pass first screening but still no sensible solution
        // found. :-( \n");
      }
    }
  }
...
bool Check_range(const double **mat_b) {
  ...
  for (int i = 1; i <= Alphsize; i++) {
    ...
    if ((pos_flag_r == -1) || (neg_flag_r == -1) || (pos_flag_c == -1) ||
        (neg_flag_c == -1)) {
...
double *Locate_det_zero(const double **mat_b, int *N_root_add) {
  double **a /*,  *q, *p */;  // a is the exponentiated matrix of socres, p and
                              // q are bg_freqs
  int i /*,j,k*/;
  ...
  for (i = 1; i < N; i++) {
    if ((flag_sign != 0) && (fabs(s_here[i]) > Epsilon)) {
      ...
      if (s_here[i] > 0.0) flag_sign = 1;
      if (fabs(s_here[i]) / exp(l_here[i] * (r_max_m + c_max_m) / 2.0) <=
          Epsilon) {
  ...
  *N_root_add = zero_monitor;
  if (zero_monitor > 0) {
    if (zero_monitor >= N / 4) {
      ...
      // printf("number of small det points = %d out of %d, exit
      // now....\n",zero_monitor, N);
      free(root_temp);
      return NULL;
      // exit(1);
...
Lambda Find_JP(const double **mat_b, double la_min, double la_max, double **JP,
               double *p_in, double *q_in) {
  double **a, *q,
      *p;  // a is the exponentiated matrix of socres, p and q are bg_freqs
  int i, j /*,k*/;
  int N;  // number of points for first round
  double lambda /*, l_tmp, sum, sum_min, sum_max*/;
  ...
  int flag_done =
      -1;  // 1 means find sensible solution, -1 means sensible not found
  int i_monitor = 0;  // record number of solution found in the range, including
                      // nonsense ones
  int j_monitor;
  ...
  if (s_here[0] < 0.0)
    ...
    while ((flag_sign == 0) && (j_monitor < N)) {
      Sum_local =
          Check_root(mat_b, a, l_here[0] + j_monitor * dlambda / N, p, q);
  ...
  for (i = 1; i < N; i++) {  // should be N-1 ???
    ...
    if (s_here[i - 1] * s_here[i] < 0) {
      ...
      lambda_max = l_here[i];
      Sum_local = Nail_lambda(mat_b, flag_sign, lambda_min, lambda_max, p, q,
                              &lambda_final);
      if (Sum_local.flag == 1) {
        i = N;
        flag_done = 1;
...
Sum Nail_lambda(const double **mat_b, int flag_sign, double lambda_min,
                double lambda_max, double *p, double *q, double *lam_add) {
  double **a;
  double lambda;
  ...
  while (fabs(Sum_local.value - 1.0) > E_bound) {
    if (flag_sign * (Sum_local.value - 1.0) < 0)
      lambda_max = lambda;
    else if (flag_sign * (Sum_local.value - 1.0) > 0)
      lambda_min = lambda;
...
double Nail_det(const double **mat_b, int flag_sign, double lambda_min,
  ...
  while ((fabs(value) > E_bound) && (lambda > 0)) {
    if (flag_sign * (value) < 0)
      lambda_max = lambda;
    else if (flag_sign * (value) > 0)
      lambda_min = lambda;
    lambda = (lambda_min + lambda_max) / 2.0;
    value = Check_det(mat_b, a, lambda);
  }
