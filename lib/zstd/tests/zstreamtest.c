...
static void FUZ_freeDictionary(buffer_t dict) { free(dict.start); }

static buffer_t FUZ_createDictionary(const void* src, size_t srcSize,
                                     size_t blockSize,
...
static size_t SEQ_roundTrip(ZSTD_CCtx* cctx, ZSTD_DCtx* dctx,
  ...
  do {
    ...
    if (ZSTD_isError(cret)) return cret;
    ...
    while (din.pos < din.size || (endOp == ZSTD_e_end && cret == 0)) {
      ...
      if (ZSTD_isError(dret)) return dret;
...
static size_t getCCtxParams(ZSTD_CCtx* zc, ZSTD_parameters* savedParams) {
  unsigned value;
  CHECK_RET_Z(ZSTD_CCtx_getParameter(zc, ZSTD_p_windowLog,
                                     &savedParams->cParams.windowLog));
  CHECK_RET_Z(ZSTD_CCtx_getParameter(zc, ZSTD_p_hashLog,
                                     &savedParams->cParams.hashLog));
  CHECK_RET_Z(ZSTD_CCtx_getParameter(zc, ZSTD_p_chainLog,
                                     &savedParams->cParams.chainLog));
  CHECK_RET_Z(ZSTD_CCtx_getParameter(zc, ZSTD_p_searchLog,
                                     &savedParams->cParams.searchLog));
  CHECK_RET_Z(ZSTD_CCtx_getParameter(zc, ZSTD_p_minMatch,
                                     &savedParams->cParams.searchLength));
  CHECK_RET_Z(ZSTD_CCtx_getParameter(zc, ZSTD_p_targetLength,
                                     &savedParams->cParams.targetLength));
  CHECK_RET_Z(ZSTD_CCtx_getParameter(zc, ZSTD_p_compressionStrategy, &value));
  savedParams->cParams.strategy = value;

  CHECK_RET_Z(ZSTD_CCtx_getParameter(zc, ZSTD_p_checksumFlag,
                                     &savedParams->fParams.checksumFlag));
  CHECK_RET_Z(ZSTD_CCtx_getParameter(zc, ZSTD_p_contentSizeFlag,
...
static U32 badParameters(ZSTD_CCtx* zc, ZSTD_parameters const savedParams) {
  ...
  CHECK_RET(4, params.cParams.searchLog != savedParams.cParams.searchLog,
            "searchLog");
  CHECK_RET(5, params.cParams.searchLength != savedParams.cParams.searchLength,
            "searchLength");
  CHECK_RET(6, params.cParams.targetLength != savedParams.cParams.targetLength,
            "targetLength");

  CHECK_RET(7, params.fParams.checksumFlag != savedParams.fParams.checksumFlag,
            "checksumFlag");
  CHECK_RET(
      8, params.fParams.contentSizeFlag != savedParams.fParams.contentSizeFlag,
      "contentSizeFlag");
  CHECK_RET(9, params.fParams.noDictIDFlag != savedParams.fParams.noDictIDFlag,
            "noDictIDFlag");
  return 0;
}
...
static int basicUnitTests(U32 seed, double compressibility) {
  ...
  size_t const compressedBufferSize =
      (8 + skippableFrameSize) + ZSTD_compressBound(COMPRESSIBLE_NOISE_LENGTH);
  ...
  CHECK_Z(ZSTD_setDStreamParameter(zd, DStream_p_maxWindowSize,
                                   1000000000)); /* large limit */
  {
  ...
  /* Re-use without init */
  DISPLAYLEVEL(
      3, "test%3i : decompress again without init (re-use previous settings): ",
      testNb++);
  outBuff.pos = 0;
  {
  ...
  { /* skippable frame */
    ...
    while (r) { /* skippable frame */
      ...
      if (ZSTD_isError(r))
        DISPLAYLEVEL(4,
                     "ZSTD_decompressStream on skippable frame error : %s \n",
                     ZSTD_getErrorName(r));
      if (ZSTD_isError(r)) goto _output_error;
    }
    /* normal frame */
  ...
  if (outBuff.pos != CNBufferSize)
    DISPLAYLEVEL(4,
                 "outBuff.pos != CNBufferSize : should have regenerated same "
                 "amount ! \n");
  if (outBuff.pos != CNBufferSize)
    goto _output_error; /* should regenerate the same amount */
  if (inBuff.pos != cSize)
  ...
  DISPLAYLEVEL(3,
               "test%3i : generate error when ZSTD_decompressStream() doesn't "
               "progress : ",
               testNb++);
  { /* skippable frame */
    size_t r = 0;
  ...
  } /* error, or some data not flushed */
  {
    ...
    if ((size_t)origSize != CNBufferSize) goto _output_error;
  ...
  /* use 2 */
  {
    size_t const inSize = 1025; /* will not continue, because tables auto-adjust
                                   and are therefore different size */
    DISPLAYLEVEL(5, "use2 ");
    ZSTD_initCStream_advanced(
        zc, NULL, 0, ZSTD_getParams(19, inSize, 0),
        inSize); /* needs btopt + search3 to trigger hashLog3 */
  ...
  DISPLAYLEVEL(3, "test%3i : wrong parameter for ZSTD_setDStreamParameter(): ",
               testNb++);
  {
    size_t const r = ZSTD_setDStreamParameter(zd, (ZSTD_DStreamParameter_e)999,
                                              1); /* large limit */
    if (!ZSTD_isError(r)) goto _output_error;
  }
  DISPLAYLEVEL(3, "OK \n");
  ...
  DISPLAYLEVEL(
      3, "test%3i : ZSTD_initCStream_usingCDict_advanced with masked dictID : ",
      testNb++);
  {
    ZSTD_compressionParameters const cParams =
        ZSTD_getCParams(1, CNBufferSize, dictionary.filled);
    ZSTD_frameParameters const fParams = {1 /* contentSize */, 1 /* checksum */,
                                          1 /* noDictID */};
    ...
    size_t const initError =
        ZSTD_initCStream_usingCDict_advanced(zc, cdict, fParams, CNBufferSize);
    if (ZSTD_isError(initError)) goto _output_error;
    outBuff.dst = compressedBuffer;
    outBuff.size = compressedBufferSize;
    outBuff.pos = 0;
  ...
  DISPLAYLEVEL(
      3, "test%3i : compress again with ZSTD_compress_generic : ", testNb++);
  outBuff.dst = compressedBuffer;
  outBuff.size = compressedBufferSize;
  outBuff.pos = 0;
  inBuff.src = CNBuffer;
      ...
      testNb++);
  {
    ...
    CHECK_Z(
        ZSTD_initCStream_advanced(zc,
                                  dictionary.start, dictionary.filled, params, 0 /* pledgedSrcSize==0 means "empty" when params.fParams.contentSizeFlag is set */));
  ...
  DISPLAYLEVEL(3,
               "test%3i : compress %u bytes with multiple threads : ", testNb++,
               COMPRESSIBLE_NOISE_LENGTH);
  {
  ...
  /* Complex multithreading + dictionary test */
  {
    ...
    DISPLAYLEVEL(
        3, "test%3i : compress %u bytes with multiple threads + dictionary : ",
        testNb++, (U32)srcSize);
    ...
    memcpy(dst, srcToCopy,
           segLength); /* create a long repetition at long distance for job 2 */
    outBuff.dst = compressedBuffer;
    outBuff.size = compressedBufferSize;
    outBuff.pos = 0;
    inBuff.src = CNBuffer;
  ...
  }
  {
    ZSTD_compressionParameters const cParams = ZSTD_getCParams(
        1, 4 KB,
        dictionary.filled); /* intentionnally lies on estimatedSrcSize, to push
                               cdict into targeting a small window size */
    ZSTD_CDict* const cdict = ZSTD_createCDict_advanced(
        dictionary.start, dictionary.filled, ZSTD_dlm_byRef, ZSTD_dct_fullDict,
        cParams, ZSTD_defaultCMem);
    DISPLAYLEVEL(5, "cParams.windowLog = %u : ", cParams.windowLog);
    CHECK_Z(ZSTD_CCtx_refCDict(zc, cdict));
    CHECK_Z(ZSTD_compress_generic(zc, &outBuff, &inBuff, ZSTD_e_end));
    CHECK_Z(ZSTD_CCtx_refCDict(
        zc, NULL)); /* do not keep a reference to cdict, as its lifetime ends */
    ZSTD_freeCDict(cdict);
  }
  if (inBuff.pos != inBuff.size)
    goto _output_error; /* entire input should be consumed */
  ...
  DISPLAYLEVEL(3,
               "test%3i : decompress large frame created from multiple threads "
               "+ dictionary : ",
               testNb++);
  {
  ...
  DISPLAYLEVEL(3, "test%3i : ZSTD_initCStream_srcSize sets requestedParams : ",
               testNb++);
  {
    unsigned level;
  ...
  DISPLAYLEVEL(3, "test%3i : ZSTD_initCStream_advanced sets requestedParams : ",
               testNb++);
  {
  ...
  DISPLAYLEVEL(
      3,
      "test%3i : wildcopy doesn't overwrite potential match data : ", testNb++);
  { /* This test has a window size of 1024 bytes and consists of 3 blocks:
       ...
       overlength write of length 2*WILDCOPY_OVERLENGTH - 3) litlength = 0,
       offset = 1021, match length = 3 (this one will try to read from
               ...
               testNb++);
  {
    ...
    CHECK_Z(ZSTD_decompress_usingDict(zd, decodedBuffer, CNBufferSize, out.dst,
                                      out.pos, dictionary.start,
                                      dictionary.filled));
...
static size_t findDiff(const void* buf1, const void* buf2, size_t max) {
  ...
  if (u >= 3) DISPLAY(" %02X %02X %02X ", b1[u - 3], b1[u - 2], b1[u - 1]);
  DISPLAY(" :%02X:  %02X %02X %02X %02X %02X \n", b1[u], b1[u + 1], b1[u + 2],
          b1[u + 3], b1[u + 4], b1[u + 5]);
  if (u >= 3) DISPLAY(" %02X %02X %02X ", b2[u - 3], b2[u - 2], b2[u - 1]);
  DISPLAY(" :%02X:  %02X %02X %02X %02X %02X \n", b2[u], b2[u + 1], b2[u + 2],
...
static int fuzzerTests(U32 seed, U32 nbTests, unsigned startTest,
  ...
  CHECK(!cNoiseBuffer[0] || !cNoiseBuffer[1] || !cNoiseBuffer[2] ||
            !cNoiseBuffer[3] || !cNoiseBuffer[4] || !copyBuffer || !dstBuffer ||
            !cBuffer || !zc || !zd || !zd_noise,
  ...
  /* test loop */
  for (; (testNb <= nbTests) || (UTIL_clockSpanMicro(startClock) < g_clockTime);
       testNb++) {
    U32 lseed;
    ...
    if ((FUZ_rand(&lseed) & 0xFF) == 132) {
      ...
      CHECK_Z(ZSTD_initDStream_usingDict(zd, NULL,
    ...
    if ((FUZ_rand(&lseed) & 1) /* at beginning, to keep same nb of rand */
      ...
      dictSize =
          ((FUZ_rand(&lseed) & 7) == 1) ? FUZ_rLogLength(&lseed, dictLog) : 0;
      {
      ...
      }
      {
        U64 const pledgedSrcSize =
            (FUZ_rand(&lseed) & 3) ? ZSTD_CONTENTSIZE_UNKNOWN : maxTestSize;
        ZSTD_parameters params =
    ...
    XXH64_reset(&xxhState, 0);
    {
      ...
      U32 n;
      for (n = 0, cSize = 0, totalTestSize = 0; totalTestSize < maxTestSize;
           n++) {
    ...
    }
    {
      ...
      CHECK(outBuff.pos != totalTestSize,
            "decompressed data : wrong size (%u != %u)", (U32)outBuff.pos,
            (U32)totalTestSize);
  ...
  for (; (testNb <= nbTests) || (UTIL_clockSpanMicro(startClock) < g_clockTime);
    ...
    /* compression init */
    {
      ...
      int const cLevelThreadAdjusted = cLevelCandidate - (nbThreads * 2) +
                                       2; /* reduce cLevel when multiple threads
      ...
      if (FUZ_rand(&lseed) & 1) { /* simple init */
        ...
        }
        {
          ...
          DISPLAYLEVEL(
              5,
              "Init with windowLog = %u, pledgedSrcSize = %u, dictSize = %u \n",
              params.cParams.windowLog, (U32)pledgedSrcSize, (U32)dictSize);
          params.fParams.checksumFlag = FUZ_rand(&lseed) & 1;
          params.fParams.noDictIDFlag = FUZ_rand(&lseed) & 1;
          params.fParams.contentSizeFlag = FUZ_rand(&lseed) & 1;
          DISPLAYLEVEL(5, "checksumFlag : %u \n", params.fParams.checksumFlag);
          CHECK_Z(ZSTDMT_setMTCtxParameter(zc, ZSTDMT_p_overlapSectionLog,
                                           FUZ_rand(&lseed) % 12));
          CHECK_Z(ZSTDMT_setMTCtxParameter(
              zc, ZSTDMT_p_jobSize,
              FUZ_rand(&lseed) % (2 * maxTestSize + 1))); /* custome job size */
          CHECK_Z(ZSTDMT_initCStream_advanced(zc, dict, dictSize, params,
                                              pledgedSrcSize));
        }
      }
    }
    ...
    XXH64_reset(&xxhState, 0);
    {
      ...
      /* final frame epilogue */
      {
        ...
        while (remainingToFlush) {
          ...
          CHECK(ZSTD_isError(remainingToFlush), "ZSTDMT_endStream error : %s",
                ZSTD_getErrorName(remainingToFlush));
          assert(outBuff.pos >= previousPos);
    ...
    }
    {
      ...
      for (totalGenSize = 0; decompressionResult;) {
        ...
        outBuff.size = outBuff.pos + dstBuffSize;
        DISPLAYLEVEL(
            6, "ZSTD_decompressStream input %u bytes into outBuff %u bytes \n",
      ...
      CHECK(inBuff.pos != cSize,
            "compressed data should be fully read (%u != %u)", (U32)inBuff.pos,
            (U32)cSize);
      {
        U64 const crcDest = XXH64(dstBuffer, totalTestSize, 0);
        if (crcDest != crcOrig) findDiff(copyBuffer, dstBuffer, totalTestSize);
  ...
  for (; (testNb <= nbTests) || (UTIL_clockSpanMicro(startClock) < g_clockTime);
    ...
    if ((FUZ_rand(&lseed) & 1) /* at beginning, to keep same nb of rand */
      ...
      if (maxTestSize >= srcBufferSize) maxTestSize = srcBufferSize - 1;
      {
        ...
        DISPLAYLEVEL(5, "t%u : compression level : %i \n", testNb,
                     compressionLevel);
        CHECK_Z(setCCtxParameter(zc, cctxParams, ZSTD_p_compressionLevel,
      ...
      /* random dictionary selection */
      dictSize =
          ((FUZ_rand(&lseed) & 63) == 1) ? FUZ_rLogLength(&lseed, dictLog) : 0;
      {
        size_t const dictStart = FUZ_rand(&lseed) % (srcBufferSize - dictSize);
        dict = srcBuffer + dictStart;
      ...
      }
      {
        ...
        if (FUZ_rand(&lseed) & 1) {
          ...
          CHECK_Z(setCCtxParameter(zc, cctxParams, ZSTD_p_windowLog,
                                   cParams.windowLog, opaqueAPI));
          assert(cParams.windowLog >=
                 ZSTD_WINDOWLOG_MIN); /* guaranteed by ZSTD_adjustCParams() */
          windowLogMalus = (cParams.windowLog - ZSTD_WINDOWLOG_MIN) / 5;
        }
        if (FUZ_rand(&lseed) & 1) {
          DISPLAYLEVEL(5, "t%u: hashLog : %u \n", testNb, cParams.hashLog);
          CHECK_Z(setCCtxParameter(zc, cctxParams, ZSTD_p_hashLog,
                                   cParams.hashLog, opaqueAPI));
        }
        if (FUZ_rand(&lseed) & 1) {
          DISPLAYLEVEL(5, "t%u: chainLog : %u \n", testNb, cParams.chainLog);
          CHECK_Z(setCCtxParameter(zc, cctxParams, ZSTD_p_chainLog,
                                   cParams.chainLog, opaqueAPI));
        }
        if (FUZ_rand(&lseed) & 1)
          CHECK_Z(setCCtxParameter(zc, cctxParams, ZSTD_p_searchLog,
                                   cParams.searchLog, opaqueAPI));
        if (FUZ_rand(&lseed) & 1)
          CHECK_Z(setCCtxParameter(zc, cctxParams, ZSTD_p_minMatch,
                                   cParams.searchLength, opaqueAPI));
        if (FUZ_rand(&lseed) & 1)
          CHECK_Z(setCCtxParameter(zc, cctxParams, ZSTD_p_targetLength,
        ...
        if (bigTests) {
          ...
          if (FUZ_rand(&lseed) & 3)
            CHECK_Z(setCCtxParameter(
                zc, cctxParams, ZSTD_p_ldmHashLog,
                FUZ_randomClampedLength(&lseed, ZSTD_HASHLOG_MIN, 23),
                opaqueAPI));
          if (FUZ_rand(&lseed) & 3)
            CHECK_Z(setCCtxParameter(
                zc, cctxParams, ZSTD_p_ldmMinMatch,
                FUZ_randomClampedLength(&lseed, ZSTD_LDM_MINMATCH_MIN,
                                        ZSTD_LDM_MINMATCH_MAX),
                opaqueAPI));
          if (FUZ_rand(&lseed) & 3)
            CHECK_Z(setCCtxParameter(
                zc, cctxParams, ZSTD_p_ldmBucketSizeLog,
                FUZ_randomClampedLength(&lseed, 0, ZSTD_LDM_BUCKETSIZELOG_MAX),
                opaqueAPI));
          if (FUZ_rand(&lseed) & 3)
            CHECK_Z(setCCtxParameter(
                zc, cctxParams, ZSTD_p_ldmHashEveryLog,
                FUZ_randomClampedLength(&lseed, 0,
        ...
        if (FUZ_rand(&lseed) & 1) {
          ...
          CHECK_Z(setCCtxParameter(zc, cctxParams, ZSTD_p_checksumFlag,
                                   checksumFlag, opaqueAPI));
        }
        if (FUZ_rand(&lseed) & 1)
          CHECK_Z(setCCtxParameter(zc, cctxParams, ZSTD_p_dictIDFlag,
                                   FUZ_rand(&lseed) & 1, opaqueAPI));
        if (FUZ_rand(&lseed) & 1)
          CHECK_Z(setCCtxParameter(zc, cctxParams, ZSTD_p_contentSizeFlag,
                                   FUZ_rand(&lseed) & 1, opaqueAPI));
        if (FUZ_rand(&lseed) & 1) {
        ...
        if (bigTests || (FUZ_rand(&lseed) & 0xF) == 0xF) {
          ...
          CHECK_Z(setCCtxParameter(zc, cctxParams, ZSTD_p_nbWorkers, nbThreads,
                                   opaqueAPI));
          if (nbThreads > 1) {
            U32 const jobLog = FUZ_rand(&lseed) % (testLog + 1);
            CHECK_Z(setCCtxParameter(zc, cctxParams, ZSTD_p_overlapSizeLog,
                                     FUZ_rand(&lseed) % 10, opaqueAPI));
            CHECK_Z(setCCtxParameter(zc, cctxParams, ZSTD_p_jobSize,
                                     (U32)FUZ_rLogLength(&lseed, jobLog),
                                     opaqueAPI));
          }
        }
        /* Enable rsyncable mode 1 in 4 times. */
        setCCtxParameter(zc, cctxParams, ZSTD_p_rsyncable,
                         (FUZ_rand(&lseed) % 4 == 0), opaqueAPI);

        if (FUZ_rand(&lseed) & 1)
          CHECK_Z(setCCtxParameter(zc, cctxParams, ZSTD_p_forceMaxWindow,
                                   FUZ_rand(&lseed) & 1, opaqueAPI));
        ...
        if (FUZ_rand(&lseed) & 1) {
          ...
          if (dict && dictSize) {
            /* test that compression parameters are rejected (correctly) after
             * loading a non-NULL dictionary */
            if (opaqueAPI) {
              size_t const setError =
                  ZSTD_CCtx_setParametersUsingCCtxParams(zc, cctxParams);
              CHECK(
                  !ZSTD_isError(setError),
                  "ZSTD_CCtx_setParametersUsingCCtxParams should have failed");
            } else {
              size_t const setError = ZSTD_CCtx_setParameter(
                  zc, ZSTD_p_windowLog, cParams.windowLog - 1);
    ...
    XXH64_reset(&xxhState, 0);
    {
      ...
      for (cSize = 0, totalTestSize = 0; (totalTestSize < maxTestSize);) {
        ...
        DISPLAYLEVEL(6,
                     "t%u: compress consumed %u bytes (total : %u) ; flush: %u "
                     "(total : %u) \n",
                     testNb, (U32)inBuff.pos, (U32)(totalTestSize + inBuff.pos),
                     (U32)flush, (U32)outBuff.pos);
      ...
      /* final frame epilogue */
      {
        ...
        while (remainingToFlush) {
          ...
          remainingToFlush =
              ZSTD_compress_generic(zc, &outBuff, &inBuff, ZSTD_e_end);
...
int main(int argc, const char** argv) {
  ...
  if (!result) {
    switch (selected_api) {
      case simple_api:
        result =
            fuzzerTests(seed, nbTests, testNb, ((double)proba) / 100, bigTests);
        break;
      case mt_api:
        result = fuzzerTests_MT(seed, nbTests, testNb, ((double)proba) / 100,
                                bigTests);
        break;
      case advanced_api:
