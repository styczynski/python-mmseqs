...
DECODECORPUS = os.environ.get(
    "DECODECORPUS", abs_join(FUZZ_DIR, "..", "decodecorpus")
)
...
def parse_env_flags(args, flags):
    ...
    def set_sanitizer(sanitizer, default, san, nosan):
        if sanitizer in san and sanitizer in nosan:
            raise RuntimeError(
                "-fno-sanitize={s} and -fsanitize={s} passed".format(
                    s=sanitizer
                )
            )
        if sanitizer in san:
...
def build_parser(args):
    ...
    parser.add_argument(
        ...
        help="Extra CPPFLAGS for MSAN (default: $MSAN_EXTRA_CPPFLAGS='{}')".format(
            MSAN_EXTRA_CPPFLAGS
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Extra CXXFLAGS for MSAN (default: $MSAN_EXTRA_CXXFLAGS='{}')".format(
            MSAN_EXTRA_CXXFLAGS
        ),
    )
    parser.add_argument(
        ...
        help="Extra LDFLAGS for MSAN (default: $MSAN_EXTRA_LDFLAGS='{}')".format(
            MSAN_EXTRA_LDFLAGS
        ),
    )
    parser.add_argument(
    ...
    args = parse_env_flags(
        args,
        " ".join([args.cppflags, args.cflags, args.cxxflags, args.ldflags]),
    )
