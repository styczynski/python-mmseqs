...
ZSTD_compressionParameters FUZZ_randomCParams(size_t srcSize, uint32_t *state) {
  ...
  cParams.searchLength =
      FUZZ_rand32(state, ZSTD_SEARCHLENGTH_MIN, ZSTD_SEARCHLENGTH_MAX);
...
void FUZZ_setRandomParameters(ZSTD_CCtx *cctx, size_t srcSize,
                              uint32_t *state) {
  ...
  setRand(cctx, ZSTD_p_ldmBucketSizeLog, 0, ZSTD_LDM_BUCKETSIZELOG_MAX, state);
