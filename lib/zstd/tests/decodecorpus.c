...
BYTE SEQUENCE_LITERAL_BUFFER[ZSTD_BLOCKSIZE_MAX]; /* storeSeq expects a place to
                                                     copy literals to */
...
typedef struct {
  size_t
      contentSize; /* 0 means unknown (unless contentSize == windowSize == 0) */
...
static void writeFrameHeader(U32* seed, frame_t* frame, dictInfo info) {
...

  {
    ...
    if (RAND(seed) & 7 && g_maxDecompressedSizeLog > 7) {
      ...
      /* do small content */
      highBit =
          1ULL << RAND_range(seed, 0, MIN(7, 1U << g_maxDecompressedSizeLog));
    } else {
      /* 0 size frame */
      highBit = 0;
    }
  ...
  if (contentSizeFlag) {
    ...
    int minFcsCode = (fh.contentSize >= 256) + (fh.contentSize >= 65536 + 256) +
...

  {
     ...
     * Frame_Content_Size (bits 7-6) singleSegment: 1-bit flag describing if
     * data must be regenerated within a single continuous memory segment. (bit
     * 5) contentChecksumFlag: 1-bit flag that is set if frame includes checksum
     * at the end -- set to 1 below (bit 2) dictBits: 2-bit flag describing how
     * many bytes Dictionary_ID uses -- set to 3 (bits 1-0) For more
     * information:
...
static size_t writeLiteralsBlockSimple(U32* seed, frame_t* frame,
                                       size_t contentSize) {
...
static size_t writeHufHeader(U32* seed, HUF_CElt* hufTable, void* dst,
  ...
  /* Scan input and build symbol stats */
  {
    size_t const largest = HIST_count_wksp(
        count, &maxSymbolValue, (const BYTE*)src, srcSize, WKSP, sizeof(WKSP));
  ...
  {
    size_t const maxBits = HUF_buildCTable_wksp(hufTable, count, maxSymbolValue,
                                                huffLog, WKSP, sizeof(WKSP));
    CHECKERR(maxBits);
    huffLog = (U32)maxBits;
  }
...
static size_t writeLiteralsBlockCompressed(U32* seed, frame_t* frame,
                                           size_t contentSize) {
...
static size_t writeLiteralsBlock(U32* seed, frame_t* frame,
                                 size_t contentSize) {
...
static U32 generateSequences(U32* seed, frame_t* frame, seqStore_t* seqStore,
  ...
  for (i = 0; i < numSequences; i++) {
    ...
    U32 literalLen =
        (RAND(seed) & 7)
            ? ROUND(RAND_exp(seed, literalsSize / (double)(numSequences - i)))
            : 0;
    ...
    do {
      if (RAND(seed) & 7) {
        ...
        if (info.useDict && (RAND(seed) & 1) && i + 1 != numSequences &&
           ...
           * adhere to predetermined contentSize */
          ...
          if (offset > frame->header.windowSize) {
            if (lenPastStart < MIN_SEQ_LEN) {
              /* when offset > windowSize, matchLen bound by end of dictionary
               * (lenPastStart) */
              /* this also means that lenPastStart must be greater than
               * MIN_SEQ_LEN */
              /* make sure lenPastStart does not go past dictionary start though
               */
              lenPastStart =
                  MIN(lenPastStart + MIN_SEQ_LEN, (U32)info.dictContentSize);
              offset =
                  (U32)((BYTE*)srcPtr - (BYTE*)frame->srcStart) + lenPastStart;
            }
            {
    ...
    DISPLAYLEVEL(6, "      LL: %5u OF: %5u ML: %5u", literalLen, offset,
                 matchLen);
...
static void initSymbolSet(const BYTE* symbols, size_t len, BYTE* set,
                          BYTE maxSymbolValue) {
  size_t i;
...
static int isSymbolSubset(const BYTE* symbols, size_t len, const BYTE* set,
                          BYTE maxSymbolValue) {
  size_t i;
...
static size_t writeSequences(U32* seed, frame_t* frame, seqStore_t* seqStorePtr,
  ...
  /* CTable for Literal Lengths */
  {
    ...
    size_t const mostFrequent = HIST_countFast_wksp(
        count, &max, llCodeTable, nbSeq, WKSP, sizeof(WKSP)); /* cannot fail */
    ...
    if (mostFrequent == nbSeq) {
      ...
      *op++ = llCodeTable[0];
      ...
      FSE_buildCTable_wksp(CTable_LitLength, norm, max, tableLog, scratchBuffer,
                           sizeof(scratchBuffer));
  ...
  /* see Literal Lengths for descriptions of mode choices */
  {
    U32 max = MaxOff;
    size_t const mostFrequent = HIST_countFast_wksp(
        count, &max, ofCodeTable, nbSeq, WKSP, sizeof(WKSP)); /* cannot fail */
    assert(!HIST_isError(mostFrequent));
    if (mostFrequent == nbSeq) {
      *op++ = ofCodeTable[0];
      ...
      Offtype = set_rle;
               ...
               isSymbolSubset(ofCodeTable, nbSeq, frame->stats.offsetSymbolSet,
                              28)) {
      Offtype = set_repeat;
    } else if (!(RAND(seed) & 3)) {
      FSE_buildCTable_wksp(CTable_OffsetBits, OF_defaultNorm, DefaultMaxOff,
                           OF_defaultNormLog, scratchBuffer,
                           sizeof(scratchBuffer));
      Offtype = set_basic;
    } else {
      size_t nbSeq_1 = nbSeq;
  ...
  /* see Literal Lengths for descriptions of mode choices */
  {
    U32 max = MaxML;
    size_t const mostFrequent = HIST_countFast_wksp(
        count, &max, mlCodeTable, nbSeq, WKSP, sizeof(WKSP)); /* cannot fail */
    assert(!HIST_isError(mostFrequent));
    if (mostFrequent == nbSeq) {
      *op++ = *mlCodeTable;
...
static size_t writeCompressedBlock(U32* seed, frame_t* frame,
                                   size_t contentSize, dictInfo info) {
...
static U32 generateCompressedBlock(U32 seed, frame_t* frame, dictInfo info) {
  ...
  while (!blockWritten) {
    ...
    if (cSize >= blockContentSize) { /* compressed size must be strictly smaller
                                        than decompressed size :
...
static int genRandomDict(U32 dictID, U32 seed, size_t dictSize,
                         BYTE* fullDict) {
...

  {
    ...
    dictWriteSize = ZDICT_finalizeDictionary(
        fullDict, dictSize, dictContent, dictContentSize, samples, sampleSizes,
...
static dictInfo initDictInfo(int useDict, size_t dictContentSize,
                             BYTE* dictContent, U32 dictID) {
...
static size_t testDecodeSimple(frame_t* fr) {
  ...
  size_t const ret =
      ZSTD_decompress(DECOMPRESSED_BUFFER, MAX_DECOMPRESSED_SIZE, fr->dataStart,
...
static int runTestMode(U32 seed, unsigned numFiles,
                       unsigned const testDurationS, genType_e genType) {
  unsigned fnum;
  ...
  for (fnum = 0;
    ...
    {
      int const ret =
          (genType == gt_frame) ? runFrameTest(&seed) : runBlockTest(&seed);
      if (ret) return ret;
    }
  }
...
static int generateCorpusWithDict(U32 seed, unsigned numFiles,
                                  const char* const path,
                                  const char* const origPath,
                                  const size_t dictSize, genType_e genType) {
  char outPath[MAX_PATH];
  BYTE* fullDict;
  U32 const dictID = RAND(&seed);
  int errorDetected = 0;
...
static void usage(const char* programName) {
  ...
  DISPLAY(
      " -o<path> : select path to output original file (default:no output)\n");
  DISPLAY(
      "                in multiple files mode this should be a directory\n");
...
static void advancedUsage(const char* programName) {
  ...
  DISPLAY(
      ...
      "frame header\n");
  DISPLAY(
      " --use-dict=#             : include a dictionary used to decompress the "
      "corpus\n");
  DISPLAY(
