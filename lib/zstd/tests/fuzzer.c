...
#define CHECK_V(var, fn) \
  size_t const var = fn; \
  if (ZSTD_isError(var)) goto _output_error
#define CHECK(fn) \
  { CHECK_V(err, fn); }
...
static void* FUZ_mallocDebug(void* counter, size_t size) {
  ...
  DISPLAYLEVEL(4, "allocating %u KB => effectively %u KB \n", (U32)(size >> 10),
               (U32)(malloc_size(ptr) >> 10)); /* OS-X specific */
...
static void FUZ_displayMallocStats(mallocCounter_t count) {
  DISPLAYLEVEL(3, "peak:%6u KB,  nbMallocs:%2u, total:%6u KB \n",
               (U32)(count.peakMalloc >> 10), count.nbMalloc,
               (U32)(count.totalMalloc >> 10));
}

static int FUZ_mallocTests_internal(unsigned seed, double compressibility,
                                    unsigned part, void* inBuffer,
                                    size_t inSize, void* outBuffer,
                                    size_t outSize) {
  ...
  if (part <= 1) {
    ...
    for (compressionLevel = 1; compressionLevel <= 6; compressionLevel++) {
      ...
      CHECK_Z(ZSTD_compressCCtx(cctx, outBuffer, outSize, inBuffer, inSize,
                                compressionLevel));
      ZSTD_freeCCtx(cctx);
  ...
  if (part <= 3) {
    ...
    for (nbThreads = 1; nbThreads <= 4; nbThreads++) {
      ...
      for (compressionLevel = 1; compressionLevel <= 6; compressionLevel++) {
        ...
        ZSTD_outBuffer out = {outBuffer, outSize, 0};
        ZSTD_inBuffer in = {inBuffer, inSize, 0};
        CHECK_Z(ZSTD_CCtx_setParameter(cctx, ZSTD_p_compressionLevel,
                                       (U32)compressionLevel));
        CHECK_Z(ZSTD_CCtx_setParameter(cctx, ZSTD_p_nbWorkers, nbThreads));
        while (ZSTD_compress_generic(cctx, &out, &in, ZSTD_e_end)) {
        }
        ZSTD_freeCCtx(cctx);
        DISPLAYLEVEL(3, "compress_generic,-T%u,end level %i : ", nbThreads,
                     compressionLevel);
        FUZ_displayMallocStats(malcount);
      }
    }
  ...
  if (part <= 4) {
    ...
    for (nbThreads = 1; nbThreads <= 4; nbThreads++) {
      ...
      for (compressionLevel = 1; compressionLevel <= 6; compressionLevel++) {
        ...
        ZSTD_freeCCtx(cctx);
        DISPLAYLEVEL(3, "compress_generic,-T%u,continue level %i : ", nbThreads,
                     compressionLevel);
        FUZ_displayMallocStats(malcount);
      }
    }
...
static int basicUnitTests(U32 seed, double compressibility) {
  ...
  {
    size_t const r = ZSTD_decompress_usingDict(
        dctx, decodedBuffer, CNBuffSize, compressedBuffer, cSize, NULL, 0);
    if (r != CNBuffSize) goto _output_error;
  }
  DISPLAYLEVEL(3, "OK \n");
  ...
  {
    size_t const r = ZSTD_decompress_usingDDict(dctx, decodedBuffer, CNBuffSize,
                                                compressedBuffer, cSize, NULL);
    if (r != CNBuffSize) goto _output_error;
  }
  DISPLAYLEVEL(3, "OK \n");
  ...
  {
    size_t const r =
        ZSTD_decompress(decodedBuffer, CNBuffSize, compressedBuffer, cSize - 1);
  ...
  DISPLAYLEVEL(3, "test%3d : check CCtx size after compressing empty input : ",
               testNb++);
  {
    ZSTD_CCtx* const cctx = ZSTD_createCCtx();
  ...
  }
  {
    ...
    CHECK_Z(ZSTD_flushStream(
        cctx, &output)); /* ensure no possibility to "concatenate" and determine
                            the content size */
  ...
  DISPLAYLEVEL(3,
               "test%3d : re-use CCtx with expanding block size : ", testNb++);
  {
    ZSTD_CCtx* const cctx = ZSTD_createCCtx();
        ...
        ZSTD_CONTENTSIZE_UNKNOWN)); /* re-use same parameters */
    {
      ...
      size_t const outSize = ZSTD_compressBound(inSize);
      CHECK_Z(
          ZSTD_compressEnd(cctx, compressedBuffer, outSize, CNBuffer, inSize));
      ...
      3, "test%3d : re-using a CCtx should compress the same : ", testNb++);
  {
    ...
    memcpy((char*)CNBuffer + 20, CNBuffer,
           10); /* create one match, starting from beginning of sample, which is
                   the difficult case (see #1241) */
    for (i = 1; i <= 19; i++) {
      ZSTD_CCtx* const cctx = ZSTD_createCCtx();
      size_t size1, size2;
      DISPLAYLEVEL(5, "l%i ", i);
      size1 = ZSTD_compressCCtx(cctx, compressedBuffer, compressedBufferSize,
                                CNBuffer, 30, i);
      CHECK_Z(size1);
      size2 = ZSTD_compressCCtx(cctx, compressedBuffer, compressedBufferSize,
                                CNBuffer, 30, i);
      CHECK_Z(size2);
      ...
      3, "test%3d : overflow protection with large windowLog : ", testNb++);
  {
    ...
    size_t const nbCompressions =
        ((1U << 31) / CNBuffSize) +
        1; /* ensure U32 overflow protection is triggered */
    size_t cnb;
    assert(cctx != NULL);
  ...
  DISPLAYLEVEL(3, "test%3d : size down context : ", testNb++);
  {
    ...
    CHECK_Z(ZSTD_compressBegin(
        largeCCtx, 19)); /* streaming implies ZSTD_CONTENTSIZE_UNKNOWN, which
                            maximizes memory usage */
    CHECK_Z(ZSTD_compressEnd(largeCCtx, compressedBuffer, compressedBufferSize,
                             CNBuffer, 1));
    {
          ...
          largeCCtx); /* size of context must be measured after compression */
      {
        ...
        CHECK_Z(ZSTD_compressCCtx(smallCCtx, compressedBuffer,
                                  compressedBufferSize, CNBuffer, 1, 1));
        {
          ...
          assert(
              largeCCtxSize >
              32 * smallCCtxSize); /* note : "too large" definition is handled
                                    * within zstd_compress.c . make this test
                                    * case extreme, so that it doesn't depend on
                                    * a possibly fluctuating definition */
        }
        ZSTD_freeCCtx(smallCCtx);
      }
      {
        U32 const maxNbAttempts =
            1100; /* nb of usages before triggering size down is handled within
                   * zstd_compress.c. currently defined as 128x, but could be
                   * adjusted in the future.
                   * make this test long enough so that it's not too much tied
...
#define STATIC_CCTX_LEVEL 3
               ...
               STATIC_CCTX_LEVEL);
  {
    ...
    }
    {
      ...
      DISPLAYLEVEL(
          3, "test%3i : simple compression test with static CCtx : ", testNb++);
      CHECKPLUS(
          r,
          ZSTD_compressCCtx(staticCCtx, compressedBuffer, compressedBufferSize,
                            CNBuffer, CNBuffSize, STATIC_CCTX_LEVEL),
          cSize = r);
      DISPLAYLEVEL(3, "OK (%u bytes : %.2f%%)\n", (U32)cSize,
                   (double)cSize / CNBuffSize * 100);

      DISPLAYLEVEL(3, "test%3i : simple decompression test with static DCtx : ",
                   testNb++);
      {
        size_t const r = ZSTD_decompressDCtx(
            staticDCtx, decodedBuffer, CNBuffSize, compressedBuffer, cSize);
        if (r != CNBuffSize) goto _output_error;
      }
      DISPLAYLEVEL(3, "OK \n");
      ...
      DISPLAYLEVEL(3, "test%3i : init CCtx for too large level (must fail) : ",
                   testNb++);
      {
      ...
      DISPLAYLEVEL(
          3, "test%3i : init CCtx for small level %u (should work again) : ",
          testNb++, 1);
      {
      ...
      DISPLAYLEVEL(3, "test%3i : init CStream for small level %u : ", testNb++,
                   1);
      {
      ...
      DISPLAYLEVEL(3, "test%3i : init CStream with dictionary (should fail) : ",
                   testNb++);
      {
        size_t const r =
      ...
      }
      {
        ...
        ZSTD_inBuffer input = {compressedBuffer, ZSTD_FRAMEHEADERSIZE_MAX + 1,
                               0};
  ...
  DISPLAYLEVEL(3, "test%3i : create ZSTDMT CCtx : ", testNb++);
  {
    ...
    DISPLAYLEVEL(3, "test%3i : compress %u bytes with 2 threads : ", testNb++,
                 (U32)CNBuffSize);
    CHECKPLUS(r,
              ZSTDMT_compressCCtx(mtctx, compressedBuffer, compressedBufferSize,
                                  CNBuffer, CNBuffSize, 1),
              cSize = r);
    DISPLAYLEVEL(3, "OK (%u bytes : %.2f%%)\n", (U32)cSize,
                 (double)cSize / CNBuffSize * 100);
    ...
    DISPLAYLEVEL(3, "test%3i : compress -T2 with checksum : ", testNb++);
    {
      ...
      CHECKPLUS(r,
                ZSTDMT_compress_advanced(
                    mtctx, compressedBuffer, compressedBufferSize, CNBuffer,
                    CNBuffSize, NULL, params, 3 /*overlapRLog*/),
                cSize = r);
    }
    DISPLAYLEVEL(3, "OK (%u bytes : %.2f%%)\n", (U32)cSize,
                 (double)cSize / CNBuffSize * 100);
  ...
  DISPLAYLEVEL(3, "test%3i : compress multiple frames : ", testNb++);
  {
    ...
    for (i = 0; i < segs; i++) {
      CHECK_V(r, ZSTD_compress((BYTE*)compressedBuffer + off, CNBuffSize - off,
  ...
  /* Dictionary and CCtx Duplication tests */
  {
    ...
    cSize = 0;
    CHECKPLUS(r,
              ZSTD_compressEnd(ctxOrig, compressedBuffer, compressedBufferSize,
    ...
    CHECKPLUS(
        ...
        ZSTD_decompress_usingDict(dctx, decodedBuffer, CNBuffSize,
                                  compressedBuffer, cSize, CNBuffer, dictSize),
    ...
    DISPLAYLEVEL(3, "test%3i : compress with duplicated context : ", testNb++);
    {
      ...
      CHECKPLUS(r,
                ZSTD_compressEnd(
                    ctxDuplicated, compressedBuffer, compressedBufferSize,
    ...
    DISPLAYLEVEL(3, "test%3i : decompress with DDict : ", testNb++);
    {
      ...
      size_t const r = ZSTD_decompress_usingDDict(
          dctx, decodedBuffer, CNBuffSize, compressedBuffer, cSize, ddict);
      if (r != CNBuffSize - dictSize) goto _output_error;
    ...
    DISPLAYLEVEL(3, "test%3i : decompress with static DDict : ", testNb++);
    {
      ...
      {
        const ZSTD_DDict* const ddict =
            ZSTD_initStaticDDict(ddictBuffer, ddictBufferSize, CNBuffer,
    ...
    DISPLAYLEVEL(
        3, "test%3i : check content size on duplicated context : ", testNb++);
    {
      size_t const testSize = CNBuffSize / 3;
      {
        ZSTD_parameters p = ZSTD_getParams(2, testSize, dictSize);
        p.fParams.contentSizeFlag = 1;
        CHECK(ZSTD_compressBegin_advanced(ctxOrig, CNBuffer, dictSize, p,
                                          testSize - 1));
      }
      CHECK(ZSTD_copyCCtx(ctxDuplicated, ctxOrig, testSize));

      CHECKPLUS(r,
                ZSTD_compressEnd(ctxDuplicated, compressedBuffer,
  ...
  /* Dictionary and dictBuilder tests */
  {
    ...
    }
    {
      size_t const sDictSize =
          ZDICT_trainFromBuffer(dictBuffer, dictBufferCapacity, decodedBuffer,
    ...
    dictSize = ZDICT_trainFromBuffer(dictBuffer, dictBufferCapacity, CNBuffer,
                                     samplesSizes, nbSamples);
    ...
    DISPLAYLEVEL(3,
                 "test%3i : Multithreaded FASTCOVER dictBuilder : ", testNb++);
    {
      U32 u;
      for (u = 0; u < nbSamples; u++) samplesSizes[u] = sampleUnitSize;
    }
    ...
    cSize =
        ZSTD_compress_usingDict(cctx, compressedBuffer, compressedBufferSize,
                                CNBuffer, CNBuffSize, dictBuffer, dictSize, 4);
    ...
    DISPLAYLEVEL(
        3, "test%3i : frame built with dictionary should be decompressible : ",
        testNb++);
    CHECKPLUS(r,
              ZSTD_decompress_usingDict(dctx, decodedBuffer, CNBuffSize,
                                        compressedBuffer, cSize, dictBuffer,
                                        dictSize),
              if (r != CNBuffSize) goto _output_error);
    DISPLAYLEVEL(3, "OK \n");
    ...
    DISPLAYLEVEL(3, "test%3i : estimate CDict size : ", testNb++);
    {
      ...
      size_t const estimatedSize =
          ZSTD_estimateCDictSize_advanced(dictSize, cParams, ZSTD_dlm_byRef);
      DISPLAYLEVEL(3, "OK : %u \n", (U32)estimatedSize);
    }
    ...
    DISPLAYLEVEL(3, "test%3i : compress with CDict ", testNb++);
    {
      ...
      ZSTD_CDict* const cdict =
          ZSTD_createCDict_advanced(dictBuffer, dictSize, ZSTD_dlm_byRef,
                                    ZSTD_dct_auto, cParams, ZSTD_defaultCMem);
      DISPLAYLEVEL(3, "(size : %u) : ", (U32)ZSTD_sizeof_CDict(cdict));
      cSize =
          ZSTD_compress_usingCDict(cctx, compressedBuffer, compressedBufferSize,
      ...
      for (level = 1; level <= maxLevel; ++level) {
        ...
        if (cdictBuffer == NULL) goto _output_error;
        {
          const ZSTD_CDict* const cdict =
              ZSTD_initStaticCDict(cdictBuffer, cdictSize, dictBuffer, dictSize,
    ...
    DISPLAYLEVEL(3,
                 "test%3i : ZSTD_compress_usingCDict_advanced, no contentSize, "
                 "no dictID : ",
                 testNb++);
    {
      ZSTD_frameParameters const fParams = {0 /* frameSize */, 1 /* checksum */,
                                            1 /* noDictID*/};
      ZSTD_compressionParameters const cParams =
          ZSTD_getCParams(1, CNBuffSize, dictSize);
      ZSTD_CDict* const cdict =
          ZSTD_createCDict_advanced(dictBuffer, dictSize, ZSTD_dlm_byRef,
                                    ZSTD_dct_auto, cParams, ZSTD_defaultCMem);
      cSize = ZSTD_compress_usingCDict_advanced(cctx, compressedBuffer,
                                                compressedBufferSize, CNBuffer,
    ...
    DISPLAYLEVEL(
        3, "test%3i : frame built without dictID should be decompressible : ",
        testNb++);
    CHECKPLUS(r,
              ZSTD_decompress_usingDict(dctx, decodedBuffer, CNBuffSize,
                                        compressedBuffer, cSize, dictBuffer,
    ...
    DISPLAYLEVEL(3, "test%3i : ZSTD_compress_advanced, no dictID : ", testNb++);
    {
      ...
      cSize =
          ZSTD_compress_advanced(cctx, compressedBuffer, compressedBufferSize,
                                 CNBuffer, CNBuffSize, dictBuffer, dictSize, p);
      if (ZSTD_isError(cSize)) goto _output_error;
    }
    DISPLAYLEVEL(3, "OK (%u bytes : %.2f%%)\n", (U32)cSize,
                 (double)cSize / CNBuffSize * 100);
    ...
    DISPLAYLEVEL(
        3, "test%3i : dictionary containing only header should return error : ",
        testNb++);
    {
    ...
    DISPLAYLEVEL(
        3,
        "test%3i : Building cdict w/ ZSTD_dm_fullDict on a good dictionary : ",
        testNb++);
    {
      ZSTD_compressionParameters const cParams =
          ZSTD_getCParams(1, CNBuffSize, dictSize);
      ZSTD_CDict* const cdict = ZSTD_createCDict_advanced(
          dictBuffer, dictSize, ZSTD_dlm_byRef, ZSTD_dct_fullDict, cParams,
          ZSTD_defaultCMem);
    ...
    DISPLAYLEVEL(3,
                 "test%3i : Building cdict w/ ZSTD_dm_fullDict on a rawContent "
                 "(must fail) : ",
                 testNb++);
    {
      ZSTD_compressionParameters const cParams =
    ...
    DISPLAYLEVEL(3,
                 "test%3i : Loading rawContent starting with dict header w/ "
                 "ZSTD_dm_auto should fail : ",
                 testNb++);
    {
      size_t ret;
      MEM_writeLE32((char*)dictBuffer + 2, ZSTD_MAGIC_DICTIONARY);
      ret = ZSTD_CCtx_loadDictionary_advanced(cctx, (const char*)dictBuffer + 2,
                                              dictSize - 2, ZSTD_dlm_byRef,
                                              ZSTD_dct_auto);
    ...
    DISPLAYLEVEL(3,
                 "test%3i : Loading rawContent starting with dict header w/ "
                 "ZSTD_dm_rawContent should pass : ",
                 testNb++);
    {
      size_t ret;
      MEM_writeLE32((char*)dictBuffer + 2, ZSTD_MAGIC_DICTIONARY);
    ...
    DISPLAYLEVEL(3,
                 "test%3i : Dictionary with non-default repcodes : ", testNb++);
    {
      U32 u;
      for (u = 0; u < nbSamples; u++) samplesSizes[u] = sampleUnitSize;
    }
  ...
  /* COVER dictionary builder tests */
  {
    ...
    DISPLAYLEVEL(3,
                 "test%3i : ZDICT_optimizeTrainFromBuffer_cover : ", testNb++);
    memset(&params, 0, sizeof(params));
    params.steps = 4;
  ...
  DISPLAYLEVEL(3, "test%3i : Content size verification : ", testNb++);
  {
    ...
    {
      size_t const result =
          ZSTD_compressEnd(cctx, decodedBuffer, CNBuffSize, CNBuffer, srcSize);
  ...
  DISPLAYLEVEL(3, "test%3i : negative compression level : ", testNb++);
  {
    ...
    assert(cctx != NULL);
    {
      ...
      size_t const cSize_1pass =
          ZSTD_compress_advanced(cctx, compressedBuffer, compressedBufferSize,
                                 CNBuffer, srcSize, NULL, 0, params);
      if (ZSTD_isError(cSize_1pass)) goto _output_error;

      CHECK(ZSTD_CCtx_setParameter(cctx, ZSTD_p_compressionLevel,
  ...
  /* parameters order test */
  {
...

    {
      ...
      CHECK(ZSTD_CCtx_setParameter(cctx, ZSTD_p_windowLog, 18));
      {
        ...
        ZSTD_outBuffer out = {compressedBuffer, ZSTD_compressBound(inputSize),
                              0};
        size_t const result =
            ZSTD_compress_generic(cctx, &out, &in, ZSTD_e_end);
...

    {
      ...
      CHECK(ZSTD_CCtx_setParameter(cctx, ZSTD_p_compressionLevel, 2));
      {
        ...
        if (in.pos != in.size) goto _output_error;
        if (out.pos != cSize)
          goto _output_error; /* must result in same compressed result, hence
                                 same size */
        if (XXH64(out.dst, out.pos, 0) != xxh64)
          goto _output_error; /* must result in exactly same content, hence same
                                 hash */
        DISPLAYLEVEL(3, "OK (compress : %u -> %u bytes)\n", (U32)inputSize,
                     (U32)out.pos);
      }
  ...
  /* block API tests */
  {
    ...
    cSize =
        ZSTD_compressBlock(cctx, compressedBuffer,
                           ZSTD_compressBound(blockSize), CNBuffer, blockSize);
    if (ZSTD_isError(cSize)) goto _output_error;
    DISPLAYLEVEL(3, "OK \n");
    ...
    CHECK(ZSTD_compressBegin(
        cctx, -99)); /* we just want to quickly overflow internal U32 index */
    CHECK(ZSTD_getBlockSize(cctx) >= blockSize);
    {
      ...
      while (compressed < toCompress) {
        size_t const blockCSize = ZSTD_compressBlock(
            cctx, compressedBuffer, ZSTD_compressBound(blockSize), CNBuffer,
            blockSize);
        if (ZSTD_isError(cSize)) goto _output_error;
    ...
    cSize2 = ZSTD_compressBlock(
        cctx, (char*)compressedBuffer + cSize, ZSTD_compressBound(blockSize),
    ...
    DISPLAYLEVEL(3,
                 "test%3i : Dictionary Block decompression test : ", testNb++);
    ...
    {
      CHECK_V(r, ZSTD_decompressBlock(
...
#define ZEROESLENGTH 100
  ...
  {
    CHECK_V(r, ZSTD_compress(compressedBuffer, ZSTD_compressBound(ZEROESLENGTH),
  ...
  {
    CHECK_V(r, ZSTD_decompress(decodedBuffer, ZEROESLENGTH, compressedBuffer,
                               cSize));
...
#define NB3BYTESSEQMASK (NB3BYTESSEQ - 1)
  ...
  DISPLAYLEVEL(3, "test%3i : compress lots 3-bytes sequences : ", testNb++);
  {
    CHECK_V(r, ZSTD_compress(compressedBuffer,
                             ZSTD_compressBound(_3BYTESTESTLENGTH), CNBuffer,
                             _3BYTESTESTLENGTH, 19));
    cSize = r;
  }
  DISPLAYLEVEL(3, "OK (%u bytes : %.2f%%)\n", (U32)cSize,
  ...
  RDG_genBuffer(CNBuffer, CNBuffSize, 0.0, 0.1, seed);
  {
    ...
    while (size <= CNBuffSize) {
      CHECK(
          ZSTD_compressCCtx(cctx, compressedBuffer, bound, CNBuffer, size, 3));
  ...
  DISPLAYLEVEL(3, "test%3i : incompressible data and ill suited dictionary : ",
               testNb++);
  { /* Train a dictionary on low characters */
    ...
    { /* Compress the data */
      ...
      CHECK(ZSTD_compress_usingDict(cctx, outputBuffer, outputSize, CNBuffer,
                                    inputSize, dictBuffer, dictSize, 1));
      free(outputBuffer);
      ZSTD_freeCCtx(cctx);
    }
  ...
  if (strcmp("No error detected",
             ZSTD_getErrorName((ZSTD_ErrorCode)(0 - ZSTD_error_no_error))) != 0)
    goto _output_error;
  ...
  RDG_genBuffer(CNBuffer, CNBuffSize, compressibility, 0., seed);
  {
    ...
    CHECK_Z(ZSTD_compress_usingCDict(
        cctx, compressedBuffer, compressedBufferSize, CNBuffer, size, lgCDict));
    CHECK_Z(
        ZSTD_getFrameHeader(&lgHeader, compressedBuffer, compressedBufferSize));
    CHECK_Z(ZSTD_compress_usingCDict(
        cctx, compressedBuffer, compressedBufferSize, CNBuffer, size, smCDict));
    CHECK_Z(
        ZSTD_getFrameHeader(&smHeader, compressedBuffer, compressedBufferSize));
...
static ZSTD_parameters FUZ_makeParams(ZSTD_compressionParameters cParams,
...
static int fuzzerTests(U32 seed, U32 nbTests, unsigned startTest,
                       U32 const maxDurationS, double compressibility,
                       int bigTests) {
  ...
  /* main test loop */
  for (;
       (testNb <= nbTests) || (UTIL_clockSpanMicro(startClock) < maxClockSpan);
       testNb++) {
    ...
    /* compression tests */
    {
      ...
      assert(cSize > 3);
      {
        ...
        DISPLAYLEVEL(5,
                     "fuzzer t%u: compress into too small buffer of size %u "
                     "(missing %u bytes) \n",
                     testNb, (unsigned)tooSmallSize, (unsigned)missing);
        {
          size_t const errorCode = ZSTD_compressCCtx(
              ctx, dstBuffer, tooSmallSize, sampleBuffer, sampleSize, cLevel);
          CHECK(!ZSTD_isError(errorCode),
                "ZSTD_compressCCtx should have failed ! (buffer too small : %u "
                "< %u)",
                (U32)tooSmallSize, (U32)cSize);
        }
        {
          U32 endCheck;
          memcpy(&endCheck, dstBuffer + tooSmallSize, 4);
          CHECK(endCheck != endMark,
                "ZSTD_compressCCtx : dst buffer overflow  (check.%08X != "
                "%08X.mark)",
                endCheck, endMark);
        }
      }
    ...
    DISPLAYLEVEL(5, "fuzzer t%u: decompression of truncated source \n", testNb);
    {
      ...
      {
        size_t const errorCode = ZSTD_decompress(dstBuffer, dstBufferSize,
    ...
    if (sampleSize > 3) {
      ...
      dstBuffer[tooSmallSize] = token;
      {
        ...
        CHECK(!ZSTD_isError(errorCode),
              "ZSTD_decompress should have failed : %u > %u (dst buffer too "
              "small)",
              (U32)errorCode, (U32)tooSmallSize);
      }
    ...
    if (cSize > 6) {
      ...
      DISPLAYLEVEL(5, "fuzzer t%u: decompress noisy source \n", testNb);
      {
        ...
        memcpy(dstBuffer + sampleSize, &endMark, 4);
        {
           ...
           * strictly respect dst buffer boundaries */
          CHECK((!ZSTD_isError(decompressResult)) &&
                    (decompressResult > sampleSize),
                "ZSTD_decompress on noisy src : result is too large : %u > %u "
                "(dst buffer)",
                (U32)decompressResult, (U32)sampleSize);
        }
        {
          U32 endCheck;
                 ...
                 testNb);
    {
      ...
      dictSize =
          FUZ_rLogLength(&lseed, dictLog); /* needed also for decompression */
      dict = srcBuffer + (FUZ_rand(&lseed) % (srcBufferSize - dictSize));

      DISPLAYLEVEL(6,
                   "fuzzer t%u: Compressing up to <=%u bytes at level %i with "
                   "dictionary size %u \n",
      ...
      if (FUZ_rand(&lseed) & 0xF) {
        ...
        ZSTD_frameParameters const fPar = {
            ...
            0 /*NodictID*/}; /* note : since dictionary is fake, dictIDflag has
                                no impact */
...

    {
      ...
      for (totalTestSize = 0, cSize = 0, n = 0; n < nbChunks; n++) {
...

        {
          size_t const compressResult =
              ZSTD_compressContinue(ctx, cBuffer + cSize, cBufferSize - cSize,
...

      {
        ...
        CHECK(ZSTD_isError(flushResult), "multi-segments epilogue error : %s",
    ...
    /* ensure memory requirement is good enough (should always be true) */
    {
      ...
      {
        size_t const roundBuffSize =
            ZSTD_decodingBufferSize_min(zfh.windowSize, zfh.frameContentSize);
        CHECK_Z(roundBuffSize);
        CHECK((roundBuffSize > totalTestSize) &&
                  (zfh.frameContentSize != ZSTD_CONTENTSIZE_UNKNOWN),
              "ZSTD_decodingBufferSize_min() requires more memory (%u) than "
              "necessary (%u)",
    ...
    CHECK(totalCSize != cSize, "compressed data should be fully read") {
      ...
      CHECK(crcOrig != crcDest,
            ...
            (U32)findDiff(mirrorBuffer, dstBuffer, totalTestSize),
            (U32)totalTestSize);
    }
  } /* for ( ; (testNb <= nbTests) */
...
/*! readU32FromChar() :
   ...
   reading. Note : function result can overflow if digit string > MAX_UINT */
 ...
 */
static unsigned longCommandWArg(const char** stringPtr,
