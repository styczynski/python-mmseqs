...
static int is_name_bad(char const* name) {
  ...
  if (len > g_max_name_len) g_max_name_len = len;
  for (; *name != '\0'; ++name)
    if (*name == ',') return 1;
  return 0;
}
  ...
  for (size_t i = 0; i < nargs; ++i) {
    if (long_options[i].val < 128) {
      ...
      fprintf(stderr, "--%s      \t- %s %s\n", long_options[i].name,
              long_extras[i].help, required_message(long_extras[i].opt_type));
    }
  }
}
...
static int parse_args(int argc, char** argv) {
  ...
  int bad = 0;
  for (size_t i = 0; i < nargs; ++i) {
    if (long_extras[i].opt_type != required_option) continue;
    if (long_extras[i].value == NULL) continue;
    if (*long_extras[i].value != NULL) continue;
...
void tprint_names(FILE* results, char const* data_name, char const* config_name,
...
static int run_all(FILE* results) {
  ...
  for (size_t method = 0; methods[method] != NULL; ++method) {
    if (g_method != NULL && strcmp(methods[method]->name, g_method)) continue;
    for (size_t datum = 0; data[datum] != NULL; ++datum) {
      if (g_data != NULL && strcmp(data[datum]->name, g_data)) continue;
...
int main(int argc, char** argv) {
  ...
  if (g_diff) /* Diff the new results with the previous results. */
