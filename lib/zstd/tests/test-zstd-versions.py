...
def dict_compress_sample(tag, sample):
    ...
    if (
        subprocess.call(
            ["./zstd." + tag, "-D", dict_name, "-f", sample], stderr=DEVNULL
        )
        == 0
    ):
    ...
    if (
        subprocess.call(
            ["./zstd." + tag, "-D", dict_name, "-5f", sample], stderr=DEVNULL
        )
        == 0
    ):
    ...
    if (
        subprocess.call(
            ["./zstd." + tag, "-D", dict_name, "-9f", sample], stderr=DEVNULL
        )
        == 0
    ):
    ...
    if (
        subprocess.call(
            ["./zstd." + tag, "-D", dict_name, "-15f", sample], stderr=DEVNULL
        )
        == 0
    ):
    ...
    if (
        subprocess.call(
            ["./zstd." + tag, "-D", dict_name, "-18f", sample], stderr=DEVNULL
        )
        == 0
    ):
...
if __name__ == "__main__":
    ...
    dict_source_path = (
        tmp_dir + "/" + dict_source
    )  # /path/to/zstd/tests/versionsTest/dict_source
    ...
    for tag in tags:
        os.chdir(base_dir)
        dst_zstd = "{}/zstd.{}".format(
            tmp_dir, tag
        )  # /path/to/zstd/tests/versionsTest/zstd.<TAG>
        if not os.path.isfile(dst_zstd) or tag == head:
            if tag != head:
                r_dir = "{}/{}".format(
                    tmp_dir, tag
                )  # /path/to/zstd/tests/versionsTest/<TAG>
                os.makedirs(r_dir, exist_ok=True)
                os.chdir(clone_dir)
                git(
                    ["--work-tree=" + r_dir, "checkout", tag, "--", "."], False
                )
                if tag == "v0.5.0":
                    os.chdir(
                        r_dir + "/dictBuilder"
                ...
                os.chdir(
                    r_dir + "/programs"
                )  # /path/to/zstd/tests/versionsTest/<TAG>/programs
