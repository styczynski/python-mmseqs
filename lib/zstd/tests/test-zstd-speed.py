...
def send_email(emails, topic, text, have_mutt, have_mail):
    ...
    with open(logFileName, "w") as myfile:
        ...
        if have_mutt:
            execute(
                'mutt -s "' + topic + '" ' + emails + " < " + logFileName,
                verbose,
            )
        elif have_mail:
            execute(
                'mail -s "' + topic + '" ' + emails + " < " + logFileName,
                verbose,
            )
        else:
...
def git_get_changes(branch, commit, last_commit):
    ...
    if last_commit is None:
        ...
        commits = execute(
            "git --no-pager log %s %s..%s" % (fmt, last_commit, commit)
        )
    return str(
...
def benchmark_and_compare(
    ...
    with open(resultsFileName, "a") as myfile:
        ...
        if last_cspeed == None:
            log(
                "WARNING: No data for comparison for branch=%s file=%s "
                % (branch, fileName)
            )
            return ""
...
def update_config_file(branch, commit):
    last_commit = None
    commitFileName = (
        working_path + "/commit_" + branch.replace("/", "_") + ".txt"
    )
...
def double_check(
    ...
    if not args.dry_run:
        ...
        if text:
            log(
                "WARNING: redoing tests for branch %s: commit %s"
                % (branch, commit)
            )
            text = benchmark_and_compare(
                branch,
...
def test_commit(
    ...
    if not args.dry_run:
        execute(
            ...
            + 'make -C programs clean zstd zstd32 MOREFLAGS="-DZSTD_GIT_COMMIT=%s"'
            % version
        )
    md5_zstd = hashfile(hashlib.md5(), clone_path + "/programs/zstd")
    md5_zstd32 = hashfile(hashlib.md5(), clone_path + "/programs/zstd32")
    md5_zstd_clang = hashfile(
        hashlib.md5(), clone_path + "/programs/zstd_clang"
    )
    print(
        "md5(zstd)=%s\nmd5(zstd32)=%s\nmd5(zstd_clang)=%s"
        % (md5_zstd, md5_zstd32, md5_zstd_clang)
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "testFileNames",
        help="file or directory names list for speed benchmark",
    )
    parser.add_argument(
        "emails", help="list of e-mail addresses to send warnings"
    )
    parser.add_argument("--dictionary", "-D", help="path to the dictionary")
    parser.add_argument(
        "--message",
        "-m",
        help="attach an additional message to e-mail",
        default="",
    )
    parser.add_argument(
        "--repoURL",
        help="changes default repository URL",
    ...
    parser.add_argument(
        "--maxLoadAvg",
        type=float,
        help="maximum load average to start testing",
        default=0.75,
    )
    parser.add_argument(
        "--lastCLevel",
        type=int,
        help="last compression level for testing",
        default=5,
    )
    parser.add_argument(
        "--sleepTime",
    ...
    parser.add_argument(
        ...
        "-t",
        type=int,
        help="timeout for executing shell commands",
        default=1800,
    )
    parser.add_argument(
        "--dry-run",
    ...
    send_email(
        args.emails,
        "[%s:%s] test-zstd-speed.py %s has been started"
        ...
        try:
            ...
            if loadavg <= args.maxLoadAvg:
                ...
                for branch in branches:
                    commit = execute(
                        "git show -s --format=%h " + branch, verbose
                    )[0]
                    last_commit = update_config_file(branch, commit)
                    if commit == last_commit:
                        log(
                            "skipping branch %s: head %s already processed"
                            % (branch, commit)
                        )
                    else:
                        log(
                ...
                log(
                    "WARNING: main loadavg=%.2f is higher than %s"
                    % (loadavg, args.maxLoadAvg)
                )
            if verbose:
                log("sleep for %s seconds" % args.sleepTime)
        except Exception as e:
            stack = traceback.format_exc()
            email_topic = "[%s:%s] ERROR in %s:%s" % (
                email_header,
                pid,
                branch,
                commit,
