...
#define WELCOME_MESSAGE                                                       \
  "*** %s %s %i-bits, by %s ***\n", PROGRAM_DESCRIPTION, ZSTD_VERSION_STRING, \
      (int)(sizeof(void*) * 8), AUTHOR
...
#define CHECKTIME(r)                            \
  {                                             \
    if (BMK_timeSpan(g_time) > g_timeLimit_s) { \
...
static const U32 mintable[NUM_PARAMS] = {ZSTD_WINDOWLOG_MIN,
                                         ZSTD_CHAINLOG_MIN,
                                         ZSTD_HASHLOG_MIN,
                                         ZSTD_SEARCHLOG_MIN,
...
static const U32 maxtable[NUM_PARAMS] = {
    ZSTD_WINDOWLOG_MAX,    ZSTD_CHAINLOG_MAX,
    ZSTD_HASHLOG_MAX,      ZSTD_SEARCHLOG_MAX,
    ZSTD_SEARCHLENGTH_MAX, ZSTD_TARGETLENGTH_MAX,
    ZSTD_btultra,          FADT_MAX};

/* # of values parameters can take on */
static const U32 rangetable[NUM_PARAMS] = {WLOG_RANGE, CLOG_RANGE, HLOG_RANGE,
                                           SLOG_RANGE, SLEN_RANGE, TLEN_RANGE,
                                           STRT_RANGE, FADT_RANGE};

/* ZSTD_cctxSetParameter() index to set */
static const ZSTD_cParameter cctxSetParamTable[NUM_PARAMS] = {
    ZSTD_p_windowLog,           ZSTD_p_chainLog,       ZSTD_p_hashLog,
    ZSTD_p_searchLog,           ZSTD_p_minMatch,       ZSTD_p_targetLength,
    ZSTD_p_compressionStrategy, ZSTD_p_forceAttachDict};

/* names of parameters */
static const char* g_paramNames[NUM_PARAMS] = {
    "windowLog",    "chainLog",     "hashLog",  "searchLog",
    "searchLength", "targetLength", "strategy", "forceAttachDict"};
...
static paramValues_t sanitizeParams(paramValues_t params) {
  ...
  if (params.vals[strt_ind] == ZSTD_dfast) params.vals[slog_ind] = 0;
  if (params.vals[strt_ind] != ZSTD_btopt &&
      params.vals[strt_ind] != ZSTD_btultra &&
...
static paramValues_t adjustParams(paramValues_t p, const size_t maxBlockSize,
...
static U32 BMK_timeSpan(const UTIL_time_t tStart) {
  return (U32)(UTIL_clockSpanMicro(tStart) / 1000000ULL);
}

static U32 FUZ_rotl32(U32 x, U32 r) { return ((x << r) | (x >> (32 - r))); }
...
static paramValues_t overwriteParams(paramValues_t base,
...
static void paramVaryOnce(const varInds_t paramIndex, const int amt,
                          paramValues_t* ptr) {
...
static void paramVariation(paramValues_t* ptr, memoTable_t* mtAll,
                           const U32 nbChanges) {
  paramValues_t p;
  U32 validated = 0;
  while (!validated) {
...
static int feasible(const BMK_benchResult_t results,
 ...
 * to 1 (with 0 at 0 and 1 being fully fulfilling the constraint), summed with a
 * logarithmic bonus to exceeding the constraint value. We also give linear
 * ratio for compression ratio. The constant factors are experimental.
 */
static double resultScore(const BMK_benchResult_t res, const size_t srcSize,
                          const constraint_t target) {
...
static double resultDistLvl(const BMK_benchResult_t result1,
...
static int compareResultLT(const BMK_benchResult_t result1,
                           ...
                           const constraint_t target, size_t srcSize) {
  ...
  return feasible(result2, target) || (!feasible(result1, target) &&
...
static void optimizerAdjustInput(paramValues_t* pc, const size_t maxBlockSize) {
  ...
  if (pc->vals[wlog_ind] != PARAM_UNSET) {
    ...
    if (maxBlockSize < (1ULL << 31) && sshb + 1 < pc->vals[wlog_ind]) {
      ...
      if (adjust != pc->vals[wlog_ind]) {
        ...
        DISPLAY(
            "Warning: windowLog larger than src/block size, adjusted to %u\n",
            pc->vals[wlog_ind]);
      }
    }
  }
  ...
  if (pc->vals[wlog_ind] != PARAM_UNSET && pc->vals[clog_ind] != PARAM_UNSET) {
    ...
    if (pc->vals[clog_ind] > maxclog) {
      ...
      DISPLAY(
          "Warning: chainlog too much larger than windowLog size, adjusted to "
          "%u\n",
          pc->vals[clog_ind]);
    }
  }
  ...
  if (pc->vals[wlog_ind] != PARAM_UNSET && pc->vals[hlog_ind] != PARAM_UNSET) {
    if (pc->vals[wlog_ind] + 1 < pc->vals[hlog_ind]) {
      ...
      DISPLAY(
          "Warning: hashlog too much larger than windowLog size, adjusted to "
          "%u\n",
          pc->vals[hlog_ind]);
    }
  }
...
static int redundantParams(const paramValues_t paramValues,
                           const constraint_t target,
...
static void BMK_displayOneResult(FILE* f, winnerInfo_t res,
 ...
 * discovered */
static void BMK_printWinner(FILE* f, const int cLevel,
  ...
  if (TIMED) {
    ...
    fprintf(
        f, "%1lu:%2lu:%05.2f - ", (unsigned long)minutes / 60,
        (unsigned long)minutes % 60,
        (double)(time - minutes * TIMELOOP_NANOSEC * 60ULL) / TIMELOOP_NANOSEC);
  }
...
static int speedSizeCompare(const BMK_benchResult_t r1,
...
static int insertWinner(const winnerInfo_t w,
...
static void BMK_printWinnerOpt(FILE* f, const U32 cLevel,
                               const BMK_benchResult_t result,
...
static void BMK_printWinners2(FILE* f, const winnerInfo_t* winners,
                              const size_t srcSize) {
  int cLevel;
  ...
  for (cLevel = 0; cLevel <= NB_LEVELS_TRACKED; cLevel++)
    BMK_printWinner(f, cLevel, winners[cLevel].result, winners[cLevel].params,
                    srcSize);
}

static void BMK_printWinners(FILE* f, const winnerInfo_t* winners,
                             const size_t srcSize) {
  fseek(f, 0, SEEK_SET);
...
static int createBuffersFromMemory(buffers_t* buff, void* srcBuffer,
...
static int createBuffers(buffers_t* buff,
...
static size_t sanitizeVarArray(varInds_t* varNew, const size_t varLength,
                               const varInds_t* varArray,
  ...
  for (i = 0; i < varLength; i++) {
    if (!((varArray[i] == clog_ind && strat == ZSTD_fast) ||
          ...
          (varArray[i] == tlen_ind && strat != ZSTD_btopt &&
           strat != ZSTD_btultra && strat != ZSTD_fast))) {
      varNew[j] = varArray[i];
      j++;
    }
  }
...
/* pass in using dict. */
static size_t variableParams(const paramValues_t paramConstraints,
                             varInds_t* res, const int usingDictionary) {
  varInds_t i;
  size_t j = 0;
  for (i = 0; i < NUM_PARAMS; i++) {
...
static unsigned memoTableIndDirect(const paramValues_t* ptr,
                                   const varInds_t* varyParams,
...
static size_t memoTableGet(const memoTable_t* memoTableArray,
...
static void memoTableSet(const memoTable_t* memoTableArray,
                         const paramValues_t p, const BYTE value) {
  const memoTable_t mt = memoTableArray[p.vals[strt_ind]];
  switch (mt.tableType) {
    case directMap:
...
static memoTable_t* createMemoTableArray(const paramValues_t p,
  ...
  memoTable_t* mtAll =
      (memoTable_t*)calloc(sizeof(memoTable_t), (ZSTD_btultra + 1));
...
/* specifiy strategy */
static void randomConstrainedParams(paramValues_t* pc,
                                    const memoTable_t* memoTableArray,
...
static BMK_benchOutcome_t BMK_benchMemInvertible(
    buffers_t buf, contexts_t ctx, int cLevel, const paramValues_t* comprParams,
...
static int BMK_benchParam(BMK_benchResult_t* resultPtr, buffers_t buf,
                          contexts_t ctx, paramValues_t cParams) {
...
#define CBENCHMARK(conditional, resultvar, tmpret, mode, sec)                 \
  {                                                                           \
    if (conditional) {                                                        \
      BMK_benchOutcome_t const outcome =                                      \
          BMK_benchMemInvertible(buf, ctx, BASE_CLEVEL, &cParams, mode, sec); \
      ...
      {                                                                       \
        ...
        if (mode != BMK_compressOnly) {                                       \
          resultvar.dSpeed = tmpResult.dSpeed;                                \
        }                                                                     \
      }                                                                       \
    }                                                                         \
  }
...
#define VARIANCE 1.2
static int allBench(BMK_benchResult_t* resultPtr, const buffers_t buf,
  ...
  *resultPtr = benchres;
  ...
  if (benchres.cSpeed) {
    ...
    uncertaintyConstantC =
        ((loopDurationC + (double)(2 * g_clockGranularity)) / loopDurationC);
  }
  ...
  if (benchres.dSpeed) {
    ...
    uncertaintyConstantD =
        ((loopDurationD + (double)(2 * g_clockGranularity)) / loopDurationD);
  }
 ...
 * benchmarked before. */
static int benchMemo(BMK_benchResult_t* resultPtr, const buffers_t buf,
                     const contexts_t ctx, const paramValues_t cParams,
                     const constraint_t target, BMK_benchResult_t* winnerResult,
  ...
  /* establish speed objectives (relative to level 1) */
  {
    ...
    for (l = 2; l <= NB_LEVELS_TRACKED; l++) {
      ...
      g_level_constraint[l].windowLog_max =
          (l < 20)
              ? 23
              : l + 5; /* only --ultra levels >= 20 can use windowlog > 23 */
      g_level_constraint[l].strategy_max =
          (l < 19) ? ZSTD_btopt
                   : ZSTD_btultra; /* level 19 is allowed to use btultra */
    }
  }
}
  ...
  for (cLevel = 1; cLevel <= NB_LEVELS_TRACKED; cLevel++) {
    ...
    if ((double)testResult.cSize <=
      ...
      double O_CSpeed_note =
          O_ratioNote * (30 + 10 * cLevel) + log(winners[cLevel].result.cSpeed);

      double W_DSpeed_note =
          W_ratioNote * (20 + 2 * cLevel) + log(testResult.dSpeed);
      double O_DSpeed_note =
          O_ratioNote * (20 + 2 * cLevel) + log(winners[cLevel].result.dSpeed);

      if (W_DMemUsed_note < O_DMemUsed_note) {
        /* uses too much Decompression memory for too little benefit */
        if (W_ratio > O_ratio)
          DISPLAY(
              "Decompression Memory : %5.3f @ %4.1f MB  vs  %5.3f @ %4.1f MB   "
              ": not enough for level %i\n",
      ...
      if (W_CMemUsed_note < O_CMemUsed_note) {
        ...
        if (W_ratio > O_ratio)
          DISPLAY(
              "Compression Memory : %5.3f @ %4.1f MB  vs  %5.3f @ %4.1f MB   : "
              "not enough for level %i\n",
      ...
      if (W_CSpeed_note < O_CSpeed_note) {
        ...
        if (W_ratio > O_ratio)
          DISPLAY(
              "Compression Speed : %5.3f @ %4.1f MB/s  vs  %5.3f @ %4.1f MB/s  "
              " : not enough for level %i\n",
      ...
      if (W_DSpeed_note < O_DSpeed_note) {
        ...
        if (W_ratio > O_ratio)
          DISPLAY(
              "Decompression Speed : %5.3f @ %4.1f MB/s  vs  %5.3f @ %4.1f "
              "MB/s   : not enough for level %i\n",
      ...
      if (W_ratio < O_ratio)
        DISPLAY(
            "Solution %4.3f selected over %4.3f at level %i, due to better "
...
static BYTE* NB_TESTS_PLAYED(paramValues_t p) {
  ...
  return &g_alreadyTested[(XXH64((void*)&p2, sizeof(p2), 0) >> 3) &
                          PARAMTABLEMASK];
}

static void playAround(FILE* f, winnerInfo_t* winners, paramValues_t p,
                       const buffers_t buf, const contexts_t ctx) {
...
static void BMK_selectRandomStart(FILE* f, winnerInfo_t* winners,
                                  const buffers_t buf, const contexts_t ctx) {
...
static int benchOnce(const buffers_t buf, const contexts_t ctx,
  ...
  BMK_benchResult_t testResult;
  g_params =
      adjustParams(overwriteParams(cParamsToPVals(ZSTD_getCParams(
                                       cLevel, buf.maxBlockSize, ctx.dictSize)),
                                   g_params),
 ...
 * to #dim + 2) if it finds a candidate exceeding winnerInfo, it will repeat.
 * Otherwise, it will stop the current stage of hill climbing. Each iteration of
 * hill climbing proceeds in 2 'phases'. Phase 1 climbs according to the
 * resultScore function, which is effectively a linear increase in reward until
 * it reaches the constraint-satisfying value, it which point any excess results
 * in only logarithmic reward. This aims to find some constraint-satisfying
 * point. Phase 2 optimizes in accordance with what the original function sets
 * out to maximize, with all feasible solutions valued over all infeasible
 * solutions.
 */
 ...
 */
static winnerInfo_t climbOnce(const constraint_t target, memoTable_t* mtAll,
                              const buffers_t buf, const contexts_t ctx,
    ...
    while (better) {
      ...
      for (i = 0; i < varLen; i++) {
        for (offset = -1; offset <= 1; offset += 2) {
          ...
          candidateInfo.params = cparam;
          paramVaryOnce(mtAll[cparam.vals[strt_ind]].varArray[i], offset,
          ...
          if (paramValid(candidateInfo.params)) {
            ...
            DEBUGOUTPUT("Res: %d\n", res);
            if (res == BETTER_RESULT) { /* synonymous with better when called w/
                                           infeasibleBM */
      ...
      for (dist = 2; dist < varLen + 2; dist++) { /* varLen is # dimensions */
        for (i = 0; i < (1 << varLen) / varLen + 2; i++) {
          ...
          if (res == BETTER_RESULT) { /* synonymous with better in this case*/
            winnerInfo = candidateInfo;
            better = 1;
            if (compareResultLT(bestFeasible1.result, winnerInfo.result, target,
                                buf.srcSize)) {
              bestFeasible1 = winnerInfo;
            }
            break;
      ...
      if (!better) { /* infeas -> feas -> stop */
        ...
        feas = 1;
        better = 1;
        winnerInfo = bestFeasible1; /* note with change, bestFeasible may not
...
/* flexible parameters: iterations of failed climbing (or if we do non-random,
   maybe this is when everything is close to visitied) weight more on visit for
   bad results, less on good results/more on later results / ones with more
   failures. allocate memoTable here.
 */
static winnerInfo_t optimizeFixedStrategy(
    const buffers_t buf, const contexts_t ctx, const constraint_t target,
    paramValues_t paramTarget, const ZSTD_strategy strat,
 ...
 * strategy = ZSTD_fast) cLevel - compression level to exceed (all solutions
 * must be > lvl in cSpeed + ratio)
 */
...
static int optimizeForSize(const char* const* const fileNamesTable,
                           const size_t nbFiles, const char* dictFileName,
  ...
  if (g_optmode) {
    ...
    BMK_printWinnerOpt(stdout, cLevelOpt, winner.result, winner.params, target,
                       buf.srcSize);
  }
  ...
  if (target.cSpeed != 0) {
    DISPLAYLEVEL(2, " - limit compression speed %u MB/s", target.cSpeed >> 20);
  }
  if (target.dSpeed != 0) {
  ...
  if (target.cMem != (U32)-1) {
    DISPLAYLEVEL(2, " - limit memory %u MB", target.cMem >> 20);
  }
    ...
    /* find best solution from default params */
    {
      ...
      if (paramTarget.vals[strt_ind] == PARAM_UNSET) {
        ...
        for (i = 1; i <= maxSeeds; i++) {
          int ec;
          CParams = overwriteParams(cParamsToPVals(ZSTD_getCParams(
          ...
          if (!ec && compareResultLT(winner.result, candidate,
        ...
        BMK_printWinnerOpt(stdout, CUSTOM_LEVEL, winner.result, winner.params,
                           target, buf.srcSize);
      }
    }
    ...
    /* start 'real' optimization */
    {
      ...
      if (paramTarget.vals[strt_ind] == PARAM_UNSET) {
        ...
        while (st && tries > 0) {
          ...
          wc = optimizeFixedStrategy(buf, ctx, target, paramBase, st, allMT,
                                     tries);
        ...
        winner = optimizeFixedStrategy(buf, ctx, target, paramBase,
                                       paramTarget.vals[strt_ind], allMT,
                                       g_maxTries);
      }
    }
 ...
 * reading. Note : function will exit() program if digit sequence overflows */
...
static int usage_advanced(void) {
  ...
  DISPLAY(
      ...
      "block) \n");
  DISPLAY(
      " --optimize=  : same as -O with more verbose syntax (see README.md)\n");
  ...
  if (filenamesStart == 0) {
    ...
    if (seperateFiles) {
      for (i = 0; i < argc - filenamesStart; i++) {
        if (g_optimizer) {
          result = optimizeForSize(argv + filenamesStart + i, 1, dictFileName,
                                   target, paramTarget, cLevelOpt, cLevelRun,
                                   memoTableLog);
          if (result) {
          ...
          result =
              benchFiles(argv + filenamesStart + i, 1, dictFileName, cLevelRun);
          if (result) {
            DISPLAY("Error on File %d", i);
            return result;
          }
      ...
      if (g_optimizer) {
        result = optimizeForSize(argv + filenamesStart, argc - filenamesStart,
                                 dictFileName, target, paramTarget, cLevelOpt,
