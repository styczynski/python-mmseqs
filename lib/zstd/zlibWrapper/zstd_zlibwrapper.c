...
static int ZWRAP_initializeCStream(ZWRAP_CCtx* zwc, const void* dict,
                                   size_t dictSize,
                                   unsigned long long pledgedSrcSize) {
  ...
  if (!pledgedSrcSize) pledgedSrcSize = zwc->pledgedSrcSize;
  {
    ...
    size_t initErr;
    LOG_WRAPPERC(
        "pledgedSrcSize=%d windowLog=%d chainLog=%d hashLog=%d searchLog=%d "
        "searchLength=%d strategy=%d\n",
        (int)pledgedSrcSize, params.cParams.windowLog, params.cParams.chainLog,
        params.cParams.hashLog, params.cParams.searchLog,
        params.cParams.searchLength, params.cParams.strategy);
...
ZEXTERN int ZEXPORT z_deflateInit_ OF((z_streamp strm, int level,
  ...
  if (level == Z_DEFAULT_COMPRESSION) level = ZWRAP_DEFAULT_CLEVEL;
...
int ZWRAP_deflateReset_keepDict(z_streamp strm) {
  ...
  if (!g_ZWRAP_useZSTDcompression) return deflateReset(strm);
...
ZEXTERN int ZEXPORT z_deflate OF((z_streamp strm, int flush)) {
  ...
  LOG_WRAPPERC(
      "- deflate2 flush=%d avail_in=%d avail_out=%d total_in=%d total_out=%d\n",
      (int)flush, (int)strm->avail_in, (int)strm->avail_out,
      (int)strm->total_in, (int)strm->total_out);
  if (strm->avail_in > 0) {
  ...
  LOG_WRAPPERC(
      "- deflate3 flush=%d avail_in=%d avail_out=%d total_in=%d total_out=%d\n",
      (int)flush, (int)strm->avail_in, (int)strm->avail_out,
      (int)strm->total_in, (int)strm->total_out);
  return Z_OK;
}
...
ZEXTERN uLong ZEXPORT z_deflateBound OF((z_streamp strm, uLong sourceLen)) {
  if (!g_ZWRAP_useZSTDcompression) return deflateBound(strm, sourceLen);

  return ZSTD_compressBound(sourceLen);
}

ZEXTERN int ZEXPORT z_deflateParams OF((z_streamp strm, int level,
                                        int strategy)) {
  if (!g_ZWRAP_useZSTDcompression) {
...
ZEXTERN int ZEXPORT z_inflateInit_ OF((z_streamp strm, const char* version,
                                       int stream_size)) {
...
ZEXTERN int ZEXPORT z_inflate OF((z_streamp strm, int flush)) {
  ...
  LOG_WRAPPERD(
      "- inflate1 flush=%d avail_in=%d avail_out=%d total_in=%d total_out=%d\n",
      (int)flush, (int)strm->avail_in, (int)strm->avail_out,
      (int)strm->total_in, (int)strm->total_out);
  ...
  if (zwd->totalInBytes < ZLIB_HEADERSIZE) {
    if (zwd->totalInBytes == 0 && strm->avail_in >= ZLIB_HEADERSIZE) {
      if (MEM_readLE32(strm->next_in) != ZSTD_MAGICNUMBER) {
        ...
        {
          int const result = (flush == Z_INFLATE_SYNC) ? inflateSync(strm)
                                                       : inflate(strm, flush);
          LOG_WRAPPERD(
              "- inflate3 flush=%d avail_in=%d avail_out=%d total_in=%d "
              "total_out=%d res=%d\n",
              (int)flush, (int)strm->avail_in, (int)strm->avail_out,
      ...
      if (MEM_readLE32(zwd->headerBuf) != ZSTD_MAGICNUMBER) {
        ...
        strm->avail_out = 0;
        {
          ...
          LOG_WRAPPERD("ZLIB inflate errorCode=%d strm->avail_in=%d\n", dErr,
                       (int)strm->avail_in);
          if (dErr != Z_OK) return ZWRAPD_finishWithError(zwd, strm, dErr);
        }
...
ZEXTERN int ZEXPORT z_deflateCopy OF((z_streamp dest, z_streamp source)) {
  if (!g_ZWRAP_useZSTDcompression) return deflateCopy(dest, source);
  return ZWRAPC_finishWithErrorMsg(source, "deflateCopy is not supported!");
}

ZEXTERN int ZEXPORT z_deflateTune OF((z_streamp strm, int good_length,
...
ZEXTERN int ZEXPORT z_deflatePending OF((z_streamp strm, unsigned* pending,
                                         int* bits)) {
  if (!g_ZWRAP_useZSTDcompression) return deflatePending(strm, pending, bits);
...
ZEXTERN int ZEXPORT z_deflatePrime OF((z_streamp strm, int bits, int value)) {
  if (!g_ZWRAP_useZSTDcompression) return deflatePrime(strm, bits, value);
...
ZEXTERN int ZEXPORT z_deflateSetHeader OF((z_streamp strm, gz_headerp head)) {
  if (!g_ZWRAP_useZSTDcompression) return deflateSetHeader(strm, head);
...
ZEXTERN int ZEXPORT z_inflateBack OF((z_streamp strm, in_func in,
...
ZEXTERN uLong ZEXPORT z_compressBound OF((uLong sourceLen)) {
  if (!g_ZWRAP_useZSTDcompression) return compressBound(sourceLen);
...
ZEXTERN uLong ZEXPORT z_adler32_z OF((uLong adler, const Bytef* buf,
                                      z_size_t len)) {
  return adler32_z(adler, buf, len);
}

ZEXTERN uLong ZEXPORT z_crc32_z OF((uLong crc, const Bytef* buf,
                                    z_size_t len)) {
