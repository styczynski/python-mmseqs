...
unsigned size;
{
  ...
  if (state.state->mode != GZ_READ && state.state->mode != GZ_WRITE) return -1;
...
int ZEXPORT gzrewind(file) gzFile file;
{
  ...
  if (LSEEK(state.state->fd, state.state->start, SEEK_SET) == -1) return -1;
  gz_reset(state);
  return 0;
}
...
int ZEXPORT gzeof(file) gzFile file;
{
  ...
  state = (gz_statep)file;
  if (state.state->mode != GZ_READ && state.state->mode != GZ_WRITE) return 0;
...
int *errnum;
{
  ...
  if (errnum != NULL) *errnum = state.state->err;
...
void ZEXPORT gzclearerr(file) gzFile file;
{
  ...
  if (file == NULL) return;
  state = (gz_statep)file;
  if (state.state->mode != GZ_READ && state.state->mode != GZ_WRITE) return;
...
/* Create an error message in allocated memory and set state.state->err and
   ...
   Do not try to free or allocate space if the error is Z_MEM_ERROR (out of
...
const char *msg;
{
  ...
  if (state.state->msg != NULL) {
    if (state.state->err != Z_MEM_ERROR) free(state.state->msg);
    state.state->msg = NULL;
  }

  /* if fatal, set state.state->x.have to 0 so that the gzgetc() macro fails */
  if (err != Z_OK && err != Z_BUF_ERROR) state.state->x.have = 0;
