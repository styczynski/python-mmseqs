...
/* Use read() to load a buffer -- return -1 on error, otherwise 0.  Read from
   ...
   state.state->msg as appropriate. This function needs to loop on read(), since
   read() is not guaranteed to
...
local int gz_avail(state) gz_statep state;
{
  ...
  if (state.state->err != Z_OK && state.state->err != Z_BUF_ERROR) return -1;
...
/* Look for gzip header, set up for inflate or copy.  state.state->x.have must
   be 0. If this is the first time in, allocate required memory.
   state.state->how will be left unchanged if there is no more input data
   available, will be set to COPY if there is no gzip header and direct copying
   will be performed, or it will be set to GZIP for decompression.  If direct
   copying, then leftover input data from the input buffer will be copied to the
   output buffer.  In that case, all further file reads will be directly to
   either the output buffer or a user buffer.  If decompressing, the inflate
   state will be initialized. gz_look() will return 0 on success or -1 on
   failure. */
...
/* Decompress from input to the provided next_out and avail_out in the state.
   ...
   decompressed data.  If the gzip stream completes, state.state->how is reset
   to LOOK to look for the next gzip stream or raw data, once
   state.state->x.have is depleted.  Returns 0 on success, -1 on failure. */
local int gz_decomp(state) gz_statep state;
{
  ...
  if (ret == Z_STREAM_END) state.state->how = LOOK;
...
/* Fetch data and put it in the output buffer.  Assumes state.state->x.have is
   0. Data is either copied from the input file or decompressed from the input
   file depending on state.state->how.  If state.state->how is LOOK, then a gzip
   header is looked for to determine whether to copy or decompress.  Returns -1
   on error, otherwise 0.  gz_fetch() will leave state.state->how as COPY or
   GZIP unless the
  ...
  do {
    switch (state.state->how) {
      case LOOK: /* -> LOOK, COPY (only if never GZIP), or GZIP */
        ...
        if (state.state->how == LOOK) return 0;
        break;
      case COPY: /* -> COPY */
...
z_size_t len;
{
  ...
  if (state.state->seek) {
    state.state->seek = 0;
    if (gz_skip(state, state.state->skip) == -1) return 0;
  }
  ...
  do {
    ...
    if (state.state->x.have) {
      if (state.state->x.have < n) n = state.state->x.have;
...
gzFile file;
{
  ...
  if (state.state->seek) {
    state.state->seek = 0;
    if (gz_skip(state, state.state->skip) == -1) return -1;
  }
  ...
  if (state.state->x.next == state.state->out) {
    ...
    while (src > state.state->out) *--dest = *--src;
...
int len;
{
  ...
  if (state.state->seek) {
    state.state->seek = 0;
    if (gz_skip(state, state.state->skip) == -1) return NULL;
  }
  ...
  if (left) do {
      ...
      if (eol != NULL) n = (unsigned)(eol - state.state->x.next) + 1;
...
int ZEXPORT gzclose_r(file) gzFile file;
{
  ...
  if (state.state->mode != GZ_READ) return Z_STREAM_ERROR;
