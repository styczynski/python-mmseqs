...
struct ZSTD_DCtx_s {
  ...
  const void *vBase; /* virtual start of previous segment if it was just before
                        curr one */
  const void *dictEnd; /* end of previous segment */
  size_t expected;
  ZSTD_frameParams fParams;
  blockType_e bType; /* used in ZSTD_decompressContinue(), to transfer blockType
  ...
  if (ZSTD_readLE32(src) != ZSTD_MAGICNUMBER) {
    if ((ZSTD_readLE32(src) & 0xFFFFFFF0U) == ZSTD_MAGIC_SKIPPABLE_START) {
      if (srcSize < ZSTD_skippableHeaderSize)
        return ZSTD_skippableHeaderSize; /* magic number + skippable frame
                                            length */
...
unsigned long long ZSTD_findDecompressedSize(const void *src, size_t srcSize) {
  {
    ...
    while (srcSize >= ZSTD_frameHeaderSize_prefix) {
      ...
      if ((magicNumber & 0xFFFFFFF0U) == ZSTD_MAGIC_SKIPPABLE_START) {
        ...
        if (srcSize < ZSTD_skippableHeaderSize) return ERROR(srcSize_wrong);
        skippableSize =
...

  {
    ...
    switch (litEncType) {
      ...
      case set_compressed:
        if (srcSize < 5)
          return ERROR(
              corruption_detected); /* srcSize >= MIN_CBLOCK_SIZE == 3; here we
                                       need up to 5 for case 3 */
        {
                  ...
                  (litEncType == set_repeat)
                      ? (singleStream
                             ? HUF_decompress1X_usingDTable(
                                   dctx->litBuffer, litSize, istart + lhSize,
...
static size_t ZSTD_buildSeqTable(FSE_DTable *DTableSpace,
  ...
  switch (type) {
    ...
    case set_compressed: {
      ...
      if ((spaceUsed32 << 2) > workspaceSize) return ERROR(GENERIC);
      ...
      workspaceSize -= (spaceUsed32 << 2);
      {
        ...
        FSE_buildDTable_wksp(DTableSpace, norm, max, tableLog, workspace,
                             workspaceSize);
        *DTablePtr = DTableSpace;
        return headerSize;
      }
  ...
  if (ip + 4 > iend) return ERROR(srcSize_wrong); /* minimum possible size */
  {
    ...
    /* Build DTables */
    {
      size_t const llhSize = ZSTD_buildSeqTable(
          ...
          iend - ip, LL_defaultDTable, dctx->fseEntropy,
    ...
    }
    {
      size_t const ofhSize = ZSTD_buildSeqTable(
          ...
          iend - ip, OF_defaultDTable, dctx->fseEntropy,
          dctx->entropy.workspace, sizeof(dctx->entropy.workspace));
    ...
    }
    {
      size_t const mlhSize = ZSTD_buildSeqTable(
          ...
          iend - ip, ML_defaultDTable, dctx->fseEntropy,
          dctx->entropy.workspace, sizeof(dctx->entropy.workspace));
...
size_t ZSTD_execSequenceLast7(BYTE *op, BYTE *const oend, seq_t sequence,
  ...
  if (oMatchEnd > oend)
    return ERROR(
        dstSize_tooSmall); /* last match must start at a minimum distance of
                              WILDCOPY_OVERLENGTH from oend */
  if (iLitEnd > litLimit)
...
size_t ZSTD_execSequence(BYTE *op, BYTE *const oend, seq_t sequence,
  ...
  if (iLitEnd > litLimit)
    return ERROR(corruption_detected); /* over-read beyond lit buffer */
  if (oLitEnd > oend_w)
    return ZSTD_execSequenceLast7(op, oend, sequence, litPtr, litLimit, base,
                                  vBase, dictEnd);
  ...
  if (sequence.litLength > 8)
    ZSTD_wildcopy(op + 8, (*litPtr) + 8,
                  sequence.litLength -
                      8); /* note : since oLitEnd <= oend-WILDCOPY_OVERLENGTH,
...
#define ADVANCED_SEQS 4
    ...
    seqState.gotoDict = (uPtrDiff)dictEnd -
    ...
    for (; (BIT_reloadDStream(&(seqState.DStream)) <= BIT_DStream_completed) &&
      ...
      size_t const oneSeqSize = ZSTD_execSequenceLong(
          op, oend, sequences[(seqNb - ADVANCED_SEQS) & STOSEQ_MASK], &litPtr,
    ...
    for (; seqNb < nbSeq; seqNb++) {
      size_t const oneSeqSize =
          ZSTD_execSequenceLong(op, oend, sequences[seqNb & STOSEQ_MASK],
...
static size_t ZSTD_decompressBlock_internal(
  ...
  if (sizeof(size_t) > 4) /* do not enable prefetching on 32-bits x86, as it's
                             performance detrimental */
    /* likely because of register pressure */
    /* if that's the correct cause, then 32-bits ARM should be affected
     * differently */
    /* it would be good to test this on ARM real hardware, to see if prefetch
     * version improves speed */
...
static size_t ZSTD_decompressFrame(ZSTD_DCtx *dctx, void *dst,
  ...
  while (1) {
    ...
    switch (blockProperties.blockType) {
      case bt_compressed:
        decodedSize =
            ZSTD_decompressBlock_internal(dctx, op, oend - op, ip, cBlockSize);
        break;
      case bt_raw:
      ...
      case bt_rle:
        decodedSize =
            ZSTD_generateNxBytes(op, oend - op, *ip, blockProperties.origSize);
        break;
      case bt_reserved:
      default:
        return ERROR(corruption_detected);
    ...
    case ZSTDds_getFrameHeaderSize:
      ...
      if ((ZSTD_readLE32(src) & 0xFFFFFFF0U) ==
        ...
        dctx->expected =
            ...
            ZSTD_frameHeaderSize_prefix; /* magic number + skippable frame
                                            length */
...
static size_t ZSTD_loadEntropy(ZSTD_entropyTables_t *entropy,
  ...
  {
    size_t const hSize =
        HUF_readDTableX4_wksp(entropy->hufTable, dictPtr, dictEnd - dictPtr,
...

  {
    ...
    CHECK_E(FSE_buildDTable_wksp(
                entropy->OFTable, offcodeNCount, offcodeMaxValue, offcodeLog,
                entropy->workspace, sizeof(entropy->workspace)),
...

  {
    ...
    CHECK_E(FSE_buildDTable_wksp(
                entropy->MLTable, matchlengthNCount, matchlengthMaxValue,
...

  {
    ...
    CHECK_E(FSE_buildDTable_wksp(
                entropy->LLTable, litlengthNCount, litlengthMaxValue,
...
static size_t ZSTD_loadEntropy_inDDict(ZSTD_DDict *ddict) {
  ...
  /* load entropy tables */
  CHECK_E(
      ZSTD_loadEntropy(&ddict->entropy, ddict->dictContent, ddict->dictSize),
      dictionary_corrupted);
...
struct ZSTD_DStream_s {
  ...
  size_t outEnd;
  size_t blockSize;
  BYTE headerBuffer[ZSTD_FRAMEHEADERSIZE_MAX]; /* tmp buffer to store frame
                                                  header */
  size_t lhSize;
  ZSTD_customMem customMem;
  void *legacyContext;
  ...
  while (someMoreWork) {
    switch (zds->stage) {
      ...
      case zdss_loadHeader: {
        ...
        if (zds->fParams.frameContentSize &&
          ...
          if (cSize <= (size_t)(iend - istart)) {
            size_t const decompressedSize = ZSTD_decompress_usingDDict(
                zds->dctx, op, oend - op, istart, cSize, zds->ddict);
        ...
        {
          size_t const h1Size = ZSTD_nextSrcSizeToDecompress(
                                          ...
                                          h1Size));
          {
            ...
            CHECK_F(ZSTD_decompressContinue(
                zds->dctx, NULL, 0, zds->headerBuffer + h1Size, h2Size));
          }
        }
      ...
      case zdss_load: {
        ...
        /* decode loaded input */
        {
          ...
          size_t const decodedSize = ZSTD_decompressContinue(
              zds->dctx, zds->outBuff + zds->outStart,
              zds->outBuffSize - zds->outStart, zds->inBuff, neededInSize);
          if (ZSTD_isError(decodedSize)) return decodedSize;
