...
#if DO_ZSTD
  if (ZSTD_isError(ZSTD_resetCStream(state, 0))) return -EIO;
#endif

#if DO_ZLIB
  if (zlib_deflateReset(&state) != Z_OK) return -EIO;
#endif
...
static ssize_t zstd_compress_test_write(struct file *file,
  ...
  while (remaining > 0) {
    ...
    if (copy_from_user(buffer_in, buf, copy_size)) return -EFAULT;
    buf += copy_size;
    remaining -= copy_size;
...
static int __init zstd_compress_test_init(void) {
  ...
  if (!(workspace = vmalloc(workspace_size))) goto fail;
  ...
  if (zlib_deflateInit(&state, DO_ZLIB) != Z_OK) goto fail;
#endif
