 ...
 ***********************************************************/
    ...
    defined(__MINGW32__) /* No point defining Large file for 64 bit but \
...
#define CHECK_IO(f)                           \
  {                                           \
    int const errcod = (f);                   \
    if (errcod < 0) return ERROR(seekableIO); \
  }
...
U32 ZSTD_seekable_offsetToFrameIndex(ZSTD_seekable* const zs,
...
unsigned long long ZSTD_seekable_getFrameCompressedOffset(
    ZSTD_seekable* const zs, U32 frameIndex) {
...
size_t ZSTD_seekable_getFrameCompressedSize(ZSTD_seekable* const zs,
                                            U32 frameIndex) {
...
size_t ZSTD_seekable_getFrameDecompressedSize(ZSTD_seekable* const zs,
                                              U32 frameIndex) {
  if (frameIndex > zs->seekTable.tableLen) return ERROR(frameIndex_tooLarge);
...
static size_t ZSTD_seekable_loadSeekTable(ZSTD_seekable* zs) {
...

  {
    ...
    U32 remaining =
        frameSize - ZSTD_seekTableFooterSize; /* don't need to re-read footer */
    {
...
size_t ZSTD_seekable_initBuff(ZSTD_seekable* zs, const void* src,
                              size_t srcSize) {
  zs->buffWrapper = (buffWrapper_t){src, srcSize, 0};
  {
    ZSTD_seekable_customFile srcFile = {
        &zs->buffWrapper, &ZSTD_seekable_read_buff, &ZSTD_seekable_seek_buff};
...
size_t ZSTD_seekable_decompress(ZSTD_seekable* zs, void* dst, size_t len,
    ...
    while (zs->decompressedOffset < offset + len) {
      ...
      if (zs->decompressedOffset < offset) {
        ...
        outTmp = (ZSTD_outBuffer){
            zs->outBuff,
            MIN(SEEKABLE_BUFF_SIZE, offset - zs->decompressedOffset), 0};
      } else {
      ...
      if (toRead == 0) {
        ...
        if (zs->decompressedOffset < offset + len) {
          ...
          targetFrame =
              ZSTD_seekable_offsetToFrameIndex(zs, zs->decompressedOffset);
        }
        break;
      }
...
size_t ZSTD_seekable_decompressFrame(ZSTD_seekable* zs, void* dst,
                                     size_t dstSize, U32 frameIndex) {
  if (frameIndex >= zs->seekTable.tableLen) {
...

  {
    ...
    return ZSTD_seekable_decompress(zs, dst, decompressedSize,
                                    zs->seekTable.entries[frameIndex].dOffset);
  }
}
