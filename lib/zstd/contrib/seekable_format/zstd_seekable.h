 ...
 *  start a new one.
 *
 *  ZSTD_seekable_endStream() will end the current frame, and then write the
 *seek table so that decompressors can efficiently find compressed frames.
 ...
 *  consecutive range of data, it will efficiently retain the decompressor
 *object and avoid redecompressing frame prefixes.  The return value is the
 *number of bytes decompressed, or an error code checkable with ZSTD_isError().
 *
