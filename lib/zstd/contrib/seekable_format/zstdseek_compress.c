...
size_t ZSTD_seekable_logFrame(ZSTD_frameLog* fl, unsigned compressedSize,
  ...
  if (fl->size == ZSTD_SEEKABLE_MAXFRAMES) return ERROR(frameIndex_tooLarge);
...
static inline size_t ZSTD_stwrite32(ZSTD_frameLog* fl, ZSTD_outBuffer* output,
                                    U32 const value, U32 const offset) {
  if (fl->seekTablePos < offset + 4) {
    BYTE tmp[4]; /* so that we can work with buffers too small to write a whole
                    word to */
    size_t const lenWrite =
        MIN(output->size - output->pos, offset + 4 - fl->seekTablePos);
    MEM_writeLE32(tmp, value);
    memcpy((BYTE*)output->dst + output->pos, tmp + (fl->seekTablePos - offset),
           lenWrite);
    output->pos += lenWrite;
...
size_t ZSTD_seekable_writeSeekTable(ZSTD_frameLog* fl, ZSTD_outBuffer* output) {
  ...
  CHECK_Z(
      ZSTD_stwrite32(fl, output, seekTableLen - ZSTD_skippableHeaderSize, 4));

  while (fl->seekTableIndex < fl->size) {
    CHECK_Z(ZSTD_stwrite32(
        fl, output, fl->entries[fl->seekTableIndex].cSize,
        ZSTD_skippableHeaderSize + sizePerFrame * fl->seekTableIndex + 0));

    CHECK_Z(ZSTD_stwrite32(
        fl, output, fl->entries[fl->seekTableIndex].dSize,
        ZSTD_skippableHeaderSize + sizePerFrame * fl->seekTableIndex + 4));

    if (fl->checksumFlag) {
      CHECK_Z(ZSTD_stwrite32(
          fl, output, fl->entries[fl->seekTableIndex].checksum,
          ZSTD_skippableHeaderSize + sizePerFrame * fl->seekTableIndex + 8));
    }
  ...
  CHECK_Z(
      ZSTD_stwrite32(fl, output, ZSTD_SEEKABLE_MAGICNUMBER, seekTableLen - 4));
