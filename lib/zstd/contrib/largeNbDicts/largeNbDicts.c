...
static void freeBuffer(buffer_t buff) { free(buff.ptr); }
...
static buffer_t createDictionaryBuffer(const char* dictionaryName,
  ...
  if (dictionaryName) {
    ...
    size_t const dictSize = ZDICT_trainFromBuffer(
        dictBuffer, requestedDictSize, srcBuffer, srcBlockSizes, nbBlocks);
...
static int loadFiles(void* buffer, size_t bufferSize, size_t* fileSizes,
 ...
 * blockSize. otherwise, creates exactly nbSlices slices, by either truncating
 * input (when smaller) or repeating input from beginning */
static slice_collection_t splitSlices(slice_collection_t srcSlices,
                                      size_t blockSize, size_t nbSlices) {
...
static size_t sliceCollection_totalCapacity(slice_collection_t sc) {
  size_t totalSize = 0;
  for (size_t n = 0; n < sc.nbSlices; n++) totalSize += sc.capacities[n];
  return totalSize;
}
...
static buffer_collection_t createBufferCollection_fromSliceCollectionSizes(
    slice_collection_t sc) {
...
/* @return : kBuffNull if any error */
static buffer_collection_t createBufferCollection_fromFiles(
    const char* const* fileNamesTable, unsigned nbFiles) {
...
static ddict_collection_t createDDictCollection(const void* dictBuffer,
                                                size_t dictSize,
                                                size_t nbDDict) {
...
static size_t compressBlocks(
    size_t* cSizes, /* optional (can be NULL). If present, must contain at least
                       nbBlocks fields */
    slice_collection_t dstBlockBuffers, slice_collection_t srcBlockBuffers,
  ...
  for (size_t blockNb = 0; blockNb < nbBlocks; blockNb++) {
    ...
    if (cdict == NULL) {
      ...
      cBlockSize =
          ZSTD_compress_usingCDict(cctx, dstBlockBuffers.slicePtrs[blockNb],
                                   dstBlockBuffers.capacities[blockNb],
                                   srcBlockBuffers.slicePtrs[blockNb],
...
decompressInstructions createDecompressInstructions(
    ddict_collection_t dictionaries) {
...
/* benched function */
size_t decompress(const void* src, size_t srcSize, void* dst,
                  size_t dstCapacity, void* payload) {
...
int bench(const char** fileNameTable, unsigned nbFiles, const char* dictionary,
  ...
  buffer_t const dictBuffer = createDictionaryBuffer(
      dictionary, srcs.buffer.ptr, srcs.slices.capacities, srcs.slices.nbSlices,
      DICTSIZE);
...
#endif
    filenameTable =
        UTIL_createFileList(nameTable, nameIdx, &buffer_containing_filenames,
