...
vector<string> get_lines(vector<string>& input, int& linenum,
                         string terminator) {
  vector<string> out;
  string line;
  size_t epos;
...
int main(int argc, char* argv[]) {
  ...
  if (argc < 4) {
    cout << "usage: " << argv[0] << " [zstd_version] [input_file] [output_html]"
         << endl;
    return 1;
  }
  ...
  for (linenum = 0; (size_t)linenum < input.size(); linenum++) {
    ...
    if (spos == string::npos) {
      spos = line.find("/*!");
      if (spos == string::npos) spos = line.find("/**");
      if (spos == string::npos) spos = line.find("/*-");
      if (spos == string::npos) spos = line.find("/*=");
    ...
    if (!comments.empty())
      comments[comments.size() - 1] = comments[comments.size() - 1].substr(
          0, comments[comments.size() - 1].find("*/"));
    ...
    /* comments of type /*! mean: this is a function declaration; switch
     * comments with declarations */
    if (exclam == '!') {
      if (!comments.empty())
        comments.erase(
            comments.begin()); /* remove first line like "ZSTD_XXX() :" */
      linenum++;
      lines = get_lines(input, linenum, "");
      ...
      sout << "</p></pre><BR>" << endl << endl;
               ...
               '=') { /* comments of type /*= and /**= mean: use a <H3> header
                         and show also all functions until first empty line */
      trim(comments[0], " ");
      ...
      sout << "</pre></b><BR>" << endl;
    } else { /* comments of type /** and /*- mean: this is a comment; use a <H2>
                header for the first line */
      if (comments.empty()) continue;

      trim(comments[0], " ");
      sout << "<a name=\"Chapter" << chapter << "\"></a><h2>" << comments[0]
           << "</h2><pre>";
