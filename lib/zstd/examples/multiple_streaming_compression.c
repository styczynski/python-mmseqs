 ...
 * files while preserving memory management. All structures and buffers will be
 * created only once, and shared across all compression operations */
...
static void compressFile_orDie(resources ress, const char* fname,
                               const char* outName, int cLevel) {
  ...
  while ((read = fread_orDie(ress.buffIn, toRead, fin))) {
    ...
    while (input.pos < input.size) {
      ...
      if (toRead > ress.buffInSize)
        toRead = ress.buffInSize; /* Safely handle when `buffInSize` is manually
