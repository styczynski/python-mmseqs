...
size_t ZSTD_CCtx_setParameter(ZSTD_CCtx* cctx, ZSTD_cParameter param,
  ...
  switch (param) {
    ...
    case ZSTD_p_forceMaxWindow: /* Force back-references to remain < windowSize,
                                 ...
                                 * content. default : 0 when using a CDict, 1
                                 * when using a Prefix */
      return ZSTD_CCtxParam_setParameter(&cctx->requestedParams, param, value);
    ...
    case ZSTD_p_nbWorkers:
      if ((value > 0) && cctx->staticSize) {
        return ERROR(
            parameter_unsupported); /* MT not compatible with static alloc */
      }
      return ZSTD_CCtxParam_setParameter(&cctx->requestedParams, param, value);
...
size_t ZSTD_CCtxParam_setParameter(ZSTD_CCtx_params* CCtxParams,
  ...
  switch (param) {
    ...
    case ZSTD_p_dictIDFlag: /* When applicable, dictionary's dictID is provided
    ...
    case ZSTD_p_overlapSizeLog:
      ...
      return ZSTDMT_CCtxParam_setMTCtxParameter(
          CCtxParams, ZSTDMT_p_overlapSectionLog, value);
#endif
    ...
    case ZSTD_p_rsyncable:
      ...
      return ERROR(parameter_unsupported);
#else
      return ZSTDMT_CCtxParam_setMTCtxParameter(CCtxParams, ZSTDMT_p_rsyncable,
                                                value);
#endif
  ...
  if (dict == NULL || dictSize == 0) { /* no dictionary mode */
    ...
    cctx->cdictLocal =
        ZSTD_createCDict_advanced(dict, dictSize, dictLoadMethod,
                                  dictContentType, cParams, cctx->customMem);
    cctx->cdict = cctx->cdictLocal;
    if (cctx->cdictLocal == NULL) return ERROR(memory_allocation);
  }
  return 0;
}
...
ZSTD_compressionParameters ZSTD_getCParamsFromCCtxParams(
  ...
  if (CCtxParams->cParams.searchLength)
...
static size_t ZSTD_sizeof_matchState(
  ...
  U32 const hashLog3 = (forCCtx && cParams->searchLength == 3)
...
static U32 ZSTD_equivalentCParams(ZSTD_compressionParameters cParams1,
  ...
  return (cParams1.hashLog == cParams2.hashLog) &
         ...
         & ((cParams1.searchLength == 3) ==
            (cParams2.searchLength == 3)); /* hashlog3 space */
}
...
static U32 ZSTD_equivalentParams(ZSTD_CCtx_params params1,
                                 ZSTD_CCtx_params params2, size_t buffSize1,
...
static void ZSTD_reset_compressedBlockState(ZSTD_compressedBlockState_t* bs) {
  int i;
  for (i = 0; i < ZSTD_REP_NUM; ++i) bs->rep[i] = repStartValue[i];
...
static size_t ZSTD_continueCCtx(ZSTD_CCtx* cctx, ZSTD_CCtx_params params,
                                U64 pledgedSrcSize) {
  ...
  cctx->blockSize = blockSize; /* previous block size could be different even
                                  for same windowLog, due to pledgedSrcSize */
  ...
  cctx->dictID = 0;
  if (params.ldmParams.enableLdm) ZSTD_window_clear(&cctx->ldmState.window);
...
static void* ZSTD_reset_matchState(ZSTD_matchState_t* ms, void* ptr,
...
#define ZSTD_WORKSPACETOOLARGE_MAXDURATION                               \
  128 /* when workspace is continuously too large                        \
       ...
       * because it's sized to handle a worst case scenario which rarely \
       * happens. In which case, resize it down to free some memory */
...

  {
    ...
    /* Check if workSpace is large enough, alloc a new one if needed */
    {
      ...
      int const workSpaceWasteful =
          workSpaceTooLarge &&
          (zc->workSpaceOversizedDuration > ZSTD_WORKSPACETOOLARGE_MAXDURATION);
      zc->workSpaceOversizedDuration =
...
static size_t ZSTD_noCompressBlock(void* dst, size_t dstCapacity,
...
#define COMPRESS_LITERALS_SIZE_MIN 63
    ...
    return ERROR(dstSize_tooSmall); /* not enough space for compression */
  {
    ...
    cLitSize =
        singleStream
            ? HUF_compress1X_repeat(ostart + lhSize, dstCapacity - lhSize, src,
                                    srcSize, 255, 11, workspace, wkspSize,
 ...
 */
static size_t ZSTD_fseBitCost(FSE_CTable const* ctable, unsigned const* count,
                              unsigned const max) {
...
MEM_STATIC size_t ZSTD_buildCTable(void* dst, size_t dstCapacity,
                                   ...
                                   symbolEncodingType_e type, U32* count,
                                   ...
                                   size_t prevCTableSize, void* workspace,
                                   size_t workspaceSize) {
  BYTE* op = (BYTE*)dst;
  ...
  switch (type) {
    ...
    case set_compressed: {
      ...
      CHECK_F(FSE_normalizeCount(norm, tableLog, count, nbSeq_1, max));
      {
        ...
        CHECK_F(FSE_buildCTable_wksp(nextCTable, norm, max, tableLog, workspace,
                                     workspaceSize));
        return NCountSize;
      }
    }
...

  {
    ...
    for (n = nbSeq - 2; n < nbSeq; n--) { /* intentional underflow */
      ...
      DEBUGLOG(6, "encoding: litlen:%2u - matchlen:%2u - offCode:%7u",
               sequences[n].litLength, sequences[n].matchLength + MINMATCH,
               sequences[n].offset);
      /* 32b*/ /* 64b*/
      /* (7)*/ /* (7)*/
...
MEM_STATIC size_t ZSTD_compressSequences_internal(
    seqStore_t* seqStorePtr, ZSTD_entropyCTables_t const* prevEntropy,
    ZSTD_entropyCTables_t* nextEntropy, ZSTD_CCtx_params const* cctxParams,
    void* dst, size_t dstCapacity, void* workspace, size_t wkspSize,
    const int bmi2) {
  ...
  /* build CTable for Literal Lengths */
  {
    ...
    size_t const mostFrequent = HIST_countFast_wksp(
        count, &max, llCodeTable, nbSeq, workspace, wkspSize); /* can't fail */
    DEBUGLOG(5, "Building LL table");
    nextEntropy->fse.litlength_repeatMode =
        prevEntropy->fse.litlength_repeatMode;
    LLtype = ZSTD_selectEncodingType(
        &nextEntropy->fse.litlength_repeatMode, count, max, mostFrequent, nbSeq,
        LLFSELog, prevEntropy->fse.litlengthCTable, LL_defaultNorm,
                 ...
                 FSE_repeat_none)); /* We don't copy tables */
    {
      ...
      if (LLtype == set_compressed) lastNCount = op;
      op += countSize;
    }
  }
  /* build CTable for Offsets */
  {
    U32 max = MaxOff;
    size_t const mostFrequent = HIST_countFast_wksp(
        count, &max, ofCodeTable, nbSeq, workspace, wkspSize); /* can't fail */
    ...
    Offtype = ZSTD_selectEncodingType(
        ...
        OffFSELog, prevEntropy->fse.offcodeCTable, OF_defaultNorm,
                 ...
                 FSE_repeat_none)); /* We don't copy tables */
    {
      ...
      if (ZSTD_isError(countSize)) return countSize;
      if (Offtype == set_compressed) lastNCount = op;
      op += countSize;
    }
  }
  /* build CTable for MatchLengths */
  {
    U32 max = MaxML;
    size_t const mostFrequent = HIST_countFast_wksp(
        count, &max, mlCodeTable, nbSeq, workspace, wkspSize); /* can't fail */
                 ...
                 FSE_repeat_none)); /* We don't copy tables */
    {
      size_t const countSize = ZSTD_buildCTable(
          ...
          sizeof(prevEntropy->fse.matchlengthCTable), workspace, wkspSize);
      if (ZSTD_isError(countSize)) return countSize;
      if (MLtype == set_compressed) lastNCount = op;
      op += countSize;
    }
  }
  ...
  *seqHead = (BYTE)((LLtype << 6) + (Offtype << 4) + (MLtype << 2));

  {
    size_t const bitstreamSize =
        ZSTD_encodeSequences(op, oend - op, CTable_MatchLength, mlCodeTable,
                             CTable_OffsetBits, ofCodeTable, CTable_LitLength,
                             llCodeTable, sequences, nbSeq, longOffsets, bmi2);
...
MEM_STATIC size_t ZSTD_compressSequences(
    ...
    void* dst, size_t dstCapacity, size_t srcSize, void* workspace,
...
ZSTD_blockCompressor ZSTD_selectBlockCompressor(ZSTD_strategy strat,
  ...
  static const ZSTD_blockCompressor blockCompressor[3][(unsigned)ZSTD_btultra +
                                                       1] = {
...
static size_t ZSTD_compressBlock_internal(ZSTD_CCtx* zc, void* dst,
  ...
  size_t cSize;
  DEBUGLOG(5,
           "ZSTD_compressBlock_internal (dstCapacity=%zu, dictLimit=%u, "
           "nextToUpdate=%u)",
  ...
  /* select and store sequences */
  {
    ...
    if (zc->externSeqStore.pos < zc->externSeqStore.size) {
      ...
      CHECK_F(ZSTD_ldm_generateSequences(&zc->ldmState, &ldmSeqStore,
                                         &zc->appliedParams.ldmParams, src,
                                         srcSize));
      /* Updates ldmSeqStore.pos */
      lastLLSize =
...
static size_t ZSTD_compress_frameChunk(ZSTD_CCtx* cctx, void* dst,
  ...
  while (remaining) {
    ...
    if (ZSTD_window_needOverflowCorrection(ms->window, ip + blockSize)) {
      U32 const cycleLog = ZSTD_cycleLog(cctx->appliedParams.cParams.chainLog,
...

    {
      size_t cSize = ZSTD_compressBlock_internal(
          cctx, op + ZSTD_blockHeaderSize, dstCapacity - ZSTD_blockHeaderSize,
          ip, blockSize);
      if (ZSTD_isError(cSize)) return cSize;
 ...
 * in zstd_internal.h)) or an error code if `dstCapcity` is too small
...
size_t ZSTD_referenceExternalSequences(ZSTD_CCtx* cctx, rawSeq* seq,
                                       size_t nbSeq) {
  if (cctx->stage != ZSTDcs_init) return ERROR(stage_wrong);
  ...
  if (!frame) {
    ...
    if (ZSTD_window_needOverflowCorrection(ms->window,
      ...
      U32 const correction = ZSTD_window_correctOverflow(
          &ms->window, cycleLog, 1 << cctx->appliedParams.cParams.windowLog,
          src);
      ZSTD_STATIC_ASSERT(ZSTD_CHAINLOG_MAX <= 30);
      ZSTD_STATIC_ASSERT(ZSTD_WINDOWLOG_MAX_32 <= 30);
      ZSTD_STATIC_ASSERT(ZSTD_WINDOWLOG_MAX <= 31);
...
static size_t ZSTD_loadZstdDictionary(ZSTD_compressedBlockState_t* bs,
...

  {
    ...
    CHECK_E(
        FSE_buildCTable_wksp(bs->entropy.fse.offcodeCTable, offcodeNCount,
                             MaxOff, offcodeLog, workspace, HUF_WORKSPACE_SIZE),
...

  {
    ...
    CHECK_F(
        ZSTD_loadDictionaryContent(ms, params, dictPtr, dictContentSize, dtlm));
    return dictID;
  }
}
...
static size_t ZSTD_compress_insertDictionary(
    ZSTD_compressedBlockState_t* bs, ZSTD_matchState_t* ms,
  ...
  if (MEM_readLE32(dict) != ZSTD_MAGIC_DICTIONARY) {
    ...
    if (dictContentType == ZSTD_dct_fullDict) return ERROR(dictionary_wrong);
    assert(0); /* impossible */
  }
...
static size_t ZSTD_compressBegin_internal(
    ZSTD_CCtx* cctx, const void* dict, size_t dictSize,
    ZSTD_dictContentType_e dictContentType, ZSTD_dictTableLoadMethod_e dtlm,
    const ZSTD_CDict* cdict, ZSTD_CCtx_params params, U64 pledgedSrcSize,
    ZSTD_buffered_policy_e zbuff) {
...
const ZSTD_CDict* ZSTD_initStaticCDict(void* workspace, size_t workspaceSize,
  ...
  DEBUGLOG(4, "(workspaceSize < neededSize) : (%u < %u) => %u",
           (U32)workspaceSize, (U32)neededSize,
...
size_t ZSTD_initCStream_internal(ZSTD_CStream* zcs, const void* dict,
  ...
  if (dict && dictSize >= 8) {
    ...
    zcs->cdictLocal = ZSTD_createCDict_advanced(dict, dictSize, ZSTD_dlm_byCopy,
    ...
    if (cdict) {
      params.cParams = ZSTD_getCParamsFromCDict(
          cdict); /* cParams are enforced from cdict; it includes windowLog */
    }
    ZSTD_freeCDict(zcs->cdictLocal);
    zcs->cdictLocal = NULL;
    zcs->cdict = cdict;
...
size_t ZSTD_initCStream_usingCDict_advanced(ZSTD_CStream* zcs,
  ...
  if (!cdict)
    return ERROR(dictionary_wrong); /* cannot handle NULL cdict (does not know
                                       what to do) */
  {
    ZSTD_CCtx_params params = zcs->requestedParams;
    params.cParams = ZSTD_getCParamsFromCDict(cdict);
  ...
  if ((pledgedSrcSize == 0) && (params.fParams.contentSizeFlag == 0))
    pledgedSrcSize =
        ZSTD_CONTENTSIZE_UNKNOWN; /* for compatibility with older programs
                                     relying on this behavior. Users should now
                                     specify ZSTD_CONTENTSIZE_UNKNOWN. This line
  ...
  U64 const pledgedSrcSize =
      (pss == 0) ? ZSTD_CONTENTSIZE_UNKNOWN
                 : pss; /* temporary : 0 interpreted as "unknown" during
                           transition period. Users willing to specify "unknown"
                           **must** use ZSTD_CONTENTSIZE_UNKNOWN. `0` will be
                           interpreted as "empty" in the future */
  ZSTD_CCtxParams_init(&zcs->requestedParams, compressionLevel);
 ...
 *  internal function for all *compressStream*() variants and
 * *compress_generic() non-static, because can be called from zstdmt_compress.c
    ...
    switch (zcs->streamStage) {
      ...
      case zcss_load:
        ...
        DEBUGLOG(5, "stream compression stage (flushMode==%u)", flushMode);
        {
          ...
          DEBUGLOG(5, "inBuffTarget:%u / inBuffSize:%u", (U32)zcs->inBuffTarget,
                   (U32)zcs->inBuffSize);
      ...
      case zcss_flush:
        DEBUGLOG(5, "flush stage");
        {
          ...
          DEBUGLOG(5, "toFlush: %u into %u ==> flushed: %u", (U32)toFlush,
                   (U32)(oend - op), (U32)flushed);
          op += flushed;
...
size_t ZSTD_compress_generic(ZSTD_CCtx* cctx, ZSTD_outBuffer* output,
    ...
    if (params.nbWorkers > 0) {
      ...
      if (cctx->mtctx == NULL) {
        DEBUGLOG(4,
                 ...
                 params.nbWorkers);
        cctx->mtctx =
            ZSTDMT_createCCtx_advanced(params.nbWorkers, cctx->customMem);
  ...
  if (cctx->appliedParams.nbWorkers > 0) {
    ...
    }
    {
      ...
      DEBUGLOG(5,
               "completed ZSTD_compress_generic delegating to "
...
size_t ZSTD_compress_generic_simpleArgs(ZSTD_CCtx* cctx, void* dst,
