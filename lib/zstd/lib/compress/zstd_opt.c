...
static void ZSTD_rescaleFreqs(optState_t* const optPtr, const BYTE* const src,
                              size_t const srcSize, int optLevel) {
  ...
  if (optPtr->litLengthSum == 0) { /* first block : init */
    ...
    if (optPtr->symbolCosts->huf.repeatMode ==
        HUF_repeat_valid) { /* huffman table presumed generated by dictionary */
      optPtr->priceType = zop_dynamic;
      ...
      {
        unsigned ll;
        for (ll = 0; ll <= MaxLL; ll++) optPtr->litLengthFreq[ll] = 1;
      }
      optPtr->litLengthSum = MaxLL + 1;

      {
        unsigned ml;
        for (ml = 0; ml <= MaxML; ml++) optPtr->matchLengthFreq[ml] = 1;
      }
      optPtr->matchLengthSum = MaxML + 1;

      {
        unsigned of;
        for (of = 0; of <= MaxOff; of++) optPtr->offCodeFreq[of] = 1;
      }
 ...
 * position */
static int ZSTD_litLengthContribution(U32 const litLength,
...
void ZSTD_updateTree_internal(ZSTD_matchState_t* ms, const BYTE* const ip,
  ...
  DEBUGLOG(5, "ZSTD_updateTree_internal, from %u to %u  (dictMode:%u)", idx,
           target, dictMode);
...
FORCE_INLINE_TEMPLATE U32 ZSTD_BtGetAllMatches(
    ZSTD_matchState_t* ms, const BYTE* ip, const BYTE* const iHighLimit,
    const ZSTD_dictMode_e dictMode, U32 rep[ZSTD_REP_NUM], U32 const ll0,
...
static repcodes_t ZSTD_updateRep(U32 const rep[3], U32 const offset,
                                 U32 const ll0) {
  repcodes_t newReps;
...
FORCE_INLINE_TEMPLATE size_t ZSTD_compressBlock_opt_generic(
  ...
  while (ip < ilimit) {
    ...
    /* find first match */
    {
      ...
      U32 const nbMatches = ZSTD_BtGetAllMatches(ms, ip, iend, dictMode, rep,
      ...
      opt[0].litlen = litlen;
      opt[0].price =
          ZSTD_literalsContribution(anchor, litlen, optStatePtr, optLevel);
    ...
    for (cur = 1; cur <= last_pos; cur++) {
...

      {
        ...
        U32 const nbMatches = ZSTD_BtGetAllMatches(
            ms, inr, iend, dictMode, opt[cur].rep, ll0, matches, minMatch);
        U32 matchNb;
        if (!nbMatches) {
