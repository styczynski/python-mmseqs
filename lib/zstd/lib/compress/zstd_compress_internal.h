...
#define ZSTD_DUBT_UNSORTED_MARK                                             \
  1 /* For btlazy2 strategy, index 1 now means "unsorted".                  \
       ...
       larger than its predecessor. It's not a big deal though : candidate                                     \
       will just be sorted again. Additionnally, candidate position 1 will be                                \
       lost. But candidate 1 cannot hide a large tree of candidates, so it's a          \
       minimal loss. The benefit is that ZSTD_DUBT_UNSORTED_MARK cannot be                      \
       misdhandled after table re-use with a different strategy Constant                                                                   \
       required by ZSTD_compressBlock_btlazy2() and                         \
 ...
 * code) into seqStore_t. `offsetCode` : distance to match + 3 (values 1-3 are
 * repCodes). `mlBase` : matchLength - MINMATCH
 */
MEM_STATIC void ZSTD_storeSeq(seqStore_t* seqStorePtr, size_t litLength,
...
MEM_STATIC void ZSTD_window_enforceMaxDist(
    ZSTD_window_t* window, void const* srcEnd, U32 maxDist,
    U32* loadedDictEndPtr, const ZSTD_matchState_t** dictMatchStatePtr) {
  U32 const current = (U32)((BYTE const*)srcEnd - window->base);
  U32 loadedDictEnd = loadedDictEndPtr != NULL ? *loadedDictEndPtr : 0;
  DEBUGLOG(5, "ZSTD_window_enforceMaxDist: current=%u, maxDist=%u", current,
           maxDist);
