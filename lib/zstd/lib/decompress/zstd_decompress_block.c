                  ...
                  (litEncType == set_repeat)
                      ? (singleStream
                             ? HUF_decompress1X_usingDTable_bmi2(
                                   ...
                                   litCSize, dctx->HUFptr, dctx->bmi2)
 ...
 * buffer. requires more careful checks, to ensure there is no overflow.
 * performance does not matter though.
 * note : this case is supposed to be never generated "naturally" by reference
 * encoder, since in most cases it needs at least 8 bytes to look for a match.
...
ZSTD_decodeSequence(seqState_t* seqState, const ZSTD_longOffset_e longOffsets) {
  ...
  seq.litLength = llBase + ((llBits > 0) ? BIT_readBitsFast(&seqState->DStream,
                                                            llBits /*>0*/)
                                         : 0); /* <=  16 bits */
  if (MEM_32bits()) BIT_reloadDStream(&seqState->DStream);
...
FORCE_INLINE_TEMPLATE size_t ZSTD_decompressSequences_body(
...
FORCE_INLINE_TEMPLATE seq_t ZSTD_decodeSequenceLong(
    seqState_t* seqState, ZSTD_longOffset_e const longOffsets) {
  seq_t seq;
  U32 const llBits =
      seqState->stateLL.table[seqState->stateLL.state].nbAdditionalBits;
  U32 const mlBits =
...

  {
    ...
    seq.match =
        matchBase +
        pos - seq.offset; /* note : this operation can overflow when seq.offset
                           * is really too large, which can only happen when
                           * input is corrupted. No consequence though : no
                           * memory access will occur, overly large offset will
                           * be detected in ZSTD_execSequenceLong() */
 ...
 * sometimes mearning "farther than memory cache distance". This function will
