 ...
 *  maximum allowed nb of calls to ZSTD_decompressStream() and
 * ZSTD_decompress_generic() without any forward progress (defined as: no byte
 * read from input, and no byte flushed to output) before triggering an error.
 */
...
size_t ZSTD_findFrameCompressedSize(const void* src, size_t srcSize) {
  ...
  if ((srcSize >= ZSTD_skippableHeaderSize) &&
    ...
    return ZSTD_skippableHeaderSize +
           MEM_readLE32((const BYTE*)src + ZSTD_FRAMEIDSIZE);
  } else {
    const BYTE* ip = (const BYTE*)src;
  ...
  while (srcSize >= ZSTD_frameHeaderSize_prefix) {
...

    {
      ...
      if ((ZSTD_getErrorCode(res) == ZSTD_error_prefix_unknown) &&
         ...
         * Note that one could be unlucky, it might be a corruption error
         * instead, happening right at the place where we expect zstd magic
         * bytes. But this is _much_ less likely than a srcSize field error. */
        return ERROR(srcSize_wrong);
      }
      ...
      if (dctx->format == ZSTD_f_zstd1) { /* allows header */
        ...
        if ((MEM_readLE32(src) & 0xFFFFFFF0U) ==
          ...
          dctx->expected = ZSTD_skippableHeaderSize -
                           srcSize; /* remaining to load to get full skippable
                                       frame header */
...
size_t ZSTD_loadDEntropy(ZSTD_entropyDTables_t* entropy, const void* const dict,
...

  {
    ...
    ZSTD_buildFSETable(entropy->MLTable, matchlengthNCount, matchlengthMaxValue,
                       ML_base, ML_bits, matchlengthLog);
...
size_t ZSTD_setDStreamParameter(ZSTD_DStream* dctx,
                                ZSTD_DStreamParameter_e paramType,
                                unsigned paramValue) {
  if (dctx->streamStage != zdss_init) return ERROR(stage_wrong);
  switch (paramType) {
    default:
...
size_t ZSTD_estimateDStreamSize_fromFrame(const void* src, size_t srcSize) {
  U32 const windowSizeMax =
      1U << ZSTD_WINDOWLOG_MAX; /* note : should be user-selectable */
  ZSTD_frameHeader zfh;
  ...
  while (someMoreWork) {
    switch (zds->streamStage) {
      ...
      case zdss_loadHeader:
          ...
          if (ZSTD_isError(hSize)) {
            ...
            if (legacyVersion) {
              const void* const dict =
                  zds->ddict ? ZSTD_DDict_dictContent(zds->ddict) : NULL;
              size_t const dictSize =
        ...
        if ((MEM_readLE32(zds->headerBuffer) & 0xFFFFFFF0U) ==
            ZSTD_MAGIC_SKIPPABLE_START) { /* skippable frame */
...
size_t ZSTD_decompress_generic(ZSTD_DCtx* dctx, ZSTD_outBuffer* output,
                               ZSTD_inBuffer* input) {
  return ZSTD_decompressStream(dctx, output, input);
}

size_t ZSTD_decompress_generic_simpleArgs(ZSTD_DCtx* dctx, void* dst,
