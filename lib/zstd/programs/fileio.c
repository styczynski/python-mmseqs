...
static int g_displayLevel =
    2; /* 0 : no display;  1: errors;  2: + result + interaction + warnings;  3:
...
static void INThandler(int sig) {
  ...
  signal(
      sig,
      SIG_IGN); /* this invocation generates a buggy warning in Visual Studio */
#endif
  if (g_artefact) {
...
static U32 g_sparseFileSupport = 1; /* 0: no sparse allowed; 1: auto (file yes,
                                       stdout no); 2: force sparse */
...
static int g_maxAdaptLevel = 22; /* initializing this value requires a constant,
                                    so ZSTD_maxCLevel() doesn't work */
void FIO_setAdaptMax(int maxCLevel) { g_maxAdaptLevel = maxCLevel; }

static U32 g_ldmFlag = 0;
void FIO_setLdmFlag(unsigned ldmFlag) { g_ldmFlag = (ldmFlag > 0); }
static U32 g_ldmHashLog = 0;
void FIO_setLdmHashLog(unsigned ldmHashLog) { g_ldmHashLog = ldmHashLog; }
static U32 g_ldmMinMatch = 0;
void FIO_setLdmMinMatch(unsigned ldmMinMatch) { g_ldmMinMatch = ldmMinMatch; }
  ...
  /* Advanced parameters, including dictionary */
  {
    ...
    CHECK(
        ZSTD_CCtx_setParameter(ress.cctx, ZSTD_p_contentSizeFlag,
                               1)); /* always enable content size when available
                                       (note: supposed to be default) */
    CHECK(ZSTD_CCtx_setParameter(ress.cctx, ZSTD_p_dictIDFlag, g_dictIDFlag));
    CHECK(
        ZSTD_CCtx_setParameter(ress.cctx, ZSTD_p_checksumFlag, g_checksumFlag));
    /* compression level */
    CHECK(ZSTD_CCtx_setParameter(ress.cctx, ZSTD_p_compressionLevel,
                                 (unsigned)cLevel));
    /* long distance matching */
    CHECK(ZSTD_CCtx_setParameter(ress.cctx, ZSTD_p_enableLongDistanceMatching,
                                 g_ldmFlag));
    ...
    if (g_ldmBucketSizeLog != FIO_LDM_PARAM_NOTSET) {
      CHECK(ZSTD_CCtx_setParameter(ress.cctx, ZSTD_p_ldmBucketSizeLog,
                                   g_ldmBucketSizeLog));
    }
    if (g_ldmHashEveryLog != FIO_LDM_PARAM_NOTSET) {
      CHECK(ZSTD_CCtx_setParameter(ress.cctx, ZSTD_p_ldmHashEveryLog,
                                   g_ldmHashEveryLog));
    }
    /* compression parameters */
    CHECK(ZSTD_CCtx_setParameter(ress.cctx, ZSTD_p_windowLog,
                                 comprParams.windowLog));
    CHECK(ZSTD_CCtx_setParameter(ress.cctx, ZSTD_p_chainLog,
                                 comprParams.chainLog));
    CHECK(
        ZSTD_CCtx_setParameter(ress.cctx, ZSTD_p_hashLog, comprParams.hashLog));
    CHECK(ZSTD_CCtx_setParameter(ress.cctx, ZSTD_p_searchLog,
                                 comprParams.searchLog));
    CHECK(ZSTD_CCtx_setParameter(ress.cctx, ZSTD_p_minMatch,
                                 comprParams.searchLength));
    CHECK(ZSTD_CCtx_setParameter(ress.cctx, ZSTD_p_targetLength,
                                 comprParams.targetLength));
    CHECK(ZSTD_CCtx_setParameter(ress.cctx, ZSTD_p_compressionStrategy,
                                 (U32)comprParams.strategy));
    /* multi-threading */
    ...
    if (g_overlapLog != FIO_OVERLAP_LOG_NOTSET) {
      ...
      CHECK(ZSTD_CCtx_setParameter(ress.cctx, ZSTD_p_overlapSizeLog,
                                   g_overlapLog));
    }
    ...
    CHECK(ZSTD_CCtx_setPledgedSrcSize(ress.cctx,
                                      ZSTD_CONTENTSIZE_UNKNOWN)); /* reset */
...
static unsigned long long FIO_compressGzFrame(cRess_t* ress,
  ...
  while (1) {
    ...
    if (srcFileSize == UTIL_FILESIZE_UNKNOWN)
      DISPLAYUPDATE(2, "\rRead : %u MB ==> %.2f%%", (U32)(inFileSize >> 20),
...
static unsigned long long FIO_compressLz4Frame(cRess_t* ress,
...

  {
    size_t readSize;
    size_t headerSize =
        LZ4F_compressBegin(ctx, ress->dstBuffer, ress->dstBufferSize, &prefs);
    ...
    while (readSize > 0) {
      ...
      /* Compress Block */
      outSize = LZ4F_compressUpdate(ctx, ress->dstBuffer, ress->dstBufferSize,
                                    ress->srcBuffer, readSize, NULL);
      if (LZ4F_isError(outSize))
        EXM_THROW(35, "zstd: %s: lz4 compression failed : %s", srcFileName,
...

    {
      ...
      if (sizeCheck != headerSize)
        EXM_THROW(39, "Write error : cannot write end of stream");
    }
    outFileSize += headerSize;
  }
...
static unsigned long long FIO_compressZstdFrame(const cRess_t* ressPtr,
                                                const char* srcFileName,
                                                U64 fileSize,
                                                int compressionLevel,
                                                U64* readsize) {
  ...
  do {
    ...
    *readsize += inSize;
    ...
    if ((inSize == 0) || (*readsize == fileSize)) directive = ZSTD_e_end;
    ...
    while (
      ...
      CHECK_V(stillToFlush,
              ZSTD_compress_generic(ress.cctx, &outBuff, &inBuff, directive));

      /* count stats */
      inputPresented++;
      if (oldIPos == inBuff.pos)
        inputBlocked++; /* input buffer is full and can't take any more : input
                           speed is faster than consumption rate */
      if (!toFlushNow) flushWaiting = 1;

      /* Write compressed stream */
      DISPLAYLEVEL(6,
                   "ZSTD_compress_generic(end:%u) => input pos(%u)<=(%u)size ; "
                   "output generated %u bytes \n",
                   (U32)directive, (U32)inBuff.pos, (U32)inBuff.size,
                   (U32)outBuff.pos);
      if (outBuff.pos) {
        size_t const sizeCheck =
      ...
      if (READY_FOR_UPDATE()) {
        ...
        double const cShare = (double)zfp.produced /
                              (zfp.consumed + !zfp.consumed /*avoid div0*/) *
                              100;
        ...
        if (g_displayLevel >= 3) {
          DISPLAYUPDATE(
              3,
              "\r(L%i) Buffered :%4u MB - Consumed :%4u MB - Compressed :%4u "
              "MB => %.2f%% ",
              compressionLevel, (U32)((zfp.ingested - zfp.consumed) >> 20),
              (U32)(zfp.consumed >> 20), (U32)(zfp.produced >> 20), cShare);
        ...
        if (g_adaptiveMode) {
          ...
          if (zfp.currentJobID > 1) { /* only possible if nbWorkers >= 1 */
             ...
             * or because input is slow and no job can start while waiting for
             * at least one buffer to be filled. note : excluse starting part,
             * since currentJobID > 1 */
            if ((zfp.consumed ==
                 previous_zfp_update
                     .consumed) /* no data compressed : no data available, or no
                                   more buffer to compress to, OR compression is
                                   really slow (compression of a single block is
            ...
            if ((newlyProduced >
                 (newlyFlushed * 9 /
                  8)) /* compression produces more data than output can flush
                         (though production can be spiky, due to work unit :
                         (N==4)*block sizes) */
                && (flushWaiting ==
                    0) /* flush speed was never slowed by lack of production, so
                          it's operating at max capacity */
            ) {
              DISPLAYLEVEL(
                  6,
                  "compression faster than flush (%llu > %llu), and flushed "
                  "was never slowed down by lack of production => slow down \n",
          ...
          if (zfp.currentJobID > lastJobID) {
            ...
            /* check input speed */
            if (zfp.currentJobID >
                g_nbWorkers + 1) { /* warm up period, to fill all workers */
              if (inputBlocked <= 0) {
                DISPLAYLEVEL(6,
                             "input is never blocked => input is slower than "
                             "ingestion \n");
                speedChange = slower;
                ...
                DISPLAYLEVEL(6,
                             "input blocked %u/%u(%.2f) - ingested:%u vs "
                             "%u:consumed - flushed:%u vs %u:produced \n",
                ...
                if ((inputBlocked >
                     inputPresented /
                         8) /* input is waiting often, because input buffers is
                               full : compression or output too slow */
                    && (newlyFlushed * 33 / 32 >
                        newlyProduced) /* flush everything that is produced */
                    && (newlyIngested * 33 / 32 >
                        newlyConsumed) /* input speed as fast or faster than
                                          compression speed */
                ) {
                  DISPLAYLEVEL(6,
                               "recommend faster as in(%llu) >= "
                               "(%llu)comp(%llu) <= out(%llu) \n",
            ...
            if (speedChange == slower) {
              ...
              ZSTD_CCtx_setParameter(ress.cctx, ZSTD_p_compressionLevel,
 ...
 */
static int FIO_compressFilename_internal(cRess_t ress, const char* dstFileName,
                                         const char* srcFileName,
                                         int compressionLevel) {
  U64 readsize = 0;
  U64 compressedfilesize = 0;
  ...
  switch (g_compressionType) {
    ...
    case FIO_lzmaCompression:
      ...
      (void)compressionLevel;
      EXM_THROW(20,
                "zstd: %s: file cannot be compressed as xz/lzma (zstd compiled "
                "without ZSTD_LZMACOMPRESS) -- ignored \n",
                srcFileName);
#endif
      break;
...
static int FIO_compressFilename_dstFile(cRess_t ress, const char* dstFileName,
  ...
  if (closeDstFile) {
    ...
    if ((result != 0) /* operation failure */
      ...
      FIO_remove(dstFileName); /* remove compression artefact; note don't do
 ...
 *            1 : missing or pb opening srcFileName
 */
static int FIO_compressFilename_srcFile(cRess_t ress, const char* dstFileName,
                                        const char* srcFileName,
                                        int compressionLevel) {
  int result;
...
int FIO_compressFilename(const char* dstFileName, const char* srcFileName,
  ...
  U64 const fileSize = UTIL_getFileSize(srcFileName);
  U64 const srcSize =
      (fileSize == UTIL_FILESIZE_UNKNOWN) ? ZSTD_CONTENTSIZE_UNKNOWN : fileSize;
 ...
 */
static const char* FIO_determineCompressedName(const char* srcFileName,
                                               const char* suffix) {
  static size_t dfnbCapacity = 0;
  static char* dstFileNameBuffer = NULL; /* using static allocation : this
                                            function cannot be multi-threaded */
 ...
 */
int FIO_compressMultipleFilenames(const char** inFileNamesTable,
  ...
  if (outFileName !=
    ...
    for (u = 0; u < nbFiles; u++) {
      ...
      const char* const dstFileName =
          FIO_determineCompressedName(srcFileName, suffix); /* cannot fail */
      error |= FIO_compressFilename_srcFile(ress, dstFileName, srcFileName,
                                            compressionLevel);
    }
  }
...
static unsigned FIO_fwriteSparse(FILE* file, const void* buffer,
                                 size_t bufferSize, unsigned storedSkips) {
  ...
  if (!g_sparseFileSupport) { /* normal write */
    ...
    if (sizeCheck != bufferSize)
      EXM_THROW(70, "Write error : cannot write decoded block");
    return 0;
  }
...
/** FIO_passThrough() : just copy input into output, for compatibility with gzip
   -df mode
    @return : 0 (no error) */
static unsigned FIO_passThrough(FILE* foutput, FILE* finput, void* buffer,
...
static void FIO_zstdErrorHelp(dRess_t* ress, size_t err,
  ...
  if (err == 0) {
    ...
    if (windowLog <= ZSTD_WINDOWLOG_MAX) {
      U32 const windowMB =
          (U32)((windowSize >> 20) + ((windowSize & ((1 MB) - 1)) != 0));
      assert(windowSize <
...
static unsigned long long FIO_decompressZstdFrame(dRess_t* ress, FILE* finput,
                                                  const char* srcFileName,
                                                  U64 alreadyDecoded) {
  U64 frameSize = 0;
  U32 storedSkips = 0;
  ...
  while (1) {
    ...
    /* Write block */
    storedSkips = FIO_fwriteSparse(ress->dstFile, ress->dstBuffer, outBuff.pos,
                                   storedSkips);
    frameSize += outBuff.pos;
    DISPLAYUPDATE(2, "\r%-20.20s : %u MB...     ", srcFileName,
                  (U32)((alreadyDecoded + frameSize) >> 20));
    ...
    if (inBuff.size != inBuff.pos) {
      DISPLAYLEVEL(1,
                   "%s : Decoding error (37) : should consume entire input \n",
                   srcFileName);
      return FIO_ERROR_FRAME_DECODING;
    }
...
static unsigned long long FIO_decompressGzFrame(dRess_t* ress, FILE* srcFile,
                                                const char* srcFileName) {
...
static unsigned long long FIO_decompressLzmaFrame(dRess_t* ress, FILE* srcFile,
                                                  const char* srcFileName,
                                                  int plain_lzma) {
  unsigned long long outFileSize = 0;
  lzma_stream strm = LZMA_STREAM_INIT;
...
static unsigned long long FIO_decompressLz4Frame(dRess_t* ress, FILE* srcFile,
                                                 const char* srcFileName) {
  ...
  for (; nextToLoad;) {
    ...
    while ((pos < readSize) ||
      ...
      nextToLoad =
          LZ4F_decompress(dCtx, ress->dstBuffer, &decodedBytes,
                          (char*)(ress->srcBuffer) + pos, &remaining, NULL);
      if (LZ4F_isError(nextToLoad)) {
...
#else
      DISPLAYLEVEL(1,
                   "zstd: %s: gzip file cannot be uncompressed (zstd compiled "
                   "without HAVE_ZLIB) -- ignored \n",
                   srcFileName);
      return 1;
#endif
...
#ifdef ZSTD_LZMADECOMPRESS
      unsigned long long const frameSize =
          FIO_decompressLzmaFrame(&ress, srcFile, srcFileName, buf[0] != 0xFD);
      if (frameSize == FIO_ERROR_FRAME_DECODING) return 1;
      filesize += frameSize;
#else
      DISPLAYLEVEL(1,
...
#else
      DISPLAYLEVEL(1,
                   "zstd: %s: lz4 file cannot be uncompressed (zstd compiled "
                   "without HAVE_LZ4) -- ignored \n",
                   srcFileName);
      return 1;
#endif
    } else if ((g_overwrite) &&
...
static int FIO_decompressDstFile(dRess_t ress, FILE* srcFile,
  ...
  if (releaseDstFile) {
    ...
    if ((result != 0) /* operation failure */
      ...
      FIO_remove(dstFileName); /* remove decompression artefact; note: don't do
                                  anything special if remove() fails */
      ...
      if (strcmp(dstFileName,
          ...
          && transfer_permissions) /* file permissions correctly extracted from
                                      src */
        UTIL_setFileStat(
            dstFileName,
...
/** FIO_decompressSrcFile() :
    ...
    @return : 0 : OK
              1 : error
*/
static int FIO_decompressSrcFile(dRess_t ress, const char* dstFileName,
                                 const char* srcFileName) {
  FILE* srcFile;
  int result;
...
static const char* FIO_determineDstName(const char* srcFileName) {
  static size_t dfnbCapacity = 0;
  static char* dstFileNameBuffer = NULL; /* using static allocation : this
                                            function cannot be multi-threaded */
    ...
    }
    {
      ...
      if (magicNumber == ZSTD_MAGICNUMBER) {
        ...
        /* move to the end of the frame header */
        {
          ...
          ERROR_IF(fseek(srcFile, ((long)headerSize) - ((long)numBytesRead),
                         SEEK_CUR) != 0,
                   info_frame_error,
        ...
        /* check if checksum is used */
        {
          ...
          if (contentChecksumFlag) {
            info->usesCheck = 1;
            ERROR_IF(fseek(srcFile, 4, SEEK_CUR) != 0, info_frame_error,
...
static InfoError getFileInfo_fileConfirmed(fileInfo_t* info,
...
static void displayInfo(const char* inFileName, const fileInfo_t* info,
                        int displayLevel) {
  ...
  if (displayLevel <= 2) {
    if (!info->decompUnavailable) {
      DISPLAYOUT("%6d  %5d  %7.2f %2s  %9.2f %2s  %5.3f  %5s  %s\n",
                 ...
                 decompressedSizeUnit, unitStr, ratio, checkString, inFileName);
    } else {
    ...
    if (!info->decompUnavailable) {
      DISPLAYOUT("Decompressed Size: %.2f %2s (%llu B)\n", decompressedSizeUnit,
                 unitStr, (unsigned long long)info->decompressedSize);
      DISPLAYOUT("Ratio: %.4f\n", ratio);
    }
    DISPLAYOUT("Check: %s\n", checkString);
    DISPLAYOUT("\n");
...
static int FIO_listFile(fileInfo_t* total, const char* inFileName,
                        int displayLevel) {
  fileInfo_t info;
