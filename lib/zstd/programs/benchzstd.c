...
static void BMK_initCCtx(ZSTD_CCtx* ctx, const void* dictBuffer,
                         size_t dictBufferSize, int cLevel,
...
static void BMK_initDCtx(ZSTD_DCtx* dctx, const void* dictBuffer,
...
static BMK_benchOutcome_t BMK_benchMemAdvancedNoAlloc(
  ...
  size_t const blockSize =
      ((adv->blockSize >= 32 && (adv->mode != BMK_decodeOnly)) ? adv->blockSize
                                                               : srcSize) +
  ...
  if (adv->mode == BMK_decodeOnly) { /* benchmark only decompression : source
    ...
    while (!(compressionCompleted && decompressionCompleted)) {
      if (!compressionCompleted) {
...

        {
          ...
          ratio = (double)srcSize / cSize;
          {
            ...
            newResult.cSpeed =
                ((U64)srcSize * TIMELOOP_NANOSEC / cResult.nanoSecPerRun);
      ...
      if (!decompressionCompleted) {
...

        {
          ...
          if (newDSpeed > benchResult.dSpeed) benchResult.dSpeed = newDSpeed;
        }

        {
          int const ratioAccuracy = (ratio < 10.) ? 3 : 2;
          DISPLAYLEVEL(
              2, "%2s-%-17.17s :%10u ->%10u (%5.*f),%6.*f MB/s ,%6.1f MB/s \r",
              marks[markNb], displayName, (U32)srcSize, (U32)benchResult.cSize,
              ratioAccuracy, ratio, benchResult.cSpeed < (10 MB) ? 2 : 1,
              (double)benchResult.cSpeed / MB_UNIT,
...

{
  ...
  int allocationincomplete =
      !srcPtrs || !srcSizes || !cPtrs || !cSizes || !cCapacities || !resPtrs ||
      !resSizes || !timeStateCompress || !timeStateDecompress || !cctx ||
      !dctx || !compressedBuffer || !resultBuffer;
  ...
  if (!allocationincomplete && !dstParamsError) {
    outcome = BMK_benchMemAdvancedNoAlloc(
        ...
        &resultBuffer, compressedBuffer, maxCompressedSize, timeStateCompress,
        timeStateDecompress, srcBuffer, srcSize, fileSizes, nbFiles, cLevel,
        comprParams, dictBuffer, dictBufferSize, cctx, dctx, displayLevel,
        displayName, adv);
  }
...
static BMK_benchOutcome_t BMK_benchCLevel(
    const void* srcBuffer, size_t benchedSize, const size_t* fileSizes,
    unsigned nbFiles, int cLevel, const ZSTD_compressionParameters* comprParams,
  ...
  if (displayLevel == 1 && !adv->additionalParam) /* --quiet mode */
    DISPLAY("bench %s %s: input %u bytes, %u seconds, %u KB blocks\n",
            ZSTD_VERSION_STRING, ZSTD_GIT_COMMIT_STRING, (U32)benchedSize,
            adv->nbSeconds, (U32)(adv->blockSize >> 10));
...
BMK_benchOutcome_t BMK_syntheticTest(
  ...
  res = BMK_benchCLevel(srcBuffer, benchedSize, &benchedSize /* ? */, 1 /* ? */,
...
static int BMK_loadFiles(void* buffer, size_t bufferSize, size_t* fileSizes,
...
BMK_benchOutcome_t BMK_benchFilesAdvanced(
  ...
  /* Load input buffer */
  {
    int const errorCode = BMK_loadFiles(srcBuffer, benchedSize, fileSizes,
  ...
  /* Bench */
  {
    ...
    snprintf(mfName, sizeof(mfName), " %u files", nbFiles);
    {
      ...
      res = BMK_benchCLevel(srcBuffer, benchedSize, fileSizes, nbFiles, cLevel,
                            compressionParams, dictBuffer, dictBufferSize,
                            displayLevel, displayName, adv);
    }
  }
