...
static void DiB_saveDict(const char* dictFileName, const void* buff,
                         size_t buffSize) {
 ...
 * sizes table.
 */
static fileStats DiB_fileStats(const char** fileNamesTable, unsigned nbFiles,
...
/*! ZDICT_trainFromBuffer_unsafe_legacy() :
   ...
   `samplesBuffer`. `samplesBuffer` must be followed by noisy guard band to
   avoid out-of-buffer reads.
    @return : size of dictionary stored into `dictBuffer` (<=
   `dictBufferCapacity`) or an error code.
*/
...
int DiB_trainFromFiles(const char* dictFileName, unsigned maxDictSize,
  ...
  if (fs.oneSampleTooLarge) {
    ...
    DISPLAYLEVEL(2,
                 "!  As a consequence, only the first %u bytes of each sample "
                 "are loaded \n",
                 SAMPLESIZE_MAX);
  }
  if (fs.nbSamples < 5) {
    ...
    EXM_THROW(14,
              "nb of samples too low"); /* we now clearly forbid this case */
  }
...

  {
    size_t dictSize;
    if (params) {
      DiB_fillNoise((char*)srcBuffer + loadedSize,
      ...
      if (optimize) {
        ...
        if (!ZDICT_isError(dictSize)) {
          ...
          DISPLAYLEVEL(2, "k=%u\nd=%u\nsteps=%u\nsplit=%u\n", coverParams->k,
                       coverParams->d, coverParams->steps, splitPercentage);
        }
      } else {
        dictSize = ZDICT_trainFromBuffer_cover(dictBuffer, maxDictSize,
                                               srcBuffer, sampleSizes,
