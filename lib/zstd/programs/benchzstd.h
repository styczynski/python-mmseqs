 ...
 * srcBuffer with specific compression parameters provided by other arguments
 * using benchFunction (cLevel, comprParams + adv in advanced Mode) */
/*  srcBuffer - data source, expected to be valid compressed data if in Decode
 * Only Mode srcSize - size of data in srcBuffer fileSizes - srcBuffer is
 * considered cut into 1+ segments, to compress separately. note :
 * sum(fileSizes) must be == srcSize.  (<== ensure it's properly checked)
 ...
 * provided. dstCapacity - capacity of destination buffer, give 0 if dstBuffer =
 * NULL adv = see advancedParams_t
 */
