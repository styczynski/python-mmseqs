...
#define RDG_DEBUG 0
#define TRACE(...) \
  if (RDG_DEBUG) fprintf(stderr, __VA_ARGS__)
...
static BYTE RDG_genChar(U32* seed, const BYTE* ldt) {
  ...
  return ldt[id]; /* memory-sanitizer fails here, stating "uninitialized value"
                     when table initialized with P==0.0. Checked : table is
                     fully initialized */
}
...
static void RDG_genBlock(void* buffer, size_t buffSize, size_t prefixSize,
                         double matchProba, const BYTE* ldt,
                         unsigned* seedPtr) {
...
void RDG_genStdout(unsigned long long size, double matchProba, double litProba,
  ...
  while (total < size) {
    ...
    RDG_genBlock(buff, stdDictSize + stdBlockSize, stdDictSize, matchProba, ldt,
                 &seed);
    total += genBlockSize;
    {
