...
int UTIL_setFileStat(const char* filename, stat_t* statbuf) {
  ...
  if (!UTIL_isRegularFile(filename)) return -1;
  ...
  do {
    ...
    if (cFile.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) {
      ...
      nbFiles += UTIL_prepareFileList(
          ...
          followLinks); /* Recursively call "UTIL_prepareFileList" with the new
                           path. */
      if (*bufStart == NULL) {
        free(path);
        FindClose(hFile);
 ...
 * inputNames, inputNamesNb), scans directories, and returns a new list of files
 * (params: return value, allocatedBuffer, allocatedNamesNb). After finishing
 * usage of the list the structures should be freed with
 * UTIL_freeFileList(params: return value, allocatedBuffer) In case of error
 * UTIL_createFileList returns NULL and UTIL_freeFileList should not be called.
 */
const char** UTIL_createFileList(const char** inputNames, unsigned inputNamesNb,
  ...
  for (i = 0, pos = 0, nbFiles = 0; i < inputNamesNb; i++) {
    if (!UTIL_isDirectory(inputNames[i])) {
      ...
      nbFiles +=
          UTIL_prepareFileList(inputNames[i], &buf, &pos, &bufend, followLinks);
