...
TEST_CASE("padding_funcname", "[pattern_formatter]") {
  ...
  spdlog::details::log_msg msg2{
      spdlog::source_loc{"ignored", 1, "func567890123"}, "test_logger",
...
class custom_test_flag : public spdlog::custom_flag_formatter {
 public:
  explicit custom_test_flag(std::string txt) : some_txt{std::move(txt)} {}
...
TEST_CASE("clone-custom_formatter", "[pattern_formatter]") {
  auto formatter_1 = std::make_shared<spdlog::pattern_formatter>();
  formatter_1->add_flag<custom_test_flag>('t', "custom_output")
      .set_pattern("[%n] [%t] %v");
  auto formatter_2 = formatter_1->clone();
  ...
  auto expected = fmt::format("[logger-name] [custom_output] some message{}",
...
TEST_CASE("custom flags", "[pattern_formatter]") {
  ...
  spdlog::details::log_msg msg(spdlog::source_loc{}, "logger-name",
                               spdlog::level::info, "some message");
  formatter->format(msg, formatted);
  auto expected =
      fmt::format("[logger-name] [custom1] [custom2] some message{}",
                  spdlog::details::os::default_eol);
