...
using filename_memory_buf_t =
    fmt::basic_memory_buffer<spdlog::filename_t::value_type, 250>;
...
struct custom_daily_file_name_calculator {
  static spdlog::filename_t calc_filename(const spdlog::filename_t &basename,
                                          const tm &now_tm) {
    filename_memory_buf_t w;
    fmt::format_to(w, SPDLOG_FILENAME_T("{}{:04d}{:02d}{:02d}"), basename,
...
TEST_CASE("daily_file_sink::daily_filename_calculator", "[daily_file_sink]]") {
  ...
  std::basic_regex<spdlog::filename_t::value_type> re(SPDLOG_FILENAME_T(
