...
using test_value_types = std::tuple<
    toml::value, toml::basic_value<toml::preserve_comments>,
...
BOOST_AUTO_TEST_CASE(test_find_throws) {
...

  {
    // value is not a table
    toml::value v(true);
    BOOST_CHECK_THROW(toml::find<toml::boolean>(std::move(v), "key"),
                      toml::type_error);
  }
  {
    // the value corresponding to the key is not the expected type
...
BOOST_AUTO_TEST_CASE(test_find_array_throws) {
  ...
  // move version
  {
    // value is not an array
    toml::value v(true);
    BOOST_CHECK_THROW(toml::find<toml::boolean>(std::move(v), 0),
                      toml::type_error);
  }
  {
    // the value corresponding to the key is not the expected type
  ...
  }
  {
    // the value corresponding to the key is not found
    toml::value v{1, 2, 3, 4, 5};
    BOOST_CHECK_THROW(toml::find<toml::integer>(std::move(v), 6),
                      std::out_of_range);
  }
  {
    // the positive control.
...
BOOST_AUTO_TEST_CASE(test_find_recursive) {
  ...
  {
    toml::value v{toml::array{"array", "of", "string"},
    ...
    toml::value v = toml::table{
        {"array",
         toml::array{toml::array{1, 2, 3},
                     toml::array{toml::table{{"foo", "bar"}, {"baz", "qux"}},
...
BOOST_AUTO_TEST_CASE_TEMPLATE(test_find_exact, value_type, test_value_types) {
  ...
  {
    toml::offset_datetime dt(
        toml::local_datetime(toml::local_date(2018, toml::month_t::Apr, 22),
                             toml::local_time(12, 30, 45)),
  ...
  }
  {
    ...
    const bool result1 =
        (vec == toml::find<typename value_type::array_type>(v, "key"));
    BOOST_CHECK(result1);

    toml::find<typename value_type::array_type>(v, "key").push_back(
        value_type(123));
    ...
    const auto moved =
        toml::find<typename value_type::array_type>(std::move(v), "key");
  ...
  }
  {
    ...
    const bool result1 =
        (tab == toml::find<typename value_type::table_type>(v, "key"));
    BOOST_CHECK(result1);

    toml::find<typename value_type::table_type>(v, "key")["key3"] =
        value_type(123);
    ...
    const auto moved =
        toml::find<typename value_type::table_type>(std::move(v), "key");
...
BOOST_AUTO_TEST_CASE_TEMPLATE(test_find_integer_type, value_type,
                              test_value_types) {
  {
    value_type v{{"key", 42}};
    ...
    BOOST_TEST(std::uint16_t(42) ==
               toml::find<std::uint16_t>(std::move(v), "key"));
  }
}

BOOST_AUTO_TEST_CASE_TEMPLATE(test_find_floating_type, value_type,
                              test_value_types) {
  {
    value_type v{{"key", 3.14}};
    ...
    BOOST_TEST(static_cast<long double>(ref) ==
               toml::find<long double>(v, "key"));
    BOOST_TEST(static_cast<float>(ref) ==
               toml::find<float>(std::move(v), "key"));
  }
}

BOOST_AUTO_TEST_CASE_TEMPLATE(test_find_string_type, value_type,
                              test_value_types) {
  {
...
BOOST_AUTO_TEST_CASE_TEMPLATE(test_find_toml_array, value_type,
                              test_value_types) {
  ...
  const std::deque<std::int64_t> deq =
      toml::find<std::deque<std::int64_t>>(v, "key");
  ...
  std::pair<double, double> pr =
      toml::find<std::pair<double, double>>(p, "key");
  BOOST_TEST(3.14 == pr.first);
  BOOST_TEST(2.71 == pr.second);
}

BOOST_AUTO_TEST_CASE_TEMPLATE(test_find_move_toml_array, value_type,
                              test_value_types) {
  ...
  const std::vector<int> vec =
      toml::find<std::vector<int>>(std::move(v1), "key");
  const std::list<short> lst =
      toml::find<std::list<short>>(std::move(v2), "key");
  const std::deque<std::int64_t> deq =
      toml::find<std::deque<std::int64_t>>(std::move(v3), "key");
  ...
  value_type p{{"key", {3.14, 2.71}}};
  std::pair<double, double> pr =
      toml::find<std::pair<double, double>>(std::move(p), "key");
  BOOST_TEST(3.14 == pr.first);
  BOOST_TEST(2.71 == pr.second);
}

BOOST_AUTO_TEST_CASE_TEMPLATE(test_find_toml_array_of_array, value_type,
                              test_value_types) {
  ...
  std::pair<std::vector<int>, std::vector<std::string>> p =
      toml::find<std::pair<std::vector<int>, std::vector<std::string>>>(v,
                                                                        "key");
  ...
  std::tuple<std::vector<int>, std::vector<std::string>> t =
      toml::find<std::tuple<std::vector<int>, std::vector<std::string>>>(v,
                                                                         "key");
...
BOOST_AUTO_TEST_CASE_TEMPLATE(test_find_move_toml_array_of_array, value_type,
                              test_value_types) {
  ...
  std::pair<std::vector<int>, std::vector<std::string>> p =
      toml::find<std::pair<std::vector<int>, std::vector<std::string>>>(
          std::move(v1), "key");
  ...
  std::tuple<std::vector<int>, std::vector<std::string>> t =
      toml::find<std::tuple<std::vector<int>, std::vector<std::string>>>(
          std::move(v2), "key");
...
BOOST_AUTO_TEST_CASE_TEMPLATE(test_find_toml_table, value_type,
                              test_value_types) {
  {
    value_type v1{
...
BOOST_AUTO_TEST_CASE_TEMPLATE(test_find_toml_local_date, value_type,
                              test_value_types) {
  {
  ...
  }
  {
    value_type v1{{"key", toml::local_date{2018, toml::month_t::Apr, 1}}};
    const auto date = std::chrono::system_clock::to_time_t(
        toml::find<std::chrono::system_clock::time_point>(std::move(v1),
                                                          "key"));
...
BOOST_AUTO_TEST_CASE_TEMPLATE(test_find_toml_local_time, value_type,
                              test_value_types) {
  {
    value_type v1{{"key", toml::local_time{12, 30, 45}}};
    const auto time = toml::find<std::chrono::seconds>(v1, "key");
    BOOST_CHECK(time == std::chrono::hours(12) + std::chrono::minutes(30) +
...
BOOST_AUTO_TEST_CASE_TEMPLATE(test_find_toml_local_datetime, value_type,
                              test_value_types) {
  {
...
BOOST_AUTO_TEST_CASE_TEMPLATE(test_get_toml_offset_datetime, value_type,
                              test_value_types) {
  {
    value_type v1{
        {"key", toml::offset_datetime(
                    toml::local_date{2018, toml::month_t::Apr, 1},
                    toml::local_time{12, 30, 0}, toml::time_offset{9, 0})}};
...

  {
    value_type v1{
        {"key", toml::offset_datetime(
                    toml::local_date{2018, toml::month_t::Apr, 1},
                    toml::local_time{12, 30, 0}, toml::time_offset{-8, 0})}};
