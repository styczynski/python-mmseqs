...
std::basic_ostream<charT, traits>& operator<<(
    std::basic_ostream<charT, traits>& os, const std::vector<T, Alloc>& v) {
  os << "[ ";
  for (const auto& i : v) {
    os << i << ' ';
  }
...
template <typename charT, typename traits, typename T, typename Alloc>
std::basic_ostream<charT, traits>& operator<<(
    std::basic_ostream<charT, traits>& os, const std::deque<T, Alloc>& v) {
  os << "[ ";
  for (const auto& i : v) {
    os << i << ' ';
  }
...
template <typename charT, typename traits, typename T, typename Alloc>
std::basic_ostream<charT, traits>& operator<<(
    std::basic_ostream<charT, traits>& os, const std::list<T, Alloc>& v) {
  os << "[ ";
  for (const auto& i : v) {
    os << i << ' ';
  }
...
template <typename charT, typename traits, typename Key, typename Value,
          typename Hash, typename Eq, typename Alloc>
std::basic_ostream<charT, traits>& operator<<(
    std::basic_ostream<charT, traits>& os,
...
BOOST_AUTO_TEST_CASE_TEMPLATE(test_find_or_exact, value_type,
                              test_value_types) {
  ...
  TOML11_TEST_FIND_OR_EXACT(local_datetime,
                            ...
                            (toml::local_date(1999, toml::month_t::Jan, 2),
                             toml::local_time(6, 0, 30)))
  TOML11_TEST_FIND_OR_EXACT(
      offset_datetime,
      (toml::local_date(2019, toml::month_t::Apr, 1),
       toml::local_time(12, 30, 45), toml::time_offset(9, 0)),
      (toml::local_date(1999, toml::month_t::Jan, 2),
       toml::local_time(6, 0, 30), toml::time_offset(-3, 0))) {
...
#define TOML11_TEST_FIND_OR_MODIFY(toml_type, init_expr, opt_expr) \
  ...
  /**/
BOOST_AUTO_TEST_CASE_TEMPLATE(test_find_or_modify, value_type,
                              test_value_types) {
...
BOOST_AUTO_TEST_CASE_TEMPLATE(test_find_or_fallback, value_type,
                              test_value_types) {
...
BOOST_AUTO_TEST_CASE(test_find_or_map) {
  ...
  {
    const toml::value v1{{"key", {{"key", "value"}}}};
  ...
  }
  {
    ...
    const auto key2 =
        toml::find_or<map_type>(std::move(v2), "key2", map_type{});
