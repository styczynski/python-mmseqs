...
struct from<extlib::foo> {
  static extlib::foo from_toml(const toml::value& v) {
    return extlib::foo{toml::find<int>(v, "a"),
...
struct from<extlib::baz> {
  static extlib::baz from_toml(const toml::value& v) {
    return extlib::baz{toml::find<int>(v, "a"),
                       toml::find<std::string>(v, "b")};
  }
};
...
struct from<extlib2::foo> {
  ...
  static extlib2::foo from_toml(const toml::basic_value<C, M, A>& v) {
    return extlib2::foo{toml::find<int>(v, "a"),
                        toml::find<std::string>(v, "b")};
  }
};
...
struct from<extlib2::baz> {
  ...
  static extlib2::baz from_toml(const toml::basic_value<C, M, A>& v) {
    return extlib2::baz{toml::find<int>(v, "a"),
                        toml::find<std::string>(v, "b")};
  }
};
...
struct into<extlib2::qux> {
  static toml::basic_value<toml::preserve_comments, std::map> into_toml(
      const extlib2::qux& f) {
    return toml::basic_value<toml::preserve_comments, std::map>{{"a", f.a},
                                                                {"b", f.b}};
  }
};
}  // namespace toml
...
BOOST_AUTO_TEST_CASE(test_conversion_by_member_methods) {
...

  {
    ...
    const toml::basic_value<toml::discard_comments, std::map, std::deque> v2(
        foo);
...
BOOST_AUTO_TEST_CASE(test_conversion_one_way) {
  ...
  {
    const toml::basic_value<toml::discard_comments, std::map> v{{"a", 42},
                                                                {"b", "baz"}};
...
BOOST_AUTO_TEST_CASE(test_recursive_conversion) {
  ...
  // via constructor
  {
    const toml::value v{{"0", toml::table{{"a", 42}, {"b", "baz"}}},
