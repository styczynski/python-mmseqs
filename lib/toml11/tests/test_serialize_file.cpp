...
BOOST_AUTO_TEST_CASE(test_example_with_comment_nocomment) {
  {
    ...
    const auto serialized =
        toml::parse<toml::preserve_comments>("tmp1_com_nocomment.toml");
...
BOOST_AUTO_TEST_CASE(test_fruit_map_dq) {
  ...
  const auto serialized =
      toml::parse<toml::discard_comments, std::map, std::deque>("tmp2.toml");
  BOOST_TEST(data == serialized);
}

BOOST_AUTO_TEST_CASE(test_fruit_with_comments) {
  const auto data =
      toml::parse<toml::preserve_comments>("toml/tests/fruit.toml");
  {
...
BOOST_AUTO_TEST_CASE(test_hard_example_map_dq) {
  ...
  const auto serialized =
      toml::parse<toml::discard_comments, std::map, std::deque>("tmp3.toml");
  BOOST_TEST(data == serialized);
}
...
array_of_table = [
  ...
  BOOST_TEST(parsed.at("array_of_table").comments().front() ==
             " comment about array itself");
  BOOST_TEST(parsed.at("array_of_table").at(0).comments().size() == 1u);
  BOOST_TEST(parsed.at("array_of_table").at(0).comments().front() ==
             " comment about the first element (table)");
}
