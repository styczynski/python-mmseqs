...
BOOST_AUTO_TEST_CASE(test_example) {
  ...
  const auto& database = toml::find(data, "database");
  {
    ...
    BOOST_CHECK(toml::find<std::vector<int>>(database, "ports") ==
                expected_ports);
  ...
  const auto& servers = toml::find(data, "servers");
  {
    ...
    BOOST_TEST(toml::get<std::string>(beta.at("country")) ==
...
BOOST_AUTO_TEST_CASE(test_hard_example_comment) {
  const auto data =
      toml::parse<toml::preserve_comments>("toml/tests/hard_example.toml");
  const auto the = toml::find(data, "the");
  BOOST_TEST(toml::find<std::string>(the, "test_string") ==
             "You'll hate me after this - #");
...
BOOST_AUTO_TEST_CASE(test_example_preserve_comment) {
  const auto data =
      toml::parse<toml::preserve_comments>("toml/tests/example.toml");
...
BOOST_AUTO_TEST_CASE(test_example_preserve_stdmap_stddeque) {
  ...
  static_assert(
      std::is_same<typename decltype(data)::table_type,
                   std::map<toml::key, typename std::remove_cv<decltype(
                                           data)>::type>>::value,
      "");
  static_assert(
      std::is_same<
          typename decltype(data)::array_type,
          std::deque<typename std::remove_cv<decltype(data)>::type>>::value,
      "");
...
BOOST_AUTO_TEST_CASE(test_file_with_BOM) {
  {
    ...
    BOOST_TEST(toml::find<std::string>(toml::find(data, "table"), "key") ==
               "value");
  }
  {
    const std::string table(
...
BOOST_AUTO_TEST_CASE(test_file_without_newline_at_the_end_of_file) {
  {
    ...
    std::istringstream iss(table);
    const auto data =
        toml::parse(iss, "test_file_without_newline_at_the_end_of_file.toml");
  ...
  }
  {
    ...
    const auto data = toml::parse(
        iss, "test_file_without_newline_at_the_end_of_file_CRLF.toml");
...

  {
    ...
    std::istringstream iss(table);
    const auto data = toml::parse(
        iss, "test_file_without_newline_at_the_end_of_file_comment.toml");
...

  {
    ...
    std::istringstream iss(table);
    const auto data = toml::parse(
        iss, "test_file_without_newline_at_the_end_of_file_ws.toml");
...
BOOST_AUTO_TEST_CASE(test_files_end_with_comment) {
  // comment w/o newline
  {
    const std::string table(
        ...
        "# comment");
    std::istringstream iss(table);
    const auto data = toml::parse(iss, "test_files_end_with_comment.toml");
...
BOOST_AUTO_TEST_CASE(test_files_end_with_empty_lines) {
  {
    const std::string table(
        ...
        "\n");
    std::istringstream iss(table);
    const auto data = toml::parse(iss, "test_files_end_with_newline.toml");
...
BOOST_AUTO_TEST_CASE(test_parse_function_compiles) {
  ...
  const std::filesystem::path fname_path(fname_string.begin(),
                                         fname_string.end());
