...
inline result<std::pair<boolean, region>, std::string> parse_boolean(
    location& loc) {
  ...
  if (const auto token = lex_boolean::invoke(loc)) {
    ...
    } else  // internal error.
    {
      throw internal_error(
          format_underline("toml::parse_boolean: internal error",
                           {{source_location(reg), "invalid token"}}),
          source_location(reg));
    }
  }
  loc.reset(first);  // rollback
  return err(format_underline(
      "toml::parse_boolean: ",
      {{source_location(loc), "the next token is not a boolean"}}));
}

inline result<std::pair<integer, region>, std::string> parse_binary_integer(
    location& loc) {
  const auto first = loc.iter();
  ...
  loc.reset(first);
  return err(format_underline(
      "toml::parse_binary_integer:",
      {{source_location(loc), "the next token is not an integer"}}));
}

inline result<std::pair<integer, region>, std::string> parse_octal_integer(
    location& loc) {
  const auto first = loc.iter();
...
parse_hexadecimal_integer(location& loc) {
  ...
  loc.reset(first);
  return err(format_underline(
      "toml::parse_hexadecimal_integer",
      {{source_location(loc), "the next token is not an integer"}}));
}

inline result<std::pair<integer, region>, std::string> parse_integer(
    location& loc) {
  const auto first = loc.iter();
  if (first != loc.end() && *first == '0') {
    ...
    if (*second == 'b') {
      return parse_binary_integer(loc);
    }  // 0b1100
    if (*second == 'o') {
      return parse_octal_integer(loc);
    }  // 0o775
    if (*second == 'x') {
      return parse_hexadecimal_integer(loc);
    }  // 0xC0FFEE

    if (std::isdigit(*second)) {
      return err(
          format_underline("toml::parse_integer: "
...
inline result<std::pair<floating, region>, std::string> parse_floating(
    location& loc) {
  const auto first = loc.iter();
  if (const auto token = lex_float::invoke(loc)) {
    ...
    if (str == "inf" || str == "+inf") {
      if (std::numeric_limits<floating>::has_infinity) {
        return ok(std::make_pair(std::numeric_limits<floating>::infinity(),
                                 token.unwrap()));
      } else {
      ...
      if (std::numeric_limits<floating>::has_quiet_NaN) {
        return ok(std::make_pair(std::numeric_limits<floating>::quiet_NaN(),
                                 token.unwrap()));
      } else if (std::numeric_limits<floating>::has_signaling_NaN) {
        return ok(std::make_pair(std::numeric_limits<floating>::signaling_NaN(),
                                 token.unwrap()));
      } else {
        throw std::domain_error(
...
inline result<std::string, std::string> parse_escape_sequence(location& loc) {
  ...
  switch (*loc.iter()) {
    ...
    case 'u': {
      if (const auto token = lex_escape_unicode_short::invoke(loc)) {
        ...
        return err(
            format_underline("parse_escape_sequence: "
  ...
  const auto msg = format_underline(
      ...
      /* Hints = */
      {"if you want to write backslash as just one backslash, "
...
parse_ml_basic_string(location& loc) {
  ...
  if (const auto token = lex_ml_basic_string::invoke(loc)) {
    ...
    if (!delim) {
      throw internal_error(
          format_underline("parse_ml_basic_string: invalid token",
    ...
    delim = none();
    while (!delim) {
      using lex_unescaped_seq =
          repeat<either<lex_ml_basic_unescaped, lex_newline>, unlimited>;
    ...
    assert(std::all_of(
        delim.unwrap().first(), delim.unwrap().last(),
    ...
    loc.reset(first);
    return err(
        format_underline("toml::parse_ml_basic_string: "
                         "the next token is not a valid multiline string",
                         {{source_location(loc), "here"}}));
  }
}

inline result<std::pair<toml::string, region>, std::string> parse_basic_string(
    location& loc) {
  const auto first = loc.iter();
  if (const auto token = lex_basic_string::invoke(loc)) {
    ...
    if (!quot) {
      throw internal_error(
          format_underline("parse_basic_string: "
                           "invalid token",
                           {{source_location(inner_loc), "should be \""}}),
          source_location(inner_loc));
    }
    ...
    loc.reset(first);  // rollback
    return err(
        format_underline("toml::parse_basic_string: "
...
parse_ml_literal_string(location& loc) {
  ...
  if (const auto token = lex_ml_literal_string::invoke(loc)) {
    ...
    if (!open) {
      throw internal_error(
          format_underline("parse_ml_literal_string: invalid token",
                           {{source_location(inner_loc), "should be '''"}}),
          source_location(inner_loc));
    }
    // immediate newline is ignored (if exists)
    ...
    assert(std::all_of(
        close.unwrap().first(), close.unwrap().last(),
        [](const char c) noexcept { return c == '\''; }));
...
parse_literal_string(location& loc) {
  ...
  if (const auto token = lex_literal_string::invoke(loc)) {
    ...
    if (!open) {
      throw internal_error(
          format_underline("parse_literal_string: invalid token",
                           {{source_location(inner_loc), "should be '"}}),
          source_location(inner_loc));
    }
    ...
    loc.reset(first);  // rollback
    return err(
        format_underline("toml::parse_literal_string: "
                         "the next token is not a valid literal string",
                         {{source_location(loc), "here"}}));
  }
}

inline result<std::pair<toml::string, region>, std::string> parse_string(
    location& loc) {
...
inline result<std::pair<local_date, region>, std::string> parse_local_date(
    location& loc) {
  const auto first = loc.iter();
  if (const auto token = lex_local_date::invoke(loc)) {
    ...
    if (!y || inner_loc.iter() == inner_loc.end() || *inner_loc.iter() != '-') {
      throw internal_error(
          format_underline("toml::parse_inner_local_date: invalid year format",
                           {{source_location(inner_loc), "should be `-`"}}),
          source_location(inner_loc));
    }
    inner_loc.advance();
    ...
    if (!m || inner_loc.iter() == inner_loc.end() || *inner_loc.iter() != '-') {
      throw internal_error(
          format_underline("toml::parse_local_date: invalid month format",
                           {{source_location(inner_loc), "should be `-`"}}),
          source_location(inner_loc));
    }
    inner_loc.advance();
    ...
    if (!d) {
      throw internal_error(
          format_underline("toml::parse_local_date: invalid day format",
    ...
    return ok(std::make_pair(
        local_date(
            ...
            static_cast<month_t>(static_cast<std::int8_t>(
                from_string<int>(m.unwrap().str(), 0) - 1)),
...
inline result<std::pair<local_time, region>, std::string> parse_local_time(
    location& loc) {
  const auto first = loc.iter();
  if (const auto token = lex_local_time::invoke(loc)) {
    ...
    if (!h || inner_loc.iter() == inner_loc.end() || *inner_loc.iter() != ':') {
      throw internal_error(
          format_underline("toml::parse_local_time: invalid year format",
                           {{source_location(inner_loc), "should be `:`"}}),
          source_location(inner_loc));
    }
    inner_loc.advance();
    ...
    if (!m || inner_loc.iter() == inner_loc.end() || *inner_loc.iter() != ':') {
      throw internal_error(
          format_underline("toml::parse_local_time: invalid month format",
                           {{source_location(inner_loc), "should be `:`"}}),
          source_location(inner_loc));
    }
    inner_loc.advance();
    ...
    if (!s) {
      throw internal_error(
          format_underline("toml::parse_local_time: invalid second format",
                           {{source_location(inner_loc), "here"}}),
          source_location(inner_loc));
    }
    local_time time(from_string<int>(h.unwrap().str(), 0),
    ...
    if (const auto secfrac = lex_time_secfrac::invoke(inner_loc)) {
      ...
      if (before_secfrac != inner_loc.iter()) {
        throw internal_error(
            format_underline("toml::parse_local_time: invalid subsecond format",
                             {{source_location(inner_loc), "here"}}),
            source_location(inner_loc));
      }
    }
...
parse_local_datetime(location& loc) {
  ...
  if (const auto token = lex_local_date_time::invoke(loc)) {
    ...
    return ok(
        std::make_pair(local_datetime(date.unwrap().first, time.unwrap().first),
                       token.unwrap()));
  } else {
    loc.reset(first);
    return err(format_underline(
...
inline result<std::pair<key, region>, std::string> parse_simple_key(
    location& loc) {
...
inline result<std::pair<std::vector<key>, region>, std::string> parse_key(
    location& loc) {
  const auto first = loc.iter();
  ...
  if (const auto token = lex_dotted_key::invoke(loc)) {
    ...
    while (inner_loc.iter() != inner_loc.end()) {
      ...
      if (inner_loc.iter() == inner_loc.end()) {
        ...
        throw internal_error(
            format_underline("toml::parse_key: "
  ...
  return err(format_underline(
      "toml::parse_key: an invalid key appeaed.",
...
template <typename Value>
result<std::pair<typename Value::array_type, region>, std::string> parse_array(
    location& loc) {
  ...
  using lex_ws_comment_newline =
      repeat<either<lex_wschar, lex_newline, lex_comment>, unlimited>;
  ...
  while (loc.iter() != loc.end()) {
    ...
    using lex_array_separator =
        sequence<maybe<lex_ws_comment_newline>, character<','>>;
  ...
  loc.reset(first);
  throw syntax_error(
      format_underline("toml::parse_array: "
...
parse_key_value_pair(location& loc) {
  ...
  return ok(
      std::make_pair(std::move(key_reg.unwrap()), std::move(val.unwrap())));
}
...
std::string format_dotted_keys(InputIterator first, const InputIterator last) {
  static_assert(
      std::is_same<
          key, typename std::iterator_traits<InputIterator>::value_type>::value,
      "");
...
result<std::pair<std::vector<key>, region>, std::string> parse_table_key(
    location& loc);
...
bool is_valid_forward_table_definition(const Value& fwd, Iterator key_first,
                                       Iterator key_curr, Iterator key_last) {
  std::string internal = "";
...
result<bool, std::string> insert_nested_key(
    typename Value::table_type& root, const Value& v, InputIterator iter,
    const InputIterator last, region key_reg,
    ...
    if (std::next(iter) == last)  // k is the last key
    {
      ...
      if (is_array_of_table) {
        if (tab->count(k) == 1)  // there is already an array of table
        {
          if (tab->at(k).is_table()) {
            ...
            throw syntax_error(
                format_underline(
                    concat_to_string("toml::insert_value: array of table (\"",
            ...
            throw syntax_error(
                format_underline(
                    ...
                    {{tab->at(k).location(),
                     ...
                     {v.location(), "while inserting this array-of-tables"}}),
                v.location());
          }
      ...
      if (tab->count(k) == 1) {
        if (tab->at(k).is_table() && v.is_table()) {
          if (!is_valid_forward_table_definition(tab->at(k), first, iter,
                                                 last)) {
            throw syntax_error(
                format_underline(
                    concat_to_string("toml::insert_value: table (\"",
                                     format_dotted_keys(first, last),
                                     "\") already exists."),
          ...
          return ok(true);
        } else if (v.is_table() && tab->at(k).is_array() &&
                   tab->at(k).as_array().size() > 0 &&
                   tab->at(k).as_array().front().is_table()) {
          throw syntax_error(
              format_underline(
                  concat_to_string("toml::insert_value: array of tables (\"",
                                   format_dotted_keys(first, last),
                                   "\") already exists."),
          ...
          throw syntax_error(
              format_underline(
                  concat_to_string("toml::insert_value: value (\"",
                                   format_dotted_keys(first, last),
                                   "\") already exists."),
    ...
    } else  // k is not the last one, we should insert recursively
    {
      ...
      } else if (tab->at(k).is_array())  // inserting to array-of-tables?
      {
        ...
        if (!a.back().is_table()) {
          throw syntax_error(
              format_underline(
                  concat_to_string("toml::insert_value: target (",
                                   format_dotted_keys(first, std::next(iter)),
...
parse_inline_table(location& loc) {
  ...
  while (loc.iter() != loc.end()) {
    ...
    if (!inserted) {
      throw internal_error(
          "toml::parse_inline_table: "
          "failed to insert value into table: " +
              inserted.unwrap_err(),
          source_location(loc));
    }
  ...
  loc.reset(first);
  throw syntax_error(
      format_underline("toml::parse_inline_table: "
...
inline result<value_t, std::string> guess_number_type(const location& l) {
  ...
  if (lex_local_date_time::invoke(loc)) {
    ...
    if (loc.iter() != loc.end() && (*loc.iter() == '+' || *loc.iter() == '-' ||
      ...
      return err(
          format_underline("bad offset: should be [+-]HH:MM or Z",
  ...
  if (lex_local_date::invoke(loc)) {
    ...
    if (loc.iter() != loc.end()) {
      ...
      if (c == 'T' || c == 't') {
        return err(format_underline(
            "bad time: should be HH:MM:SS.subsec",
  ...
  if (lex_float::invoke(loc)) {
    if (loc.iter() != loc.end() && *loc.iter() == '_') {
      return err(
          format_underline("bad float: `_` should be surrounded by digits",
                           {{source_location(loc), "here"}},
  ...
  if (lex_integer::invoke(loc)) {
    if (loc.iter() != loc.end()) {
      ...
      if (c == '_') {
        return err(format_underline(
            "bad integer: `_` should be surrounded by digits",
            {{source_location(loc), "here"}},
      ...
      if ('0' <= c && c <= '9') {
        ...
        loc.retrace();
        return err(format_underline(
            "bad integer: leading zero", {{source_location(loc), "here"}},
            {"pass: -42, 1_000, 1_2_3_4_5, 0xC0FFEE, 0b0010, 0o755",
             "fail: 1__000, 0123"}));
      }
      if (c == ':' || c == '-') {
        return err(format_underline(
            "bad datetime: invalid format", {{source_location(loc), "here"}},
            {"pass: 1979-05-27T07:32:00-07:00, 1979-05-27 07:32:00.999999Z",
             "fail: 1979-05-27T7:32:00-7:00, 1979-05-27 7:32-00:30"}));
      }
      if (c == '.' || c == 'e' || c == 'E') {
        return err(format_underline(
            "bad float: invalid format", {{source_location(loc), "here"}},
            {"pass: +1.0, -2e-2, 3.141_592_653_589, inf, nan",
             "fail: .0, 1., _1.0, 1.0_, 1_.0, 1.0__0"}));
      }
    }
  ...
  if (loc.iter() != loc.end() && *loc.iter() == '_') {
    return err(format_underline(
        "bad number: `_` should be surrounded by digits",
...
result<Value, std::string> parse_value_helper(
    result<std::pair<T, region>, std::string> rslt) {
  if (rslt.is_ok()) {
...
inline result<std::pair<std::vector<key>, region>, std::string> parse_table_key(
    location& loc) {
  if (auto token = lex_std_table::invoke(loc)) {
    ...
    if (!open || inner_loc.iter() == inner_loc.end()) {
      throw internal_error(
          format_underline("toml::parse_table_key: no `[`",
                           {{source_location(inner_loc), "should be `[`"}}),
          source_location(inner_loc));
    }
    // to skip [ a . b . c ]
    ...
    if (!keys) {
      throw internal_error(
          format_underline("toml::parse_table_key: invalid key",
    ...
    return err(
        format_underline("toml::parse_table_key: "
                         "not a valid table key",
                         {{source_location(loc), "here"}}));
  }
}
...
parse_array_table_key(location& loc) {
  if (auto token = lex_array_table::invoke(loc)) {
    ...
    if (!open || inner_loc.iter() == inner_loc.end()) {
      throw internal_error(
          format_underline("toml::parse_array_table_key: no `[[`",
                           {{source_location(inner_loc), "should be `[[`"}}),
          source_location(inner_loc));
    }
    lex_ws::invoke(inner_loc);
    ...
    if (!keys) {
      throw internal_error(
          format_underline("toml::parse_array_table_key: invalid key",
    ...
    return err(
        format_underline("toml::parse_array_table_key: "
                         "not a valid table key",
                         {{source_location(loc), "here"}}));
  }
}
...
template <typename Value>
result<typename Value::table_type, std::string> parse_ml_table(location& loc) {
  using value_type = Value;
  using table_type = typename value_type::table_type;
  ...
  using skip_line =
      repeat<sequence<maybe<lex_ws>, maybe<lex_comment>, lex_newline>,
             at_least<1>>;
  ...
  while (loc.iter() != loc.end()) {
    ...
    if (!newline && loc.iter() != loc.end()) {
      ...
      const auto msg = format_underline(
          ...
          "invalid line format",
          {{source_location(loc),
            concat_to_string("expected newline, but got '",
                             show_char(*loc.iter()), "'.")}});
      loc.reset(before2);
      return err(msg);
    }
...
result<Value, std::string> parse_toml_file(location& loc) {
  ...
  while (loc.iter() != loc.end()) {
    ...
    if (const auto tabkey = parse_array_table_key(loc)) {
      ...
      const auto inserted =
          insert_nested_key(data, value_type(tab.unwrap(), reg, reg.comments()),
    ...
    return err(
        format_underline("toml::parse_toml_file: "
                         "unknown line appeared",
                         {{source_location(loc), "unknown format"}}));
  }
...
basic_value<Comment, Table, Array> parse(
