...
struct location final : public region_base {
  ...
  void reset(const_iterator rollback) noexcept {
    ...
    {
      this->line_number_ -=
          static_cast<std::size_t>(std::count(rollback, this->iter_, '\n'));
    } else  // iter < rollback [[unlikely]]
    {
      this->line_number_ +=
          static_cast<std::size_t>(std::count(this->iter_, rollback, '\n'));
    }
    this->iter_ = rollback;
    return;
  }
...
struct region final : public region_base {
  ...
  std::vector<std::string> comments() const override {
    ...
    std::vector<std::string> com{};
    {
      ...
      // a = [1, 2, 3, 4, 5]
      if (this->first() == std::find_if(
                               this->line_begin(), this->first(),
        ...
        while (iter != this->begin()) {
          ...
          const auto line_start =
              std::find(rev_iter(iter), rev_iter(this->begin()), '\n').base();
          ...
          // > b = "current value"
          if (std::all_of(
                  line_start, comment_found, [](const char c) noexcept->bool {
                    return c == ' ' || c == '\t';
                  })) {
...

    {
      ...
      // ]
      // ```
      const auto comment_found = std::find(this->last(), this->line_end(), '#');
      if (comment_found != this->line_end())  // '#' found
      {
        ...
        if (comment_found ==
            std::find_if(
                this->last(), comment_found, [](const char c) noexcept->bool {
                  return !(c == ' ' || c == '\t' || c == ',');
                })) {
          // unwrap the first '#' by std::next.
