...
std::basic_string<charT, traits, Alloc> format_key(
...
template <typename charT, typename traits, typename Alloc>
std::basic_string<charT, traits, Alloc> format_keys(
    const std::vector<std::basic_string<charT, traits, Alloc>>& keys) {
  std::basic_string<charT, traits, Alloc> serialized;
  if (keys.empty()) {
    return serialized;
  }
...
struct serializer {
  ...
  std::string operator()(const floating_type f) const {
    ...
    const auto has_fraction =
        (token.cend() != std::find(token.cbegin(), token.cend(), '.'));
 ...
 private:
  ...
  std::string make_inline_array(const array_type& v) const {
    ...
    for (const auto& item : v) {
      ...
      token += visit(
          serializer((std::numeric_limits<std::size_t>::max)(),
                     this->float_prec_,
  ...
  std::string make_multiline_table(const table_type& v) const {
    ...
    for (const auto& kv : v) {
      ...
      const auto residual_width = (this->width_ > key_and_sep.size())
                                      ? this->width_ - key_and_sep.size()
                                      : 0;
      token += key_and_sep;
      token += visit(
          serializer(residual_width, this->float_prec_,
    ...
    for (const auto& kv : v) {
      ...
      auto tmp =
          visit(serializer(this->width_, this->float_prec_,
...
std::string format(
    const basic_value<C, M, V>& v, std::size_t w = 80u,
  ...
  if (v.is_table()) {
    ...
    const auto serialized =
        visit(serializer<value_type>(w, fprec, no_comment, false), v);
    oss << serialized;
    return oss.str();
  }
...
template <typename charT, typename traits>
std::basic_ostream<charT, traits>& nocomment(
...
template <typename charT, typename traits>
std::basic_ostream<charT, traits>& showcomment(
    std::basic_ostream<charT, traits>& os) {
  // by default, it is zero. and by defalut, it shows comments.
...
std::basic_ostream<charT, traits>& operator<<(
    std::basic_ostream<charT, traits>& os, const basic_value<C, M, V>& v) {
  using value_type = basic_value<C, M, V>;
