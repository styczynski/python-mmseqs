...
inline std::string show_char(const char c) {
  ...
  // (c++17) are valid.
  if (std::isgraph(
          *reinterpret_cast<unsigned char const*>(std::addressof(c)))) {
    return std::string(1, c);
  } else {
    std::array<char, 5> buf;
    buf.fill('\0');
    const auto r = std::snprintf(buf.data(), buf.size(), "0x%02x",
...
struct character {
  ...
  static result<region, none_t> invoke(location& loc) {
    if (loc.iter() == loc.end()) {
      return none();
    }
...
constexpr char in_range<L, U>::upper;
template <char L, char U>
constexpr char in_range<L, U>::lower;
...
struct sequence<Head, Tail...> {
  ...
  static result<region, none_t> invoke(location& loc, region reg,
                                       Iterator first) {
    const auto rslt = Head::invoke(loc);
    if (rslt.is_err()) {
      loc.reset(first);
